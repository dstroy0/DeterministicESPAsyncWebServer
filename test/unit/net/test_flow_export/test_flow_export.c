...
void test_flow_guards_and_overflows()
{
    ...
    TEST_ASSERT_FALSE(flow_ipfix_begin(NULL, buf, sizeof(buf), 0, 0, 0));
    TEST_ASSERT_FALSE(flow_ipfix_begin(&w, NULL, sizeof(buf), 0, 0, 0));
    TEST_ASSERT_FALSE(flow_v9_begin(NULL, buf, sizeof(buf), 0, 0, 0, 0));
    TEST_ASSERT_FALSE(flow_v9_begin(&w, NULL, sizeof(buf), 0, 0, 0, 0));
    TEST_ASSERT_EQUAL_size_t(0, flow_export_finish(NULL));
    ...
    TEST_ASSERT_FALSE(flow_export_template(NULL, 256, &f, 1));
    TEST_ASSERT_FALSE(flow_export_template(&w, 256, NULL, 1));
    TEST_ASSERT_FALSE(flow_export_template(&w, 256, &f, 0));
    TEST_ASSERT_FALSE(flow_export_data_begin(NULL, 256));
    TEST_ASSERT_FALSE(flow_export_data_begin(&w, 255)); // id < 256
    uint8_t rec[4] = {1, 2, 3, 4};
    TEST_ASSERT_FALSE(flow_export_data_record(NULL, rec, 4));
    TEST_ASSERT_FALSE(flow_export_data_record(&w, rec, 4)); // no set open
    TEST_ASSERT_FALSE(flow_export_data_record(&w, NULL, 4));
    TEST_ASSERT_FALSE(flow_export_data_end(NULL));
...
// v5 writers' null-arg guards: buf==NULL and header/record==NULL, each in isolation
