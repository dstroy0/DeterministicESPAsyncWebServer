...
void test_build_then_parse_round_trip()
{
    ...
    uint8_t buf[64];
    uint16_t n = pc_esp3_build(ESP3_RADIO_ERP1, data, 7, opt, 3, buf, sizeof(buf));
...
static uint16_t sample(uint8_t *buf, uint16_t cap)
{
    ...
    return pc_esp3_build(ESP3_RADIO_ERP1, data, 4, NULL, 0, buf, cap);
}
...
void test_parse_rejects_bad_sync()
{
    ...
    buf[0] = 0x00;
    TEST_ASSERT_EQUAL_INT(-1, pc_esp3_parse(buf, n, NULL));
}
...
void test_parse_needs_more_bytes()
{
    uint8_t buf[32];
    uint16_t n = sample(buf, sizeof(buf));
    TEST_ASSERT_EQUAL_INT(0, pc_esp3_parse(buf, 3, NULL));     // header not complete
    TEST_ASSERT_EQUAL_INT(0, pc_esp3_parse(buf, n - 1, NULL)); // data not complete
}
...
void test_parse_rejects_over_length()
{
    ...
    TEST_ASSERT_EQUAL_INT(-1, pc_esp3_parse(buf, sizeof(buf), NULL));
}
...
{
    pc_esp3_packet pkt;
    TEST_ASSERT_EQUAL_INT(0, pc_esp3_parse(NULL, 10, &pkt)); // null raw
    uint8_t tiny[1] = {0};
...
void test_parse_succeeds_with_null_out()
{
    ...
    uint8_t buf[32];
    uint16_t n = sample(buf, sizeof(buf));
    TEST_ASSERT_EQUAL_INT((int)n, pc_esp3_parse(buf, n, NULL));
}

void test_build_rejects_null_out()
{
    const uint8_t data[4] = {0xD5, 0x08, 0x11, 0x22};
    TEST_ASSERT_EQUAL_UINT16(0, pc_esp3_build(ESP3_RADIO_ERP1, data, 4, NULL, 0, NULL, 32));
}
...
void test_erp1_parse()
{
    ...
    TEST_ASSERT_FALSE(pc_erp1_parse(NULL, 7, &t));
    TEST_ASSERT_FALSE(pc_erp1_parse(rps, 7, NULL));
}
...
void test_erp1_build()
{
    ...
    n = pc_erp1_build(buf, sizeof(buf), PC_ERP_RORG_1BS, NULL, 0, 0x11223344, 0x55);
    const uint8_t minimal[6] = {0xD5, 0x11, 0x22, 0x33, 0x44, 0x55};
    ...
    TEST_ASSERT_EQUAL_UINT16(0, pc_erp1_build(NULL, sizeof(buf), PC_ERP_RORG_RPS, rps_payload, 1, 0x1, 0x0));
    TEST_ASSERT_EQUAL_UINT16(0, pc_erp1_build(buf, sizeof(buf), PC_ERP_RORG_RPS, NULL, 1, 0x1, 0x0));
