...
void test_classify_v4()
{
    pc_ip ip;
    Ip.parse("0.0.0.0", &ip);
    TEST_ASSERT_EQUAL(PC_IP_SCOPE_UNSPECIFIED, Ip.classify(&ip));
    Ip.parse("127.0.0.1", &ip);
    TEST_ASSERT_EQUAL(PC_IP_SCOPE_LOOPBACK, Ip.classify(&ip));
    Ip.parse("169.254.3.4", &ip);
    TEST_ASSERT_EQUAL(PC_IP_SCOPE_LINK_LOCAL, Ip.classify(&ip));
    Ip.parse("10.1.2.3", &ip);
    TEST_ASSERT_EQUAL(PC_IP_SCOPE_PRIVATE, Ip.classify(&ip));
    Ip.parse("172.16.0.1", &ip);
    TEST_ASSERT_EQUAL(PC_IP_SCOPE_PRIVATE, Ip.classify(&ip));
    Ip.parse("172.32.0.1", &ip); // just outside 172.16/12
    TEST_ASSERT_EQUAL(PC_IP_SCOPE_GLOBAL, Ip.classify(&ip));
    Ip.parse("192.168.5.5", &ip);
    TEST_ASSERT_EQUAL(PC_IP_SCOPE_PRIVATE, Ip.classify(&ip));
    Ip.parse("224.0.0.1", &ip);
    TEST_ASSERT_EQUAL(PC_IP_SCOPE_MULTICAST, Ip.classify(&ip));
    Ip.parse("8.8.8.8", &ip);
    TEST_ASSERT_EQUAL(PC_IP_SCOPE_GLOBAL, Ip.classify(&ip));
...
void test_classify_v6()
{
    ...
    TEST_ASSERT_EQUAL(PC_IP_SCOPE_LINK_LOCAL, Ip.classify(&ip));
    Ip.parse("fc00::1", &ip);
    TEST_ASSERT_EQUAL(PC_IP_SCOPE_PRIVATE, Ip.classify(&ip)); // ULA
...
void test_ip_classify_equal_cidr_and_parse_edges()
{
    ...
    TEST_ASSERT_EQUAL_INT(PC_IP_SCOPE_UNSPECIFIED, Ip.classify(NULL));
    pc_ip none = {0};
    pc_ip none2 = {0};
    TEST_ASSERT_EQUAL_INT(PC_IP_SCOPE_UNSPECIFIED, Ip.classify(&none));
    ...
    TEST_ASSERT_EQUAL_UINT(0, Ip.format(&none, buf, sizeof(buf)));  // PC_IP_NONE family
