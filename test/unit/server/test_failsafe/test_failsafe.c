...
{
    pc_failsafe_on_breach(NULL, NULL); // overrides the setUp() default
...
{
    char buf[8];
    TEST_ASSERT_EQUAL_INT(0, pc_failsafe_json_at(0, NULL, sizeof(buf)));
...
{
    pc_failsafe_register_at(NULL, 500, 1000);
    char buf[128];
    int n = pc_failsafe_json_at(1200, buf, sizeof(buf));
    TEST_ASSERT_TRUE(n > 0);
