...
{
    char buf[64];
    size_t n = pc_spb_build_topic(buf, sizeof(buf), "group1", "NDATA", "edge1", NULL);
...
void test_overflow_fails_closed()
{
    ...
    char tsmall[8];
    TEST_ASSERT_EQUAL_size_t(0, pc_spb_build_topic(tsmall, sizeof(tsmall), "group1", "NDATA", "edge1", NULL));
}
...
{
    char tbuf[64];
    TEST_ASSERT_EQUAL_UINT(0, pc_spb_build_topic(NULL, sizeof(tbuf), "g", "NDATA", "e", NULL));
    TEST_ASSERT_EQUAL_UINT(0, pc_spb_build_topic(tbuf, sizeof(tbuf), NULL, "NDATA", "e", NULL));
    ...
    TEST_ASSERT_EQUAL_UINT(0, pc_spb_build_payload(buf, sizeof(buf), 1, 0, NULL, 2)); // n>0, null metrics
...
{
    char tbuf[64];
    TEST_ASSERT_EQUAL_UINT(0, pc_spb_build_topic(tbuf, sizeof(tbuf), "g", NULL, "e", NULL));
    ...
    TEST_ASSERT_EQUAL_UINT(0, pc_spb_build_payload(NULL, 128, 1, 0, NULL, 0)); // null buf

    // n == 0: timestamp + seq only, no metrics field.
    uint8_t pbuf[32];
    size_t pn = pc_spb_build_payload(pbuf, sizeof(pbuf), 42, 7, NULL, 0);
...
void test_decode_payload_and_metrics()
{
    ...
    TEST_ASSERT_FALSE(pc_spb_parse_payload(NULL, n, &hdr));
    TEST_ASSERT_FALSE(pc_spb_parse_metric(NULL, mlen, &d));
    TEST_ASSERT_FALSE(pc_spb_payload_next_metric(NULL, n, &pos, &mb, &mlen));
}
