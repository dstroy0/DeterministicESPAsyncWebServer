...
//
// Unit tests for the DNS answer classifier / verifier (network_drivers/network/dns/dns_resolver).
...
void test_resolve_verified_paths()
{
    ...
    Resolver.test_set_resolve(PROTO_FALSE, 0);                        // reset the hook
}

void test_resolve_host_ok_null_out_ip()
{
    // Call Resolver.resolve() (the host stub) directly - not via the _verified
    ...
    Resolver.test_set_resolve(PROTO_TRUE, IPV4(8, 8, 8, 8));
    TEST_ASSERT_TRUE(Resolver.resolve("example.com", NULL));
    Resolver.test_set_resolve(PROTO_FALSE, 0); // reset the hook
}
