...
{
    pc_link_set(&g_m, 1, true, nullptr, nullptr);
    int from = 5, to = 5;
...
{
    TEST_ASSERT_EQUAL_INT(-1, pc_link_select(nullptr));
    LinkManager m;
    pc_link_init(&m, nullptr, 3); // null ifaces -> n forced to 0, active -1
    TEST_ASSERT_EQUAL_INT(-1, pc_link_select(&m));
    TEST_ASSERT_EQUAL_INT(-1, pc_link_active(&m));
}
...
{
    pc_link_init(nullptr, g_ifaces, 3); // must simply return
    TEST_ASSERT_EQUAL_INT(-1, pc_link_active(nullptr));
}
...
void test_set_guard_paths(void)
{
    ...
    TEST_ASSERT_FALSE(pc_link_set(nullptr, 0, true, &from, &to));
    TEST_ASSERT_EQUAL_INT(-1, from);
    TEST_ASSERT_EQUAL_INT(-1, to);

    // Null interface table: reports the manager's active (-1), returns false.
    LinkManager m;
    pc_link_init(&m, nullptr, 3);
    from = 7;
    to = 7;
    ...
    TEST_ASSERT_FALSE(pc_link_set(&g_m, 9, true, nullptr, nullptr));
}
