...
void test_submessage_endianness(void)
{
    ...
    size_t n = pc_rtps_submessage(Rtps::RTPS_SM_INFO_TS, Rtps::RTPS_FLAG_ENDIAN, body, 8, out, sizeof(out));
    ...
    n = pc_rtps_submessage(Rtps::RTPS_SM_DATA, 0x00, body, 8, out, sizeof(out));
...
void test_parse_message(void)
{
    ...
    uint8_t ts[8] = {0};
    n += pc_rtps_submessage(Rtps::RTPS_SM_INFO_TS, Rtps::RTPS_FLAG_ENDIAN, ts, 8, msg + n, sizeof(msg) - n);
    uint8_t data[4] = {0xDE, 0xAD, 0xBE, 0xEF};
    n += pc_rtps_submessage(Rtps::RTPS_SM_DATA, Rtps::RTPS_FLAG_ENDIAN, data, 4, msg + n, sizeof(msg) - n);
...
void test_parse_rejects(void)
{
    ...
    // Bad magic.
    msg[0] = 'X';
    TEST_ASSERT_FALSE(pc_rtps_parse(msg, 20, nullptr, nullptr));
    msg[0] = 'R';
    // A newer minor version than ours is rejected.
    msg[5] = 99;
    TEST_ASSERT_FALSE(pc_rtps_parse(msg, 20, nullptr, nullptr));
    // Too short for a header.
    TEST_ASSERT_FALSE(pc_rtps_parse(msg, 10, nullptr, nullptr));
}
...
{
    uint8_t out[24];
    TEST_ASSERT_EQUAL_size_t(0, pc_rtps_header(nullptr, VENDOR, out, sizeof(out)));
    TEST_ASSERT_EQUAL_size_t(0, pc_rtps_header(GUID, nullptr, out, sizeof(out)));
    TEST_ASSERT_EQUAL_size_t(0, pc_rtps_header(GUID, VENDOR, nullptr, sizeof(out)));
...
void test_submessage_null_args(void)
{
    uint8_t out[16];
    uint8_t body[4] = {1, 2, 3, 4};
    TEST_ASSERT_EQUAL_size_t(0, pc_rtps_submessage(Rtps::RTPS_SM_DATA, 0, body, 4, nullptr, sizeof(out)));
    TEST_ASSERT_EQUAL_size_t(0, pc_rtps_submessage(Rtps::RTPS_SM_DATA, 0, nullptr, 4, out, sizeof(out)));
    // body_len == 0 with a null body: header only, no memcpy.
    size_t n = pc_rtps_submessage(Rtps::RTPS_SM_PAD, Rtps::RTPS_FLAG_ENDIAN, nullptr, 0, out, sizeof(out));
...
{
    TEST_ASSERT_FALSE(pc_rtps_parse(nullptr, 20, nullptr, nullptr));
}
...
void test_parse_rejects_each_magic_byte(void)
{
    ...
    TEST_ASSERT_TRUE(pc_rtps_parse(msg, 20, nullptr, nullptr));
}
...
void test_parse_big_endian_submessage(void)
{
    ...
    size_t n = pc_rtps_header(GUID, VENDOR, msg, sizeof(msg));
    uint8_t data[4] = {0xDE, 0xAD, 0xBE, 0xEF};
    n += pc_rtps_submessage(Rtps::RTPS_SM_DATA, 0x00, data, 4, msg + n, sizeof(msg) - n);
...
void test_parse_zero_length_terminates(void)
{
    uint8_t msg[32];
    size_t n = pc_rtps_header(GUID, VENDOR, msg, sizeof(msg));
    n += pc_rtps_submessage(Rtps::RTPS_SM_PAD, Rtps::RTPS_FLAG_ENDIAN, nullptr, 0, msg + n, sizeof(msg) - n);
...
void test_parse_without_callback(void)
{
    ...
    n += pc_rtps_submessage(Rtps::RTPS_SM_DATA, Rtps::RTPS_FLAG_ENDIAN, data, 4, msg + n, sizeof(msg) - n);
    TEST_ASSERT_TRUE(pc_rtps_parse(msg, n, nullptr, nullptr));
}
