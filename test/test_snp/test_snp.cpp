...
void test_bcc(void)
{
    ...
    TEST_ASSERT_EQUAL_HEX8(0x64, dws_snp_bcc(b, sizeof(b)));
    // Wrap-around low byte.
    const uint8_t w[] = {0xFF, 0x02};
    TEST_ASSERT_EQUAL_HEX8(0x01, dws_snp_bcc(w, sizeof(w)));
}
...
void test_build_and_parse(void)
{
    ...
    uint8_t buf[16];
    size_t n = dws_snp_build(Snp::SNP_SOH, data, 3, buf, sizeof(buf));
    ...
    SnpFrame f;
    TEST_ASSERT_TRUE(dws_snp_parse(buf, n, &f));
...
void test_parse_rejects(void)
{
    ...
    uint8_t buf[8];
    size_t n = dws_snp_build(Snp::SNP_SOH, data, 2, buf, sizeof(buf));
    SnpFrame f;
    buf[n - 1] ^= 0xFF; // bad BCC
    TEST_ASSERT_FALSE(dws_snp_parse(buf, n, &f));
    TEST_ASSERT_FALSE(dws_snp_parse(buf, 2, &f)); // too short
    // Length field says more data than the buffer holds.
    uint8_t bad[4] = {0x01, 0x10, 0x00, 0x11};
    TEST_ASSERT_FALSE(dws_snp_parse(bad, sizeof(bad), &f));
}

// dws_snp_build rejects a null output, a length with a null data pointer, and a
