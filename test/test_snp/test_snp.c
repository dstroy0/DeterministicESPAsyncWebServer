...
void test_build_and_parse(void)
{
    ...
    uint8_t buf[16];
    size_t n = pc_snp_build(SNP_SOH, data, 3, buf, sizeof(buf));
...
{
    uint8_t buf[8];
    size_t n = pc_snp_build(SNP_ENQ, NULL, 0, buf, sizeof(buf));
...
void test_parse_rejects(void)
{
    ...
    uint8_t buf[8];
    size_t n = pc_snp_build(SNP_SOH, data, 2, buf, sizeof(buf));
    SnpFrame f;
...
void test_snp_parse_guards(void)
{
    ...
    SnpFrame f;
    size_t n = pc_snp_build(SNP_ENQ, NULL, 0, buf, sizeof(buf));
    TEST_ASSERT_FALSE(pc_snp_parse(NULL, n, &f));  // null frame
    TEST_ASSERT_FALSE(pc_snp_parse(buf, n, NULL)); // null out
}
