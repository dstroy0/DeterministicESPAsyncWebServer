...
void test_guards_and_datacontenttype_only()
{
    ...
    ce.source = "/s";
    ce.type = "t";
    TEST_ASSERT_EQUAL_size_t(0, pc_cloudevents_build_json(nullptr, sizeof(buf), &ce)); // null buf
    TEST_ASSERT_EQUAL_size_t(0, pc_cloudevents_build_json(buf, 0, &ce));               // zero cap
    TEST_ASSERT_EQUAL_size_t(0, pc_cloudevents_build_json(buf, sizeof(buf), nullptr)); // null event
    ...
    CloudEvent out;
    TEST_ASSERT_FALSE(pc_cloudevents_from_headers(nullptr, &out)); // null request
}
...
{
    feed_request(0, "POST /events HTTP/1.1\r\nHost: x\r\nce-id: a\r\nce-source: /p\r\nce-type: t\r\n\r\n");
    TEST_ASSERT_FALSE(pc_cloudevents_from_headers(&http_pool[0], nullptr));
}
...
void test_numparse_strtol()
{
    ...
    v = pc_strtol("123", nullptr); // null end pointer must not be dereferenced
