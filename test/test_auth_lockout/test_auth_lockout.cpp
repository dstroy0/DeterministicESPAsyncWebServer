...
// Every bucket keys on the FULL family-tagged address (DWSIp) - never a hash or a
...
// Build a v4 DWSIp from a host-order word (0x0A000001 -> 10.0.0.1).
...
{
    auth_lockout_reset();
    DWSIp ip = v4w(0x0A000001u);
    for (int i = 0; i < DWS_AUTH_LOCKOUT_THRESHOLD - 1; i++)
...
{
    auth_lockout_reset();
    DWSIp ip = v4w(0x0A000002u);
    for (int i = 0; i < DWS_AUTH_LOCKOUT_THRESHOLD; i++)
        auth_lockout_fail(&ip, 0);
    TEST_ASSERT_EQUAL_UINT32(DWS_AUTH_LOCKOUT_BASE_MS, auth_lockout_remaining_ms(&ip, 0));
}
...
void test_exponential_backoff()
{
    ...
    TEST_ASSERT_EQUAL_UINT32(DWS_AUTH_LOCKOUT_BASE_MS, auth_lockout_remaining_ms(&ip, 0));
    auth_lockout_fail(&ip, 0);
    TEST_ASSERT_EQUAL_UINT32(2u * DWS_AUTH_LOCKOUT_BASE_MS, auth_lockout_remaining_ms(&ip, 0));
    auth_lockout_fail(&ip, 0);
    TEST_ASSERT_EQUAL_UINT32(4u * DWS_AUTH_LOCKOUT_BASE_MS, auth_lockout_remaining_ms(&ip, 0));
}
...
{
    auth_lockout_reset();
    DWSIp ip = v4w(0x0A000005u);
    for (int i = 0; i < DWS_AUTH_LOCKOUT_THRESHOLD; i++)
        auth_lockout_fail(&ip, 0);
    TEST_ASSERT_EQUAL_UINT32(1, auth_lockout_remaining_ms(&ip, DWS_AUTH_LOCKOUT_BASE_MS - 1));
    TEST_ASSERT_EQUAL_UINT32(0, auth_lockout_remaining_ms(&ip, DWS_AUTH_LOCKOUT_BASE_MS));
    TEST_ASSERT_EQUAL_UINT32(0, auth_lockout_remaining_ms(&ip, DWS_AUTH_LOCKOUT_BASE_MS + 100));
}
...
{
    auth_lockout_reset();
    DWSIp a = v4w(0x0A000007u), b = v4w(0x0A000008u);
    for (int i = 0; i < DWS_AUTH_LOCKOUT_THRESHOLD; i++)
...
void test_v6_distinct_from_v4_and_each_other()
{
    auth_lockout_reset();
    DWSIp v6a;
    v6a.family = DWSIpFamily::DWS_IP_NONE;
    DWSIp v6b;
    v6b.family = DWSIpFamily::DWS_IP_NONE;
    TEST_ASSERT_TRUE(dws_ip_parse("2001:db8::1", &v6a));
    TEST_ASSERT_TRUE(dws_ip_parse("2001:db8::2", &v6b));
    for (int i = 0; i < DWS_AUTH_LOCKOUT_THRESHOLD; i++)
    ...
    DWSIp v4 = v4w(0x00000001u); // 0.0.0.1 - distinct family, distinct bucket
...
void test_zero_ip_never_locked()
{
    ...
    for (int i = 0; i < DWS_AUTH_LOCKOUT_THRESHOLD + 10; i++)
...
{
    auth_lockout_reset();
    for (int i = 0; i < DWS_AUTH_LOCKOUT_SLOTS; i++)
    {
        DWSIp ip = v4w(0xC0000000u + (uint32_t)i);
        auth_lockout_fail(&ip, 0);
    }
    DWSIp fresh = v4w(0xD0000000u);
    for (int i = 0; i < DWS_AUTH_LOCKOUT_THRESHOLD; i++)
...
{
    auth_lockout_reset();
    DWSIp victim = v4w(0x0A0000FFu);
    for (int i = 0; i < DWS_AUTH_LOCKOUT_THRESHOLD; i++)
        auth_lockout_fail(&victim, 0);
    TEST_ASSERT_TRUE(auth_lockout_remaining_ms(&victim, 0) > 0);
    for (uint32_t i = 1; i <= (uint32_t)DWS_AUTH_LOCKOUT_SLOTS * 2u; i++)
    {
        DWSIp other = v4w(0xB0000000u + i);
