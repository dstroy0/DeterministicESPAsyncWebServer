...
void test_104_i_format_roundtrip()
{
    ...
    uint8_t buf[32];
    size_t n = dws_iec104_build_i(buf, sizeof(buf), 100, 50, asdu, 6);
    ...
    Iec104Apci a;
    size_t c;
    TEST_ASSERT_TRUE(dws_iec104_parse(buf, n, &a, &c));
...
{
    uint8_t buf[8];
    size_t n = dws_iec104_build_s(buf, sizeof(buf), 1234);
...
{
    uint8_t buf[8];
    size_t n = dws_iec104_build_u(buf, sizeof(buf), IEC104_STARTDT_ACT);
    TEST_ASSERT_EQUAL_size_t(6, n);
    Iec104Apci a;
    size_t c;
    TEST_ASSERT_TRUE(dws_iec104_parse(buf, n, &a, &c));
...
{
    uint8_t buf[8];
    dws_iec104_build_i(buf, sizeof(buf), 0x7FFF, 0x4001, nullptr, 0);
    Iec104Apci a;
    size_t c;
    TEST_ASSERT_TRUE(dws_iec104_parse(buf, 6, &a, &c));
...
void test_asdu_header_roundtrip()
{
    ...
    IecAsduHeader g;
    size_t c;
    TEST_ASSERT_TRUE(dws_iec_asdu_parse_header(buf, 6, &g, &c));
...
{
    uint8_t buf[4] = {0};
    TEST_ASSERT_EQUAL_size_t(3, dws_iec_put_ioa(buf, sizeof(buf), 0x123456));
    TEST_ASSERT_EQUAL_HEX8(0x56, buf[0]); // little-endian
    TEST_ASSERT_EQUAL_HEX8(0x34, buf[1]);
    TEST_ASSERT_EQUAL_HEX8(0x12, buf[2]);
    TEST_ASSERT_EQUAL_HEX32(0x123456u, dws_iec_get_ioa(buf));
}

void test_101_fixed_frame()
{
    uint8_t buf[8];
    size_t n = dws_iec101_build_fixed(buf, sizeof(buf), IEC_FC_REQUEST_CLASS2, 0x01);
    ...
    Iec101Frame f;
    size_t c;
    TEST_ASSERT_TRUE(dws_iec101_parse(buf, n, &f, &c));
...
void test_101_variable_frame_roundtrip()
{
    ...
    uint8_t buf[32];
    size_t n = dws_iec101_build_variable(buf, sizeof(buf), IEC_FC_USER_DATA_CONFIRM, 0x01, asdu, 6);
...
void test_104_build_guards()
{
    ...
    TEST_ASSERT_EQUAL_size_t(0, dws_iec104_build_i(nullptr, sizeof(buf), 0, 0, asdu, 6)); // null buf
    TEST_ASSERT_EQUAL_size_t(0, dws_iec104_build_i(buf, sizeof(buf), 0, 0, nullptr, 6));  // len but null asdu
    TEST_ASSERT_EQUAL_size_t(0, dws_iec104_build_i(buf, 512, 0, 0, asdu, 250));           // asdu_len > 249
    TEST_ASSERT_EQUAL_size_t(0, dws_iec104_build_i(buf, 8, 0, 0, asdu, 6));               // cap < 6 + asdu_len
    TEST_ASSERT_EQUAL_size_t(0, dws_iec104_build_s(nullptr, sizeof(buf), 0));             // null buf
    TEST_ASSERT_EQUAL_size_t(0, dws_iec104_build_s(buf, 4, 0));                           // cap < APCI len
    TEST_ASSERT_EQUAL_size_t(0, dws_iec104_build_u(nullptr, sizeof(buf), 0));             // null buf
    TEST_ASSERT_EQUAL_size_t(0, dws_iec104_build_u(buf, 4, 0));                           // cap < APCI len
}
...
void test_104_parse_rejects()
{
    ...
    TEST_ASSERT_FALSE(dws_iec104_parse(bad_start, sizeof(bad_start), &a, &c)); // buf[0] != 0x68
    TEST_ASSERT_FALSE(dws_iec104_parse(nullptr, 6, &a, &c));                   // null buf
    uint8_t short_l[6] = {IEC_START_104, 0x03, 0, 0, 0, 0};
    TEST_ASSERT_FALSE(dws_iec104_parse(short_l, sizeof(short_l), &a, &c)); // L < 4
    uint8_t trunc[2] = {IEC_START_104, 0x0A};
    TEST_ASSERT_FALSE(dws_iec104_parse(trunc, sizeof(trunc), &a, &c)); // len < 2 + L
}
...
void test_asdu_ioa_guards()
{
    ...
    IecAsduHeader g;
    size_t c;
    TEST_ASSERT_EQUAL_size_t(0, dws_iec_asdu_build_header(buf, 4, &h)); // cap < 6
    TEST_ASSERT_EQUAL_size_t(0, dws_iec_asdu_build_header(nullptr, 8, &h));
    TEST_ASSERT_FALSE(dws_iec_asdu_parse_header(buf, 5, &g, &c));   // len < 6
    TEST_ASSERT_EQUAL_size_t(0, dws_iec_put_ioa(buf, 2, 0x123456)); // cap < 3
    TEST_ASSERT_EQUAL_size_t(0, dws_iec_put_ioa(nullptr, 8, 0));
}
...
void test_101_build_guards()
{
    uint8_t buf[32];
    const uint8_t asdu[6] = {0};
    TEST_ASSERT_EQUAL_size_t(0, dws_iec101_build_fixed(buf, 4, 0x49, 0x01));                    // cap < 5
    TEST_ASSERT_EQUAL_size_t(0, dws_iec101_build_fixed(nullptr, sizeof(buf), 0x49, 0x01));      // null buf
    TEST_ASSERT_EQUAL_size_t(0, dws_iec101_build_variable(buf, sizeof(buf), 0, 0, nullptr, 6)); // len but null asdu
    TEST_ASSERT_EQUAL_size_t(0, dws_iec101_build_variable(buf, 512, 0, 0, asdu, 254));          // asdu_len > 253
    TEST_ASSERT_EQUAL_size_t(0, dws_iec101_build_variable(buf, 10, 0, 0, asdu, 6));             // cap < 6 + L
}
...
void test_101_parse_rejects()
{
    Iec101Frame f;
    size_t c;
    TEST_ASSERT_FALSE(dws_iec101_parse(nullptr, 5, &f, &c)); // null buf
    uint8_t empty[1] = {0};
    TEST_ASSERT_FALSE(dws_iec101_parse(empty, 0, &f, &c)); // len < 1

    uint8_t bad_cksum[5] = {IEC_START_FIXED, 0x49, 0x01, 0xFF, IEC_STOP};
    TEST_ASSERT_FALSE(dws_iec101_parse(bad_cksum, sizeof(bad_cksum), &f, &c)); // checksum mismatch
    uint8_t bad_stop[5] = {IEC_START_FIXED, 0x49, 0x01, 0x4A, 0x00};
    TEST_ASSERT_FALSE(dws_iec101_parse(bad_stop, sizeof(bad_stop), &f, &c)); // no 0x16 stop

    uint8_t var_trunc[3] = {IEC_START_104, 0x08, 0x08};
    TEST_ASSERT_FALSE(dws_iec101_parse(var_trunc, sizeof(var_trunc), &f, &c)); // len < 4
    uint8_t var_badhdr[8] = {IEC_START_104, 0x01, 0x01, IEC_START_104, 0, 0, 0, 0};
    TEST_ASSERT_FALSE(dws_iec101_parse(var_badhdr, sizeof(var_badhdr), &f, &c)); // L < 2
    uint8_t var_mismatch[8] = {IEC_START_104, 0x08, 0x09, IEC_START_104, 0, 0, 0, 0};
    TEST_ASSERT_FALSE(dws_iec101_parse(var_mismatch, sizeof(var_mismatch), &f, &c)); // buf[2] != L
