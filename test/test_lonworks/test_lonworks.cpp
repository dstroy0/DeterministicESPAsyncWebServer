...
void test_nv_pdu_roundtrip(void)
{
    ...
    uint8_t out[8];
    size_t n = dws_lon_build_nv(Lon::LON_MSG_NV_UPDATE, 0x1234, value, 2, out, sizeof(out));
    ...
    LonNv nv;
    TEST_ASSERT_TRUE(dws_lon_parse_nv(out, n, &nv));
...
void test_nv_selector_masked_to_14_bits(void)
{
    ...
    uint8_t out[8];
    dws_lon_build_nv(Lon::LON_MSG_NV_POLL, 0x3FFF, nullptr, 0, out, sizeof(out));
    LonNv nv;
    // Simulate a peer that set reserved high bits.
    out[1] |= 0xC0;
    TEST_ASSERT_TRUE(dws_lon_parse_nv(out, 3, &nv));
    TEST_ASSERT_EQUAL_UINT16(0x3FFF, nv.selector);
    // Reject a selector > 14 bits at build.
    TEST_ASSERT_EQUAL_size_t(0, dws_lon_build_nv(Lon::LON_MSG_NV_UPDATE, 0x4000, nullptr, 0, out, sizeof(out)));
}

void test_snvt_temp(void)
{
    uint8_t enc[2];
    dws_lon_snvt_temp_encode(25.0, enc);
    // (25 + 273.15) * 100 = 29815 = 0x747... check decode round-trip.
    double c = dws_lon_snvt_temp_decode(enc);
    TEST_ASSERT_TRUE(absd(c - 25.0) < 0.01);
    // 0 C -> 273.15 K -> 27315.
    dws_lon_snvt_temp_encode(0.0, enc);
...
void test_snvt_clamps_and_guards()
{
    uint8_t out[16];
    uint8_t val[2] = {0, 0};
    TEST_ASSERT_EQUAL_size_t(0, dws_lon_build_nv(0x00, 0, val, sizeof(val), out, 2)); // cap too small
    LonNv nv;
    uint8_t tiny[1] = {0};
    TEST_ASSERT_FALSE(dws_lon_parse_nv(tiny, sizeof(tiny), &nv)); // too short
    uint8_t t[2];
    dws_lon_snvt_temp_encode(1e9, t);  // saturates at +32767
    dws_lon_snvt_temp_encode(-1e9, t); // saturates at -32768
    uint8_t sw[2];
    dws_lon_snvt_switch_encode(-5.0, 1, sw); // negative percent clamps to 0
}
...
void test_nv_build_null_guards(void)
{
    uint8_t out[8];
    uint8_t val[2] = {1, 2};
    // out == nullptr guard branch.
    TEST_ASSERT_EQUAL_size_t(0, dws_lon_build_nv(Lon::LON_MSG_NV_UPDATE, 0, val, sizeof(val), nullptr, sizeof(out)));
    // value_len > 0 but value == nullptr guard branch.
    TEST_ASSERT_EQUAL_size_t(0, dws_lon_build_nv(Lon::LON_MSG_NV_UPDATE, 0, nullptr, 2, out, sizeof(out)));
}
...
void test_nv_parse_null_guards(void)
{
    ...
    LonNv nv;
    // pdu == nullptr guard branch.
    TEST_ASSERT_FALSE(dws_lon_parse_nv(nullptr, 3, &nv));
    // out == nullptr guard branch.
    TEST_ASSERT_FALSE(dws_lon_parse_nv(pdu, 3, nullptr));
}
...
void test_snvt_temp_clamp_high_in_range(void)
{
    ...
    uint8_t enc[2];
    dws_lon_snvt_temp_encode(200.0, enc);
...
void test_snvt_switch_decode_null_outputs(void)
{
    ...
    double pct = -1.0;
    uint8_t st = 0xAA;
    // percent == nullptr: only state should be written.
    dws_lon_snvt_switch_decode(enc, nullptr, &st);
    TEST_ASSERT_EQUAL_UINT8(1, st);
    // state == nullptr: only percent should be written.
    dws_lon_snvt_switch_decode(enc, &pct, nullptr);
    TEST_ASSERT_TRUE(absd(pct - 50.0) < 0.01);
}
