...
{
    char v[64];
    TEST_ASSERT_TRUE(detws_prov_form_field("ssid=MyAP&psk=secret", "ssid", v, sizeof(v)));
    TEST_ASSERT_EQUAL_STRING("MyAP", v);
    TEST_ASSERT_TRUE(detws_prov_form_field("ssid=MyAP&psk=secret", "psk", v, sizeof(v)));
...
{
    char v[64];
    TEST_ASSERT_TRUE(detws_prov_form_field("ssid=My+AP&psk=p%40ss%21", "ssid", v, sizeof(v)));
    TEST_ASSERT_EQUAL_STRING("My AP", v);
    TEST_ASSERT_TRUE(detws_prov_form_field("ssid=My+AP&psk=p%40ss%21", "psk", v, sizeof(v)));
...
{
    char v[64];
    TEST_ASSERT_FALSE(detws_prov_form_field("ssid=x", "psk", v, sizeof(v)));
...
{
    char v[64];
    TEST_ASSERT_TRUE(detws_prov_form_field("myssid=wrong&ssid=right", "ssid", v, sizeof(v)));
...
{
    char v[4];
    TEST_ASSERT_TRUE(detws_prov_form_field("ssid=abcdef", "ssid", v, sizeof(v)));
