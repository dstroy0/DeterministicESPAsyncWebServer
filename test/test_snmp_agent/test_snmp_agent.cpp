                        ...
                        long f3, const uint32_t *oid, size_t oidn, const SnmpValue *setval)
{
    ...
    pc_ber_put_octet_string(&e, (uint8_t)SnmpTag::BER_OCTET_STRING, (const uint8_t *)comm, strlen(comm));
    ...
    {
        pc_ber_put_octet_string(&e, (uint8_t)SnmpTag::BER_OCTET_STRING, (const uint8_t *)setval->str, setval->str_len);
    }
    else
    {
        pc_ber_put_null(&e);
...
static size_t build_pdu(uint8_t *buf, size_t cap, int knob)
{
    BerEnc e;
    pc_ber_enc_init(&e, buf, cap);
    size_t pdus = pc_ber_seq_begin(&e, (uint8_t)SnmpTag::SNMP_PDU_GET);
    pc_ber_put_integer(&e, 42); // request-id
    pc_ber_put_integer(&e, 0);
    pc_ber_put_integer(&e, 0);
    if (knob == VB_BAD_VBL_TAG)
    {
        pc_ber_put_octet_string(&e, (uint8_t)SnmpTag::BER_OCTET_STRING, (const uint8_t *)"x",
    ...
    else
    {
        ...
        else if (knob == VB_BAD_OID_VALUE) // an OID-typed value that fails to decode (empty OID)
        {
            size_t vb = pc_ber_seq_begin(&e, (uint8_t)SnmpTag::BER_SEQUENCE);
            pc_ber_put_oid(&e, OID_SYSDESCR, 9);
            uint8_t empty_oid[2] = {(uint8_t)SnmpTag::BER_OID, 0x00}; // OID tag, zero length -> pc_ber_read_oid rejects
...
static bool parse_resp(const uint8_t *buf, size_t len, RespView *rv)
{
    ...
    uint8_t vt;
    size_t vl;
    if (!pc_ber_read_header(&d, &vt, &vl) || vt != (uint8_t)SnmpTag::BER_SEQUENCE)
    {
        return false;
    }
    ...
    while (d.pos < vend && d.ok)
    {
        uint8_t st;
        size_t sl;
        if (!pc_ber_read_header(&d, &st, &sl) || st != (uint8_t)SnmpTag::BER_SEQUENCE)
        {
            return false;
        }
...
void test_registration_and_rw_edges()
{
    ...
    TEST_ASSERT_FALSE(pc_snmp_agent_add_string(shortoid, 1, "x", nullptr));
    TEST_ASSERT_FALSE(pc_snmp_agent_add_integer(shortoid, 1, 5, nullptr));
    TEST_ASSERT_FALSE(pc_snmp_agent_add_dynamic(shortoid, 1, (uint8_t)SnmpTag::BER_INTEGER, nullptr));
    ...
    pc_snmp_agent_set_rw_community(nullptr);
    uint8_t req[256], resp[256];
    SnmpValue sv;
    memset(&sv, 0, sizeof(sv));
    sv.type = (uint8_t)SnmpTag::BER_INTEGER;
...
{
    TEST_ASSERT_TRUE(pc_snmp_agent_add_dynamic(OID_IP, 9, (uint8_t)SnmpTag::SNMP_IPADDRESS, ip_getter));
    uint8_t req[256], resp[256];
...
static size_t build_pdu_with_value(uint8_t *buf, size_t cap, uint8_t pdu_tag, int value_kind)
{
    ...
    if (value_kind == 1)
    {
        pc_ber_put_uint(&e, (uint8_t)SnmpTag::SNMP_GAUGE32, 500); // application uint value
    }
...
void test_get_failing_getter_is_nosuchinstance()
{
    ...
    TEST_ASSERT_TRUE(pc_snmp_agent_add_dynamic(OID_FAIL, 9, (uint8_t)SnmpTag::SNMP_COUNTER32, failing_getter));
    uint8_t req[256], resp[256];
                                  ...
                                  size_t oidn, size_t nvb)
{
    ...
    pc_ber_enc_init(&e, buf, cap);
    size_t msg = pc_ber_seq_begin(&e, (uint8_t)SnmpTag::BER_SEQUENCE);
    pc_ber_put_integer(&e, (int)SnmpVersion::SNMP_V2C);
    pc_ber_put_octet_string(&e, (uint8_t)SnmpTag::BER_OCTET_STRING, (const uint8_t *)"public", 6);
    size_t pdus = pc_ber_seq_begin(&e, (uint8_t)SnmpTag::SNMP_PDU_GETBULK);
    pc_ber_put_integer(&e, reqid);
...
void test_dispatch_empty_varbind_list_tiny_buffer()
{
    ...
    size_t p = pc_ber_seq_begin(&e, (uint8_t)SnmpTag::SNMP_PDU_GET);
    pc_ber_put_integer(&e, 1);
    pc_ber_put_integer(&e, 0);
    pc_ber_put_integer(&e, 0);
    size_t vbl = pc_ber_seq_begin(&e, (uint8_t)SnmpTag::BER_SEQUENCE);
