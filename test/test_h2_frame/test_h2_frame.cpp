...
//
// Unit tests for the HTTP/2 frame layer (network_drivers/presentation/http2/dws_h2_frame, RFC 9113):
...
void test_header_roundtrip()
{
    ...
    TEST_ASSERT_EQUAL_INT(9,
                          (int)dws_h2_write_header(b, sizeof b, 0x123456, H2FrameType::H2_HEADERS, 0x05, 0x7FFFFFFF));
    const uint8_t exp[9] = {0x12, 0x34, 0x56, 0x01, 0x05, 0x7F, 0xFF, 0xFF, 0xFF};
    TEST_ASSERT_EQUAL_UINT8_ARRAY(exp, b, 9);
    H2FrameHeader h;
    TEST_ASSERT_TRUE(dws_h2_parse_header(b, 9, &h));
    ...
    TEST_ASSERT_TRUE(dws_h2_parse_header(r, 9, &h));
    TEST_ASSERT_EQUAL_UINT32(0x7FFFFFFF, h.stream_id);
...
void test_settings_build_parse()
{
    ...
    uint8_t f[64];
    size_t n = dws_h2_build_settings(f, sizeof f, ids, vals, 2);
    ...
    H2Settings s;
    dws_h2_settings_defaults(&s);
    TEST_ASSERT_TRUE(dws_h2_parse_settings(f + 9, 12, &s));
...
void test_control_frames()
{
    ...
    TEST_ASSERT_EQUAL_INT(17, (int)dws_h2_build_ping_ack(b, sizeof b, op));
...
void test_headers_and_data()
{
    ...
    TEST_ASSERT_EQUAL_INT(10, (int)dws_h2_build_headers(b, sizeof b, 1, block, 1, true));
...
void test_settings_all_ids_and_build_guards()
{
    ...
    uint8_t f[128];
    size_t n = dws_h2_build_settings(f, sizeof f, ids, vals, 6);
    TEST_ASSERT_TRUE(dws_h2_parse_settings(f + 9, n - 9, &s));
    ...
    TEST_ASSERT_FALSE(dws_h2_parse_settings(bad_iws, 6, &s));
    const uint8_t bad_mfs_hi[6] = {0x00, 0x05, 0x01, 0x00, 0x00, 0x00}; // MAX_FRAME_SIZE = 2^24
    TEST_ASSERT_FALSE(dws_h2_parse_settings(bad_mfs_hi, 6, &s));
    // A too-short frame header is rejected.
    H2FrameHeader h;
    uint8_t tiny[4] = {0};
    TEST_ASSERT_FALSE(dws_h2_parse_header(tiny, sizeof(tiny), &h));
    ...
    uint8_t op[8] = {0};
    uint8_t blk[4] = {0};
    TEST_ASSERT_EQUAL_size_t(0, dws_h2_build_settings(small, sizeof(small), sid, sv, 1));
    TEST_ASSERT_EQUAL_size_t(0, dws_h2_build_window_update(small, sizeof(small), 1, 1));
    TEST_ASSERT_EQUAL_size_t(0, dws_h2_build_rst_stream(small, sizeof(small), 1, 0));
    TEST_ASSERT_EQUAL_size_t(0, dws_h2_build_goaway(small, sizeof(small), 1, 0));
    TEST_ASSERT_EQUAL_size_t(0, dws_h2_build_ping_ack(small, sizeof(small), op));
    TEST_ASSERT_EQUAL_size_t(0, dws_h2_build_headers(small, sizeof(small), 1, blk, sizeof(blk), false));
    TEST_ASSERT_EQUAL_size_t(0, dws_h2_build_data(small, sizeof(small), 1, blk, sizeof(blk), false));
}
