...
static bool resolver(const char *path, const pc_gql_args *args, pc_gql_value *out)
{
    ...
    if (!strcmp(path, "temp"))
    {
        out->type = pc_gql_type::PC_GQL_FLOAT;
        out->f = 21.5;
        return true;
    }
    ...
    {
        out->type = pc_gql_type::PC_GQL_NULL;
        return true;
    }
...
{
    pc_gql_result rc = pc_graphql_execute(q, strlen(q), resolver, out, sizeof(out));
    (void)rc;
    return out;
}

static pc_gql_result run_rc(const char *q)
{
    return pc_graphql_execute(q, strlen(q), resolver, out, sizeof(out));
}
...
{
    pc_gql_result rc = pc_graphql_execute("{ name ", 7, resolver, out, sizeof(out));
    TEST_ASSERT_EQUAL_INT(pc_gql_result::PC_GQL_ERR_PARSE, rc);
    TEST_ASSERT_NOT_NULL(strstr(out, "\"errors\""));
}

void test_mutation_rejected()
{
    pc_gql_result rc = pc_graphql_execute("mutation { x }", 14, resolver, out, sizeof(out));
    TEST_ASSERT_EQUAL_INT(pc_gql_result::PC_GQL_ERR_PARSE, rc);
}

void test_depth_limit()
{
    pc_gql_result rc = pc_graphql_execute("{a{b{c{d{e{f{g}}}}}}}", 21, resolver, out, sizeof(out));
    TEST_ASSERT_EQUAL_INT(pc_gql_result::PC_GQL_ERR_LIMIT, rc);
}

void test_overflow_fails_closed()
{
    char tiny[8];
    pc_gql_result rc = pc_graphql_execute("{ name uptime }", 15, resolver, tiny, sizeof(tiny));
    TEST_ASSERT_EQUAL_INT(pc_gql_result::PC_GQL_ERR_OVERFLOW, rc);
}

void test_string_escapes_decoded()
{
    // \n \t \r \\ \/ and an unknown escape (\z) are all decoded by the arg lexer.
    TEST_ASSERT_EQUAL_INT(pc_gql_result::PC_GQL_OK, run_rc("{ greet(name: \"a\\nb\\tc\\rd\\\\e\\/f\\z\") }"));
...
void test_number_arg_variants_parse()
{
    ...
    TEST_ASSERT_EQUAL_INT(pc_gql_result::PC_GQL_OK,
...
{
    TEST_ASSERT_EQUAL_INT(pc_gql_result::PC_GQL_ERR_PARSE, run_rc("{ greet(name: \"oops) }"));
}

void test_arg_missing_colon_fails()
{
    TEST_ASSERT_EQUAL_INT(pc_gql_result::PC_GQL_ERR_PARSE, run_rc("{ sensor(id 7) { value } }"));
}

void test_bad_arg_value_fails()
{
    TEST_ASSERT_EQUAL_INT(pc_gql_result::PC_GQL_ERR_PARSE, run_rc("{ greet(name: @) }"));
}

void test_trailing_junk_fails()
{
    TEST_ASSERT_EQUAL_INT(pc_gql_result::PC_GQL_ERR_PARSE, run_rc("{ name } junk"));
}
...
void test_long_field_name_hits_limit()
{
    ...
    q[n++] = '}';
    q[n] = '\0';
    TEST_ASSERT_EQUAL_INT(pc_gql_result::PC_GQL_ERR_LIMIT,
                          pc_graphql_execute(q, strlen(q), resolver, out, sizeof(out)));
}

void test_null_inputs_fail_closed()
{
    TEST_ASSERT_EQUAL_INT(pc_gql_result::PC_GQL_ERR_PARSE, pc_graphql_execute(0, 0, resolver, out, sizeof(out)));
    TEST_ASSERT_EQUAL_INT(pc_gql_result::PC_GQL_ERR_PARSE, pc_graphql_execute("{ name }", 8, resolver, 0, 0));
}

void test_unknown_operation_keyword_fails()
{
    TEST_ASSERT_EQUAL_INT(pc_gql_result::PC_GQL_ERR_PARSE, run_rc("subscription { name }"));
}
...
{
    TEST_ASSERT_EQUAL_INT(pc_gql_result::PC_GQL_ERR_PARSE, run_rc("{ f(a: -) }"));
    TEST_ASSERT_EQUAL_INT(pc_gql_result::PC_GQL_ERR_PARSE, run_rc("{ f(: 1) }"));
    TEST_ASSERT_EQUAL_INT(pc_gql_result::PC_GQL_ERR_PARSE, run_rc("{ 1 }"));
}

// `query Op` with no selection set, and `query` followed by a non-name, are parse errors.
void test_query_keyword_forms_fail()
{
    TEST_ASSERT_EQUAL_INT(pc_gql_result::PC_GQL_ERR_PARSE, run_rc("query Op"));  // no selection set follows
    TEST_ASSERT_EQUAL_INT(pc_gql_result::PC_GQL_ERR_PARSE, run_rc("query 123")); // bad operation name
}
...
void test_pool_limits()
{
    ...
    for (int i = 0; i < 25; i++) // > PC_GQL_MAX_ARGS (24)
    {
        n += snprintf(q + n, sizeof(q) - n, "a%d:1 ", i);
    }
    n += snprintf(q + n, sizeof(q) - n, ") }");
    TEST_ASSERT_EQUAL_INT(pc_gql_result::PC_GQL_ERR_LIMIT, run_rc(q));

    n = 0;
    n += snprintf(q + n, sizeof(q) - n, "{ ");
    for (int i = 0; i < 49; i++) // > PC_GQL_MAX_NODES (48)
    {
...
void test_string_pool_exhaustion()
{
    ...
    int n = 0;
    n += snprintf(q + n, sizeof(q) - n, "{ f(");
    for (int i = 0; i < 8; i++) // 8 x 40 bytes = 320 > PC_GQL_STRBUF (256)
    {
...
// A resolver path longer than PC_GQL_PATH_MAX (96) fails closed as an overflow, both
...
void test_resolver_path_overflow()
{
    ...
    TEST_ASSERT_EQUAL_INT(pc_gql_result::PC_GQL_ERR_OVERFLOW, run_rc(q));
...
void test_arg_accessors_edges()
{
    ...
    bool b = false;
    TEST_ASSERT_FALSE(pc_gql_arg_int(nullptr, "x", &i));
    TEST_ASSERT_FALSE(pc_gql_arg_str(nullptr, "x", &s));
    TEST_ASSERT_FALSE(pc_gql_arg_bool(nullptr, "x", &b));
...
void test_query_keyword_without_operation_name()
{
    ...
    TEST_ASSERT_EQUAL_INT(pc_gql_result::PC_GQL_ERR_PARSE, run_rc("query"));
    TEST_ASSERT_EQUAL_INT(pc_gql_result::PC_GQL_ERR_PARSE, run_rc("@ { name }"));
}
...
{
    TEST_ASSERT_EQUAL_INT(pc_gql_result::PC_GQL_ERR_PARSE, run_rc("{ greet(name: \"ab\\"));
}
...
{
    TEST_ASSERT_EQUAL_INT(pc_gql_result::PC_GQL_ERR_PARSE, run_rc("{ f(a:12"));
    TEST_ASSERT_EQUAL_INT(pc_gql_result::PC_GQL_ERR_PARSE, run_rc("{ f(a:1.5"));
    TEST_ASSERT_EQUAL_INT(pc_gql_result::PC_GQL_ERR_PARSE, run_rc("{ f(a:1e"));
    TEST_ASSERT_EQUAL_INT(pc_gql_result::PC_GQL_ERR_PARSE, run_rc("{ f(a:1e5"));
}
...
void test_number_exponent_and_sign_forms()
{
    ...
    TEST_ASSERT_EQUAL_INT(pc_gql_result::PC_GQL_ERR_PARSE, run_rc("{ f(a:1e2}"));
}
...
{
    TEST_ASSERT_EQUAL_INT(pc_gql_result::PC_GQL_ERR_PARSE, run_rc("{ f(a: !) }"));
    TEST_ASSERT_EQUAL_INT(pc_gql_result::PC_GQL_ERR_PARSE, run_rc("{ f(a: ENUM) }"));
}
...
{
    TEST_ASSERT_EQUAL_INT(pc_gql_result::PC_GQL_OK,
                          pc_graphql_execute("{ name uptime }", 15, nullptr, out, sizeof(out)));
...
{
    char buf[4] = {'x', 'x', 'x', 'x'};
    TEST_ASSERT_EQUAL_INT(pc_gql_result::PC_GQL_ERR_PARSE, pc_graphql_execute("{ name }", 8, resolver, buf, 0));
...
void test_error_document_capacity_edges()
{
    ...
    TEST_ASSERT_EQUAL_INT(pc_gql_result::PC_GQL_ERR_PARSE, pc_graphql_execute("{", 1, resolver, buf, 8));
    TEST_ASSERT_EQUAL_CHAR('\0', buf[0]); // overflowed on the first write -> nothing emitted

    memset(buf, 0, sizeof(buf));
    TEST_ASSERT_EQUAL_INT(pc_gql_result::PC_GQL_ERR_PARSE, pc_graphql_execute("{", 1, resolver, buf, sizeof(kErr) - 1));
...
void test_data_document_exact_fit_is_overflow()
{
    ...
    TEST_ASSERT_EQUAL_INT(pc_gql_result::PC_GQL_ERR_OVERFLOW, pc_graphql_execute("{a}", 3, resolver, buf, 19));
    TEST_ASSERT_EQUAL_INT(pc_gql_result::PC_GQL_OK, pc_graphql_execute("{a}", 3, resolver, buf, 20));
...
// REGRESSION TEST for a stack buffer overflow: parse_name() was bounded only by PC_GQL_NAME_MAX
...
static void test_long_bareword_argument_does_not_overflow_keyword_scratch()
{
    ...
    TEST_ASSERT_EQUAL_INT(pc_gql_result::PC_GQL_ERR_PARSE, run_rc("{ f(a: LONGENUMVALUE) }"));
    // Well past PC_GQL_NAME_MAX (32) as well, not just past the scratch.
    TEST_ASSERT_EQUAL_INT(pc_gql_result::PC_GQL_ERR_PARSE,
                          run_rc("{ f(a: AAAAAAAAAABBBBBBBBBBCCCCCCCCCCDDDDDDDDDD) }"));
    // Exactly at the old 8-byte scratch boundary.
    TEST_ASSERT_EQUAL_INT(pc_gql_result::PC_GQL_ERR_PARSE, run_rc("{ f(a: ENUMVALU) }"));
    // The keywords the scratch actually exists for still parse.
    TEST_ASSERT_EQUAL_INT(pc_gql_result::PC_GQL_OK, run_rc("{ f(a: true) }"));
    TEST_ASSERT_EQUAL_INT(pc_gql_result::PC_GQL_OK, run_rc("{ f(a: false) }"));
    TEST_ASSERT_EQUAL_INT(pc_gql_result::PC_GQL_OK, run_rc("{ f(a: null) }"));
}
