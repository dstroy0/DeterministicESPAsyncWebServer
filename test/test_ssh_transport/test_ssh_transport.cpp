...
{
    dws_ssh_hostkey_ed25519_set(BASELINE_ED25519_SEEDS[0]); // deterministic baseline host key
...
void test_kexinit_parse_selects_ecdsa()
{
    ...
    TEST_ASSERT_TRUE(dws_ssh_hostkey_ecdsa_available());
...
{
    dws_ssh_hostkey_ed25519_set(BASELINE_ED25519_SEEDS[0]); // any host key so negotiation can complete
    uint8_t buf[SSH_KEXINIT_MAX];
...
void test_kexdh_handle_curve25519_rejects_low_order()
{
    ...
    ssh_transport_init(0);
    dws_ssh_hostkey_ed25519_set(zero_seed);
...
void test_kexdh_handle_rsa_sha512_signature()
{
    ...
    uint8_t em[256];
    const size_t total = DWS_PKCS1_SHA512_DIGESTINFO_LEN + DWS_SHA512_DIGEST_LEN; // 83
    const size_t pad = 256 - 3 - total;                                           // 170
    em[0] = 0x00;
    em[1] = 0x01;
    memset(em + 2, 0xFF, pad);
    em[2 + pad] = 0x00;
    memcpy(em + 3 + pad, dws_pkcs1_sha512_digestinfo, DWS_PKCS1_SHA512_DIGESTINFO_LEN);
    memcpy(em + 3 + pad + DWS_PKCS1_SHA512_DIGESTINFO_LEN, d512, DWS_SHA512_DIGEST_LEN);
