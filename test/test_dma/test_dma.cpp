...
//
// The env sizes DWS_DMA_BUF_SIZE = 8, DWS_DMA_CHANNELS = 2 (staging = 24).
...
static void on_ev(const dws_dma_event *ev, void *ctx)
{
    (void)ctx;
    Ev e;
    e.dir = ev->dir;
    ...
    e.seq = ev->seq;
    e.ptr = ev->data;
    if (ev->dir == dws_dma_dir::DWS_DMA_RX && ev->data)
...
static bool open_ch(uint8_t ch, bool loop)
{
    dws_dma_config c = {};
    c.channel = ch;
    c.periph = dws_dma_periph::DWS_DMA_UART;
    c.loopback = loop;
    c.on_complete = on_ev;
    c.ctx = nullptr;
    return dws_dma_open(&c);
}
...
static std::vector<uint8_t> rx_concat()
{
    ...
    for (size_t i = 0; i < g_ev.size(); i++)
        if (g_ev[i].dir == dws_dma_dir::DWS_DMA_RX)
            v.insert(v.end(), g_ev[i].data.begin(), g_ev[i].data.end());
    return v;
}

static size_t count_dir(dws_dma_dir dir)
{
    size_t n = 0;
    for (size_t i = 0; i < g_ev.size(); i++)
        if (g_ev[i].dir == dir)
...
{
    TEST_ASSERT_FALSE(dws_dma_open(nullptr));
    dws_dma_config c = {};
    c.channel = 0;
    c.on_complete = nullptr; // null callback
    TEST_ASSERT_FALSE(dws_dma_open(&c));
    c.on_complete = on_ev;
    c.channel = DWS_DMA_CHANNELS; // out of range
    TEST_ASSERT_FALSE(dws_dma_open(&c));
...
void test_ingress_emits_rx_event()
{
    ...
    TEST_ASSERT_TRUE(dws_dma_sim_feed(0, msg, sizeof(msg)));
    TEST_ASSERT_EQUAL_size_t(0, g_ev.size()); // nothing until we pump the engine
    dws_dma_poll();
    TEST_ASSERT_EQUAL_size_t(1, count_dir(dws_dma_dir::DWS_DMA_RX));
...
{
    TEST_ASSERT_TRUE(open_ch(0, false));
    uint8_t msg[DWS_DMA_BUF_SIZE + 3];
    for (size_t i = 0; i < sizeof(msg); i++)
        msg[i] = (uint8_t)i;
    TEST_ASSERT_TRUE(dws_dma_sim_feed(0, msg, sizeof(msg)));
    dws_dma_poll();
    // one full-buffer completion + one partial idle-line flush
    TEST_ASSERT_EQUAL_size_t(2, count_dir(dws_dma_dir::DWS_DMA_RX));
    TEST_ASSERT_EQUAL_UINT16(DWS_DMA_BUF_SIZE, g_ev[0].len);
...
{
    TEST_ASSERT_TRUE(open_ch(0, false));
    uint8_t msg[DWS_DMA_BUF_SIZE * 2]; // exactly two full buffers
    for (size_t i = 0; i < sizeof(msg); i++)
...
void test_egress_captures_tx()
{
    ...
    TEST_ASSERT_TRUE(dws_dma_tx_submit(0, out, sizeof(out)));
    dws_dma_poll();
    TEST_ASSERT_EQUAL_size_t(1, count_dir(dws_dma_dir::DWS_DMA_TX));
    TEST_ASSERT_EQUAL_size_t(0, count_dir(dws_dma_dir::DWS_DMA_RX)); // no loopback -> no RX
    TEST_ASSERT_EQUAL_UINT16(4, g_ev[0].len);
    TEST_ASSERT_NULL(g_ev[0].ptr); // TX events carry no buffer

    uint8_t cap[16];
    uint16_t n = dws_dma_sim_capture(0, cap, sizeof(cap));
...
void test_tx_one_in_flight_fail_closed()
{
    ...
    const uint8_t b[] = {4, 5};
    TEST_ASSERT_TRUE(dws_dma_tx_submit(0, a, sizeof(a)));
    TEST_ASSERT_FALSE(dws_dma_tx_submit(0, b, sizeof(b))); // busy -> fail-closed
    dws_dma_poll();                                        // TX completes, frees the channel
    TEST_ASSERT_TRUE(dws_dma_tx_submit(0, b, sizeof(b)));
    dws_dma_poll();
    TEST_ASSERT_EQUAL_size_t(2, count_dir(dws_dma_dir::DWS_DMA_TX));
}
...
void test_tx_rejects_bad_len()
{
    TEST_ASSERT_TRUE(open_ch(0, false));
    const uint8_t x[1] = {9};
    TEST_ASSERT_FALSE(dws_dma_tx_submit(0, x, 0)); // zero length
    uint8_t big[DWS_DMA_BUF_SIZE + 1] = {0};
    TEST_ASSERT_FALSE(dws_dma_tx_submit(0, big, DWS_DMA_BUF_SIZE + 1)); // oversize
    TEST_ASSERT_FALSE(dws_dma_tx_submit(0, nullptr, 4));                // null buffer
}
...
void test_loopback_round_trip()
{
    ...
    TEST_ASSERT_TRUE(dws_dma_tx_submit(0, ping, sizeof(ping)));
    dws_dma_poll(); // one poll: TX drains, loops into ingress, RX completes
    TEST_ASSERT_EQUAL_size_t(1, count_dir(dws_dma_dir::DWS_DMA_TX));
    TEST_ASSERT_EQUAL_size_t(1, count_dir(dws_dma_dir::DWS_DMA_RX));
    std::vector<uint8_t> got = rx_concat();
...
{
    TEST_ASSERT_TRUE(open_ch(0, false));
    uint8_t big[DWS_DMA_BUF_SIZE * 3 + 1] = {0};
    TEST_ASSERT_FALSE(dws_dma_sim_feed(0, big, sizeof(big))); // past staging -> reject whole
    uint8_t ok[DWS_DMA_BUF_SIZE * 3] = {0};
    TEST_ASSERT_TRUE(dws_dma_sim_feed(0, ok, sizeof(ok))); // exactly fits
}

void test_closed_channel_is_inert()
{
    const uint8_t x[] = {1, 2, 3};
    TEST_ASSERT_FALSE(dws_dma_sim_feed(0, x, sizeof(x))); // never opened
    TEST_ASSERT_FALSE(dws_dma_tx_submit(0, x, sizeof(x)));
    dws_dma_poll();
    TEST_ASSERT_EQUAL_size_t(0, g_ev.size());
    TEST_ASSERT_TRUE(open_ch(0, false));
    dws_dma_close(0);
    TEST_ASSERT_FALSE(dws_dma_sim_feed(0, x, sizeof(x))); // closed again
}
...
void test_two_channels_independent()
{
    ...
    TEST_ASSERT_TRUE(dws_dma_sim_feed(0, a, sizeof(a)));
    TEST_ASSERT_TRUE(dws_dma_sim_feed(1, b, sizeof(b)));
    dws_dma_poll();
    size_t ch0 = 0, ch1 = 0;
    for (size_t i = 0; i < g_ev.size(); i++)
    {
...
{
    dws_dma_close(255); // out-of-range close is a no-op
    dws_dma_close(0);   // ensure channel 0 is closed
    uint8_t b[4] = {0};
