...
#include "services/smb/smb2.h" // dws_smb3_derive_signing_key
#include <string.h>
#include <unity.h>
...
void test_sha512_vectors()
{
    ...
    dws_sha512(big, 136, d); // 136 = 128 + 8: one full block in update, then 8 bytes into the final block
    const uint8_t b136[8] = {0xfe, 0xda, 0x01, 0x51, 0x00, 0x00, 0x00, 0x00};
    TEST_ASSERT_EQUAL_HEX8_ARRAY(b136, d, 4);
    dws_sha512(big, 200, d);
    ...
    dws_sha512_init(&sc);
    dws_sha512_update(&sc, A, 64);
    dws_sha512_update(&sc, B, 72);
    dws_sha512_final(&sc, strm);
...
void test_smb3_derive_signing_key()
{
    ...
    uint8_t out[16] = {0};
    TEST_ASSERT_TRUE(dws_smb3_derive_signing_key(session_key, (uint16_t)Smb2Dialect::SMB2_DIALECT_0300, nullptr, out));
    TEST_ASSERT_EQUAL_HEX8_ARRAY(k30, out, 16);
    // 3.0.2 derives identically (pre-3.1.1 path); the preauth hash is ignored there.
    memset(out, 0, 16);
    TEST_ASSERT_TRUE(dws_smb3_derive_signing_key(session_key, (uint16_t)Smb2Dialect::SMB2_DIALECT_0302, preauth, out));
    TEST_ASSERT_EQUAL_HEX8_ARRAY(k30, out, 16);
    ...
    memset(out, 0, 16);
    TEST_ASSERT_TRUE(dws_smb3_derive_signing_key(session_key, (uint16_t)Smb2Dialect::SMB2_DIALECT_0311, preauth, out));
    TEST_ASSERT_EQUAL_HEX8_ARRAY(k311, out, 16);

    // fail-closed: null pointers, and a 3.1.1 request with no preauth hash.
    TEST_ASSERT_FALSE(dws_smb3_derive_signing_key(nullptr, (uint16_t)Smb2Dialect::SMB2_DIALECT_0300, nullptr, out));
    TEST_ASSERT_FALSE(
        dws_smb3_derive_signing_key(session_key, (uint16_t)Smb2Dialect::SMB2_DIALECT_0300, nullptr, nullptr));
    TEST_ASSERT_FALSE(dws_smb3_derive_signing_key(session_key, (uint16_t)Smb2Dialect::SMB2_DIALECT_0311, nullptr, out));
}
