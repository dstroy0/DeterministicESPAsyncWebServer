...
{
    pc_config_begin("ns");
    TEST_ASSERT_FALSE(pc_config_set_str("k", nullptr));                     // null value
    TEST_ASSERT_EQUAL_size_t(0, pc_config_get_str("k", nullptr, 0, "def")); // null out
    TEST_ASSERT_FALSE(pc_config_set_blob("k", nullptr, 4));                 // null data
}

void test_key_ok_rejects_null_and_empty_key()
{
    TEST_ASSERT_FALSE(pc_config_set_str(nullptr, "v")); // null key
    char out[8];
    size_t n = pc_config_get_str("", out, sizeof(out), nullptr); // empty key, no default
...
{
    TEST_ASSERT_EQUAL_UINT32(5u, pc_config_get_u32(nullptr, 5u));
}

void test_get_blob_rejects_invalid_key()
{
    uint8_t out[4];
    TEST_ASSERT_EQUAL_size_t(0, pc_config_get_blob(nullptr, out, sizeof(out)));
}
...
void test_get_blob_null_out_with_existing_key()
{
    ...
    TEST_ASSERT_EQUAL_size_t(0, pc_config_get_blob("bk", nullptr, 8));
}
...
{
    TEST_ASSERT_FALSE(pc_config_erase(nullptr));
}
