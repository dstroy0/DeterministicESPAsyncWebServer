...
void test_asserts_on_active_and_counts_edge()
{
    ...
    TEST_ASSERT_FALSE(pc_sen0192_motion_update(&m, false, 500));
    TEST_ASSERT_FALSE(pc_sen0192_motion_present(&m));

    // First active (high) sample -> presence starts (returns the edge), one motion event.
    TEST_ASSERT_TRUE(pc_sen0192_motion_update(&m, true, 1000));
    ...
    TEST_ASSERT_FALSE(pc_sen0192_motion_update(&m, true, 1500));
    TEST_ASSERT_EQUAL_UINT32(1, pc_sen0192_motion_events(&m));
}
...
void test_holds_then_clears_after_window()
{
    Sen0192Motion m;
    pc_sen0192_motion_init(&m, 2000, true);
    pc_sen0192_motion_update(&m, true, 1000); // present, last_active=1000
...
{
    Sen0192Motion m;
    pc_sen0192_motion_init(&m, 1000, true);
    TEST_ASSERT_TRUE(pc_sen0192_motion_update(&m, true, 100)); // event 1
    pc_sen0192_motion_tick(&m, 1200);                          // clears (1100 > 1000)
    TEST_ASSERT_FALSE(pc_sen0192_motion_present(&m));
    TEST_ASSERT_TRUE(pc_sen0192_motion_update(&m, true, 2000)); // event 2 (new edge)
...
{
    Sen0192Motion m;
    pc_sen0192_motion_init(&m, 1000, false);                    // OUT reads LOW on motion
    TEST_ASSERT_FALSE(pc_sen0192_motion_update(&m, true, 100)); // HIGH is inactive here
    TEST_ASSERT_FALSE(pc_sen0192_motion_present(&m));
    TEST_ASSERT_TRUE(pc_sen0192_motion_update(&m, false, 200)); // LOW is active
    TEST_ASSERT_TRUE(pc_sen0192_motion_present(&m));
    TEST_ASSERT_EQUAL_UINT32(1, pc_sen0192_motion_events(&m));
}
