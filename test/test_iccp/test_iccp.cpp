...
{
    uint8_t out[16];
    size_t n = pc_iccp_state_q(Iccp::ICCP_STATE_ON, Iccp::ICCP_QUAL_VALID, nullptr, out, sizeof(out));
...
void test_state_q_with_time(void)
{
    ...
    uint8_t out[24];
    size_t n = pc_iccp_state_q(Iccp::ICCP_STATE_OFF, Iccp::ICCP_QUAL_SUSPECT, time, out, sizeof(out));
...
{
    uint8_t out[24];
    size_t n = pc_iccp_real_q(12345, Iccp::ICCP_QUAL_VALID, nullptr, out, sizeof(out));
...
{
    uint8_t out[24];
    size_t n = pc_iccp_real_q(-1, Iccp::ICCP_QUAL_VALID, nullptr, out, sizeof(out));
    // -1 -> minimal two's complement INTEGER 02 01 FF.
    const uint8_t iv[] = {0x02, 0x01, 0xFF};
    TEST_ASSERT_TRUE(find(out, n, iv, 3) >= 0);
    // -256 -> 02 02 FF 00.
    n = pc_iccp_real_q(-256, Iccp::ICCP_QUAL_VALID, nullptr, out, sizeof(out));
...
void test_state_and_real_q_guards()
{
    uint8_t out[64];
    uint8_t t[4] = {0, 0, 0, 0};
    TEST_ASSERT_EQUAL_size_t(0, pc_iccp_state_q(1, 0, t, nullptr, sizeof(out)));  // null out
    ...
    TEST_ASSERT_EQUAL_size_t(0, pc_iccp_real_q(100, 0, t, nullptr, sizeof(out))); // null out
...
void test_real_q_positive_needs_pad_byte(void)
{
    ...
    uint8_t out[24];
    size_t n = pc_iccp_real_q(128, Iccp::ICCP_QUAL_VALID, nullptr, out, sizeof(out));
