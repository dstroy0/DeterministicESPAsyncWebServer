...
static void test_build_status()
{
    ...
    TEST_ASSERT_TRUE(pc_sftp_rd_string(&r, nullptr, &ml)); // language tag (empty)
...
static void test_build_name1_realpath()
{
    ...
    pc_sftp_rd_string(&r, nullptr, nullptr); // longname
    SftpAttrs ra;
...
static void test_reader_latches_the_first_underflow()
{
    ...
    TEST_ASSERT_FALSE(pc_sftp_rd_string(&r, nullptr, nullptr));
    TEST_ASSERT_FALSE(r.ok);
}
...
static void test_build_status_without_a_message()
{
    ...
    uint8_t buf[64];
    size_t n = pc_sftp_build_status(4, PC_SSH_FX_OK, nullptr, buf, sizeof(buf));
    TEST_ASSERT_TRUE(n > 0);
    SftpReader r;
    pc_sftp_rd_init(&r, buf + 4, n - 4);
    TEST_ASSERT_EQUAL_UINT8(PC_SSH_FXP_STATUS, pc_sftp_rd_u8(&r));
    ...
    uint32_t ml = 0xFFFF;
    TEST_ASSERT_TRUE(pc_sftp_rd_string(&r, nullptr, &ml));
