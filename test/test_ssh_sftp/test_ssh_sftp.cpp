...
static void test_rw_roundtrip()
{
    uint8_t buf[64];
    SftpWriter w;
    sftp_wr_init(&w, buf, sizeof(buf));
    sftp_wr_u8(&w, 0xAB);
    sftp_wr_u32(&w, 0x11223344);
    sftp_wr_u64(&w, 0x0102030405060708ULL);
    sftp_wr_string(&w, "hello", 5);
    size_t total = sftp_wr_finish(&w);
    ...
    TEST_ASSERT_EQUAL_HEX8(0xAB, sftp_rd_u8(&r));
    TEST_ASSERT_EQUAL_HEX32(0x11223344, sftp_rd_u32(&r));
    TEST_ASSERT_EQUAL_HEX64(0x0102030405060708ULL, sftp_rd_u64(&r));
    const uint8_t *s = nullptr;
    uint32_t sl = 0;
    TEST_ASSERT_TRUE(sftp_rd_string(&r, &s, &sl));
...
static void test_reader_bounds()
{
    uint8_t buf[3] = {0, 0, 1};
    SftpReader r;
    sftp_rd_init(&r, buf, sizeof(buf));
    sftp_rd_u32(&r); // wants 4, only 3 -> underflow
    ...
    sftp_rd_init(&r, s, sizeof(s));
    const uint8_t *p = nullptr;
    uint32_t n = 0;
    TEST_ASSERT_FALSE(sftp_rd_string(&r, &p, &n));
    TEST_ASSERT_FALSE(r.ok);
}
...
static void test_attrs_roundtrip()
{
    ...
    SftpReader r;
    sftp_rd_init(&r, buf + 4, plen);
    SftpAttrs out;
    TEST_ASSERT_TRUE(sftp_rd_attrs(&r, &out));
...
static void test_attrs_skips_uidgid_and_extended()
{
    ...
    SftpWriter w;
    sftp_wr_init(&w, buf, sizeof(buf));
    sftp_wr_u32(&w, SSH_FILEXFER_ATTR_UIDGID | SSH_FILEXFER_ATTR_PERMISSIONS | SSH_FILEXFER_ATTR_EXTENDED);
    sftp_wr_u32(&w, 1000); // uid
    sftp_wr_u32(&w, 1000); // gid
    sftp_wr_u32(&w, SFTP_S_IFDIR | 0755);
    sftp_wr_u32(&w, 1); // extended_count
    sftp_wr_string(&w, "x@y", 3);
    sftp_wr_string(&w, "v", 1);
    size_t total = sftp_wr_finish(&w);
...
static void test_framing()
{
    ...
    TEST_ASSERT_EQUAL_UINT(0, sftp_frame_len(buf, 3, sizeof(buf))); // < 4 -> need more
    TEST_ASSERT_EQUAL_UINT(9, sftp_frame_len(buf, 4, sizeof(buf))); // header present -> total 4+5
    TEST_ASSERT_EQUAL_UINT(9, sftp_frame_len(buf, 9, sizeof(buf))); // whole packet
    uint8_t zero[4] = {0, 0, 0, 0};                                 // zero-length -> malformed
    TEST_ASSERT_EQUAL_UINT((size_t)-1, sftp_frame_len(zero, 4, sizeof(buf)));
    uint8_t big[4] = {0, 0, 0xFF, 0xFF}; // 65535 payload, max 16 -> too big
    TEST_ASSERT_EQUAL_UINT((size_t)-1, sftp_frame_len(big, 4, 16));
}
...
static void test_parse_open_request()
{
    ...
    SftpWriter w;
    sftp_wr_init(&w, buf, sizeof(buf));
    sftp_wr_u8(&w, SSH_FXP_OPEN);
    sftp_wr_u32(&w, 42);
    sftp_wr_string(&w, "/foo.txt", 8);
    sftp_wr_u32(&w, SSH_FXF_WRITE | SSH_FXF_CREAT | SSH_FXF_TRUNC);
    SftpAttrs empty = {0, 0, 0, 0, 0};
    sftp_wr_attrs(&w, &empty);
    size_t total = sftp_wr_finish(&w);

    TEST_ASSERT_EQUAL_UINT(total, sftp_frame_len(buf, total, sizeof(buf)));
    SftpReader r;
    sftp_rd_init(&r, buf + 4, total - 4);
    TEST_ASSERT_EQUAL_UINT8(SSH_FXP_OPEN, sftp_rd_u8(&r));
    TEST_ASSERT_EQUAL_UINT32(42, sftp_rd_u32(&r));
    const uint8_t *path = nullptr;
    uint32_t plen = 0;
    TEST_ASSERT_TRUE(sftp_rd_string(&r, &path, &plen));
    TEST_ASSERT_EQUAL_UINT(8, plen);
    TEST_ASSERT_EQUAL_MEMORY("/foo.txt", path, 8);
    TEST_ASSERT_EQUAL_HEX32(SSH_FXF_WRITE | SSH_FXF_CREAT | SSH_FXF_TRUNC, sftp_rd_u32(&r));
    SftpAttrs a;
    TEST_ASSERT_TRUE(sftp_rd_attrs(&r, &a));
...
{
    uint8_t buf[16];
    size_t n = sftp_build_version(buf, sizeof(buf));
    SftpReader r;
    sftp_rd_init(&r, buf + 4, n - 4);
    TEST_ASSERT_EQUAL_UINT8(SSH_FXP_VERSION, sftp_rd_u8(&r));
    TEST_ASSERT_EQUAL_UINT32(SFTP_VERSION, sftp_rd_u32(&r));
}

static void test_build_status()
{
    uint8_t buf[64];
    size_t n = sftp_build_status(7, SSH_FX_NO_SUCH_FILE, "nope", buf, sizeof(buf));
    SftpReader r;
    sftp_rd_init(&r, buf + 4, n - 4);
    TEST_ASSERT_EQUAL_UINT8(SSH_FXP_STATUS, sftp_rd_u8(&r));
    TEST_ASSERT_EQUAL_UINT32(7, sftp_rd_u32(&r));
    TEST_ASSERT_EQUAL_UINT32(SSH_FX_NO_SUCH_FILE, sftp_rd_u32(&r));
    const uint8_t *m = nullptr;
    uint32_t ml = 0;
    TEST_ASSERT_TRUE(sftp_rd_string(&r, &m, &ml));
    TEST_ASSERT_EQUAL_MEMORY("nope", m, 4);
    TEST_ASSERT_TRUE(sftp_rd_string(&r, nullptr, &ml)); // language tag (empty)
...
{
    uint8_t buf[64];
    size_t n = sftp_build_handle(3, "\x00\x00\x00\x02", 4, buf, sizeof(buf));
    SftpReader r;
    sftp_rd_init(&r, buf + 4, n - 4);
    TEST_ASSERT_EQUAL_UINT8(SSH_FXP_HANDLE, sftp_rd_u8(&r));
    TEST_ASSERT_EQUAL_UINT32(3, sftp_rd_u32(&r));
    ...
    n = sftp_build_data(9, payload, sizeof(payload), buf, sizeof(buf));
    sftp_rd_init(&r, buf + 4, n - 4);
    TEST_ASSERT_EQUAL_UINT8(SSH_FXP_DATA, sftp_rd_u8(&r));
    TEST_ASSERT_EQUAL_UINT32(9, sftp_rd_u32(&r));
    const uint8_t *d = nullptr;
    uint32_t dl = 0;
    sftp_rd_string(&r, &d, &dl);
...
static void test_build_name1_realpath()
{
    ...
    a.atime = a.mtime = 0;
    uint8_t buf[128];
    size_t n = sftp_build_name1(5, "/gcode/part.nc", "-rw-r--r-- 1 0 0 1234 Jan  1 2026 part.nc", &a, buf, sizeof(buf));
    SftpReader r;
    sftp_rd_init(&r, buf + 4, n - 4);
    TEST_ASSERT_EQUAL_UINT8(SSH_FXP_NAME, sftp_rd_u8(&r));
    TEST_ASSERT_EQUAL_UINT32(5, sftp_rd_u32(&r));
    TEST_ASSERT_EQUAL_UINT32(1, sftp_rd_u32(&r)); // one entry
    const uint8_t *nm = nullptr;
    uint32_t nl = 0;
    sftp_rd_string(&r, &nm, &nl);
    TEST_ASSERT_EQUAL_MEMORY("/gcode/part.nc", nm, nl);
    sftp_rd_string(&r, nullptr, nullptr); // longname
    SftpAttrs ra;
    TEST_ASSERT_TRUE(sftp_rd_attrs(&r, &ra));
...
static void test_name_multi_entry()
{
    ...
    sftp_wr_u32(&w, 11);
    size_t count_at = sftp_wr_pos(&w); // remember where the count u32 goes
    sftp_wr_u32(&w, 0);                // placeholder count
    uint32_t count = 0;
    for (int i = 0; i < 3; i++)
    {
        SftpAttrs a;
        ...
        a.atime = a.mtime = 0;
        char ln[64];
        sftp_format_longname(dirs[i], a.permissions, 0, 0, names[i], ln, sizeof(ln));
        sftp_wr_string(&w, names[i], (uint32_t)strlen(names[i]));
    ...
    sftp_rd_init(&r, buf + 4, total - 4);
    TEST_ASSERT_EQUAL_UINT8(SSH_FXP_NAME, sftp_rd_u8(&r));
    TEST_ASSERT_EQUAL_UINT32(11, sftp_rd_u32(&r));
    TEST_ASSERT_EQUAL_UINT32(3, sftp_rd_u32(&r)); // count patched
    for (int i = 0; i < 3; i++)
    {
        const uint8_t *nm = nullptr;
        uint32_t nl = 0;
...
{
    char out[64];
    size_t n = sftp_format_longname(false, 0644, 1234, 0, "file.nc", out, sizeof(out));
    ...
    n = sftp_format_longname(true, 0755, 0, 0, "dir", out, sizeof(out));
...
static void test_builder_overflow()
{
    ...
    TEST_ASSERT_EQUAL_UINT(
        0, sftp_build_status(1, SSH_FX_FAILURE, "a message too long for the buffer", tiny, sizeof(tiny)));
    TEST_ASSERT_EQUAL_UINT(0, sftp_build_data(1, "0123456789", 10, tiny, sizeof(tiny)));
}
