...
// codec (pc_modbus_process_adu) via a mock transaction seam - no hardware needed.
...
{
    (void)io;
    size_t pn = pc_modbus_process_adu(req, req_len, resp, resp_cap);
...
void test_read_single_holding()
{
    ...
    TEST_ASSERT_EQUAL_INT(
        Sb::SB_OK, pc_sb_modbus_init(&g_ctx, loopback_txn, nullptr, ModbusFunction::MODBUS_FC_READ_HOLDING_REGS, 1));
    TEST_ASSERT_EQUAL_INT(Sb::SB_OK, pc_sb_modbus_driver(&g_drv, "plc", &g_ctx));
    TEST_ASSERT_EQUAL_INT(Sb::SB_OK, pc_southbound_register(&g_drv));

    int32_t v = 0;
    TEST_ASSERT_EQUAL_INT(Sb::SB_OK, pc_southbound_read("plc", 10, &v));
...
void test_read_block_matrix()
{
    ...
    {
        pc_modbus_set_holding_reg((uint16_t)(20 + i), (uint16_t)(0x1000 + i));
    }
    pc_sb_modbus_init(&g_ctx, loopback_txn, nullptr, ModbusFunction::MODBUS_FC_READ_HOLDING_REGS, 1);
    pc_sb_modbus_driver(&g_drv, "plc", &g_ctx);
    pc_southbound_register(&g_drv);

    int32_t vals[4] = {0, 0, 0, 0};
    TEST_ASSERT_EQUAL_INT(4, pc_southbound_read_block("plc", 20, vals, 4));
...
{
    pc_modbus_set_input_reg(5, 0x0777);
    pc_sb_modbus_init(&g_ctx, loopback_txn, nullptr, ModbusFunction::MODBUS_FC_READ_INPUT_REGS, 1);
    pc_sb_modbus_driver(&g_drv, "sensor", &g_ctx);
    pc_southbound_register(&g_drv);

    int32_t v = 0;
    TEST_ASSERT_EQUAL_INT(Sb::SB_OK, pc_southbound_read("sensor", 5, &v));
    TEST_ASSERT_EQUAL_INT32(0x0777, v);
}

// A Modbus exception reply (out-of-range address) surfaces as PC_SB_MODBUS_EXCEPTION with the raw
...
void test_modbus_exception_surfaces()
{
    ...
    int32_t v = 123;
    TEST_ASSERT_EQUAL_INT(PC_SB_MODBUS_EXCEPTION, pc_southbound_read("plc", 60000, &v));
...
{
    pc_sb_modbus_init(&g_ctx, fail_txn, nullptr, ModbusFunction::MODBUS_FC_READ_HOLDING_REGS, 1);
    pc_sb_modbus_driver(&g_drv, "plc", &g_ctx);
    pc_southbound_register(&g_drv);

    int32_t v = 0;
    TEST_ASSERT_EQUAL_INT(-42, pc_southbound_read("plc", 0, &v));
    int32_t blk[2] = {0, 0};
    TEST_ASSERT_EQUAL_INT(-42, pc_southbound_read_block("plc", 0, blk, 2));
}
...
void test_write_single_round_trip()
{
    ...
    TEST_ASSERT_EQUAL_INT(Sb::SB_OK, pc_southbound_write("plc", 8, 0x4242));
    TEST_ASSERT_EQUAL_HEX16(0x4242, pc_modbus_get_holding_reg(8)); // stored on the slave
    int32_t v = 0;
    TEST_ASSERT_EQUAL_INT(Sb::SB_OK, pc_southbound_read("plc", 8, &v)); // and reads back through the facade
...
void test_write_block_round_trip()
{
    ...
    TEST_ASSERT_EQUAL_INT(3, pc_southbound_write_block("plc", 40, in, 3)); // count written
    int32_t back[3] = {0, 0, 0};
    TEST_ASSERT_EQUAL_INT(3, pc_southbound_read_block("plc", 40, back, 3));
...
void test_input_registers_read_only()
{
    ...
    TEST_ASSERT_EQUAL_INT(Sb::SB_ERR_UNSUPPORTED, pc_southbound_write("sensor", 0, 1));
    int32_t in[2] = {1, 2};
    TEST_ASSERT_EQUAL_INT(Sb::SB_ERR_UNSUPPORTED, pc_southbound_write_block("sensor", 0, in, 2));
}
...
void test_write_bounds()
{
    ...
    TEST_ASSERT_EQUAL_INT(Sb::SB_ERR_ARG, pc_southbound_write("plc", 0, 0x10000)); // > 16-bit value
    TEST_ASSERT_EQUAL_INT(Sb::SB_ERR_ARG, pc_southbound_write("plc", 0, -1));      // negative value
    int32_t big[124];
    for (int i = 0; i < 124; i++)
    {
        big[i] = 1;
    }
    TEST_ASSERT_EQUAL_INT(Sb::SB_ERR_ARG, pc_southbound_write_block("plc", 0, big, 124)); // > 123 regs/request
    int32_t bad[2] = {0, 0x10000};
    TEST_ASSERT_EQUAL_INT(Sb::SB_ERR_ARG, pc_southbound_write_block("plc", 0, bad, 2)); // out-of-range value
}
...
void test_init_rejects_bad_args()
{
    TEST_ASSERT_EQUAL_INT(Sb::SB_ERR_ARG,
                          pc_sb_modbus_init(&g_ctx, nullptr, nullptr, ModbusFunction::MODBUS_FC_READ_HOLDING_REGS, 1));
    TEST_ASSERT_EQUAL_INT(Sb::SB_ERR_ARG, pc_sb_modbus_init(&g_ctx, loopback_txn, nullptr,
                                                            ModbusFunction::MODBUS_FC_WRITE_SINGLE_REG, 1));
    TEST_ASSERT_EQUAL_INT(Sb::SB_ERR_ARG, pc_sb_modbus_init(nullptr, loopback_txn, nullptr,
...
void test_read_bounds()
{
    ...
    int32_t v = 0;
    TEST_ASSERT_EQUAL_INT(Sb::SB_ERR_ARG, pc_southbound_read("plc", 0x10000, &v)); // > 0xFFFF
    int32_t blk[130];
    TEST_ASSERT_EQUAL_INT(Sb::SB_ERR_ARG, pc_southbound_read_block("plc", 0, blk, 126)); // > 125 regs/request
    // A span that would run past the 16-bit address space.
    TEST_ASSERT_EQUAL_INT(Sb::SB_ERR_ARG, pc_southbound_read_block("plc", 0xFFFF, blk, 2));
}

// The transaction id rolls forward one per request (the caller's slave-correlation token).
void test_txid_increments()
{
    pc_modbus_set_holding_reg(0, 0x0001);
    pc_sb_modbus_init(&g_ctx, loopback_txn, nullptr, ModbusFunction::MODBUS_FC_READ_HOLDING_REGS, 1);
    pc_sb_modbus_driver(&g_drv, "plc", &g_ctx);
    pc_southbound_register(&g_drv);
