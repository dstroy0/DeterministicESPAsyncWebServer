...
static void test_rsa_pkcs1_pad_structure(void)
{
    ...
    uint8_t sig[256];
    int rc = ssh_rsa_sign(msg, sizeof(msg) - 1, pc_rsa_hash::SHA256, sig);
...
static void test_rsa_sign_verify_roundtrip(void)
{
    ...
    TEST_ASSERT_EQUAL_INT(0, pc_rsa_verify(n, e, msg, sizeof(msg) - 1, sig, 256, pc_rsa_hash::SHA256));

    // A tampered message must fail verification of the same signature.
    TEST_ASSERT_EQUAL_INT(-1, pc_rsa_verify(n, e, (const uint8_t *)"different", 9, sig, 256, pc_rsa_hash::SHA256));
}
...
static void test_rsa_verify_and_encode_guards(void)
{
    ...
    TEST_ASSERT_EQUAL_INT(
        -1, pc_rsa_verify(n, e, (const uint8_t *)"m", 1, sig, 255, pc_rsa_hash::SHA256)); // sig length mismatch
    memset(sig, 0xFF, sizeof(sig));                                                       // all-ones >= any modulus
    TEST_ASSERT_EQUAL_INT(
        -1, pc_rsa_verify(n, e, (const uint8_t *)"m", 1, sig, 256, pc_rsa_hash::SHA256)); // sig not reduced mod n
    ...
    pc_ssh_rsa_load_pubkey();
    uint8_t sig0[256];
    TEST_ASSERT_EQUAL_INT(0, ssh_rsa_sign((const uint8_t *)"x", 1, pc_rsa_hash::SHA256, sig0));
    for (int i = 0; i < 255; i++)
    {
...
static void test_rsa_verify_valid_signature(void)
{
    ...
    TEST_ASSERT_EQUAL_INT(0, pc_rsa_verify(n, e, (const uint8_t *)msg, 9, sig, 256, pc_rsa_hash::SHA256));
}
...
static void test_rsa_verify_rejects_tampered_signature(void)
{
    ...
    TEST_ASSERT_EQUAL_INT(-1, pc_rsa_verify(n, e, (const uint8_t *)"hello ssh", 9, sig, 256, pc_rsa_hash::SHA256));
}
...
static void test_rsa_verify_rejects_wrong_message(void)
{
    ...
    hex_to_bytes(n, RSA_KAT_N, 256);
    hex_to_bytes(sig, RSA_KAT_SIG, 256);
    TEST_ASSERT_EQUAL_INT(-1, pc_rsa_verify(n, e, (const uint8_t *)"goodbye!", 8, sig, 256, pc_rsa_hash::SHA256));
}
...
static void test_rsa_sha512_kat_sign_verify(void)
{
    ...
    uint8_t sig[256];
    TEST_ASSERT_EQUAL_INT(0, ssh_rsa_sign((const uint8_t *)RSA512_MSG, mlen, pc_rsa_hash::SHA512, sig));
    uint8_t ref[256];
    ...
    TEST_ASSERT_EQUAL_INT(0, pc_rsa_verify(n, e, (const uint8_t *)RSA512_MSG, mlen, sig, 256, pc_rsa_hash::SHA512));
    TEST_ASSERT_EQUAL_INT(0, pc_rsa_verify(n, e, (const uint8_t *)RSA512_MSG, mlen, ref, 256, pc_rsa_hash::SHA512));

    // Hash-algorithm binding: a SHA-512 signature must NOT verify as SHA-256.
    TEST_ASSERT_EQUAL_INT(-1, pc_rsa_verify(n, e, (const uint8_t *)RSA512_MSG, mlen, sig, 256, pc_rsa_hash::SHA256));

    // A different message (same length) must fail SHA-512 verification.
    TEST_ASSERT_EQUAL_INT(
        -1, pc_rsa_verify(n, e, (const uint8_t *)"hello rsa-sha2-256", mlen, sig, 256, pc_rsa_hash::SHA512));
