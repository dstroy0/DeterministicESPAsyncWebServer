...
{
    char buf[32];
    pc_gpib_read(buf, sizeof(buf), UNTIL_TIMEOUT, 0);
    TEST_ASSERT_EQUAL_STRING("++read\n", buf);
    pc_gpib_read(buf, sizeof(buf), UNTIL_EOI, 0);
    TEST_ASSERT_EQUAL_STRING("++read eoi\n", buf);
    pc_gpib_read(buf, sizeof(buf), UNTIL_CHAR, 10);
...
void test_builders_reject_null_buffer_and_zero_cap()
{
    ...
    char buf[32];
    TEST_ASSERT_EQUAL_size_t(0, pc_gpib_command(NULL, sizeof(buf), "clr"));
    TEST_ASSERT_EQUAL_size_t(0, pc_gpib_command(buf, 0, "clr"));
    TEST_ASSERT_EQUAL_size_t(0, pc_gpib_command(buf, sizeof(buf), NULL));
    TEST_ASSERT_EQUAL_size_t(0, pc_gpib_addr(NULL, sizeof(buf), 9, -1));
    TEST_ASSERT_EQUAL_size_t(0, pc_gpib_addr(buf, 0, 9, -1));
    TEST_ASSERT_EQUAL_size_t(0, pc_gpib_read(NULL, sizeof(buf), UNTIL_EOI, 0));
...
void test_build_data_guards_and_empty()
{
    ...
    uint8_t buf[16];
    const uint8_t src[] = {'A'};
    TEST_ASSERT_EQUAL_size_t(0, pc_gpib_build_data(NULL, sizeof(buf), src, sizeof(src)));
    TEST_ASSERT_EQUAL_size_t(0, pc_gpib_build_data(buf, 0, src, sizeof(src)));
    TEST_ASSERT_EQUAL_size_t(0, pc_gpib_build_data(buf, sizeof(buf), NULL, 1));
    size_t n = pc_gpib_build_data(buf, sizeof(buf), NULL, 0);
...
{
    TEST_ASSERT_FALSE(pc_gpib_is_command(NULL, 8));
}
...
void test_parse_addr_edges()
{
    ...
    uint8_t pad = 0;
    int sad = 0;
    TEST_ASSERT_FALSE(pc_gpib_parse_addr(NULL, 2, &pad, &sad));
    ...
    TEST_ASSERT_TRUE(pc_gpib_parse_addr("9", 1, NULL, &sad));
    TEST_ASSERT_EQUAL_INT(-1, sad);
    TEST_ASSERT_TRUE(pc_gpib_parse_addr("9", 1, &pad, NULL));
    TEST_ASSERT_EQUAL_UINT8(9, pad);
}
...
void test_parse_version_edges()
{
    ...
    const char *ver = NULL;
    size_t vlen = 0;
    TEST_ASSERT_FALSE(pc_gpib_parse_version(NULL, 10, &ver, &vlen));
    ...
    TEST_ASSERT_TRUE(pc_gpib_parse_version(resp, strlen(resp), NULL, &vlen));
    TEST_ASSERT_EQUAL_size_t(3, vlen);
    TEST_ASSERT_TRUE(pc_gpib_parse_version(resp, strlen(resp), &ver, NULL));
