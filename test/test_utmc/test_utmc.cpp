...
{
    char buf[256];
    pc_utmc_response("SIGN_7", "SLOW", Utmc::UTMC_QUALITY_GOOD, "2026-07-06T12:00:00Z", buf, sizeof(buf));
...
{
    char buf[256];
    pc_utmc_response("A&B", "x<y", Utmc::UTMC_QUALITY_SUSPECT, "t", buf, sizeof(buf));
...
{
    char out[64];
    TEST_ASSERT_EQUAL_size_t(0, pc_utmc_parse_request(nullptr, 10, out, sizeof(out))); // null xml
...
void test_null_out_and_zero_cap_guards(void)
{
    ...
    TEST_ASSERT_EQUAL_size_t(0, pc_utmc_request("id", nullptr, 64));
    TEST_ASSERT_EQUAL_size_t(0, pc_utmc_request("id", buf, 0));
    // pc_utmc_response: same two guard paths.
    TEST_ASSERT_EQUAL_size_t(0, pc_utmc_response("id", "v", Utmc::UTMC_QUALITY_GOOD, "t", nullptr, 64));
    TEST_ASSERT_EQUAL_size_t(0, pc_utmc_response("id", "v", Utmc::UTMC_QUALITY_GOOD, "t", buf, 0));
    // pc_utmc_parse_request: null out buffer, then zero-capacity buffer (valid xml both times).
    const char *xml = "<x id=\"ABC\"/>";
    TEST_ASSERT_EQUAL_size_t(0, pc_utmc_parse_request(xml, strlen(xml), nullptr, 64));
