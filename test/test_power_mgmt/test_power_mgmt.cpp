...
{
    PowerPlan p = pc_power_plan(nullptr, 50, 40, false, 0, false);
    TEST_ASSERT_EQUAL_UINT16(0, p.cpu_mhz);
}

void test_null_cfg_defaults_is_not_a_crash()
{
    pc_power_cfg_defaults(nullptr); // must not crash; there is no output to assert on
}
...
void test_json_overflow_is_fail_closed()
{
    ...
    TEST_ASSERT_EQUAL_UINT32(0, pc_power_json(nullptr, 41, tiny, sizeof(tiny)));
}

void test_json_null_out_is_rejected()
{
    PowerPlan p = plan(0, 41, false, 60000, false);
    TEST_ASSERT_EQUAL_UINT32(0, pc_power_json(&p, 41, nullptr, 128));
}
