...
void test_client_parsers_reject_malformed()
{
    ...
    rr.count = 0;
    uint8_t resp[128];
    size_t sn = pc_opcua_build_read_response(&rr, nullptr, nullptr, 0, 0, resp, sizeof(resp));
    TEST_ASSERT_TRUE(sn > 0);
...
{
    uint8_t req[256];
    size_t rn = pc_opcua_client_hello(nullptr, req, sizeof(req));
    TEST_ASSERT_TRUE(rn > 0);
    OpcUaHello hello;
    TEST_ASSERT_TRUE(pc_opcua_parse_hello(req, rn, &hello));
    ...
    OpcUaClient c;
    pc_opcua_client_init(&c);
    rn = pc_opcua_client_get_endpoints(&c, nullptr, req, sizeof(req));
    TEST_ASSERT_TRUE(rn > 0);
    OpcUaMsg m;
    TEST_ASSERT_TRUE(pc_opcua_parse_msg(req, rn, &m));
    TEST_ASSERT_EQUAL_UINT32(OPCUA_ID_GET_ENDPOINTS_REQ, m.type_id);

    rn = pc_opcua_client_create_session(&c, nullptr, nullptr, req, sizeof(req));
    TEST_ASSERT_TRUE(rn > 0);
    TEST_ASSERT_TRUE(pc_opcua_parse_msg(req, rn, &m));
    TEST_ASSERT_EQUAL_UINT32(OPCUA_ID_CREATE_SESSION_REQ, m.type_id);
}
...
void test_on_read_optional_fields_and_limits()
{
    ...
    uint8_t resp[256];
    size_t sn = pc_opcua_build_read_response(&rr, sv, nullptr, 1, 0, resp, sizeof(resp));
    TEST_ASSERT_TRUE(sn > 0);
    ...
    TEST_ASSERT_EQUAL_INT32(4, pc_opcua_client_on_read(resp, sn, nullptr, cs, 4));
    TEST_ASSERT_EQUAL_INT32(4, pc_opcua_client_on_read(resp, sn, cv, nullptr, 4));
}
...
void test_on_write_limits_and_null_sink()
{
    ...
    TEST_ASSERT_EQUAL_INT32(3, pc_opcua_client_on_write(resp, sn, nullptr, 3)); // count only
}
...
void test_on_browse_limits_and_null_sink()
{
    ...
    TEST_ASSERT_EQUAL_INT32(2, pc_opcua_client_on_browse(resp, sn, nullptr, 2)); // count only
}
