...
{
    char buf[256];
    size_t n = syslog_format(buf, sizeof(buf), SyslogFacility::SYSLOG_FAC_LOCAL0, SyslogSeverity::SYSLOG_INFO, "esp32",
                             "app", "hello");
...
void test_pri_computation_varies()
{
    char buf[256];
    // daemon(3)*8 + err(3) = 27
    syslog_format(buf, sizeof(buf), SyslogFacility::SYSLOG_FAC_DAEMON, SyslogSeverity::SYSLOG_ERR, "h", "a", "m");
    TEST_ASSERT_EQUAL_STRING("<27>1 - h a - - - m", buf);
    // local7(23)*8 + emerg(0) = 184
    syslog_format(buf, sizeof(buf), SyslogFacility::SYSLOG_FAC_LOCAL7, SyslogSeverity::SYSLOG_EMERG, "h", "a", "m");
...
{
    char buf[256];
    syslog_format(buf, sizeof(buf), SyslogFacility::SYSLOG_FAC_USER, SyslogSeverity::SYSLOG_DEBUG, nullptr, "", "msg");
...
{
    char buf[256];
    size_t n = syslog_format(buf, sizeof(buf), SyslogFacility::SYSLOG_FAC_LOCAL0, SyslogSeverity::SYSLOG_NOTICE, "h",
                             "a", nullptr);
    TEST_ASSERT_GREATER_THAN_UINT(0, n);
...
{
    char buf[256];
    size_t n = syslog_format(buf, sizeof(buf), SyslogFacility::SYSLOG_FAC_LOCAL0, SyslogSeverity::SYSLOG_INFO, "host",
                             "app", "payload");
...
void test_format_null_and_pri_clamp()
{
    ...
    TEST_ASSERT_EQUAL_UINT(0, syslog_format(nullptr, sizeof(buf), (SyslogFacility)0, (SyslogSeverity)0, "h", "a", "m"));
    TEST_ASSERT_EQUAL_UINT(0, syslog_format(buf, 0, (SyslogFacility)0, (SyslogSeverity)0, "h", "a", "m"));
    // facility/severity are unsigned enums, so PRI is never negative; an over-range value cast in at the
    // boundary clamps to 191.
    syslog_format(buf, sizeof(buf), (SyslogFacility)25, (SyslogSeverity)0, "h", "a", "m"); // 200 clamps to 191
