...
// pc_syslog_log over the udp_transport host capture seam).
...
{
    char buf[256];
    size_t n = pc_syslog_format(buf, sizeof(buf), SyslogFacility::SYSLOG_FAC_LOCAL0, SyslogSeverity::SYSLOG_INFO,
                                "esp32", "app", "hello");
...
void test_pri_computation_varies()
{
    char buf[256];
    // daemon(3)*8 + err(3) = 27
    pc_syslog_format(buf, sizeof(buf), SyslogFacility::SYSLOG_FAC_DAEMON, SyslogSeverity::SYSLOG_ERR, "h", "a", "m");
    TEST_ASSERT_EQUAL_STRING("<27>1 - h a - - - m", buf);
    // local7(23)*8 + emerg(0) = 184
    pc_syslog_format(buf, sizeof(buf), SyslogFacility::SYSLOG_FAC_LOCAL7, SyslogSeverity::SYSLOG_EMERG, "h", "a", "m");
...
{
    char buf[256];
    pc_syslog_format(buf, sizeof(buf), SyslogFacility::SYSLOG_FAC_USER, SyslogSeverity::SYSLOG_DEBUG, nullptr, "",
                     "msg");
...
{
    char buf[256];
    size_t n = pc_syslog_format(buf, sizeof(buf), SyslogFacility::SYSLOG_FAC_LOCAL0, SyslogSeverity::SYSLOG_NOTICE, "h",
                                "a", nullptr);
    TEST_ASSERT_GREATER_THAN_UINT(0, n);
...
void test_init_and_log_captured()
{
    ...
    pc_syslog_init("192.168.1.1", 514, "host1", "myapp", SyslogFacility::SYSLOG_FAC_LOCAL0);
    TEST_ASSERT_TRUE(pc_syslog_log(SyslogSeverity::SYSLOG_INFO, "hello"));
...
void test_log_not_ready_when_no_server()
{
    pc_udp_capture_enable();
    pc_udp_capture_reset();
    pc_syslog_init(nullptr, 514, "h", "a", SyslogFacility::SYSLOG_FAC_USER); // null server_ip -> copy_field empties it
    TEST_ASSERT_FALSE(pc_syslog_log(SyslogSeverity::SYSLOG_INFO, "x"));
...
void test_format_null_and_pri_clamp()
{
    ...
    TEST_ASSERT_EQUAL_UINT(0,
                           pc_syslog_format(nullptr, sizeof(buf), (SyslogFacility)0, (SyslogSeverity)0, "h", "a", "m"));
    ...
    pc_syslog_format(buf, sizeof(buf), (SyslogFacility)25, (SyslogSeverity)0, "h", "a", "m"); // 200 clamps to 191
...
void test_init_truncates_long_fields()
{
    ...
    pc_syslog_init("10.0.0.1", 514, longname, "a", SyslogFacility::SYSLOG_FAC_LOCAL0);
    TEST_ASSERT_TRUE(pc_syslog_log(SyslogSeverity::SYSLOG_INFO, "m"));
    ...
    TEST_ASSERT_EQUAL_UINT((size_t)(PC_SYSLOG_FIELD_MAX - 1), hcount);
}
...
void test_init_empty_server_ip_not_ready()
{
    pc_udp_capture_enable();
    pc_udp_capture_reset();
    pc_syslog_init("", 514, "host", "app", SyslogFacility::SYSLOG_FAC_LOCAL0);
    TEST_ASSERT_FALSE(pc_syslog_log(SyslogSeverity::SYSLOG_INFO, "x"));
    TEST_ASSERT_EQUAL_UINT(0, pc_udp_captured_len());
}
...
{
    char buf[64];
    size_t n = pc_syslog_format(buf, sizeof(buf), SyslogFacility::SYSLOG_FAC_USER, SyslogSeverity::SYSLOG_WARNING, "",
                                nullptr, "msg2");
    TEST_ASSERT_GREATER_THAN_UINT(0, n);
...
void test_format_append_boundaries()
{
    ...
    {
        size_t n = pc_syslog_format(buf, fail_caps[i], SyslogFacility::SYSLOG_FAC_USER, SyslogSeverity::SYSLOG_EMERG,
                                    "h", "a", "m");
        TEST_ASSERT_EQUAL_UINT(0, n);
    }
    // One byte past the last boundary above: every span now fits exactly.
    size_t n = pc_syslog_format(buf, 19, SyslogFacility::SYSLOG_FAC_USER, SyslogSeverity::SYSLOG_EMERG, "h", "a", "m");
    TEST_ASSERT_GREATER_THAN_UINT(0, n);
    TEST_ASSERT_EQUAL_STRING("<8>1 - h a - - - m", buf);
}

// With the client ready, a message long enough to overflow the PC_SYSLOG_MSG_MAX scratch buffer
// makes the internal pc_syslog_format() call return 0, so pc_syslog_log() must report false
...
void test_log_overflow_when_ready()
{
    ...
    memset(longname, 'H', sizeof(longname) - 1);
    longname[sizeof(longname) - 1] = '\0';
    pc_syslog_init("10.0.0.1", 514, longname, longname, SyslogFacility::SYSLOG_FAC_LOCAL0);
    char longmsg[240];
    ...
    TEST_ASSERT_FALSE(pc_syslog_log(SyslogSeverity::SYSLOG_INFO, longmsg));
    TEST_ASSERT_EQUAL_UINT(0, pc_udp_captured_len());
}
