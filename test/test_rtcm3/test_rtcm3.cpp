...
void test_build_1005_matches_pyrtcm()
{
    ...
    size_t n = rtcm3_build_1005(out, sizeof(out), SID, EX, EY, EZ);
...
{
    uint8_t out[RTCM3_MAX_FRAME];
    size_t n = rtcm3_build_1006(out, sizeof(out), SID, EX, EY, EZ, AH);
...
void test_parse_frame_and_1005()
{
    Rtcm3Frame f;
    memset(&f, 0, sizeof(f));
    size_t n = rtcm3_frame_parse(FRAME_1005, sizeof(FRAME_1005), &f);
    TEST_ASSERT_EQUAL_size_t(sizeof(FRAME_1005), n);
    ...
    TEST_ASSERT_TRUE(rtcm3_parse_1005(f.payload, f.payload_len, &arp));
...
void test_parse_frame_and_1006()
{
    Rtcm3Frame f;
    memset(&f, 0, sizeof(f));
    size_t n = rtcm3_frame_parse(FRAME_1006, sizeof(FRAME_1006), &f);
    TEST_ASSERT_EQUAL_size_t(sizeof(FRAME_1006), n);
...
void test_crc24q_matches_frame()
{
    ...
    uint32_t crc = rtcm3_crc24q(FRAME_1005, body);
    uint32_t appended =
...
void test_crc_detects_corruption()
{
    ...
    Rtcm3Frame f;
    memset(&f, 0, sizeof(f));
    size_t n = rtcm3_frame_parse(bad, sizeof(bad), &f);
...
{
    Rtcm3Frame f;
    TEST_ASSERT_EQUAL_size_t(0, rtcm3_frame_parse(FRAME_1005, 2, &f));                      // < header
    TEST_ASSERT_EQUAL_size_t(0, rtcm3_frame_parse(FRAME_1005, sizeof(FRAME_1005) - 1, &f)); // one byte short
}

void test_sync_finds_preamble()
{
    const uint8_t stream[] = {0x11, 0x22, 0x33, 0xD3, 0x00, 0x13};
    TEST_ASSERT_EQUAL_size_t(3, rtcm3_sync(stream, sizeof(stream)));
    const uint8_t none[] = {0x01, 0x02, 0x03};
    TEST_ASSERT_EQUAL_size_t(sizeof(none), rtcm3_sync(none, sizeof(none)));
}
...
void test_bit_io_roundtrip()
{
    ...
    RtcmBitWriter w;
    rtcm_bw_init(&w, buf, sizeof(buf));
    rtcm_bw_u(&w, 1005, 12);
    rtcm_bw_u(&w, 0x2A, 6);
    rtcm_bw_s(&w, -1, 12);             // all-ones two's complement
    rtcm_bw_s(&w, -48500020000LL, 38); // a 38-bit signed like an ECEF coordinate
    TEST_ASSERT_TRUE(w.ok);

    size_t p = 0;
    TEST_ASSERT_EQUAL_UINT64(1005, rtcm_br_u(buf, &p, 12));
    TEST_ASSERT_EQUAL_UINT64(0x2A, rtcm_br_u(buf, &p, 6));
    TEST_ASSERT_EQUAL_INT64(-1, rtcm_br_s(buf, &p, 12));
    TEST_ASSERT_EQUAL_INT64(-48500020000LL, rtcm_br_s(buf, &p, 38));
}
...
void test_writer_overflow_fails_closed()
{
    ...
    RtcmBitWriter w;
    rtcm_bw_init(&w, small, sizeof(small));
    rtcm_bw_u(&w, 0, 12);
    rtcm_bw_u(&w, 0, 12); // would need 24 bits > 16 -> overflow
...
void test_frame_build_roundtrip()
{
    ...
    size_t n = rtcm3_frame_build(frame, sizeof(frame), payload, sizeof(payload));
    TEST_ASSERT_EQUAL_size_t(RTCM3_HDR_LEN + sizeof(payload) + RTCM3_CRC_LEN, n);

    Rtcm3Frame f;
    memset(&f, 0, sizeof(f));
    TEST_ASSERT_EQUAL_size_t(n, rtcm3_frame_parse(frame, n, &f));
    TEST_ASSERT_TRUE(f.crc_ok);
    ...
    uint8_t tiny[4];
    TEST_ASSERT_EQUAL_size_t(0, rtcm3_frame_build(tiny, sizeof(tiny), payload, sizeof(payload)));
}
