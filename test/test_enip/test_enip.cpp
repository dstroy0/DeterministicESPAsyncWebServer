...
void test_header_round_trip()
{
    ...
    uint8_t buf[32];
    size_t n = dws_eip_build(buf, sizeof(buf), &h, data, sizeof(data));
    ...
    const uint8_t *d;
    size_t dlen;
    TEST_ASSERT_TRUE(dws_eip_parse(buf, n, &p, &d, &dlen));
...
void test_send_rr_data_bytes()
{
    ...
    uint8_t buf[64];
    size_t n = dws_eip_build_send_rr_data(buf, sizeof(buf), 0x12345678, nullptr, 10, cip, sizeof(cip));
...
void test_send_rr_data_round_trip()
{
    ...
    uint8_t buf[64];
    size_t n = dws_eip_build_send_rr_data(buf, sizeof(buf), 0x01, nullptr, 5, cip, sizeof(cip));
    ...
    size_t out_len;
    TEST_ASSERT_TRUE(dws_eip_parse_send_rr_data(d, dlen, &out_cip, &out_len));
...
void test_parse_rejects_bad()
{
    ...
    TEST_ASSERT_FALSE(dws_eip_parse(short_hdr, sizeof(short_hdr), &p, &d, &dlen));

    // A 24-octet header that declares 8 data octets but has none.
    uint8_t hdr[EIP_HEADER_SIZE] = {0};
    hdr[0] = 0x6F;
    hdr[2] = 0x08; // length 8
    TEST_ASSERT_FALSE(dws_eip_parse(hdr, sizeof(hdr), &p, &d, &dlen));
    ...
    const uint8_t *cip;
    size_t clen;
    TEST_ASSERT_FALSE(dws_eip_parse_send_rr_data(no_item, sizeof(no_item), &cip, &clen));
}
...
void test_build_overflow_fails_closed()
{
    ...
    TEST_ASSERT_EQUAL_size_t(0, dws_eip_build_send_rr_data(small, sizeof(small), 1, nullptr, 1, cip, sizeof(cip)));
    TEST_ASSERT_EQUAL_size_t(0, dws_eip_build_register_session(small, 16, nullptr));
}
...
void test_build_and_parse_guards()
{
    ...
    TEST_ASSERT_EQUAL_size_t(0, dws_eip_build(nullptr, sizeof(buf), &h, data, 2));   // null buf
    TEST_ASSERT_EQUAL_size_t(0, dws_eip_build(buf, sizeof(buf), nullptr, data, 2));  // null header
    TEST_ASSERT_EQUAL_size_t(0, dws_eip_build(buf, sizeof(buf), &h, data, 0x10000)); // data_len > 0xFFFF

    // Passing a sender context exercises the memcpy in each convenience builder.
    const uint8_t ctx[8] = {1, 2, 3, 4, 5, 6, 7, 8};
    size_t n = dws_eip_build_register_session(buf, sizeof(buf), ctx);
    TEST_ASSERT_TRUE(n > 0);
    TEST_ASSERT_EQUAL_HEX8_ARRAY(ctx, buf + 12, 8); // sender context at offset 12
    n = dws_eip_build_send_rr_data(buf, sizeof(buf), 0x01, ctx, 5, data, 2);
    TEST_ASSERT_TRUE(n > 0);
    TEST_ASSERT_EQUAL_HEX8_ARRAY(ctx, buf + 12, 8);

    TEST_ASSERT_EQUAL_size_t(0, dws_eip_build_send_rr_data(nullptr, sizeof(buf), 1, nullptr, 5, data, 2)); // null buf
    TEST_ASSERT_EQUAL_size_t(
        0, dws_eip_build_send_rr_data(buf, sizeof(buf), 1, nullptr, 5, nullptr, 2)); // dws_cip_len && !cip

    const uint8_t *cip;
    size_t clen;
    TEST_ASSERT_FALSE(dws_eip_parse_send_rr_data(nullptr, 8, &cip, &clen)); // null data
    const uint8_t tooshort[7] = {0};
    TEST_ASSERT_FALSE(dws_eip_parse_send_rr_data(tooshort, sizeof(tooshort), &cip, &clen)); // data_len < 8
    const uint8_t trunc_item[8] = {0, 0, 0, 0, 0, 0, 0x01, 0x00}; // count 1 but no item octets
    TEST_ASSERT_FALSE(dws_eip_parse_send_rr_data(trunc_item, sizeof(trunc_item), &cip, &clen));
