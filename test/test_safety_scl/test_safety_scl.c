...
{
    TEST_ASSERT_EQUAL_UINT8((uint8_t)INIT, (uint8_t)pc_scl_state(&g));
    TEST_ASSERT_EQUAL_UINT8((uint8_t)PC_NONE, (uint8_t)pc_scl_fault(&g));
...
{
    run_ok(0, 5, 1000);
    TEST_ASSERT_EQUAL_UINT8((uint8_t)RUNNING, (uint8_t)pc_scl_state(&g));
...
void test_bad_signature_trips_signature_fault(void)
{
    ...
    TEST_ASSERT_EQUAL_UINT8((uint8_t)FAILSAFE, (uint8_t)pc_scl_state(&g));
    TEST_ASSERT_EQUAL_UINT8((uint8_t)SIGNATURE, (uint8_t)pc_scl_fault(&g));
...
void test_lost_frame_trips_counter_fault(void)
{
    ...
    TEST_ASSERT_EQUAL_UINT8((uint8_t)COUNTER, (uint8_t)pc_scl_fault(&g));
}
...
void test_watchdog_trips_on_a_silent_channel(void)
{
    ...
    TEST_ASSERT_EQUAL_UINT8((uint8_t)TIMEOUT, (uint8_t)pc_scl_fault(&g));
}
...
void test_null_guards(void)
{
    ...
    TEST_ASSERT_EQUAL_UINT8((uint8_t)PC_NONE, (uint8_t)pc_scl_fault(NULL));
    // a missing connection must never read as usable
    TEST_ASSERT_EQUAL_UINT8((uint8_t)FAILSAFE, (uint8_t)pc_scl_state(NULL));
