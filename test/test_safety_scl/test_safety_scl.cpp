...
static void run_ok(uint32_t from, uint32_t n, uint32_t t0)
{
    ...
    {
        TEST_ASSERT_TRUE(pc_scl_on_frame(&g, true, from + i, t0 + i));
    }
}

void test_starts_in_init_and_is_usable(void)
{
    TEST_ASSERT_EQUAL_UINT8((uint8_t)SclState::INIT, (uint8_t)pc_scl_state(&g));
    TEST_ASSERT_EQUAL_UINT8((uint8_t)SclFault::PC_NONE, (uint8_t)pc_scl_fault(&g));
...
{
    run_ok(0, 5, 1000);
    TEST_ASSERT_EQUAL_UINT8((uint8_t)SclState::RUNNING, (uint8_t)pc_scl_state(&g));
...
{
    run_ok(0, 2, 1000);
    TEST_ASSERT_FALSE(pc_scl_on_frame(&g, false, 2, 1002));
    TEST_ASSERT_EQUAL_UINT8((uint8_t)SclState::FAILSAFE, (uint8_t)pc_scl_state(&g));
    TEST_ASSERT_EQUAL_UINT8((uint8_t)SclFault::SIGNATURE, (uint8_t)pc_scl_fault(&g));
...
{
    run_ok(0, 3, 1000);
    TEST_ASSERT_FALSE(pc_scl_on_frame(&g, true, 4, 1003)); // 3 was lost
    TEST_ASSERT_EQUAL_UINT8((uint8_t)SclFault::COUNTER, (uint8_t)pc_scl_fault(&g));
}
...
{
    run_ok(0, 3, 1000);
    TEST_ASSERT_FALSE(pc_scl_on_frame(&g, true, 2, 1003)); // 2 again
    TEST_ASSERT_EQUAL_UINT8((uint8_t)SclFault::COUNTER, (uint8_t)pc_scl_fault(&g));
}
...
{
    run_ok(0, 4, 1000);
    TEST_ASSERT_FALSE(pc_scl_on_frame(&g, true, 1, 1004));
    TEST_ASSERT_EQUAL_UINT8((uint8_t)SclFault::COUNTER, (uint8_t)pc_scl_fault(&g));
}
...
{
    run_ok(0, 2, 1000);
    TEST_ASSERT_FALSE(pc_scl_on_frame(&g, true, 99, 1002));
    TEST_ASSERT_EQUAL_UINT8((uint8_t)SclFault::COUNTER, (uint8_t)pc_scl_fault(&g));
}
...
void test_watchdog_trips_on_a_silent_channel(void)
{
    ...
    TEST_ASSERT_EQUAL_UINT8((uint8_t)SclFault::TIMEOUT, (uint8_t)pc_scl_fault(&g));
}
...
void test_watchdog_is_wrap_safe(void)
{
    ...
    TEST_ASSERT_TRUE(pc_scl_on_frame(&g, true, 0, 0xFFFFFFF0u));
...
void test_zero_watchdog_disables_the_timeout(void)
{
    ...
    TEST_ASSERT_TRUE(pc_scl_on_frame(&g, true, 0, 1000));
...
{
    run_ok(0, 2, 1000);
    TEST_ASSERT_FALSE(pc_scl_on_frame(&g, false, 2, 1002)); // SIGNATURE
    TEST_ASSERT_EQUAL_UINT8((uint8_t)SclFault::SIGNATURE, (uint8_t)pc_scl_fault(&g));

    // A subsequent perfectly good frame must NOT bring it back, and must not rewrite the fault.
    TEST_ASSERT_FALSE(pc_scl_on_frame(&g, true, 2, 1003));
    TEST_ASSERT_EQUAL_UINT8((uint8_t)SclState::FAILSAFE, (uint8_t)pc_scl_state(&g));
    TEST_ASSERT_EQUAL_UINT8((uint8_t)SclFault::SIGNATURE, (uint8_t)pc_scl_fault(&g));
...
{
    run_ok(0, 2, 1000);
    TEST_ASSERT_FALSE(pc_scl_on_frame(&g, true, 77, 1002)); // COUNTER
    ...
    TEST_ASSERT_TRUE(pc_scl_on_frame(&g, true, 10, 2000)); // armed for the new first counter
}
...
void test_counter_wraps_at_the_modulus(void)
{
    ...
    TEST_ASSERT_TRUE(pc_scl_on_frame(&g, true, 254, 1000));
    TEST_ASSERT_TRUE(pc_scl_on_frame(&g, true, 255, 1001));
    TEST_ASSERT_TRUE(pc_scl_on_frame(&g, true, 0, 1002)); // wrapped, not 256
    TEST_ASSERT_TRUE(pc_scl_on_frame(&g, true, 1, 1003));
    TEST_ASSERT_TRUE(pc_scl_ok(&g));
...
void test_init_normalises_the_first_counter(void)
{
    ...
    TEST_ASSERT_TRUE(pc_scl_on_frame(&g, true, 44, 1000)); // 300 % 256
}

void test_null_guards(void)
{
    TEST_ASSERT_FALSE(pc_scl_on_frame(nullptr, true, 0, 0));
    TEST_ASSERT_FALSE(pc_scl_poll(nullptr, 0));
    TEST_ASSERT_FALSE(pc_scl_ok(nullptr));
    TEST_ASSERT_EQUAL_UINT8((uint8_t)SclFault::PC_NONE, (uint8_t)pc_scl_fault(nullptr));
    // a missing connection must never read as usable
    TEST_ASSERT_EQUAL_UINT8((uint8_t)SclState::FAILSAFE, (uint8_t)pc_scl_state(nullptr));
    pc_scl_init(nullptr, 0, 0, 0, 0);
    pc_scl_reset(nullptr, 0, 0);
}
