...
void test_sum(void)
{
    ...
    TEST_ASSERT_EQUAL_HEX8(0x00, pc_cclink_sum(b, sizeof(b))); // wraps to 0
}
...
void test_build_and_parse(void)
{
    ...
    uint8_t buf[16];
    size_t n = pc_cclink_build(5, CclinkCmd::CCLINK_CMD_REFRESH, bits, 2, words, 4, buf, sizeof(buf));
    ...
    CcLinkFrame f;
    TEST_ASSERT_TRUE(pc_cclink_parse(buf, n, &f));
    ...
    TEST_ASSERT_EQUAL_UINT16(0x1234, pc_cclink_get_word(f.payload + 2, 4, 0));
    TEST_ASSERT_EQUAL_UINT16(0x5678, pc_cclink_get_word(f.payload + 2, 4, 1));
}

void test_bit_accessors(void)
{
    uint8_t bits[2] = {0xA5, 0x00}; // 1010_0101
    TEST_ASSERT_TRUE(pc_cclink_get_bit(bits, 2, 0));
...
void test_parse_rejects(void)
{
    uint8_t bits[1] = {0x11};
    uint8_t buf[8];
    size_t n = pc_cclink_build(1, CclinkCmd::CCLINK_CMD_POLL, bits, 1, nullptr, 0, buf, sizeof(buf));
    CcLinkFrame f;
    ...
    TEST_ASSERT_EQUAL_size_t(0,
                             pc_cclink_build(64, CclinkCmd::CCLINK_CMD_POLL, nullptr, 0, nullptr, 0, buf, sizeof(buf)));
}
...
void test_build_null_args(void)
{
    ...
    TEST_ASSERT_EQUAL_size_t(
        0, pc_cclink_build(1, CclinkCmd::CCLINK_CMD_POLL, nullptr, 0, nullptr, 0, nullptr, sizeof(buf)));
    // bit_len > 0 but bits == nullptr -> rejected.
    TEST_ASSERT_EQUAL_size_t(0,
                             pc_cclink_build(1, CclinkCmd::CCLINK_CMD_POLL, nullptr, 3, nullptr, 0, buf, sizeof(buf)));
    // word_len > 0 but words == nullptr -> rejected.
    TEST_ASSERT_EQUAL_size_t(0,
                             pc_cclink_build(1, CclinkCmd::CCLINK_CMD_POLL, nullptr, 0, nullptr, 3, buf, sizeof(buf)));
}
...
void test_build_zero_bit_len(void)
{
    ...
    uint8_t buf[16];
    size_t n = pc_cclink_build(2, CclinkCmd::CCLINK_CMD_POLL, nullptr, 0, words, 4, buf, sizeof(buf));
...
void test_parse_null_args(void)
{
    ...
    CcLinkFrame f;
    TEST_ASSERT_FALSE(pc_cclink_parse(nullptr, sizeof(buf), &f));
    TEST_ASSERT_FALSE(pc_cclink_parse(buf, sizeof(buf), nullptr));
}
...
void test_parse_no_payload(void)
{
    ...
    uint8_t buf[8];
    size_t n = pc_cclink_build(3, CclinkCmd::CCLINK_CMD_TEST, nullptr, 0, nullptr, 0, buf, sizeof(buf));
...
{
    TEST_ASSERT_FALSE(pc_cclink_get_bit(nullptr, 2, 0));
    pc_cclink_set_bit(nullptr, 2, 0, true); // out-of-range on null bits -> no-op, must not crash
    TEST_ASSERT_EQUAL_UINT16(0, pc_cclink_get_word(nullptr, 2, 0));
}
