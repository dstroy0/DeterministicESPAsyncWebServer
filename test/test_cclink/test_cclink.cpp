...
void test_sum(void)
{
    ...
    TEST_ASSERT_EQUAL_HEX8(0x00, detws_cclink_sum(b, sizeof(b))); // wraps to 0
}
...
void test_build_and_parse(void)
{
    ...
    uint8_t buf[16];
    size_t n = detws_cclink_build(5, CclinkCmd::CCLINK_CMD_REFRESH, bits, 2, words, 4, buf, sizeof(buf));
    ...
    CcLinkFrame f;
    TEST_ASSERT_TRUE(detws_cclink_parse(buf, n, &f));
    ...
    TEST_ASSERT_EQUAL_UINT16(0x1234, detws_cclink_get_word(f.payload + 2, 4, 0));
    TEST_ASSERT_EQUAL_UINT16(0x5678, detws_cclink_get_word(f.payload + 2, 4, 1));
}

void test_bit_accessors(void)
{
    uint8_t bits[2] = {0xA5, 0x00}; // 1010_0101
    TEST_ASSERT_TRUE(detws_cclink_get_bit(bits, 2, 0));
    TEST_ASSERT_FALSE(detws_cclink_get_bit(bits, 2, 1));
    TEST_ASSERT_TRUE(detws_cclink_get_bit(bits, 2, 7));
    TEST_ASSERT_FALSE(detws_cclink_get_bit(bits, 2, 99)); // out of range
    detws_cclink_set_bit(bits, 2, 8, true);
    TEST_ASSERT_TRUE(detws_cclink_get_bit(bits, 2, 8));
    TEST_ASSERT_EQUAL_HEX8(0x01, bits[1]);
    detws_cclink_set_bit(bits, 2, 0, false);
    TEST_ASSERT_FALSE(detws_cclink_get_bit(bits, 2, 0));
}
...
void test_parse_rejects(void)
{
    uint8_t bits[1] = {0x11};
    uint8_t buf[8];
    size_t n = detws_cclink_build(1, CclinkCmd::CCLINK_CMD_POLL, bits, 1, nullptr, 0, buf, sizeof(buf));
    CcLinkFrame f;
    buf[n - 1] ^= 0xFF; // bad checksum
    TEST_ASSERT_FALSE(detws_cclink_parse(buf, n, &f));
    TEST_ASSERT_FALSE(detws_cclink_parse(buf, 2, &f)); // too short
    // station > 63 rejected at build.
    TEST_ASSERT_EQUAL_size_t(
        0, detws_cclink_build(64, CclinkCmd::CCLINK_CMD_POLL, nullptr, 0, nullptr, 0, buf, sizeof(buf)));
}
