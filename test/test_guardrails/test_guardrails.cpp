...
void test_json_guards_fail_closed()
{
    ...
    TEST_ASSERT_EQUAL_INT(0, detws_health_json(&h, nullptr, sizeof(buf)));
    TEST_ASSERT_EQUAL_INT(0, detws_health_json(&h, buf, 0));
    // Null health -> empty string, 0.
    TEST_ASSERT_EQUAL_INT(0, detws_health_json(nullptr, buf, sizeof(buf)));
