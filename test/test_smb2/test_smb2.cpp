...
{
    uint8_t buf[64];
    TEST_ASSERT_EQUAL_size_t(64, pc_smb2_build_header(buf, sizeof(buf), Smb2Command::SMB2_TREE_CONNECT, 8,
...
{
    uint8_t buf[64];
    pc_smb2_build_header(buf, sizeof(buf), Smb2Command::SMB2_NEGOTIATE, 1, 0, 0, 0);
    Smb2Header h;
...
void test_build_negotiate()
{
    ...
    uint8_t buf[160];
    size_t n = pc_smb2_build_negotiate(buf, sizeof(buf), gid, Smb2SecurityMode::SMB2_NEGOTIATE_SIGNING_ENABLED);
...
{
    pc_smb2_build_header(m, 256, Smb2Command::SMB2_NEGOTIATE, 1, 5, 0, 0);
...
void test_build_negotiate_311()
{
    ...
    uint8_t buf[256];
    size_t n = pc_smb2_build_negotiate_311(buf, sizeof(buf), gid, Smb2SecurityMode::SMB2_NEGOTIATE_SIGNING_ENABLED,
    ...
    size_t n0 = pc_smb2_build_negotiate_311(buf, sizeof(buf), gid, Smb2SecurityMode::SMB2_NEGOTIATE_SIGNING_ENABLED,
    ...
    TEST_ASSERT_EQUAL_size_t(0, pc_smb2_build_negotiate_311(buf, sizeof(buf), gid, 0, nullptr, 32, offer, 4));
    TEST_ASSERT_EQUAL_size_t(0, pc_smb2_build_negotiate_311(buf, sizeof(buf), gid, 0, salt, 0, offer, 4));
    TEST_ASSERT_EQUAL_size_t(0,
                             pc_smb2_build_negotiate_311(buf, sizeof(buf), gid, 0, salt, 32, nullptr, 4));  // null list
...
{
    pc_smb2_build_header(m, 512, Smb2Command::SMB2_NEGOTIATE, 1, 5, 0, 0);
    uint8_t *b = m + 64;
    memset(b, 0, 64);
    w16(b + 0, 65); // StructureSize
...
static size_t build_ss_resp(uint8_t *m, uint64_t session_id, uint32_t status, uint16_t flags, const uint8_t *sec,
                            uint16_t sec_len)
{
    pc_smb2_build_header(m, 512, Smb2Command::SMB2_SESSION_SETUP, 1, 6, 0, session_id);
...
{
    uint8_t neg[64];
    size_t neg_n = pc_ntlmssp_build_negotiate(neg, sizeof(neg), NtlmsspFlags::NTLMSSP_CLIENT_DEFAULT_FLAGS);
    uint8_t spnego[128];
    size_t sp_n = pc_spnego_wrap_negotiate(neg, neg_n, spnego, sizeof(spnego));
    uint8_t req[256];
    size_t req_n = pc_smb2_build_session_setup(req, sizeof(req), 1, 0, Smb2SecurityMode::SMB2_NEGOTIATE_SIGNING_ENABLED,
                                               spnego, sp_n);
...
{
    pc_smb2_build_header(m, 128, Smb2Command::SMB2_TREE_CONNECT, 1, 2, tree_id, 0xABCD);
...
void test_build_create()
{
    ...
    uint8_t buf[256];
    size_t n = pc_smb2_build_create(buf, sizeof(buf), 3, 0xAAAA, 0x777, Smb2Access::SMB2_FILE_GENERIC_READ,
...
{
    pc_smb2_build_header(m, 256, Smb2Command::SMB2_CREATE, 1, 3, 0x777, 0xAAAA);
    m[16] |= 0x01;
    uint8_t *b = m + 64;
    memset(b, 0, 88);
...
void test_close_roundtrip()
{
    ...
    uint8_t m[128];
    pc_smb2_build_header(m, 128, Smb2Command::SMB2_CLOSE, 1, 4, 0x777, 0xAAAA);
    m[16] |= 0x01;
    uint8_t *rb = m + 64;
    memset(rb, 0, 60);
...
{
    pc_smb2_build_header(m, 512, Smb2Command::SMB2_READ, 1, 5, 0x777, 0xAAAA);
    m[16] |= 0x01;
    uint8_t *b = m + 64;
    memset(b, 0, 16);
...
{
    uint8_t m[128];
    pc_smb2_build_header(m, 128, Smb2Command::SMB2_WRITE, 1, 6, 0x777, 0xAAAA);
    m[16] |= 0x01;
    uint8_t *b = m + 64;
    memset(b, 0, 16);
    w16(b + 0, 17);   // StructureSize
...
void test_transport_rejects_null_and_oversize()
{
    const uint8_t msg[] = {1, 2, 3, 4, 5};
    uint8_t out[16];
    TEST_ASSERT_EQUAL_size_t(0, pc_smb2_transport_frame(nullptr, sizeof(out), msg, sizeof(msg)));
    TEST_ASSERT_EQUAL_size_t(0, pc_smb2_transport_frame(out, sizeof(out), nullptr, sizeof(msg)));
    TEST_ASSERT_EQUAL_size_t(0, pc_smb2_transport_frame(out, sizeof(out), msg, 0x01000000)); // past 24 bits
    TEST_ASSERT_EQUAL_UINT32(0, pc_smb2_transport_len(nullptr, 4));
...
{
    uint8_t buf[64];
    TEST_ASSERT_EQUAL_size_t(0, pc_smb2_build_header(nullptr, 64, Smb2Command::SMB2_NEGOTIATE, 1, 0, 0, 0));
    TEST_ASSERT_EQUAL_size_t(0, pc_smb2_build_header(buf, 63, Smb2Command::SMB2_NEGOTIATE, 1, 0, 0, 0));
}
...
void test_parse_header_null_args()
{
    ...
    pc_smb2_build_header(buf, sizeof(buf), Smb2Command::SMB2_NEGOTIATE, 1, 0, 0, 0);
    Smb2Header h;
    TEST_ASSERT_FALSE(pc_smb2_parse_header(nullptr, 64, &h));
    TEST_ASSERT_FALSE(pc_smb2_parse_header(buf, 64, nullptr));
}
...
void test_build_negotiate_null_args()
{
    uint8_t gid[16] = {0};
    uint8_t buf[160];
    TEST_ASSERT_EQUAL_size_t(0, pc_smb2_build_negotiate(nullptr, sizeof(buf), gid, 0));
    TEST_ASSERT_EQUAL_size_t(0, pc_smb2_build_negotiate(buf, sizeof(buf), nullptr, 0));
}
...
void test_parse_negotiate_response_null_and_low_offset()
{
    ...
    size_t n = build_neg_resp(m, Smb2Dialect::SMB2_DIALECT_0202, token, sizeof(token));
    Smb2NegotiateResp r;
    TEST_ASSERT_FALSE(pc_smb2_parse_negotiate_response(nullptr, n, &r));
    TEST_ASSERT_FALSE(pc_smb2_parse_negotiate_response(m, n, nullptr));
...
void test_build_session_setup_null_args()
{
    uint8_t tok[8] = {0};
    uint8_t buf[256];
    TEST_ASSERT_EQUAL_size_t(0, pc_smb2_build_session_setup(nullptr, sizeof(buf), 1, 0, 0, tok, sizeof(tok)));
    TEST_ASSERT_EQUAL_size_t(0, pc_smb2_build_session_setup(buf, sizeof(buf), 1, 0, 0, nullptr, sizeof(tok)));
...
void test_parse_session_setup_null_and_low_offset()
{
    ...
    size_t n = build_ss_resp(m, 1, 0, 0, tok, sizeof(tok));
    Smb2SessionSetupResp r;
    TEST_ASSERT_FALSE(pc_smb2_parse_session_setup_response(nullptr, n, &r));
    TEST_ASSERT_FALSE(pc_smb2_parse_session_setup_response(m, n, nullptr));
...
void test_build_tree_connect_null_args()
{
    ...
    uint8_t buf[128];
    TEST_ASSERT_EQUAL_size_t(0, pc_smb2_build_tree_connect(nullptr, sizeof(buf), 2, 0, path, sizeof(path)));
    TEST_ASSERT_EQUAL_size_t(0, pc_smb2_build_tree_connect(buf, sizeof(buf), 2, 0, nullptr, sizeof(path)));
...
void test_parse_tree_connect_null_and_command()
{
    ...
    size_t n = build_tc_resp(m, 0x777, Smb2ShareType::SMB2_SHARE_TYPE_DISK, 0x001f01ff);
    Smb2TreeConnectResp r;
    TEST_ASSERT_FALSE(pc_smb2_parse_tree_connect_response(nullptr, n, &r));
    TEST_ASSERT_FALSE(pc_smb2_parse_tree_connect_response(m, n, nullptr));
...
void test_build_create_null_args()
{
    ...
    uint8_t buf[256];
    TEST_ASSERT_EQUAL_size_t(0, pc_smb2_build_create(nullptr, sizeof(buf), 3, 0, 0, 0, 0, 0, 0, name, sizeof(name)));
    TEST_ASSERT_EQUAL_size_t(0, pc_smb2_build_create(buf, sizeof(buf), 3, 0, 0, 0, 0, 0, 0, nullptr, sizeof(name)));
...
void test_parse_create_null_and_command()
{
    ...
    Smb2CreateResp r;
    TEST_ASSERT_FALSE(pc_smb2_parse_create_response(nullptr, n, &r));
    TEST_ASSERT_FALSE(pc_smb2_parse_create_response(m, n, nullptr));
...
void test_build_close_null_args()
{
    uint8_t fid[16] = {0};
    uint8_t buf[128];
    TEST_ASSERT_EQUAL_size_t(0, pc_smb2_build_close(nullptr, sizeof(buf), 4, 0, 0, fid));
    TEST_ASSERT_EQUAL_size_t(0, pc_smb2_build_close(buf, sizeof(buf), 4, 0, 0, nullptr));
}
...
void test_parse_close_null_command_and_truncated()
{
    ...
    Smb2CloseResp r;
    TEST_ASSERT_FALSE(pc_smb2_parse_close_response(nullptr, n, &r));
    TEST_ASSERT_FALSE(pc_smb2_parse_close_response(m, n, nullptr));
...
void test_build_read_null_args()
{
    uint8_t fid[16] = {0};
    uint8_t buf[128];
    TEST_ASSERT_EQUAL_size_t(0, pc_smb2_build_read(nullptr, sizeof(buf), 5, 0, 0, fid, 16, 0));
    TEST_ASSERT_EQUAL_size_t(0, pc_smb2_build_read(buf, sizeof(buf), 5, 0, 0, nullptr, 16, 0));
}
...
void test_parse_read_null_command_and_low_offset()
{
    ...
    size_t n = build_read_resp(m, data, sizeof(data));
    Smb2ReadResp r;
    TEST_ASSERT_FALSE(pc_smb2_parse_read_response(nullptr, n, &r));
    TEST_ASSERT_FALSE(pc_smb2_parse_read_response(m, n, nullptr));
...
void test_build_write_null_args()
{
    ...
    uint8_t buf[256];
    TEST_ASSERT_EQUAL_size_t(0, pc_smb2_build_write(nullptr, sizeof(buf), 6, 0, 0, fid, data, sizeof(data), 0));
    TEST_ASSERT_EQUAL_size_t(0, pc_smb2_build_write(buf, sizeof(buf), 6, 0, 0, nullptr, data, sizeof(data), 0));
    TEST_ASSERT_EQUAL_size_t(0, pc_smb2_build_write(buf, sizeof(buf), 6, 0, 0, fid, nullptr, sizeof(data), 0));
...
void test_parse_write_null_and_command()
{
    ...
    Smb2WriteResp r;
    TEST_ASSERT_FALSE(pc_smb2_parse_write_response(nullptr, 64 + 16, &r));
    TEST_ASSERT_FALSE(pc_smb2_parse_write_response(m, 64 + 16, nullptr));
...
void test_smb3_derive_encryption_keys()
{
    ...
    TEST_ASSERT_FALSE(pc_smb3_derive_encryption_keys(sk, 0x0311, nullptr, 16, c2s, s2c));
    ...
    TEST_ASSERT_TRUE(pc_smb3_derive_encryption_keys(sk, 0x0302, nullptr, 16, c2s, s2c));
