...
{
    uint8_t buf[32];
    size_t n = pc_cotp_build_cr(buf, sizeof(buf), 0x0001, 0x0A, NULL, 0);
...
void test_cotp_cc_bytes()
{
    ...
    size_t n = pc_cotp_build_cc(buf, sizeof(buf), 0x0001, 0x0042, 0x0A, NULL, 0);
    ...
    uint8_t crbuf[32];
    size_t crn = pc_cotp_build_cr(crbuf, sizeof(crbuf), 0x1234, 0x0A, NULL, 0);
    CotpHeader cr;
    TEST_ASSERT_TRUE(pc_cotp_parse(crbuf, crn, &cr));
    n = pc_cotp_build_cc(buf, sizeof(buf), cr.src_ref, 0x0055, 0x0A, NULL, 0);
    TEST_ASSERT_TRUE(pc_cotp_parse(buf, n, &h));
...
void test_guards_and_types()
{
    ...
    const uint8_t *p;
    size_t plen, consumed;
    TEST_ASSERT_FALSE(pc_tpkt_parse(NULL, 4, &p, &plen, &consumed)); // null buf
...
void test_tpkt_parse_edge_cases()
{
    ...
    uint8_t buf[16];
    size_t n = pc_tpkt_build(buf, sizeof(buf), payload, sizeof(payload));
    TEST_ASSERT_TRUE(pc_tpkt_parse(buf, n, NULL, NULL, NULL)); // every out-param optional
}
...
void test_cotp_parse_guard_edge_cases()
{
    ...
    CotpHeader h;
    TEST_ASSERT_FALSE(pc_cotp_parse(NULL, sizeof(buf), &h));  // null buf
    TEST_ASSERT_FALSE(pc_cotp_parse(buf, sizeof(buf), NULL)); // null out
