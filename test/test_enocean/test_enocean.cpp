...
void test_crc8_known_answers()
{
    ...
    TEST_ASSERT_EQUAL_HEX8(0x07, esp3_crc8(one, 1)); // hand-derived for poly 0x07
    // CRC-8 (poly 0x07, init 0, no reflection) check value for "123456789" is 0xF4.
    const uint8_t check[9] = {'1', '2', '3', '4', '5', '6', '7', '8', '9'};
    TEST_ASSERT_EQUAL_HEX8(0xF4, esp3_crc8(check, 9));
}
...
void test_parse_rejects_bad_sync()
{
    ...
    buf[0] = 0x00;
    TEST_ASSERT_EQUAL_INT(-1, esp3_parse(buf, n, nullptr));
}
...
void test_parse_needs_more_bytes()
{
    uint8_t buf[32];
    uint16_t n = sample(buf, sizeof(buf));
    TEST_ASSERT_EQUAL_INT(0, esp3_parse(buf, 3, nullptr));     // header not complete
    TEST_ASSERT_EQUAL_INT(0, esp3_parse(buf, n - 1, nullptr)); // data not complete
}
...
void test_parse_resynchronises_after_junk()
{
    ...
    memcpy(buf + 1, tg, n);
    TEST_ASSERT_EQUAL_INT(-1, esp3_parse(buf, (uint16_t)(n + 1), nullptr)); // junk at [0]
    esp3_packet p = {};
    TEST_ASSERT_EQUAL_INT((int)n, esp3_parse(buf + 1, n, &p)); // resynced at the sync byte
