...
{
    uint8_t out[8];
    size_t n = pc_epl_soc(Epl::EPL_NODE_MN, out, sizeof(out));
...
void test_preq_pres_roundtrip(void)
{
    ...
    size_t n = pc_epl_preq(5, Epl::EPL_NODE_MN, pdo, 4, out, sizeof(out));
    TEST_ASSERT_EQUAL_size_t(7, n);
    EplFrame f;
    TEST_ASSERT_TRUE(pc_epl_parse(out, n, &f));
    ...
    n = pc_epl_pres(5, pdo, 4, out, sizeof(out));
    TEST_ASSERT_TRUE(pc_epl_parse(out, n, &f));
...
void test_soa_asnd(void)
{
    ...
    size_t n = pc_epl_soa(Epl::EPL_NODE_MN, soa_fields, 4, out, sizeof(out));
    ...
    n = pc_epl_asnd(5, Epl::EPL_NODE_MN, asnd_svc, 3, out, sizeof(out));
    ...
    n = pc_epl_soa(Epl::EPL_NODE_MN, nullptr, 0, out, sizeof(out));
    TEST_ASSERT_EQUAL_size_t(3, n);
    TEST_ASSERT_EQUAL_size_t(0, pc_epl_soa(Epl::EPL_NODE_MN, soa_fields, 4, out, 2));
    TEST_ASSERT_EQUAL_size_t(0, pc_epl_asnd(5, Epl::EPL_NODE_MN, asnd_svc, 3, out, 2));
}
...
void test_parse_rejects(void)
{
    ...
    TEST_ASSERT_FALSE(pc_epl_parse(tooshort, 2, &f));
    // Unknown message type.
    uint8_t bad[3] = {0x99, 0xFF, 0xF0};
    TEST_ASSERT_FALSE(pc_epl_parse(bad, 3, &f));
}
...
void test_epl_build_guards()
{
    ...
    TEST_ASSERT_EQUAL_size_t(0, pc_epl_build(0x01, 0, 0, nullptr, 4, out, sizeof(out))); // null payload with len
...
void test_epl_build_null_out(void)
{
    ...
    TEST_ASSERT_EQUAL_size_t(0, pc_epl_build(Epl::EPL_MSG_SOC, 0, 0, nullptr, 0, nullptr, 64));
}
...
void test_parse_null_args(void)
{
    ...
    TEST_ASSERT_FALSE(pc_epl_parse(nullptr, 3, &f));    // null frame
    TEST_ASSERT_FALSE(pc_epl_parse(frame, 3, nullptr)); // null out
}
...
void test_parse_all_message_types(void)
{
    ...
    TEST_ASSERT_TRUE(pc_epl_parse(soc, 3, &f));
    ...
    TEST_ASSERT_TRUE(pc_epl_parse(soa, 3, &f));
    TEST_ASSERT_EQUAL_HEX8(Epl::EPL_MSG_SOA, f.msg_type);

    uint8_t asnd[3] = {Epl::EPL_MSG_ASND, 5, Epl::EPL_NODE_MN};
    TEST_ASSERT_TRUE(pc_epl_parse(asnd, 3, &f));
    TEST_ASSERT_EQUAL_HEX8(Epl::EPL_MSG_ASND, f.msg_type);
}
