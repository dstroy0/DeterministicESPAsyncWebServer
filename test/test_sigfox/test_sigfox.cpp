...
void test_build_uplink_hex_encode()
{
    ...
    char out[32];
    uint16_t n = pc_sigfox_build_uplink(payload, 2, out, sizeof(out));
...
void test_build_uplink_single_byte()
{
    ...
    char out[32];
    uint16_t n = pc_sigfox_build_uplink(payload, 1, out, sizeof(out));
...
void test_build_uplink_bounds()
{
    uint8_t big[13] = {0};
    char out[64];
    TEST_ASSERT_EQUAL_UINT16(0, pc_sigfox_build_uplink(big, 13, out, sizeof(out))); // > 12-byte cap
    TEST_ASSERT_EQUAL_UINT16(0, pc_sigfox_build_uplink(big, 0, out, sizeof(out)));  // empty
    uint8_t p[4] = {1, 2, 3, 4};
    char small[10];
    TEST_ASSERT_EQUAL_UINT16(0, pc_sigfox_build_uplink(p, 4, small, sizeof(small))); // needs 17, cap 10
}
...
void test_build_uplink_null_args()
{
    const uint8_t payload[2] = {0xAB, 0x12};
    char out[32];
    TEST_ASSERT_EQUAL_UINT16(0, pc_sigfox_build_uplink(payload, 2, nullptr, sizeof(out))); // null out
    TEST_ASSERT_EQUAL_UINT16(0, pc_sigfox_build_uplink(nullptr, 2, out, sizeof(out)));     // null payload
}

void test_parse_response_ok()
{
    TEST_ASSERT_EQUAL_INT(pc_sigfox_result::SIGFOX_OK, pc_sigfox_parse_response("OK\r\n", 4));
}

void test_parse_response_error()
{
    TEST_ASSERT_EQUAL_INT(pc_sigfox_result::SIGFOX_ERROR, pc_sigfox_parse_response("ERROR\r\n", 7));
}

void test_parse_response_pending()
{
    TEST_ASSERT_EQUAL_INT(pc_sigfox_result::SIGFOX_PENDING,
                          pc_sigfox_parse_response("AT$SF=AB12\r\n", 12)); // echo, no verdict yet
    TEST_ASSERT_EQUAL_INT(pc_sigfox_result::SIGFOX_PENDING, pc_sigfox_parse_response("", 0));
}

void test_parse_response_null_buf()
{
    TEST_ASSERT_EQUAL_INT(pc_sigfox_result::SIGFOX_PENDING, pc_sigfox_parse_response(nullptr, 5));
}
...
void test_parse_response_error_wins()
{
    ...
    TEST_ASSERT_EQUAL_INT(pc_sigfox_result::SIGFOX_ERROR, pc_sigfox_parse_response(both, (uint16_t)strlen(both)));
}
