...
{
    TEST_ASSERT_TRUE(pc_cia402_state(0x0000) == Cia402State::not_ready_to_switch_on);
    TEST_ASSERT_TRUE(pc_cia402_state(0x0040) == Cia402State::switch_on_disabled);
    TEST_ASSERT_TRUE(pc_cia402_state(0x0021) == Cia402State::ready_to_switch_on);
    TEST_ASSERT_TRUE(pc_cia402_state(0x0023) == Cia402State::switched_on);
    TEST_ASSERT_TRUE(pc_cia402_state(0x0027) == Cia402State::operation_enabled);
    TEST_ASSERT_TRUE(pc_cia402_state(0x0007) == Cia402State::quick_stop_active);
    TEST_ASSERT_TRUE(pc_cia402_state(0x000F) == Cia402State::fault_reaction_active);
    TEST_ASSERT_TRUE(pc_cia402_state(0x0008) == Cia402State::fault);
}
...
void test_state_decode_ignores_high_bits()
{
    ...
    TEST_ASSERT_TRUE(pc_cia402_state(0x0637) == Cia402State::operation_enabled); // 0x27 + remote + target
    TEST_ASSERT_TRUE(pc_cia402_state(0x1237) == Cia402State::operation_enabled);
    TEST_ASSERT_TRUE(pc_cia402_state(0x0233) == Cia402State::switched_on);
}

void test_controlword_commands()
{
    TEST_ASSERT_EQUAL_HEX16(0x0006, pc_cia402_controlword(Cia402Command::shutdown));
    TEST_ASSERT_EQUAL_HEX16(0x0007, pc_cia402_controlword(Cia402Command::switch_on));
    TEST_ASSERT_EQUAL_HEX16(0x0007, pc_cia402_controlword(Cia402Command::disable_operation));
    TEST_ASSERT_EQUAL_HEX16(0x000F, pc_cia402_controlword(Cia402Command::enable_operation));
    TEST_ASSERT_EQUAL_HEX16(0x0000, pc_cia402_controlword(Cia402Command::disable_voltage));
    TEST_ASSERT_EQUAL_HEX16(0x0002, pc_cia402_controlword(Cia402Command::quick_stop));
    TEST_ASSERT_EQUAL_HEX16(0x0080, pc_cia402_controlword(Cia402Command::fault_reset));
}

void test_enable_sequence()
{
    TEST_ASSERT_EQUAL_HEX16(0x0080, pc_cia402_enable_sequence(Cia402State::fault));
    TEST_ASSERT_EQUAL_HEX16(0x0080, pc_cia402_enable_sequence(Cia402State::fault_reaction_active));
    TEST_ASSERT_EQUAL_HEX16(0x0006, pc_cia402_enable_sequence(Cia402State::switch_on_disabled));
    TEST_ASSERT_EQUAL_HEX16(0x0007, pc_cia402_enable_sequence(Cia402State::ready_to_switch_on));
    TEST_ASSERT_EQUAL_HEX16(0x000F, pc_cia402_enable_sequence(Cia402State::switched_on));
    TEST_ASSERT_EQUAL_HEX16(0x000F, pc_cia402_enable_sequence(Cia402State::operation_enabled));
    TEST_ASSERT_EQUAL_HEX16(0x0000, pc_cia402_enable_sequence(Cia402State::not_ready_to_switch_on));
}

void test_statusword_flags()
{
    TEST_ASSERT_TRUE(pc_cia402_target_reached(0x0400));
    TEST_ASSERT_FALSE(pc_cia402_target_reached(0x0000));
    TEST_ASSERT_TRUE(pc_cia402_has_fault(0x0008));
    TEST_ASSERT_TRUE(pc_cia402_warning(0x0080));
    TEST_ASSERT_TRUE(pc_cia402_voltage_enabled(0x0010));
    TEST_ASSERT_TRUE(pc_cia402_remote(0x0200));
    TEST_ASSERT_TRUE(pc_cia402_internal_limit(0x0800));
}

void test_sdo_set_controlword()
{
    CanFrame f;
    TEST_ASSERT_TRUE(pc_cia402_sdo_set_controlword(&f, 5, 0x000F));
...
{
    CanFrame f;
    TEST_ASSERT_TRUE(pc_cia402_sdo_set_target_position(&f, 1, 0x01020304));
    ...
    TEST_ASSERT_TRUE(pc_cia402_sdo_set_mode(&f, 3, Cia402Mode::cyclic_sync_position));
...
void test_sdo_get_roundtrip()
{
    ...
    CanFrame req;
    TEST_ASSERT_TRUE(pc_cia402_sdo_read(&req, 7, CIA402_OD_STATUSWORD, 0));
    ...
    TEST_ASSERT_TRUE(pc_cia402_state(sw) == Cia402State::operation_enabled);

    // wrong-index guard
    uint16_t v = 0;
    TEST_ASSERT_FALSE(pc_cia402_sdo_get_u16(&resp, CIA402_OD_CONTROLWORD, &v));
}

void test_pdo_pack_unpack()
{
    uint8_t buf[8];
    size_t n = pc_cia402_pack_command(buf, sizeof(buf), 0x000F, -12345);
    ...
    TEST_ASSERT_TRUE(pc_cia402_unpack_status(tpdo, sizeof(tpdo), &sw, &actual));
    TEST_ASSERT_EQUAL_HEX16(0x0637, sw);
    TEST_ASSERT_EQUAL_INT32(-12345, actual);

    TEST_ASSERT_EQUAL_size_t(0, pc_cia402_pack_command(buf, 4, 0, 0)); // too small
    TEST_ASSERT_FALSE(pc_cia402_unpack_status(tpdo, 5, &sw, &actual)); // too short
}
...
{
    TEST_ASSERT_TRUE(pc_cia402_state(0x0001) == Cia402State::unknown);
}
...
{
    TEST_ASSERT_EQUAL_HEX16(0x0000, pc_cia402_controlword((Cia402Command)99));
}
...
void test_sdo_set_velocity_torque()
{
    ...
    TEST_ASSERT_TRUE(pc_cia402_sdo_set_target_torque(&f, 4, -2)); // -2 = 0xFFFE
    ...
    TEST_ASSERT_FALSE(pc_cia402_sdo_set_controlword(nullptr, 5, 0x000F));
    TEST_ASSERT_FALSE(pc_cia402_sdo_set_mode(nullptr, 3, Cia402Mode::cyclic_sync_position));
    TEST_ASSERT_FALSE(pc_cia402_sdo_set_target_position(nullptr, 1, 0));
    TEST_ASSERT_FALSE(pc_cia402_sdo_set_target_velocity(nullptr, 2, 0));
    TEST_ASSERT_FALSE(pc_cia402_sdo_set_target_torque(nullptr, 4, 0));
}
...
void test_sdo_get_i32_roundtrip()
{
    ...
    int32_t val = 0;
    TEST_ASSERT_TRUE(pc_cia402_sdo_get_i32(&resp, CIA402_OD_POSITION_ACTUAL, &val));
    TEST_ASSERT_EQUAL_INT32(-20, val);
    // want_index 0 skips the index match (the short-circuited first arm) and still decodes.
    val = 0;
    TEST_ASSERT_TRUE(pc_cia402_sdo_get_i32(&resp, 0, &val));
    TEST_ASSERT_EQUAL_INT32(-20, val);
    // null value pointer -> false.
    TEST_ASSERT_FALSE(pc_cia402_sdo_get_i32(&resp, 0, nullptr));
}
...
void test_pdo_null_guards()
{
    ...
    TEST_ASSERT_EQUAL_size_t(0, pc_cia402_pack_command(nullptr, sizeof(buf), 0, 0));  // null buffer
    TEST_ASSERT_FALSE(pc_cia402_unpack_status(nullptr, sizeof(tpdo), &sw, &actual));  // null buffer
    TEST_ASSERT_FALSE(pc_cia402_unpack_status(tpdo, sizeof(tpdo), nullptr, &actual)); // null statusword
    TEST_ASSERT_FALSE(pc_cia402_unpack_status(tpdo, sizeof(tpdo), &sw, nullptr));     // null actual
}
