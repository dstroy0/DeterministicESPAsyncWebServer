...
static bool add_if(uint8_t id)
{
    ...
    return pc_forward_add_if(id, pc_if_kind::PC_IF_OTHER, cap_send, nullptr);
}
...
void test_allow_forwards()
{
    add_if(1);
    add_if(2);
    TEST_ASSERT_TRUE(pc_forward_add_rule(1, 2, pc_fwd_action::PC_FWD_ALLOW, 0));
...
{
    add_if(1);
    pc_forward_add_rule(1, 1, pc_fwd_action::PC_FWD_ALLOW, 0); // even an explicit self rule
...
void test_deny_wins_over_allow()
{
    add_if(1);
    add_if(2);
    pc_forward_add_rule(1, 2, pc_fwd_action::PC_FWD_ALLOW, 0);
    pc_forward_add_rule(1, 2, pc_fwd_action::PC_FWD_DENY, 0); // deny wins regardless of order
...
void test_multi_destination_fanout()
{
    ...
    add_if(3);
    pc_forward_add_rule(1, 2, pc_fwd_action::PC_FWD_ALLOW, 0);
    pc_forward_add_rule(1, 3, pc_fwd_action::PC_FWD_ALLOW, 0);
...
void test_rate_cap_drops_then_reopens()
{
    add_if(1);
    add_if(2);
    pc_forward_add_rule(1, 2, pc_fwd_action::PC_FWD_ALLOW, 2); // 2 frames / second
...
void test_add_if_validation_and_table_full()
{
    ...
    TEST_ASSERT_FALSE(pc_forward_add_if(9, pc_if_kind::PC_IF_OTHER, nullptr, nullptr)); // null send
    TEST_ASSERT_TRUE(add_if(2));
...
void test_add_rule_table_full()
{
    ...
    TEST_ASSERT_FALSE(pc_forward_add_rule(1, 3, pc_fwd_action::PC_FWD_ALLOW, 0)); // full
}

void test_unregistered_destination_is_inert()
{
    add_if(1);
    pc_forward_add_rule(1, 9, pc_fwd_action::PC_FWD_ALLOW, 0); // dst 9 never registered
...
void test_rule_with_mismatched_src_is_ignored()
{
    add_if(1);
    add_if(2);
    pc_forward_add_rule(9, 2, pc_fwd_action::PC_FWD_ALLOW, 0); // registered but for a different src
...
void test_duplicate_allow_rule_first_one_governs()
{
    add_if(1);
    add_if(2);
    pc_forward_add_rule(1, 2, pc_fwd_action::PC_FWD_ALLOW, 0); // first ALLOW: unlimited, governs
    pc_forward_add_rule(1, 2, pc_fwd_action::PC_FWD_ALLOW, 1); // duplicate ALLOW for the same pair: ignored
    TEST_ASSERT_EQUAL_UINT8(1, ingress(1, "a"));
    TEST_ASSERT_EQUAL_UINT8(1, ingress(1, "b"));
...
void test_get_stats_null_pointer_is_noop()
{
    ...
    pc_forward_add_rule(1, 2, pc_fwd_action::PC_FWD_ALLOW, 0);
    ingress(1, "x");
    pc_forward_get_stats(nullptr);                  // must be a safe no-op
...
void test_acl_deny_by_byte_pattern()
{
    ...
    TEST_ASSERT_TRUE(pc_forward_acl_add(1, 0, pat, msk, 1, pc_fwd_action::PC_FWD_DENY)); // deny byte0 == 0xFF
...
void test_acl_allowlist_default_deny()
{
    ...
    add_if(2);
    pc_forward_add_rule(1, 2, pc_fwd_action::PC_FWD_ALLOW, 0);
    pc_forward_acl_set_default(pc_fwd_action::PC_FWD_DENY); // allowlist: only permitted frames pass
    uint8_t pat[1] = {0xAA}, msk[1] = {0xFF};
    pc_forward_acl_add(1, 0, pat, msk, 1, pc_fwd_action::PC_FWD_ALLOW); // permit byte0 == 0xAA
...
void test_acl_first_match_wins()
{
    ...
    pc_forward_acl_add(1, 0, p1, m1, 1, pc_fwd_action::PC_FWD_ALLOW);                      // 1st: permit byte0 == 0x01
    pc_forward_acl_add(PC_FWD_IF_ANY, 0, nullptr, nullptr, 0, pc_fwd_action::PC_FWD_DENY); // 2nd: deny everything
...
void test_acl_src_any_content_wildcard()
{
    ...
    add_if(2);
    pc_forward_add_rule(1, 2, pc_fwd_action::PC_FWD_ALLOW, 0);
    pc_forward_acl_add(PC_FWD_IF_ANY, 0, nullptr, nullptr, 0, pc_fwd_action::PC_FWD_DENY); // any src, any content
...
void test_acl_entry_src_mismatch_falls_through()
{
    ...
    pc_forward_add_rule(1, 2, pc_fwd_action::PC_FWD_ALLOW, 0);
    uint8_t pat[1] = {0xAA}, msk[1] = {0xFF};
    pc_forward_acl_add(2, 0, pat, msk, 1, pc_fwd_action::PC_FWD_DENY); // entry scoped to src 2 only
...
void test_acl_short_frame_skips_entry()
{
    ...
    pc_forward_acl_add(1, 4, pat, msk, 2, pc_fwd_action::PC_FWD_DENY); // needs len >= 6
...
void test_acl_add_validation_and_table_full()
{
    ...
    TEST_ASSERT_FALSE(
        pc_forward_acl_add(1, 0, big, bm, PC_FWD_ACL_PATLEN + 1, pc_fwd_action::PC_FWD_DENY)); // patlen too big
    for (int i = 0; i < PC_FWD_MAX_ACL; i++)
    {
        TEST_ASSERT_TRUE(pc_forward_acl_add(PC_FWD_IF_ANY, 0, nullptr, nullptr, 0, pc_fwd_action::PC_FWD_ALLOW));
    }
    TEST_ASSERT_FALSE(pc_forward_acl_add(PC_FWD_IF_ANY, 0, nullptr, nullptr, 0, pc_fwd_action::PC_FWD_ALLOW)); // full
}

void test_acl_add_null_pointer_validation()
{
    uint8_t pat[1] = {0x01}, msk[1] = {0xFF};
    TEST_ASSERT_FALSE(pc_forward_acl_add(1, 0, nullptr, msk, 1, pc_fwd_action::PC_FWD_DENY)); // null pattern
    TEST_ASSERT_FALSE(pc_forward_acl_add(1, 0, pat, nullptr, 1, pc_fwd_action::PC_FWD_DENY)); // null mask
}
...
void test_route_selects_egress_and_falls_through()
{
    ...
    add_if(2);
    add_if(3);
    pc_forward_add_rule(1, 2, pc_fwd_action::PC_FWD_ALLOW, 0);   // normal path 1 -> 2
...
void test_route_src_specific_filters_by_source()
{
    ...
    pc_forward_add_rule(2, 3, pc_fwd_action::PC_FWD_ALLOW, 0); // normal fallback path for src 2
...
void test_route_default_any_content()
{
    add_if(1);
    add_if(2);
    TEST_ASSERT_TRUE(pc_forward_route_add(PC_FWD_IF_ANY, 0, nullptr, nullptr, 0, 2, 0)); // patlen 0
...
void test_route_add_validation_and_table_full()
{
    ...
    TEST_ASSERT_FALSE(pc_forward_route_add(PC_FWD_IF_ANY, 0, nullptr, msk, 1, 2, 0)); // null pattern, patlen > 0
    TEST_ASSERT_FALSE(pc_forward_route_add(PC_FWD_IF_ANY, 0, pat, nullptr, 1, 2, 0)); // null mask, patlen > 0
...
void test_inspect_pass_and_drop()
{
    ...
    add_if(2);
    pc_forward_add_rule(1, 2, pc_fwd_action::PC_FWD_ALLOW, 0);
    pc_forward_set_inspector(inspect_drop_D, nullptr);
...
void test_inspect_runs_after_acl()
{
    ...
    pc_forward_acl_add(PC_FWD_IF_ANY, 0, pat, msk, 1, pc_fwd_action::PC_FWD_DENY);
...
void test_inspect_cleared_by_null()
{
    ...
    pc_forward_add_rule(1, 2, pc_fwd_action::PC_FWD_ALLOW, 0);
    pc_forward_set_inspector(inspect_drop_D, nullptr);
    pc_forward_set_inspector(nullptr, nullptr);     // clear it
