...
//
// The env sizes DWS_FWD_MAX_IFACES = 4, DWS_FWD_MAX_RULES = 4.
...
static bool add_if(uint8_t id)
{
    ...
    return dws_forward_add_if(id, dws_if_kind::DWS_IF_OTHER, cap_send, nullptr);
}

static uint8_t ingress(uint8_t src, const char *s)
{
    return dws_forward_ingress(src, (const uint8_t *)s, (uint16_t)strlen(s));
}

static dws_forward_stats stats()
{
...
void test_allow_forwards()
{
    add_if(1);
    add_if(2);
    TEST_ASSERT_TRUE(dws_forward_add_rule(1, 2, dws_fwd_action::DWS_FWD_ALLOW, 0));
...
{
    add_if(1);
    dws_forward_add_rule(1, 1, dws_fwd_action::DWS_FWD_ALLOW, 0); // even an explicit self rule
...
void test_deny_wins_over_allow()
{
    ...
    dws_forward_add_rule(1, 2, dws_fwd_action::DWS_FWD_DENY, 0); // deny wins regardless of order
...
void test_rate_cap_drops_then_reopens()
{
    add_if(1);
    add_if(2);
    dws_forward_add_rule(1, 2, dws_fwd_action::DWS_FWD_ALLOW, 2); // 2 frames / second
    ...
    dws_forward_test_set_now(1000); // next window
...
void test_add_if_validation_and_table_full()
{
    ...
    TEST_ASSERT_FALSE(dws_forward_add_if(9, dws_if_kind::DWS_IF_OTHER, nullptr, nullptr)); // null send
    TEST_ASSERT_TRUE(add_if(2));
    TEST_ASSERT_TRUE(add_if(3));
    TEST_ASSERT_TRUE(add_if(4));
    TEST_ASSERT_FALSE(add_if(5)); // table full (DWS_FWD_MAX_IFACES = 4)
}

void test_add_rule_table_full()
{
    for (int i = 0; i < DWS_FWD_MAX_RULES; i++)
        TEST_ASSERT_TRUE(dws_forward_add_rule(1, 2, dws_fwd_action::DWS_FWD_ALLOW, 0));
    TEST_ASSERT_FALSE(dws_forward_add_rule(1, 3, dws_fwd_action::DWS_FWD_ALLOW, 0)); // full
}

void test_unregistered_destination_is_inert()
{
    add_if(1);
    dws_forward_add_rule(1, 9, dws_fwd_action::DWS_FWD_ALLOW, 0); // dst 9 never registered
...
{
    return dws_forward_ingress(1, b, n);
}
...
void test_acl_deny_by_byte_pattern()
{
    ...
    TEST_ASSERT_TRUE(dws_forward_acl_add(1, 0, pat, msk, 1, dws_fwd_action::DWS_FWD_DENY)); // deny byte0 == 0xFF
...
void test_acl_allowlist_default_deny()
{
    ...
    add_if(2);
    dws_forward_add_rule(1, 2, dws_fwd_action::DWS_FWD_ALLOW, 0);
    dws_forward_acl_set_default(dws_fwd_action::DWS_FWD_DENY); // allowlist: only permitted frames pass
    uint8_t pat[1] = {0xAA}, msk[1] = {0xFF};
    dws_forward_acl_add(1, 0, pat, msk, 1, dws_fwd_action::DWS_FWD_ALLOW); // permit byte0 == 0xAA
...
void test_acl_first_match_wins()
{
    ...
    dws_forward_acl_add(1, 0, p1, m1, 1, dws_fwd_action::DWS_FWD_ALLOW); // 1st: permit byte0 == 0x01
    dws_forward_acl_add(DWS_FWD_IF_ANY, 0, nullptr, nullptr, 0, dws_fwd_action::DWS_FWD_DENY); // 2nd: deny everything
...
void test_acl_src_any_content_wildcard()
{
    ...
    add_if(2);
    dws_forward_add_rule(1, 2, dws_fwd_action::DWS_FWD_ALLOW, 0);
    dws_forward_acl_add(DWS_FWD_IF_ANY, 0, nullptr, nullptr, 0, dws_fwd_action::DWS_FWD_DENY); // any src, any content
...
void test_acl_short_frame_skips_entry()
{
    ...
    dws_forward_acl_add(1, 4, pat, msk, 2, dws_fwd_action::DWS_FWD_DENY); // needs len >= 6
...
{
    uint8_t big[DWS_FWD_ACL_PATLEN + 1] = {0}, bm[DWS_FWD_ACL_PATLEN + 1] = {0};
    TEST_ASSERT_FALSE(
        dws_forward_acl_add(1, 0, big, bm, DWS_FWD_ACL_PATLEN + 1, dws_fwd_action::DWS_FWD_DENY)); // patlen too big
    for (int i = 0; i < DWS_FWD_MAX_ACL; i++)
...
static bool route_firstbyte(uint8_t src, char c, uint8_t egress, uint16_t cap)
{
    ...
    uint8_t msk[1] = {0xFF};
    return dws_forward_route_add(src, 0, pat, msk, 1, egress, cap);
}
...
void test_route_selects_egress_and_falls_through()
{
    ...
    add_if(2);
    add_if(3);
    dws_forward_add_rule(1, 2, dws_fwd_action::DWS_FWD_ALLOW, 0); // normal path 1 -> 2
    TEST_ASSERT_TRUE(route_firstbyte(DWS_FWD_IF_ANY, 'X', 3, 0)); // policy: 'X...' -> if 3 only
...
void test_route_never_reflects_to_source()
{
    add_if(1);
    add_if(2);
    route_firstbyte(DWS_FWD_IF_ANY, 'X', 1, 0); // egress == source
...
{
    add_if(1);
    route_firstbyte(DWS_FWD_IF_ANY, 'X', 9, 0); // if 9 is not registered
    TEST_ASSERT_EQUAL_UINT8(0, ingress(1, "Xyz"));
    TEST_ASSERT_EQUAL_UINT32(1, stats().policy_routed);
    TEST_ASSERT_EQUAL_UINT32(1, stats().send_fail);
}
...
void test_route_rate_cap()
{
    add_if(1);
    add_if(2);
    route_firstbyte(DWS_FWD_IF_ANY, 'X', 2, 1); // 1 frame/sec to the egress
...
void test_route_default_any_content()
{
    add_if(1);
    add_if(2);
    TEST_ASSERT_TRUE(dws_forward_route_add(DWS_FWD_IF_ANY, 0, nullptr, nullptr, 0, 2, 0)); // patlen 0
...
void test_route_first_match_wins()
{
    ...
    add_if(2);
    add_if(3);
    route_firstbyte(DWS_FWD_IF_ANY, 'X', 2, 0); // added first -> if 2
    route_firstbyte(DWS_FWD_IF_ANY, 'X', 3, 0); // also matches -> if 3
...
// --- inspection hook (DWS_FWD_INSPECT) ---

#if DWS_FWD_INSPECT
static int g_inspect_calls = 0;
// Inspector that drops frames whose first byte is 'D', passes the rest, and counts calls.
static dws_fwd_verdict inspect_drop_D(uint8_t src, const uint8_t *d, uint16_t n, void *ctx)
{
    (void)src;
    (void)ctx;
    g_inspect_calls++;
...
void test_inspect_pass_and_drop()
{
    ...
    add_if(2);
    dws_forward_add_rule(1, 2, dws_fwd_action::DWS_FWD_ALLOW, 0);
    dws_forward_set_inspector(inspect_drop_D, nullptr);
...
void test_inspect_runs_after_acl()
{
    ...
    dws_forward_acl_add(DWS_FWD_IF_ANY, 0, pat, msk, 1, dws_fwd_action::DWS_FWD_DENY);
...
void test_inspect_cleared_by_null()
{
    ...
    dws_forward_add_rule(1, 2, dws_fwd_action::DWS_FWD_ALLOW, 0);
    dws_forward_set_inspector(inspect_drop_D, nullptr);
    dws_forward_set_inspector(nullptr, nullptr);    // clear it
