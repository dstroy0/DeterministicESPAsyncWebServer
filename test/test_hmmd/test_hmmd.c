...
void test_parse_report_not_detected()
{
    ...
    TEST_ASSERT_FALSE(pc_hmmd_present(NULL));
    TEST_ASSERT_EQUAL_UINT16(0, pc_hmmd_distance_cm(NULL));
}
...
void test_reject_malformed_report()
{
    ...
    TEST_ASSERT_FALSE(pc_hmmd_parse_report(NULL, PC_HMMD_FRAME_MAX, &r));
    TEST_ASSERT_FALSE(pc_hmmd_parse_report(REPORT, PC_HMMD_FRAME_MAX, NULL));
}
...
void test_stream_push_rejects_null_out()
{
    ...
    TEST_ASSERT_FALSE(pc_hmmd_stream_push(&s, 0xF4, NULL));
}
...
void test_command_encoder_guards()
{
    ...
    TEST_ASSERT_EQUAL_UINT32(0, (uint32_t)pc_hmmd_cmd_open(NULL, sizeof(f)));
    // a non-zero value length with a null value pointer must be refused, not read
    TEST_ASSERT_EQUAL_UINT32(0, (uint32_t)pc_hmmd_cmd_build(f, sizeof(f), 0x0002, NULL, 2));
}
...
void test_ack_decoding()
{
    ...
    TEST_ASSERT_FALSE(pc_hmmd_ack_matches(NULL, 0x0008));
...
void test_ack_rejects_malformed()
{
    ...
    TEST_ASSERT_FALSE(pc_hmmd_parse_ack(NULL, 12, &a));
    TEST_ASSERT_FALSE(pc_hmmd_parse_ack(good, 12, NULL));
...
void test_host_binding_stubs()
{
    ...
    HmmdReport r;
    TEST_ASSERT_FALSE(pc_hmmd_stream_push(NULL, 0, &r));
}
