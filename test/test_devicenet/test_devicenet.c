...
{
    uint32_t id = 0;
    TEST_ASSERT_TRUE(pc_devicenet_encode_id(&id, DEVICENET_GROUP_1, 0x0A, 0x05)); // msgid 10, mac 5
...
{
    uint32_t id = 0;
    TEST_ASSERT_TRUE(pc_devicenet_encode_id(&id, DEVICENET_GROUP_3, 5, 0x09)); // 11 MsgID(3) MAC(6)
    ...
    TEST_ASSERT_TRUE(pc_devicenet_encode_id(&id, DEVICENET_GROUP_4, 0x2A, 0)); // 11111 MsgID(6)
    ...
    TEST_ASSERT_FALSE(pc_devicenet_encode_id(&id, DEVICENET_GROUP_1, 16, 0));   // msgid > 4 bits
    TEST_ASSERT_FALSE(pc_devicenet_encode_id(&id, DEVICENET_GROUP_2, 0, 64));   // mac > 6 bits
    TEST_ASSERT_FALSE(pc_devicenet_encode_id(&id, DEVICENET_GROUP_4, 0x30, 0)); // msgid > 0x2F
}
...
void test_build_explicit_single_frame()
{
    ...
    uint8_t big[8] = {0};
    TEST_ASSERT_FALSE(pc_devicenet_build_explicit(&f, DEVICENET_GROUP_2, 4, 0x21, big, 8));
}
...
void test_frag_non_fragmented()
{
    ...
    TEST_ASSERT_EQUAL_INT(DEVICENET_FRAG_COMPLETE, pc_devicenet_frag_feed(&rx, body, 4));
...
void test_frag_reassembly_roundtrip()
{
    ...
    TEST_ASSERT_EQUAL_INT(DEVICENET_FRAG_STARTED, pc_devicenet_frag_feed(&rx, f0, 8));

    // Middle fragment (count 1): + 6 more data.
    uint8_t f1[8] = {0x80 | 0x21, pc_devicenet_frag_octet(DEVICENET_FRAG_MIDDLE, 1), 7, 8, 9, 10, 11, 12};
    TEST_ASSERT_EQUAL_INT(DEVICENET_FRAG_PROGRESS, pc_devicenet_frag_feed(&rx, f1, 8));

    // Last fragment (count 2): + 2 data.
    uint8_t f2[4] = {0x80 | 0x21, pc_devicenet_frag_octet(DEVICENET_FRAG_LAST, 2), 13, 14};
    TEST_ASSERT_EQUAL_INT(DEVICENET_FRAG_COMPLETE, pc_devicenet_frag_feed(&rx, f2, 4));
...
void test_build_fragment_roundtrip()
{
    ...
    DeviceNetFragRx rx;
    pc_devicenet_frag_reset(&rx);
    TEST_ASSERT_EQUAL_INT(DEVICENET_FRAG_STARTED, pc_devicenet_frag_feed(&rx, f0.data, f0.dlc));
    TEST_ASSERT_EQUAL_INT(DEVICENET_FRAG_COMPLETE, pc_devicenet_frag_feed(&rx, f1.data, f1.dlc));
...
void test_frag_out_of_order_errors()
{
    ...
    TEST_ASSERT_EQUAL_INT(DEVICENET_FRAG_STARTED, pc_devicenet_frag_feed(&rx, f0, 3));
    ...
    TEST_ASSERT_EQUAL_INT(DEVICENET_FRAG_ERR, pc_devicenet_frag_feed(&rx, bad, 3));
...
void test_frag_reject_paths()
{
    ...
    TEST_ASSERT_EQUAL_INT(DEVICENET_FRAG_IGNORED, pc_devicenet_frag_feed(&rx, ack, sizeof(ack)));
}
...
void test_frag_overflow()
{
    ...
    TEST_ASSERT_EQUAL_INT(DEVICENET_FRAG_STARTED, pc_devicenet_frag_feed(&rx, frag, sizeof(frag)));
    frag[1] = pc_devicenet_frag_octet(DEVICENET_FRAG_MIDDLE, 1); // 253 + 253 = 506 > 256
    TEST_ASSERT_EQUAL_INT(DEVICENET_FRAG_ERR, pc_devicenet_frag_feed(&rx, frag, sizeof(frag)));
    TEST_ASSERT_FALSE(rx.active);
...
void test_null_arguments()
{
    ...
    TEST_ASSERT_FALSE(pc_devicenet_encode_id(NULL, DEVICENET_GROUP_1, 0, 0));
    // frag_reset tolerates a null context (no crash, nothing to clear).
    pc_devicenet_frag_reset(NULL);
    // A null frame destination is rejected before the body is looked at.
    const uint8_t one[1] = {0xAB};
    TEST_ASSERT_FALSE(pc_devicenet_build_explicit(NULL, DEVICENET_GROUP_2, 4, 0x21, one, 1));
}
...
void test_build_explicit_body_arguments()
{
    ...
    TEST_ASSERT_FALSE(pc_devicenet_build_explicit(&f, DEVICENET_GROUP_2, 4, 0x21, NULL, 3));
}
...
void test_frag_non_fragmented_header_only()
{
    ...
    DeviceNetFragRx rx;
    pc_devicenet_frag_reset(&rx);
    TEST_ASSERT_EQUAL_INT(DEVICENET_FRAG_COMPLETE, pc_devicenet_frag_feed(&rx, body, 1));
...
void test_frag_empty_data_fragments()
{
    ...
    TEST_ASSERT_EQUAL_INT(DEVICENET_FRAG_STARTED, pc_devicenet_frag_feed(&rx, f, 2));
    TEST_ASSERT_EQUAL_UINT16(0, rx.len);
    f[1] = pc_devicenet_frag_octet(DEVICENET_FRAG_MIDDLE, 1);
    TEST_ASSERT_EQUAL_INT(DEVICENET_FRAG_PROGRESS, pc_devicenet_frag_feed(&rx, f, 2));
    TEST_ASSERT_EQUAL_UINT16(0, rx.len);
    f[1] = pc_devicenet_frag_octet(DEVICENET_FRAG_LAST, 2);
    TEST_ASSERT_EQUAL_INT(DEVICENET_FRAG_COMPLETE, pc_devicenet_frag_feed(&rx, f, 2));
    TEST_ASSERT_EQUAL_UINT16(0, rx.len);
...
void test_frag_sequence_rejects()
{
    ...
    TEST_ASSERT_EQUAL_INT(DEVICENET_FRAG_ERR, pc_devicenet_frag_feed(&rx, mid, 3));
    TEST_ASSERT_FALSE(rx.active);
    ...
    TEST_ASSERT_EQUAL_INT(DEVICENET_FRAG_STARTED, pc_devicenet_frag_feed(&rx, first, 3));
    uint8_t last[3] = {0x80 | 0x21, pc_devicenet_frag_octet(DEVICENET_FRAG_LAST, 5), 0xCC};
    TEST_ASSERT_EQUAL_INT(DEVICENET_FRAG_ERR, pc_devicenet_frag_feed(&rx, last, 3));
    TEST_ASSERT_FALSE(rx.active);
    TEST_ASSERT_EQUAL_UINT16(0, rx.len);
}
