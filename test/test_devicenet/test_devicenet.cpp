...
{
    uint32_t id = 0;
    TEST_ASSERT_TRUE(devicenet_encode_id(&id, DeviceNetGroup::DEVICENET_GROUP_1, 0x0A, 0x05)); // msgid 10, mac 5
...
void test_id_group2()
{
    ...
    TEST_ASSERT_TRUE(
        devicenet_encode_id(&id, DeviceNetGroup::DEVICENET_GROUP_2, DEVICENET_G2_UNCONNECTED_EXPLICIT_REQ, 0x21));
...
{
    uint32_t id = 0;
    TEST_ASSERT_TRUE(devicenet_encode_id(&id, DeviceNetGroup::DEVICENET_GROUP_3, 5, 0x09)); // 11 MsgID(3) MAC(6)
    ...
    TEST_ASSERT_TRUE(devicenet_encode_id(&id, DeviceNetGroup::DEVICENET_GROUP_4, 0x2A, 0)); // 11111 MsgID(6)
    ...
    TEST_ASSERT_FALSE(devicenet_encode_id(&id, DeviceNetGroup::DEVICENET_GROUP_1, 16, 0));   // msgid > 4 bits
    TEST_ASSERT_FALSE(devicenet_encode_id(&id, DeviceNetGroup::DEVICENET_GROUP_2, 0, 64));   // mac > 6 bits
    TEST_ASSERT_FALSE(devicenet_encode_id(&id, DeviceNetGroup::DEVICENET_GROUP_4, 0x30, 0)); // msgid > 0x2F
}
...
void test_build_explicit_single_frame()
{
    ...
    CanFrame f;
    TEST_ASSERT_TRUE(devicenet_build_explicit(&f, DeviceNetGroup::DEVICENET_GROUP_2,
    ...
    uint8_t big[8] = {0};
    TEST_ASSERT_FALSE(devicenet_build_explicit(&f, DeviceNetGroup::DEVICENET_GROUP_2, 4, 0x21, big, 8));
}
...
void test_frag_non_fragmented()
{
    ...
    TEST_ASSERT_EQUAL_INT(DeviceNetFragResult::DEVICENET_FRAG_COMPLETE, devicenet_frag_feed(&rx, body, 4));
...
void test_frag_reassembly_roundtrip()
{
    ...
    TEST_ASSERT_EQUAL_INT(DeviceNetFragResult::DEVICENET_FRAG_STARTED, devicenet_frag_feed(&rx, f0, 8));

    // Middle fragment (count 1): + 6 more data.
    uint8_t f1[8] = {0x80 | 0x21, devicenet_frag_octet(DEVICENET_FRAG_MIDDLE, 1), 7, 8, 9, 10, 11, 12};
    TEST_ASSERT_EQUAL_INT(DeviceNetFragResult::DEVICENET_FRAG_PROGRESS, devicenet_frag_feed(&rx, f1, 8));

    // Last fragment (count 2): + 2 data.
    uint8_t f2[4] = {0x80 | 0x21, devicenet_frag_octet(DEVICENET_FRAG_LAST, 2), 13, 14};
    TEST_ASSERT_EQUAL_INT(DeviceNetFragResult::DEVICENET_FRAG_COMPLETE, devicenet_frag_feed(&rx, f2, 4));
...
void test_frag_out_of_order_errors()
{
    ...
    TEST_ASSERT_EQUAL_INT(DeviceNetFragResult::DEVICENET_FRAG_STARTED, devicenet_frag_feed(&rx, f0, 3));
    ...
    TEST_ASSERT_EQUAL_INT(DeviceNetFragResult::DEVICENET_FRAG_ERR, devicenet_frag_feed(&rx, bad, 3));
...
{
    uint32_t id = 0;
    TEST_ASSERT_FALSE(devicenet_encode_id(&id, DeviceNetGroup::DEVICENET_GROUP_2, 8, 0)); // group 2 msg_id > 7
    TEST_ASSERT_FALSE(devicenet_encode_id(&id, DeviceNetGroup::DEVICENET_GROUP_3, 8, 0)); // group 3 msg_id > 7
    ...
    TEST_ASSERT_FALSE(devicenet_build_explicit(&f, DeviceNetGroup::DEVICENET_GROUP_2, 8, 0, one, 1)); // id encode fails
}
...
void test_frag_reject_paths()
{
    ...
    TEST_ASSERT_EQUAL_INT(DeviceNetFragResult::DEVICENET_FRAG_IGNORED,
                          devicenet_frag_feed(nullptr, body, 3)); // null rx
    TEST_ASSERT_EQUAL_INT(DeviceNetFragResult::DEVICENET_FRAG_IGNORED,
                          devicenet_frag_feed(&rx, nullptr, 3));                                           // null body
    TEST_ASSERT_EQUAL_INT(DeviceNetFragResult::DEVICENET_FRAG_IGNORED, devicenet_frag_feed(&rx, body, 0)); // empty

    const uint8_t hdr_only[1] = {0x80 | 0x21}; // FRAG set but no fragmentation octet
    TEST_ASSERT_EQUAL_INT(DeviceNetFragResult::DEVICENET_FRAG_ERR, devicenet_frag_feed(&rx, hdr_only, 1));
    ...
    TEST_ASSERT_EQUAL_INT(DeviceNetFragResult::DEVICENET_FRAG_ERR,
                          devicenet_frag_feed(&rx, last_no_first, sizeof(last_no_first)));
...
void test_frag_overflow()
{
    ...
    TEST_ASSERT_EQUAL_INT(DeviceNetFragResult::DEVICENET_FRAG_STARTED, devicenet_frag_feed(&rx, frag, sizeof(frag)));
    frag[1] = devicenet_frag_octet(DEVICENET_FRAG_MIDDLE, 1); // 253 + 253 = 506 > 256
    TEST_ASSERT_EQUAL_INT(DeviceNetFragResult::DEVICENET_FRAG_ERR, devicenet_frag_feed(&rx, frag, sizeof(frag)));
    TEST_ASSERT_FALSE(rx.active);
