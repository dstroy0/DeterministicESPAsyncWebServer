...
{
    uint8_t buf[64];
    size_t len = pc_mqtt_build_publish(buf, sizeof(buf), "a/b", (const uint8_t *)"hi", 2, 0, 0, false, false);
    TEST_ASSERT_GREATER_THAN(0, len);
...
{
    uint8_t buf[64];
    size_t len = pc_mqtt_build_publish(buf, sizeof(buf), "t", (const uint8_t *)"x", 1, 1, 0x1234, true, true);
    TEST_ASSERT_GREATER_THAN(0, len);
...
{
    uint8_t buf[64];
    size_t len = pc_mqtt_build_publish(buf, sizeof(buf), "abcdef", (const uint8_t *)"", 0, 0, 0, false, false);
    uint8_t type, flags;
    uint32_t rl;
    size_t hl;
...
{
    uint8_t buf[64];
    size_t len = pc_mqtt_build_publish(buf, sizeof(buf), "t", (const uint8_t *)"x", 1, 1, 1, false, false);
    uint8_t type, flags;
    uint32_t rl;
    size_t hl;
    pc_mqtt_parse_fixed_header(buf, len, &type, &flags, &rl, &hl);
...
void test_publish_wildcard_topic_rejected()
{
    ...
    TEST_ASSERT_EQUAL_size_t(
        0, pc_mqtt_build_publish(buf, sizeof(buf), "a/+/b", (const uint8_t *)"x", 1, 0, 0, false, false));
    TEST_ASSERT_EQUAL_size_t(
        0, pc_mqtt_build_publish(buf, sizeof(buf), "a/#", (const uint8_t *)"x", 1, 0, 0, false, false));
    // A plain topic with no wildcards still builds.
    TEST_ASSERT_GREATER_THAN(
        0, pc_mqtt_build_publish(buf, sizeof(buf), "a/b/c", (const uint8_t *)"x", 1, 0, 0, false, false));
}
...
{
    uint8_t buf[8];
    TEST_ASSERT_EQUAL_size_t(4, pc_mqtt_build_ack(buf, sizeof(buf), MqttType::MQTT_PUBACK, 0x2222));
    TEST_ASSERT_EQUAL_HEX8(0x40, buf[0]);
    TEST_ASSERT_EQUAL_UINT16(0x2222, pc_mqtt_parse_ack(buf + 2, 2));

    TEST_ASSERT_EQUAL_size_t(4, pc_mqtt_build_ack(buf, sizeof(buf), MqttType::MQTT_PUBREL, 0x3333));
...
void test_build_guards_and_overflow()
{
    ...
    memset(&o, 0, sizeof(o));
    o.client_id = "c";
    TEST_ASSERT_EQUAL_UINT(0, pc_mqtt_build_connect(nullptr, sizeof(out), &o));
    TEST_ASSERT_EQUAL_UINT(0, pc_mqtt_build_connect(out, sizeof(out), nullptr));
    ...
    TEST_ASSERT_EQUAL_UINT(0, pc_mqtt_build_publish(nullptr, sizeof(out), "t", nullptr, 0, 0, 0, false, false));
    TEST_ASSERT_EQUAL_UINT(0, pc_mqtt_build_publish(out, sizeof(out), "t", nullptr, 0, 3, 0, false, false));    // qos>2
    TEST_ASSERT_EQUAL_UINT(0, pc_mqtt_build_publish(out, sizeof(out), "t", nullptr, 2000, 0, 0, false, false)); // body
    TEST_ASSERT_EQUAL_UINT(0,
                           pc_mqtt_build_publish(out, 2, "topic", (const uint8_t *)"hi", 2, 0, 0, false, false)); // cap

    TEST_ASSERT_EQUAL_UINT(0, pc_mqtt_build_subscribe(nullptr, sizeof(out), 1, "t", 0));
    ...
    TEST_ASSERT_EQUAL_UINT(0, pc_mqtt_build_unsubscribe(nullptr, sizeof(out), 1, "t"));
    TEST_ASSERT_EQUAL_UINT(0, pc_mqtt_build_unsubscribe(out, sizeof(out), 1, big_topic)); // body overflow

    TEST_ASSERT_EQUAL_UINT(0, pc_mqtt_build_ack(nullptr, 4, (MqttType)4, 1));
    TEST_ASSERT_EQUAL_UINT(0, pc_mqtt_build_ack(out, 3, (MqttType)4, 1)); // cap < 4
    TEST_ASSERT_EQUAL_UINT(0, pc_mqtt_build_pingreq(nullptr, 2));
    TEST_ASSERT_EQUAL_UINT(0, pc_mqtt_build_pingreq(out, 1)); // cap < 2
    TEST_ASSERT_EQUAL_UINT(0, pc_mqtt_build_disconnect(nullptr, 2));
...
void test_parse_guards()
{
    ...
    const uint8_t *pay;
    uint16_t pid;
    TEST_ASSERT_FALSE(pc_mqtt_parse_publish(nullptr, 0, 0, topic, sizeof(topic), &tl, &pay, &pl, &pid));
    ...
    TEST_ASSERT_EQUAL_UINT16(0, pc_mqtt_parse_ack(nullptr, 0));
    uint16_t spid;
    uint8_t rc;
    uint8_t two[2] = {0, 0};
...
{
    pc_mqtt_set_message_cb(nullptr);
    MqttConnectOpts o;
    memset(&o, 0, sizeof(o));
    o.client_id = "c";
    TEST_ASSERT_FALSE(pc_mqtt_connect("h", 1883, false, &o));
    TEST_ASSERT_FALSE(pc_mqtt_publish("t", nullptr, 0, 0, false));
...
{
    uint8_t out[64];
    TEST_ASSERT_EQUAL_UINT(0, pc_mqtt_build_publish(out, sizeof(out), nullptr, nullptr, 0, 0, 0, false, false));
    TEST_ASSERT_EQUAL_UINT(0, pc_mqtt_build_subscribe(out, sizeof(out), 1, nullptr, 0));
    TEST_ASSERT_EQUAL_UINT(0, pc_mqtt_build_unsubscribe(out, sizeof(out), 1, nullptr));
...
void test_parse_short_len_and_null_outparam_guards()
{
    ...
    TEST_ASSERT_EQUAL_INT(-1, pc_mqtt_parse_connack(nullptr, 2, nullptr));
    TEST_ASSERT_EQUAL_INT(0, pc_mqtt_parse_connack(ok, 2, nullptr)); // session_present output not requested

    uint8_t sb[3] = {0x00, 0x0A, 0x01};
    TEST_ASSERT_FALSE(pc_mqtt_parse_suback(nullptr, 3, &pid, nullptr));
    TEST_ASSERT_TRUE(pc_mqtt_parse_suback(sb, 3, nullptr, nullptr)); // caller wants neither out-param
}
