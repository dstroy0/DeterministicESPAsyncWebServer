...
{
    uint8_t b[4];
    size_t n = det_mqtt_encode_remlen(b, v);
    TEST_ASSERT_EQUAL_size_t(expect_bytes, n);
    uint32_t out = 0;
    size_t used = 0;
    TEST_ASSERT_TRUE(det_mqtt_decode_remlen(b, n, &out, &used));
...
{
    uint8_t b[4];
    TEST_ASSERT_EQUAL_size_t(0, det_mqtt_encode_remlen(b, 268435456u));
}
...
void test_remlen_decode_incomplete()
{
    ...
    uint32_t v;
    size_t used;
    TEST_ASSERT_FALSE(det_mqtt_decode_remlen(b, 2, &v, &used));
}
...
void test_remlen_decode_malformed()
{
    ...
    uint32_t v;
    size_t used;
    TEST_ASSERT_FALSE(det_mqtt_decode_remlen(b, 5, &v, &used));
}
...
void test_connect_minimal()
{
    ...
    o.keepalive_s = 30;
    uint8_t buf[64];
    size_t len = det_mqtt_build_connect(buf, sizeof(buf), &o);
    ...
    uint32_t rl;
    size_t hl;
    TEST_ASSERT_TRUE(det_mqtt_parse_fixed_header(buf, len, &type, &flags, &rl, &hl));
...
{
    uint8_t buf[64];
    size_t len = det_mqtt_build_publish(buf, sizeof(buf), "a/b", (const uint8_t *)"hi", 2, 0, 0, false, false);
    TEST_ASSERT_GREATER_THAN(0, len);
    ...
    uint16_t pid;
    TEST_ASSERT_TRUE(det_mqtt_parse_publish(buf + hl, rl, flags, topic, sizeof(topic), &tlen, &payload, &plen, &pid));
...
{
    uint8_t buf[64];
    size_t len = det_mqtt_build_publish(buf, sizeof(buf), "t", (const uint8_t *)"x", 1, 1, 0x1234, true, true);
    TEST_ASSERT_GREATER_THAN(0, len);
...
{
    uint8_t buf[64];
    size_t len = det_mqtt_build_publish(buf, sizeof(buf), "abcdef", (const uint8_t *)"", 0, 0, 0, false, false);
    uint8_t type, flags;
    uint32_t rl;
    size_t hl;
    det_mqtt_parse_fixed_header(buf, len, &type, &flags, &rl, &hl);
    char topic[4]; // too small for "abcdef"
    size_t tlen, plen;
    const uint8_t *payload;
    uint16_t pid;
    TEST_ASSERT_FALSE(det_mqtt_parse_publish(buf + hl, rl, flags, topic, sizeof(topic), &tlen, &payload, &plen, &pid));
}
...
{
    uint8_t buf[64];
    size_t len = det_mqtt_build_publish(buf, sizeof(buf), "t", (const uint8_t *)"x", 1, 1, 1, false, false);
    uint8_t type, flags;
    uint32_t rl;
    size_t hl;
    det_mqtt_parse_fixed_header(buf, len, &type, &flags, &rl, &hl);
...
void test_publish_wildcard_topic_rejected()
{
    ...
    TEST_ASSERT_EQUAL_size_t(
        0, det_mqtt_build_publish(buf, sizeof(buf), "a/+/b", (const uint8_t *)"x", 1, 0, 0, false, false));
    TEST_ASSERT_EQUAL_size_t(
        0, det_mqtt_build_publish(buf, sizeof(buf), "a/#", (const uint8_t *)"x", 1, 0, 0, false, false));
    // A plain topic with no wildcards still builds.
    TEST_ASSERT_GREATER_THAN(
        0, det_mqtt_build_publish(buf, sizeof(buf), "a/b/c", (const uint8_t *)"x", 1, 0, 0, false, false));
}
...
void test_publish_topic_nul_or_bad_utf8_rejected()
{
    ...
    TEST_ASSERT_FALSE(
        det_mqtt_parse_publish(bad_utf8, sizeof(bad_utf8), 0, topic, sizeof(topic), &tlen, &payload, &plen, &pid));
    // topic length 2, bytes {'a',0x00} = embedded NUL.
    const uint8_t embedded_nul[] = {0x00, 0x02, 'a', 0x00};
    TEST_ASSERT_FALSE(det_mqtt_parse_publish(embedded_nul, sizeof(embedded_nul), 0, topic, sizeof(topic), &tlen,
                                             &payload, &plen, &pid));
    // A well-formed topic of the same shape still parses.
    const uint8_t ok[] = {0x00, 0x03, 'a', '/', 'b'};
    TEST_ASSERT_TRUE(det_mqtt_parse_publish(ok, sizeof(ok), 0, topic, sizeof(topic), &tlen, &payload, &plen, &pid));
    TEST_ASSERT_EQUAL_STRING("a/b", topic);
}
...
{
    uint8_t buf[64];
    size_t len = det_mqtt_build_subscribe(buf, sizeof(buf), 10, "s/#", 1);
    TEST_ASSERT_GREATER_THAN(0, len);
...
{
    uint8_t buf[64];
    size_t len = det_mqtt_build_unsubscribe(buf, sizeof(buf), 11, "s/#");
    TEST_ASSERT_GREATER_THAN(0, len);
...
{
    uint8_t buf[8];
    TEST_ASSERT_EQUAL_size_t(4, det_mqtt_build_ack(buf, sizeof(buf), MqttType::MQTT_PUBACK, 0x2222));
    TEST_ASSERT_EQUAL_HEX8(0x40, buf[0]);
    TEST_ASSERT_EQUAL_UINT16(0x2222, det_mqtt_parse_ack(buf + 2, 2));

    TEST_ASSERT_EQUAL_size_t(4, det_mqtt_build_ack(buf, sizeof(buf), MqttType::MQTT_PUBREL, 0x3333));
...
void test_connack()
{
    ...
    bool sp = false;
    TEST_ASSERT_EQUAL_INT(0, det_mqtt_parse_connack(ok, 2, &sp));
    TEST_ASSERT_TRUE(sp);
    uint8_t bad[2] = {0x00, 0x05};
    TEST_ASSERT_EQUAL_INT(5, det_mqtt_parse_connack(bad, 2, &sp));
    TEST_ASSERT_FALSE(sp);
    TEST_ASSERT_EQUAL_INT(-1, det_mqtt_parse_connack(bad, 1, &sp));
}
...
void test_suback()
{
    ...
    uint16_t pid;
    uint8_t rc;
    TEST_ASSERT_TRUE(det_mqtt_parse_suback(b, 3, &pid, &rc));
...
{
    uint8_t buf[4];
    TEST_ASSERT_EQUAL_size_t(2, det_mqtt_build_pingreq(buf, sizeof(buf)));
    TEST_ASSERT_EQUAL_HEX8(0xC0, buf[0]);
    TEST_ASSERT_EQUAL_HEX8(0x00, buf[1]);
    TEST_ASSERT_EQUAL_size_t(2, det_mqtt_build_disconnect(buf, sizeof(buf)));
...
void test_fixed_header_multibyte_remlen()
{
    ...
    uint32_t rl;
    size_t hl;
    TEST_ASSERT_TRUE(det_mqtt_parse_fixed_header(buf, 3, &type, &flags, &rl, &hl));
...
void test_build_guards_and_overflow()
{
    ...
    memset(&o, 0, sizeof(o));
    o.client_id = "c";
    TEST_ASSERT_EQUAL_UINT(0, det_mqtt_build_connect(nullptr, sizeof(out), &o));
    TEST_ASSERT_EQUAL_UINT(0, det_mqtt_build_connect(out, sizeof(out), nullptr));
    MqttConnectOpts no_id = o;
    no_id.client_id = nullptr;
    TEST_ASSERT_EQUAL_UINT(0, det_mqtt_build_connect(out, sizeof(out), &no_id));
    // Oversized will payload overflows the body scratch.
    MqttConnectOpts wo = o;
    wo.will_topic = "w";
    wo.will_len = 2000;
    TEST_ASSERT_EQUAL_UINT(0, det_mqtt_build_connect(out, sizeof(out), &wo));
    // A valid CONNECT that does not fit the output buffer (compose cap check).
    TEST_ASSERT_EQUAL_UINT(0, det_mqtt_build_connect(out, 2, &o));

    TEST_ASSERT_EQUAL_UINT(0, det_mqtt_build_publish(nullptr, sizeof(out), "t", nullptr, 0, 0, 0, false, false));
    TEST_ASSERT_EQUAL_UINT(0, det_mqtt_build_publish(out, sizeof(out), "t", nullptr, 0, 3, 0, false, false)); // qos>2
    TEST_ASSERT_EQUAL_UINT(0, det_mqtt_build_publish(out, sizeof(out), "t", nullptr, 2000, 0, 0, false, false)); // body
    TEST_ASSERT_EQUAL_UINT(
        0, det_mqtt_build_publish(out, 2, "topic", (const uint8_t *)"hi", 2, 0, 0, false, false)); // cap

    TEST_ASSERT_EQUAL_UINT(0, det_mqtt_build_subscribe(nullptr, sizeof(out), 1, "t", 0));
    TEST_ASSERT_EQUAL_UINT(0, det_mqtt_build_subscribe(out, sizeof(out), 1, "t", 3));       // qos>2
    TEST_ASSERT_EQUAL_UINT(0, det_mqtt_build_subscribe(out, sizeof(out), 1, big_topic, 0)); // body overflow

    TEST_ASSERT_EQUAL_UINT(0, det_mqtt_build_unsubscribe(nullptr, sizeof(out), 1, "t"));
    TEST_ASSERT_EQUAL_UINT(0, det_mqtt_build_unsubscribe(out, sizeof(out), 1, big_topic)); // body overflow

    TEST_ASSERT_EQUAL_UINT(0, det_mqtt_build_ack(nullptr, 4, (MqttType)4, 1));
    TEST_ASSERT_EQUAL_UINT(0, det_mqtt_build_ack(out, 3, (MqttType)4, 1)); // cap < 4
    TEST_ASSERT_EQUAL_UINT(0, det_mqtt_build_pingreq(nullptr, 2));
    TEST_ASSERT_EQUAL_UINT(0, det_mqtt_build_pingreq(out, 1)); // cap < 2
    TEST_ASSERT_EQUAL_UINT(0, det_mqtt_build_disconnect(nullptr, 2));
    TEST_ASSERT_EQUAL_UINT(0, det_mqtt_build_disconnect(out, 1)); // cap < 2
}
...
void test_parse_guards()
{
    ...
    size_t hl;
    uint8_t one[1] = {0x30};
    TEST_ASSERT_FALSE(det_mqtt_parse_fixed_header(one, 1, &type, &flags, &rl, &hl)); // avail < 2
    uint8_t bad_rl[5] = {0x30, 0x80, 0x80, 0x80, 0x80};
    TEST_ASSERT_FALSE(det_mqtt_parse_fixed_header(bad_rl, 5, &type, &flags, &rl, &hl)); // malformed remlen

    char topic[32];
    size_t tl, pl;
    const uint8_t *pay;
    uint16_t pid;
    TEST_ASSERT_FALSE(det_mqtt_parse_publish(nullptr, 0, 0, topic, sizeof(topic), &tl, &pay, &pl, &pid));
    uint8_t claim[2] = {0x00, 0x10}; // tlen=16 but remaining_len=2
    TEST_ASSERT_FALSE(det_mqtt_parse_publish(claim, 2, 0, topic, sizeof(topic), &tl, &pay, &pl, &pid));
    uint8_t q1[4] = {0x00, 0x02, 'a', 'b'}; // qos1, topic fills the buffer, no room for packet id
    TEST_ASSERT_FALSE(det_mqtt_parse_publish(q1, 4, 0x02, topic, sizeof(topic), &tl, &pay, &pl, &pid));

    TEST_ASSERT_EQUAL_UINT16(0, det_mqtt_parse_ack(nullptr, 0));
    uint16_t spid;
    uint8_t rc;
    uint8_t two[2] = {0, 0};
    TEST_ASSERT_FALSE(det_mqtt_parse_suback(two, 2, &spid, &rc)); // remaining_len < 3
}

// On a host build the transport entry points are inert stubs that fail closed.
void test_host_transport_stubs()
{
    det_mqtt_set_message_cb(nullptr);
    MqttConnectOpts o;
    memset(&o, 0, sizeof(o));
    o.client_id = "c";
    TEST_ASSERT_FALSE(det_mqtt_connect("h", 1883, false, &o));
    TEST_ASSERT_FALSE(det_mqtt_publish("t", nullptr, 0, 0, false));
    TEST_ASSERT_FALSE(det_mqtt_subscribe("t", 0));
    TEST_ASSERT_FALSE(det_mqtt_unsubscribe("t"));
    TEST_ASSERT_FALSE(det_mqtt_loop());
    TEST_ASSERT_FALSE(det_mqtt_connected());
    det_mqtt_disconnect();
}
