...
{
    char buf[160];
    int n = pc_ifttt_url("button_pressed", "abc123", buf, sizeof(buf));
...
{
    char buf[128];
    int n = pc_ifttt_payload("a", "b", "c", buf, sizeof(buf));
    TEST_ASSERT_TRUE(n > 0);
...
{
    char buf[128];
    pc_ifttt_payload("only1", nullptr, nullptr, buf, sizeof(buf));
    TEST_ASSERT_EQUAL_STRING("{\"value1\":\"only1\"}", buf);

    pc_ifttt_payload(nullptr, "mid", nullptr, buf, sizeof(buf));
    TEST_ASSERT_EQUAL_STRING("{\"value2\":\"mid\"}", buf);

    pc_ifttt_payload(nullptr, nullptr, nullptr, buf, sizeof(buf));
...
{
    char buf[128];
    pc_ifttt_payload("he said \"hi\"", "a\\b", nullptr, buf, sizeof(buf));
...
{
    char buf[8];
    TEST_ASSERT_EQUAL_INT(0, pc_ifttt_url("event", "key", buf, sizeof(buf)));
    TEST_ASSERT_EQUAL_INT(0, pc_ifttt_payload("aaaa", "bbbb", "cccc", buf, sizeof(buf)));
}
...
void test_ifttt_trigger_and_post_stub()
{
    ...
    TEST_ASSERT_EQUAL_INT(-1, pc_webhook_post("http://x/y", "{}"));
    TEST_ASSERT_EQUAL_INT(-1, pc_ifttt_trigger("evt", "key", "1", "2", "3"));
}
...
void test_builder_arg_guards()
{
    char buf[64];
    buf[0] = 'x';
    TEST_ASSERT_EQUAL_INT(0, pc_ifttt_url(nullptr, "k", buf, sizeof(buf)));         // null event
    TEST_ASSERT_EQUAL_STRING("", buf);                                              // cleared
    TEST_ASSERT_EQUAL_INT(0, pc_ifttt_url("e", nullptr, buf, sizeof(buf)));         // null key
    TEST_ASSERT_EQUAL_INT(0, pc_ifttt_url("e", "k", nullptr, 10));                  // null out (no clear)
    TEST_ASSERT_EQUAL_INT(0, pc_ifttt_url("e", "k", buf, 0));                       // zero cap
    TEST_ASSERT_EQUAL_INT(0, pc_ifttt_payload("a", nullptr, nullptr, nullptr, 64)); // null out
    TEST_ASSERT_EQUAL_INT(0, pc_ifttt_payload("a", nullptr, nullptr, buf, 0));      // zero cap
}
...
void test_payload_escape_overflow_fails_closed()
{
    ...
    TEST_ASSERT_EQUAL_INT(0, pc_ifttt_payload("aaaaaaaaaa", nullptr, nullptr, buf, sizeof(buf)));
    // A value whose escape ('"' -> two bytes) lands with < 2 bytes left also fails closed.
    TEST_ASSERT_EQUAL_INT(0, pc_ifttt_payload("aaa\"", nullptr, nullptr, buf, sizeof(buf)));
}

// pc_ifttt_trigger returns -1 when the url or payload cannot be built (too long for its
...
void test_trigger_build_failures()
{
    ...
    memset(bigev, 'e', 190);
    bigev[190] = '\0';
    TEST_ASSERT_EQUAL_INT(-1, pc_ifttt_trigger(bigev, "k", "1", nullptr, nullptr)); // url overflows url[160]

    char bigval[400];
    memset(bigval, 'v', 350);
    bigval[350] = '\0';
    TEST_ASSERT_EQUAL_INT(-1, pc_ifttt_trigger("e", "k", bigval, nullptr, nullptr)); // payload overflows body[256]
}
...
void test_payload_write_fails_at_each_field(void)
{
    ...
    TEST_ASSERT_EQUAL_INT(0, pc_ifttt_payload("z", nullptr, nullptr, buf, 2));

    // cap=11: "{" + '"' + "value1" fit (pos=8); the "\":\"" separator does not (8+3>=11).
    TEST_ASSERT_EQUAL_INT(0, pc_ifttt_payload("x", nullptr, nullptr, buf, 11));

    // cap=13: everything through the escaped 1-char value fits (pos=12); the closing
    // '"' does not (12+1>=13).
    TEST_ASSERT_EQUAL_INT(0, pc_ifttt_payload("y", nullptr, nullptr, buf, 13));

    // cap=14: the whole first (and only) value fits (pos=13); the final "}" does not
    // (13+1>=14).
    TEST_ASSERT_EQUAL_INT(0, pc_ifttt_payload("y", nullptr, nullptr, buf, 14));
}

// When the comma before a second value fails to fit, ok goes false mid-iteration and
// every later put() call in that same pc_ifttt_payload() invocation is skipped by
...
void test_payload_comma_failure_skips_rest(void)
{
    ...
    TEST_ASSERT_EQUAL_INT(0, pc_ifttt_payload("a", "b", nullptr, buf, 14));
}
