...
void test_bounds_and_empty_value(void)
{
    ...
    TEST_ASSERT_TRUE(pc_dbm_put(&g_db, "empty", 5, nullptr, 0));
    uint8_t b[4];
...
void test_replay_skips_malformed_records(void)
{
    ...
    TEST_ASSERT_TRUE(raw_append(0, PC_DBM_KEY_MAX + 1, 0, nullptr, 0)); // key longer than the bound
...
void test_iterate_visits_live_keys_and_honours_an_early_stop(void)
{
    ...
    TEST_ASSERT_EQUAL_UINT32(2, pc_dbm_iterate(&g_db, nullptr, nullptr));
