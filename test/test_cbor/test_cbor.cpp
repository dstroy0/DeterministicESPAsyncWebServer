...
void test_decode_uint()
{
    ...
    TEST_ASSERT_EQUAL_INT(pc_codec_type::PC_CODEC_UINT, pc_cbor_peek(&r));
    uint64_t v;
...
void test_decode_roundtrip_map()
{
    ...
    TEST_ASSERT_TRUE(b);
    TEST_ASSERT_TRUE(pc_cspan_ok(r));
    TEST_ASSERT_EQUAL_INT(pc_codec_type::PC_CODEC_INVALID, pc_cbor_peek(&r)); // everything consumed
}
...
void test_cbor_reader_sticky_err_repeat()
{
    ...
    TEST_ASSERT_EQUAL_INT(pc_codec_type::PC_CODEC_INVALID, pc_cbor_peek(&r));
}
...
void test_peek_edge_cases()
{
    ...
    TEST_ASSERT_EQUAL_INT(pc_codec_type::PC_CODEC_BOOL, pc_cbor_peek(&r));

    uint8_t dbl[] = {0xfb, 0, 0, 0, 0, 0, 0, 0, 0}; // double marker -> info == 27
    r = pc_cspan_from(dbl, sizeof(dbl));
    TEST_ASSERT_EQUAL_INT(pc_codec_type::PC_CODEC_FLOAT, pc_cbor_peek(&r));
}
...
void test_peek_each_type()
{
    ...
    TEST_ASSERT_EQUAL_INT(pc_codec_type::PC_CODEC_INT, pc_cbor_peek(&r));
    w = pc_span_from(b, sizeof(b));
    pc_cbor_bytes(&w, d, 2);
    r = pc_cspan_from(b, pc_span_len(w));
    TEST_ASSERT_EQUAL_INT(pc_codec_type::PC_CODEC_BYTES, pc_cbor_peek(&r));
    w = pc_span_from(b, sizeof(b));
    pc_cbor_str(&w, "x");
    r = pc_cspan_from(b, pc_span_len(w));
    TEST_ASSERT_EQUAL_INT(pc_codec_type::PC_CODEC_STR, pc_cbor_peek(&r));
    w = pc_span_from(b, sizeof(b));
    pc_cbor_array(&w, 1);
    r = pc_cspan_from(b, pc_span_len(w));
    TEST_ASSERT_EQUAL_INT(pc_codec_type::PC_CODEC_ARRAY, pc_cbor_peek(&r));
    w = pc_span_from(b, sizeof(b));
    pc_cbor_map(&w, 1);
    r = pc_cspan_from(b, pc_span_len(w));
    TEST_ASSERT_EQUAL_INT(pc_codec_type::PC_CODEC_MAP, pc_cbor_peek(&r));
    w = pc_span_from(b, sizeof(b));
    pc_cbor_bool(&w, true);
    r = pc_cspan_from(b, pc_span_len(w));
    TEST_ASSERT_EQUAL_INT(pc_codec_type::PC_CODEC_BOOL, pc_cbor_peek(&r));
    ...
    pc_cbor_null(&w);
    r = pc_cspan_from(b, pc_span_len(w));
    TEST_ASSERT_EQUAL_INT(pc_codec_type::PC_CODEC_NULL, pc_cbor_peek(&r));
    w = pc_span_from(b, sizeof(b));
