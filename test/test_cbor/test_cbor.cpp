...
void test_uint()
{
    ...
    uint8_t e0[] = {0x00};
    check(b, dws_cbor_len(&w), e0, 1);
    dws_cbor_init(&w, b, sizeof(b));
    dws_cbor_uint(&w, 23);
    uint8_t e23[] = {0x17};
    check(b, dws_cbor_len(&w), e23, 1);
    dws_cbor_init(&w, b, sizeof(b));
    dws_cbor_uint(&w, 24);
    uint8_t e24[] = {0x18, 0x18};
    check(b, dws_cbor_len(&w), e24, 2);
    dws_cbor_init(&w, b, sizeof(b));
    dws_cbor_uint(&w, 1000);
    uint8_t e1k[] = {0x19, 0x03, 0xe8};
    check(b, dws_cbor_len(&w), e1k, 3);
    dws_cbor_init(&w, b, sizeof(b));
    dws_cbor_uint(&w, 1000000);
    uint8_t e1m[] = {0x1a, 0x00, 0x0f, 0x42, 0x40};
    check(b, dws_cbor_len(&w), e1m, 5);
}
...
void test_int()
{
    ...
    check(b, dws_cbor_len(&w), e2, 2);
    dws_cbor_init(&w, b, sizeof(b));
    dws_cbor_int(&w, -1000);
    uint8_t e3[] = {0x39, 0x03, 0xe7};
    check(b, dws_cbor_len(&w), e3, 3);
    dws_cbor_init(&w, b, sizeof(b));
    dws_cbor_int(&w, 42); // positive routed through dws_cbor_int
    uint8_t e4[] = {0x18, 0x2a};
    check(b, dws_cbor_len(&w), e4, 2);
}
...
void test_text()
{
    ...
    check(b, dws_cbor_len(&w), e2, 2);
    dws_cbor_init(&w, b, sizeof(b));
    dws_cbor_text(&w, "IETF");
    uint8_t e3[] = {0x64, 0x49, 0x45, 0x54, 0x46};
    check(b, dws_cbor_len(&w), e3, 5);
}
...
void test_bytes()
{
    ...
    dws_cbor_bytes(&w, data, 4);
    uint8_t e[] = {0x44, 0x01, 0x02, 0x03, 0x04};
    check(b, dws_cbor_len(&w), e, 5);
}
...
void test_simple()
{
    ...
    CborWriter w;
    dws_cbor_init(&w, b, sizeof(b));
    dws_cbor_bool(&w, false);
    uint8_t e[] = {0xf4};
    check(b, dws_cbor_len(&w), e, 1);
    dws_cbor_init(&w, b, sizeof(b));
    dws_cbor_bool(&w, true);
    uint8_t e2[] = {0xf5};
    check(b, dws_cbor_len(&w), e2, 1);
    dws_cbor_init(&w, b, sizeof(b));
    dws_cbor_null(&w);
    uint8_t e3[] = {0xf6};
    check(b, dws_cbor_len(&w), e3, 1);
}
...
void test_overflow_fails_closed()
{
    ...
    CborWriter w;
    dws_cbor_init(&w, b, sizeof(b));
    dws_cbor_uint(&w, 1000000); // needs 5 bytes
    TEST_ASSERT_FALSE(dws_cbor_ok(&w));
    TEST_ASSERT_EQUAL_size_t(5, dws_cbor_len(&w));
}
...
void test_decode_uint()
{
    ...
    dws_cbor_uint(&w, 1000);
    CborReader r;
    dws_cbor_reader_init(&r, buf, dws_cbor_len(&w));
    TEST_ASSERT_EQUAL_INT(CborType::CBOR_TYPE_UINT, dws_cbor_peek(&r));
    uint64_t v;
    TEST_ASSERT_TRUE(dws_cbor_read_uint(&r, &v));
    TEST_ASSERT_EQUAL_UINT64(1000, v);
    TEST_ASSERT_TRUE(dws_cbor_reader_ok(&r));
}
...
void test_decode_int()
{
    ...
    dws_cbor_reader_init(&r, buf, dws_cbor_len(&w));
    int64_t v;
    TEST_ASSERT_TRUE(dws_cbor_read_int(&r, &v));
...
void test_decode_float_roundtrip()
{
    ...
    CborWriter w;
    dws_cbor_init(&w, buf, sizeof(buf));
    dws_cbor_float(&w, 3.5f);
    CborReader r;
    dws_cbor_reader_init(&r, buf, dws_cbor_len(&w));
    float f;
    TEST_ASSERT_TRUE(dws_cbor_read_float(&r, &f));
...
void test_decode_roundtrip_map()
{
    ...
    dws_cbor_uint(&w, 42000);
    dws_cbor_text(&w, "name");
    dws_cbor_text(&w, "esp");
    dws_cbor_text(&w, "on");
    dws_cbor_bool(&w, true);
    ...
    dws_cbor_reader_init(&r, buf, dws_cbor_len(&w));
    size_t n;
    TEST_ASSERT_TRUE(dws_cbor_read_map(&r, &n));
...
void test_decode_truncated()
{
    ...
    CborWriter w;
    dws_cbor_init(&w, buf, sizeof(buf));
    dws_cbor_uint(&w, 1000000); // 5 bytes
    CborReader r;
    dws_cbor_reader_init(&r, buf, 3); // only 3 bytes visible
    uint64_t v;
    TEST_ASSERT_FALSE(dws_cbor_read_uint(&r, &v));
...
// dws_cbor_peek classifies every major type, and reports INVALID for tags / unassigned.
void test_peek_each_type()
{
    ...
    dws_cbor_init(&w, b, sizeof(b));
    dws_cbor_int(&w, -5);
    dws_cbor_reader_init(&r, b, dws_cbor_len(&w));
    TEST_ASSERT_EQUAL_INT(CborType::CBOR_TYPE_INT, dws_cbor_peek(&r));
    dws_cbor_init(&w, b, sizeof(b));
    dws_cbor_bytes(&w, d, 2);
    dws_cbor_reader_init(&r, b, dws_cbor_len(&w));
    TEST_ASSERT_EQUAL_INT(CborType::CBOR_TYPE_BYTES, dws_cbor_peek(&r));
    dws_cbor_init(&w, b, sizeof(b));
    dws_cbor_text(&w, "x");
    dws_cbor_reader_init(&r, b, dws_cbor_len(&w));
    TEST_ASSERT_EQUAL_INT(CborType::CBOR_TYPE_TEXT, dws_cbor_peek(&r));
    dws_cbor_init(&w, b, sizeof(b));
    dws_cbor_array(&w, 1);
    dws_cbor_reader_init(&r, b, dws_cbor_len(&w));
    TEST_ASSERT_EQUAL_INT(CborType::CBOR_TYPE_ARRAY, dws_cbor_peek(&r));
    dws_cbor_init(&w, b, sizeof(b));
    dws_cbor_map(&w, 1);
    dws_cbor_reader_init(&r, b, dws_cbor_len(&w));
    TEST_ASSERT_EQUAL_INT(CborType::CBOR_TYPE_MAP, dws_cbor_peek(&r));
    dws_cbor_init(&w, b, sizeof(b));
    dws_cbor_bool(&w, true);
    dws_cbor_reader_init(&r, b, dws_cbor_len(&w));
    ...
    dws_cbor_reader_init(&r, simple, 1);
    TEST_ASSERT_EQUAL_INT(CborType::CBOR_TYPE_INVALID, dws_cbor_peek(&r));
}
...
void test_uint_8byte()
{
    ...
    CborWriter w;
    dws_cbor_init(&w, b, sizeof(b));
    dws_cbor_uint(&w, 0x123456789ULL);
    TEST_ASSERT_TRUE(dws_cbor_ok(&w));
    TEST_ASSERT_EQUAL_size_t(9, dws_cbor_len(&w));
...
// dws_cbor_read_map on a non-map sets the error flag.
void test_read_map_type_mismatch()
{
    ...
    dws_cbor_reader_init(&r, b, dws_cbor_len(&w));
    size_t n;
    TEST_ASSERT_FALSE(dws_cbor_read_map(&r, &n));
    TEST_ASSERT_FALSE(dws_cbor_reader_ok(&r));
}
...
void test_cbor_head_reserved_and_trunc()
{
    ...
    dws_cbor_reader_init(&r, f4, sizeof(f4));
    TEST_ASSERT_FALSE(dws_cbor_read_float(&r, &fv));
    uint8_t f8[] = {0xfb, 0x00, 0x00}; // float64 arg short
    dws_cbor_reader_init(&r, f8, sizeof(f8));
    TEST_ASSERT_FALSE(dws_cbor_read_float(&r, &fv));
}
...
void test_cbor_read_empty()
{
    ...
    TEST_ASSERT_FALSE(dws_cbor_read_float(&r, &fv));
    dws_cbor_reader_init(&r, &d, 0);
    TEST_ASSERT_FALSE(dws_cbor_read_text(&r, &s, &n));
    dws_cbor_reader_init(&r, &d, 0);
    TEST_ASSERT_FALSE(dws_cbor_read_bytes(&r, &bp, &n));
    dws_cbor_reader_init(&r, &d, 0);
    TEST_ASSERT_FALSE(dws_cbor_read_array(&r, &c));
    dws_cbor_reader_init(&r, &d, 0);
