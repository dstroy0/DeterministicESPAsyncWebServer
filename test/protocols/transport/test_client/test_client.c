...
{
    TEST_ASSERT_TRUE(Tcp.client->open("example.com", 80, 1000) < 0);
    TEST_ASSERT_EQUAL_INT(-1, Tcp.client->open(NULL, 0, 0));
}
...
void test_is_closed_always_true()
{
    ...
    TEST_ASSERT_TRUE(Tcp.client->is_closed(0));
    TEST_ASSERT_TRUE(Tcp.client->is_closed(1));
}
...
void test_send_always_false()
{
    ...
    TEST_ASSERT_FALSE(Tcp.client->send(-1, payload, sizeof(payload)));
    TEST_ASSERT_FALSE(Tcp.client->send(0, payload, sizeof(payload)));
    TEST_ASSERT_FALSE(Tcp.client->send(0, NULL, 0));
}

void test_available_always_zero()
{
    TEST_ASSERT_EQUAL_size_t(0, Tcp.client->available(-1));
    TEST_ASSERT_EQUAL_size_t(0, Tcp.client->available(0));
}
...
void test_read_always_zero()
{
    ...
    TEST_ASSERT_EQUAL_size_t(0, Tcp.client->read(0, buf, sizeof(buf)));
