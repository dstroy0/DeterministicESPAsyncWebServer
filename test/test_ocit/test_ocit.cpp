...
void test_build_and_parse(void)
{
    ...
    uint8_t out[16];
    size_t n = pc_ocit_build(OcitMsgType::OCIT_MSG_SET, 0x0102, 0x0003, OcitType::OCIT_TYPE_UINT32, value, 4, out,
                             sizeof(out));
...
{
    uint8_t out[16];
    size_t n = pc_ocit_set_u16(0x00A0, 0x0005, 0xBEEF, out, sizeof(out));
    TEST_ASSERT_EQUAL_size_t(8, n);
    OcitMsg m;
    TEST_ASSERT_TRUE(pc_ocit_parse(out, n, &m));
    TEST_ASSERT_EQUAL_HEX8(OcitMsgType::OCIT_MSG_SET, m.msg_type);
    TEST_ASSERT_EQUAL_HEX8(OcitType::OCIT_TYPE_UINT16, m.data_type);
    TEST_ASSERT_EQUAL_HEX16(0xBEEF, pc_ocit_value_u16(&m));
}

void test_get_no_value(void)
{
    uint8_t out[8];
    size_t n = pc_ocit_build(OcitMsgType::OCIT_MSG_GET, 0x0102, 0x0003, OcitType::OCIT_TYPE_UINT16, nullptr, 0, out,
                             sizeof(out));
...
void test_parse_rejects_short(void)
{
    ...
    TEST_ASSERT_FALSE(pc_ocit_parse(tooshort, 5, &m));
}
...
void test_build_rejects_null_out(void)
{
    ...
    size_t n =
        pc_ocit_build(OcitMsgType::OCIT_MSG_SET, 0x0001, 0x0002, OcitType::OCIT_TYPE_UINT16, value, 2, nullptr, 16);
...
{
    uint8_t out[16];
    size_t n = pc_ocit_build(OcitMsgType::OCIT_MSG_SET, 0x0001, 0x0002, OcitType::OCIT_TYPE_UINT16, nullptr, 2, out,
                             sizeof(out));
    TEST_ASSERT_EQUAL_size_t(0, n);
}
...
void test_build_rejects_overflow(void)
{
    ...
    size_t n = pc_ocit_build(OcitMsgType::OCIT_MSG_SET, 0x0001, 0x0002, OcitType::OCIT_TYPE_UINT32, value, 4, out,
                             sizeof(out));
    TEST_ASSERT_EQUAL_size_t(0, n);
}
...
{
    OcitMsg m;
    TEST_ASSERT_FALSE(pc_ocit_parse(nullptr, 6, &m));
}

void test_parse_rejects_null_out(void)
{
    uint8_t msg[6] = {0x01, 0x00, 0x01, 0x00, 0x02, 0x03};
    TEST_ASSERT_FALSE(pc_ocit_parse(msg, sizeof(msg), nullptr));
}

void test_value_u16_rejects_null_msg(void)
{
    TEST_ASSERT_EQUAL_HEX16(0, pc_ocit_value_u16(nullptr));
}
...
void test_value_u16_rejects_wrong_type(void)
{
    ...
    uint8_t value[2] = {0x00, 0x01};
    size_t n =
        pc_ocit_build(OcitMsgType::OCIT_MSG_SET, 0x0001, 0x0002, OcitType::OCIT_TYPE_BYTE, value, 2, out, sizeof(out));
    OcitMsg m;
    TEST_ASSERT_TRUE(pc_ocit_parse(out, n, &m));
    TEST_ASSERT_EQUAL_HEX16(0, pc_ocit_value_u16(&m));
}
...
{
    // Hand-built OcitMsg (not reachable via pc_ocit_parse) exercising the !m->value guard
