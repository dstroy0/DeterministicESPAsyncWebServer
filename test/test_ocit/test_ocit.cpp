...
{
    uint8_t out[16];
    size_t n = detws_ocit_set_u16(0x00A0, 0x0005, 0xBEEF, out, sizeof(out));
...
void test_get_no_value(void)
{
    ...
    TEST_ASSERT_EQUAL_HEX16(0, detws_ocit_value_u16(&m));
}
...
void test_parse_rejects_short(void)
{
    ...
    TEST_ASSERT_FALSE(detws_ocit_parse(tooshort, 5, &m));
}
