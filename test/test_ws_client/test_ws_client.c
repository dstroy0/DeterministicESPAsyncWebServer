...
{
    uint8_t buf[256];
    size_t n = ws_client_build_handshake(buf, sizeof(buf), "example.com", "/chat", "dGhlIHNhbXBsZSBub25jZQ==", NULL);
...
void test_accept_for_key_guards()
{
    ...
    out[0] = 'x';
    ws_client_accept_for_key(NULL, out, sizeof(out)); // key == NULL
...
{
    uint8_t out[256];
    TEST_ASSERT_EQUAL_UINT(0, ws_client_build_handshake(NULL, sizeof(out), "h", "/", "k", NULL));
    TEST_ASSERT_EQUAL_UINT(0, ws_client_build_handshake(out, sizeof(out), NULL, "/", "k", NULL));
    TEST_ASSERT_EQUAL_UINT(0, ws_client_build_handshake(out, sizeof(out), "h", NULL, "k", NULL));
    TEST_ASSERT_EQUAL_UINT(0, ws_client_build_handshake(out, sizeof(out), "h", "/", NULL, NULL));
    TEST_ASSERT_EQUAL_UINT(0, ws_client_build_handshake(out, 10, "host", "/path", "key", NULL)); // overflow
}
...
{
    TEST_ASSERT_FALSE(ws_client_check_response(NULL, 100, "acc"));
    uint8_t s[4] = {'a', 'b', 'c', 'd'};
    TEST_ASSERT_FALSE(ws_client_check_response(s, 4, "acc")); // len < 12
    const char *ok = "HTTP/1.1 101 x";
    TEST_ASSERT_FALSE(ws_client_check_response((const uint8_t *)ok, strlen(ok), NULL)); // null accept
