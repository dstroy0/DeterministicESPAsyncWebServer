...
void test_kbdint_correct_password_succeeds()
{
    ...
    TEST_ASSERT_EQUAL_INT(0, SshAuth.handle_info_response(0, pkt, n, out, &olen, sizeof(out)));
...
void test_kbdint_wrong_password_fails()
{
    ...
    size_t n, olen = 0;
    n = build_kbdint_request(pkt, "alice");
    SshAuth.handle_request(0, pkt, n, out, &olen, sizeof(out));
...
void test_kbdint_response_without_request_fails()
{
    ...
    TEST_ASSERT_EQUAL_INT(-1, SshAuth.handle_info_response(0, pkt, n, out, &olen, sizeof(out)));
    TEST_ASSERT_FALSE(ssh_sess[0].authed);
}
...
void test_kbdint_response_replay_fails()
{
    ...
    SshAuth.handle_request(0, pkt, n, out, &olen, sizeof(out));
    n = build_info_response(pkt, 1, "s3cret");
    SshAuth.handle_info_response(0, pkt, n, out, &olen, sizeof(out)); // consumes the exchange
    TEST_ASSERT_EQUAL_INT(-1, SshAuth.handle_info_response(0, pkt, n, out, &olen, sizeof(out)));
}
...
void test_kbdint_request_without_verifier_or_room()
{
    ...
    SshAuth.set_password_cb(NULL); // nothing could ever verify an answer -> do not prompt
    TEST_ASSERT_EQUAL_INT(0, SshAuth.handle_request(0, pkt, n, out, &olen, sizeof(out)));
    TEST_ASSERT_EQUAL_UINT8(SSH_MSG_USERAUTH_FAILURE, out[0]);

    SshAuth.set_password_cb(check_uv); // verifier present, but the prompt does not fit
    TEST_ASSERT_EQUAL_INT(-1, SshAuth.handle_request(0, pkt, n, out, &olen, 8));
}
...
void test_kbdint_info_response_wire_guards()
{
    ...
    size_t n = build_info_response(pkt, 1, "s3cret");
    TEST_ASSERT_EQUAL_INT(-1, SshAuth.handle_info_response(MAX_SSH_CONNS, pkt, n, out, &olen, sizeof(out)));

    TEST_ASSERT_EQUAL_INT(0, SshAuth.handle_request(0, req, rq, out, &olen, sizeof(out)));
    TEST_ASSERT_EQUAL_INT(-1, SshAuth.handle_info_response(0, pkt, 0, out, &olen, sizeof(out))); // empty
    ...
    TEST_ASSERT_EQUAL_INT(-1, SshAuth.handle_info_response(0, stub, sizeof(stub), out, &olen, sizeof(out)));

    TEST_ASSERT_EQUAL_INT(0, SshAuth.handle_request(0, req, rq, out, &olen, sizeof(out)));
    uint8_t trunc[9] = {SSH_MSG_USERAUTH_INFO_RESPONSE, 0, 0, 0, 1, 0, 0, 0, 40}; // declares 40 bytes
    TEST_ASSERT_EQUAL_INT(0, SshAuth.handle_info_response(0, trunc, sizeof(trunc), out, &olen, sizeof(out)));
    TEST_ASSERT_EQUAL_UINT8(SSH_MSG_USERAUTH_FAILURE, out[0]);

    TEST_ASSERT_EQUAL_INT(0, SshAuth.handle_request(0, req, rq, out, &olen, sizeof(out)));
    SshAuth.set_password_cb(NULL); // verifier removed mid-exchange
    n = build_info_response(pkt, 1, "s3cret");
    TEST_ASSERT_EQUAL_INT(0, SshAuth.handle_info_response(0, pkt, n, out, &olen, sizeof(out)));
    TEST_ASSERT_EQUAL_UINT8(SSH_MSG_USERAUTH_FAILURE, out[0]);
    TEST_ASSERT_FALSE(ssh_sess[0].authed);
...
static int dispatch_kbdint_round(const char *answer)
{
    uint8_t pkt[128];
    size_t n = build_kbdint_request(pkt, "alice");
    int rc = SshServer.dispatch(0, SSH_MSG_USERAUTH_REQUEST, pkt, n);
    if (rc != 0)
    {
        return rc;
    }
    n = build_info_response(pkt, 1, answer);
    return SshServer.dispatch(0, SSH_MSG_USERAUTH_INFO_RESPONSE, pkt, n);
}
...
void test_kbdint_dispatch_guards_and_success()
{
    ...
    TEST_ASSERT_EQUAL_INT(-1, SshServer.dispatch(0, SSH_MSG_USERAUTH_INFO_RESPONSE, pkt, n));
...
// ---------------------------------------------------------------------------
// Full SshServer.dispatch() switch coverage: every SSH_MSG_* arm of the
...
void test_dispatch_all_switch_arms()
{
    ...
    TEST_ASSERT_EQUAL_INT(-1, SshServer.dispatch(0, SSH_MSG_USERAUTH_INFO_RESPONSE, ir, sizeof(ir)));
...
void test_dispatch_guard_and_error_arms()
{
    ...
    s->phase = SSH_PHASE_SERVICE;
    TEST_ASSERT_EQUAL_INT(-1, SshServer.dispatch(0, SSH_MSG_USERAUTH_INFO_RESPONSE, badir, sizeof(badir)));
    s->phase = SSH_PHASE_AUTH;
    TEST_ASSERT_EQUAL_INT(-1, SshServer.dispatch(0, SSH_MSG_USERAUTH_INFO_RESPONSE, badir, sizeof(badir)));
