...
void test_crc_check_vector(void)
{
    ...
    TEST_ASSERT_EQUAL_HEX16(0x906E, dws_nema_ts2_crc(msg, sizeof(msg)));
}
...
void test_build_and_parse(void)
{
    ...
    uint8_t buf[16];
    size_t n = dws_nema_ts2_build(0x05, 0x10, NemaTs2::NEMA_TS2_FT_CMD_LOADSWITCH, data, 3, buf, sizeof(buf));
    ...
    NemaTs2Frame f;
    TEST_ASSERT_TRUE(dws_nema_ts2_parse(buf, n, &f));
...
{
    uint8_t buf[8];
    size_t n = dws_nema_ts2_build(0x01, 0x00, NemaTs2::NEMA_TS2_FT_DETECTOR, nullptr, 0, buf, sizeof(buf));
...
void test_parse_rejects_bad_crc_and_short(void)
{
    ...
    TEST_ASSERT_FALSE(dws_nema_ts2_parse(buf, n, &f));
