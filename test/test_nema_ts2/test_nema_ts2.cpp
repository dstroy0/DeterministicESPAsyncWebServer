...
void test_crc_check_vector(void)
{
    ...
    TEST_ASSERT_EQUAL_HEX16(0x906E, pc_nema_ts2_crc(msg, sizeof(msg)));
}
...
void test_build_and_parse(void)
{
    ...
    uint8_t buf[16];
    size_t n = pc_nema_ts2_build(0x05, 0x10, NemaTs2::NEMA_TS2_FT_CMD_LOADSWITCH, data, 3, buf, sizeof(buf));
    ...
    NemaTs2Frame f;
    TEST_ASSERT_TRUE(pc_nema_ts2_parse(buf, n, &f));
...
{
    uint8_t buf[8];
    size_t n = pc_nema_ts2_build(0x01, 0x00, NemaTs2::NEMA_TS2_FT_DETECTOR, nullptr, 0, buf, sizeof(buf));
...
void test_parse_rejects_bad_crc_and_short(void)
{
    ...
    uint8_t buf[8];
    size_t n = pc_nema_ts2_build(0x02, 0x00, NemaTs2::NEMA_TS2_FT_CMD_MMU, data, 2, buf, sizeof(buf));
    NemaTs2Frame f;
    buf[n - 1] ^= 0xFF; // corrupt FCS high byte
    TEST_ASSERT_FALSE(pc_nema_ts2_parse(buf, n, &f));
    TEST_ASSERT_FALSE(pc_nema_ts2_parse(buf, 4, &f)); // too short
}
...
void test_build_rejects_bad_args(void)
{
    ...
    TEST_ASSERT_EQUAL_size_t(
        0, pc_nema_ts2_build(0x01, 0x00, NemaTs2::NEMA_TS2_FT_CMD_MMU, data, 2, nullptr, sizeof(buf)));

    // non-zero data_len with a null data pointer.
    TEST_ASSERT_EQUAL_size_t(0,
                             pc_nema_ts2_build(0x01, 0x00, NemaTs2::NEMA_TS2_FT_CMD_MMU, nullptr, 2, buf, sizeof(buf)));
}
...
void test_build_rejects_undersized_cap(void)
{
    ...
    TEST_ASSERT_EQUAL_size_t(0, pc_nema_ts2_build(0x01, 0x00, NemaTs2::NEMA_TS2_FT_CMD_MMU, data, 2, buf, 6));
}
...
void test_parse_rejects_null_args(void)
{
    ...
    TEST_ASSERT_FALSE(pc_nema_ts2_parse(nullptr, n, &f));  // null frame
    TEST_ASSERT_FALSE(pc_nema_ts2_parse(buf, n, nullptr)); // null out
}
