...
void test_bearer_header()
{
    ...
    TEST_ASSERT_FALSE(pc_jwt_bearer_valid(nullptr, sec(), seclen()));
}
...
void test_scope_allows()
{
    ...
    TEST_ASSERT_FALSE(pc_jwt_scope_allows(nullptr, "read"));
}
...
{
    long v = 0;
    TEST_ASSERT_FALSE(pc_jwt_claim_int(nullptr, 5, "x", &v));
    TEST_ASSERT_FALSE(pc_jwt_claim_int(TOKEN, strlen(TOKEN), nullptr, &v));
    TEST_ASSERT_FALSE(pc_jwt_claim_int(TOKEN, strlen(TOKEN), "exp", nullptr));
...
{
    char buf[32];
    TEST_ASSERT_FALSE(pc_jwt_claim_str(nullptr, 5, "x", buf, sizeof(buf)));
...
void test_bearer_header_guards()
{
    ...
    TEST_ASSERT_FALSE(pc_jwt_bearer_valid(nullptr, secret, sizeof(secret)));               // null header
...
{
    TEST_ASSERT_FALSE(pc_jwt_verify_hs256(nullptr, 20, sec(), seclen()));
...
{
    TEST_ASSERT_TRUE(pc_jwt_time_valid(nullptr, 0, 1700000000, 0));
}
...
void test_bearer_valid_at_header_guards()
{
    char t[400], hdr[470];
    mk_signed(t, sizeof(t), "{\"exp\":1700000000}");
    TEST_ASSERT_FALSE(pc_jwt_bearer_valid_at(nullptr, sec(), seclen(), 1699999000, 0));
...
{
    char buf[32], t[256];
    TEST_ASSERT_FALSE(pc_jwt_claim_str(TOKEN, strlen(TOKEN), nullptr, buf, sizeof(buf)));
    TEST_ASSERT_FALSE(pc_jwt_claim_str(TOKEN, strlen(TOKEN), "sub", nullptr, sizeof(buf)));
...
{
    TEST_ASSERT_FALSE(pc_jwt_scope_allows("read write", nullptr));
}
