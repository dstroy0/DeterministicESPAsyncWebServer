...
void test_rail_monitor(void)
{
    ...
    TEST_ASSERT_EQUAL_INT(HwRailVerdict::HW_RAIL_OK, detws_hwhealth_rail_sample(&m, 3300));
    TEST_ASSERT_EQUAL_INT(HwRailVerdict::HW_RAIL_OK, detws_hwhealth_rail_sample(&m, 3100)); // boundary: == warn is OK
    TEST_ASSERT_EQUAL_INT(HwRailVerdict::HW_RAIL_SAG, detws_hwhealth_rail_sample(&m, 3050));
    TEST_ASSERT_EQUAL_INT(HwRailVerdict::HW_RAIL_SAG, detws_hwhealth_rail_sample(&m, 2900)); // == crit is still SAG
    TEST_ASSERT_EQUAL_INT(HwRailVerdict::HW_RAIL_BROWNOUT, detws_hwhealth_rail_sample(&m, 2800));
...
{
    TEST_ASSERT_EQUAL_INT(HwGpioVerdict::HW_GPIO_OK, detws_hwhealth_gpio_short(true, true));
    TEST_ASSERT_EQUAL_INT(HwGpioVerdict::HW_GPIO_OK, detws_hwhealth_gpio_short(false, false));
    TEST_ASSERT_EQUAL_INT(HwGpioVerdict::HW_GPIO_SHORT_GND, detws_hwhealth_gpio_short(true, false));
    TEST_ASSERT_EQUAL_INT(HwGpioVerdict::HW_GPIO_SHORT_VCC, detws_hwhealth_gpio_short(false, true));
}

void test_cap_leak(void)
{
    // Expected 100ms decay, 10% tolerance -> [90, 110].
    TEST_ASSERT_EQUAL_INT(HwCapVerdict::HW_CAP_OK, detws_hwhealth_cap_leak(100, 100, 10));
    TEST_ASSERT_EQUAL_INT(HwCapVerdict::HW_CAP_OK, detws_hwhealth_cap_leak(90, 100, 10));        // boundary lo
    TEST_ASSERT_EQUAL_INT(HwCapVerdict::HW_CAP_OK, detws_hwhealth_cap_leak(110, 100, 10));       // boundary hi
    TEST_ASSERT_EQUAL_INT(HwCapVerdict::HW_CAP_LEAK, detws_hwhealth_cap_leak(80, 100, 10));      // too fast
    TEST_ASSERT_EQUAL_INT(HwCapVerdict::HW_CAP_HIGH_ESR, detws_hwhealth_cap_leak(130, 100, 10)); // too slow
    // Degenerate expected.
    TEST_ASSERT_EQUAL_INT(HwCapVerdict::HW_CAP_OK, detws_hwhealth_cap_leak(50, 0, 10));
}
...
void test_rail_ok_spi_clamps_probes()
{
    ...
    TEST_ASSERT_EQUAL_INT(HwRailVerdict::HW_RAIL_OK, detws_hwhealth_rail_sample(&m, 3300)); // nominal -> OK
    HwSpiBackoff s;
...
void test_hwhealth_null_guards_and_init_clamps(void)
{
    ...
    TEST_ASSERT_EQUAL_INT(HwRailVerdict::HW_RAIL_OK, detws_hwhealth_rail_sample(nullptr, 1000));
