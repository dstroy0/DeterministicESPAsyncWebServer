...
{
    HwRailMonitor m;
    pc_hwhealth_rail_init(&m, 3300, 3100, 2900); // nominal 3.3V, warn 3.1V, crit 2.9V
    TEST_ASSERT_EQUAL_INT(HwRailVerdict::HW_RAIL_OK, pc_hwhealth_rail_sample(&m, 3300));
    TEST_ASSERT_EQUAL_INT(HwRailVerdict::HW_RAIL_OK, pc_hwhealth_rail_sample(&m, 3100)); // boundary: == warn is OK
    TEST_ASSERT_EQUAL_INT(HwRailVerdict::HW_RAIL_SAG, pc_hwhealth_rail_sample(&m, 3050));
    TEST_ASSERT_EQUAL_INT(HwRailVerdict::HW_RAIL_SAG, pc_hwhealth_rail_sample(&m, 2900)); // == crit is still SAG
    TEST_ASSERT_EQUAL_INT(HwRailVerdict::HW_RAIL_BROWNOUT, pc_hwhealth_rail_sample(&m, 2800));
    ...
    char buf[96];
    size_t n = pc_hwhealth_rail_json(&m, buf, sizeof(buf));
    TEST_ASSERT_EQUAL_size_t(strlen(buf), n);
    TEST_ASSERT_EQUAL_STRING("{\"nominal_mv\":3300,\"min_mv\":2800,\"sag\":2,\"brownout\":1}", buf);
    // Overflow path returns 0.
    char tiny[8];
    TEST_ASSERT_EQUAL_size_t(0, pc_hwhealth_rail_json(&m, tiny, sizeof(tiny)));
}
...
{
    TEST_ASSERT_EQUAL_INT(HwGpioVerdict::HW_GPIO_OK, pc_hwhealth_gpio_short(true, true));
    TEST_ASSERT_EQUAL_INT(HwGpioVerdict::HW_GPIO_OK, pc_hwhealth_gpio_short(false, false));
    TEST_ASSERT_EQUAL_INT(HwGpioVerdict::HW_GPIO_SHORT_GND, pc_hwhealth_gpio_short(true, false));
    TEST_ASSERT_EQUAL_INT(HwGpioVerdict::HW_GPIO_SHORT_VCC, pc_hwhealth_gpio_short(false, true));
}

void test_cap_leak(void)
{
    // Expected 100ms decay, 10% tolerance -> [90, 110].
    TEST_ASSERT_EQUAL_INT(HwCapVerdict::HW_CAP_OK, pc_hwhealth_cap_leak(100, 100, 10));
    TEST_ASSERT_EQUAL_INT(HwCapVerdict::HW_CAP_OK, pc_hwhealth_cap_leak(90, 100, 10));        // boundary lo
    TEST_ASSERT_EQUAL_INT(HwCapVerdict::HW_CAP_OK, pc_hwhealth_cap_leak(110, 100, 10));       // boundary hi
    TEST_ASSERT_EQUAL_INT(HwCapVerdict::HW_CAP_LEAK, pc_hwhealth_cap_leak(80, 100, 10));      // too fast
    TEST_ASSERT_EQUAL_INT(HwCapVerdict::HW_CAP_HIGH_ESR, pc_hwhealth_cap_leak(130, 100, 10)); // too slow
    // Degenerate expected.
    TEST_ASSERT_EQUAL_INT(HwCapVerdict::HW_CAP_OK, pc_hwhealth_cap_leak(50, 0, 10));
}

void test_rail_ok_spi_clamps_probes()
{
    HwRailMonitor m;
    pc_hwhealth_rail_init(&m, 3300, 3000, 2800);
    TEST_ASSERT_EQUAL_INT(HwRailVerdict::HW_RAIL_OK, pc_hwhealth_rail_sample(&m, 3300)); // nominal -> OK
    HwSpiBackoff s;
...
{
    char buf[64];
    pc_hwhealth_rail_init(nullptr, 3300, 3100, 2900); // no-op, no crash
    TEST_ASSERT_EQUAL_INT(HwRailVerdict::HW_RAIL_OK, pc_hwhealth_rail_sample(nullptr, 1000));
    TEST_ASSERT_EQUAL_size_t(0, pc_hwhealth_rail_json(nullptr, buf, sizeof(buf))); // null monitor

    HwRailMonitor m;
    pc_hwhealth_rail_init(&m, 3300, 3100, 2900);
    TEST_ASSERT_EQUAL_size_t(0, pc_hwhealth_rail_json(&m, nullptr, sizeof(buf))); // null out
    TEST_ASSERT_EQUAL_size_t(0, pc_hwhealth_rail_json(&m, buf, 0));               // zero cap

    pc_hwhealth_spi_init(nullptr, 1, 1, 1, 1, 1); // no-op, no crash
    TEST_ASSERT_EQUAL_UINT32(0, pc_hwhealth_spi_result(nullptr, true));
...
void test_hwhealth_trip_defaults_overflow_and_band_clamp(void)
{
    ...
    TEST_ASSERT_EQUAL_INT(HwCapVerdict::HW_CAP_OK, pc_hwhealth_cap_leak(0, 50, 100)); // band(50) >= expected(50)
}
