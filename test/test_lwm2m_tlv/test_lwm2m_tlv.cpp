...
void test_write_int_1byte()
{
    ...
    dws_lwm2m_tlv_write_int(&w, 3, 42);
    size_t n = dws_lwm2m_tlv_finish(&w);
...
void test_write_int_2byte()
{
    ...
    Lwm2mTlvWriter w;
    dws_lwm2m_tlv_init(&w, buf, sizeof(buf));
    dws_lwm2m_tlv_write_int(&w, 1, 300);
    size_t n = dws_lwm2m_tlv_finish(&w);
...
void test_write_string_8bit_length()
{
    ...
    Lwm2mTlvWriter w;
    dws_lwm2m_tlv_init(&w, buf, sizeof(buf));
    dws_lwm2m_tlv_write_string(&w, 0, "Open Mobile Alliance");
    size_t n = dws_lwm2m_tlv_finish(&w);
...
void test_write_16bit_id()
{
    ...
    Lwm2mTlvWriter w;
    dws_lwm2m_tlv_init(&w, buf, sizeof(buf));
    dws_lwm2m_tlv_write_int(&w, 0x0405, 1);
    size_t n = dws_lwm2m_tlv_finish(&w);
...
void test_round_trip_and_value_int()
{
    ...
    Lwm2mTlvWriter w;
    dws_lwm2m_tlv_init(&w, buf, sizeof(buf));
    dws_lwm2m_tlv_write_int(&w, 0, 42);
    dws_lwm2m_tlv_write_int(&w, 1, 300);
    dws_lwm2m_tlv_write_int(&w, 2, -1);
    dws_lwm2m_tlv_write_bool(&w, 3, true);
    dws_lwm2m_tlv_write_string(&w, 4, "hi");
    ...
    TEST_ASSERT_TRUE(dws_lwm2m_tlv_read(buf, total, &pos, &t));
    TEST_ASSERT_EQUAL_UINT8(LWM2M_TLV_RESOURCE, t.id_type);
    TEST_ASSERT_EQUAL_UINT16(0, t.id);
    TEST_ASSERT_TRUE(dws_lwm2m_tlv_value_int(t.value, t.value_len, &v));
    ...
    TEST_ASSERT_FALSE(dws_lwm2m_tlv_read(buf, total, &pos, &t)); // end
}
...
void test_object_instance_nested()
{
    uint8_t inner[16];
    Lwm2mTlvWriter iw;
    dws_lwm2m_tlv_init(&iw, inner, sizeof(inner));
    dws_lwm2m_tlv_write_int(&iw, 0, 1);
    dws_lwm2m_tlv_write_int(&iw, 1, 2);
    size_t inner_len = dws_lwm2m_tlv_finish(&iw);

    uint8_t buf[32];
    Lwm2mTlvWriter w;
    dws_lwm2m_tlv_init(&w, buf, sizeof(buf));
    dws_lwm2m_tlv_write(&w, LWM2M_TLV_OBJECT_INSTANCE, 0, inner, inner_len);
    size_t total = dws_lwm2m_tlv_finish(&w);
    ...
    size_t ipos = 0;
    Lwm2mTlv it;
    TEST_ASSERT_TRUE(dws_lwm2m_tlv_read(t.value, t.value_len, &ipos, &it));
...
void test_write_16bit_length()
{
    ...
    Lwm2mTlvWriter w;
    dws_lwm2m_tlv_init(&w, buf, sizeof(buf));
    TEST_ASSERT_TRUE(dws_lwm2m_tlv_write(&w, LWM2M_TLV_RESOURCE, 5, val, sizeof(val)));
    size_t total = dws_lwm2m_tlv_finish(&w);
...
void test_read_24bit_length()
{
    ...
    size_t pos = 0;
    Lwm2mTlv t;
    TEST_ASSERT_TRUE(dws_lwm2m_tlv_read(tlv, sizeof(tlv), &pos, &t));
    TEST_ASSERT_EQUAL_UINT8(LWM2M_TLV_RESOURCE, t.id_type);
    ...
    pos = 0;
    TEST_ASSERT_FALSE(dws_lwm2m_tlv_read(trunc, sizeof(trunc), &pos, &t));
}
...
void test_value_int_4_and_8_byte()
{
    ...
    Lwm2mTlvWriter w;
    dws_lwm2m_tlv_init(&w, buf, sizeof(buf));
    dws_lwm2m_tlv_write_int(&w, 0, 70000);         // > int16 -> 4 octets
    dws_lwm2m_tlv_write_int(&w, 1, 5000000000LL);  // > int32 -> 8 octets
    dws_lwm2m_tlv_write_int(&w, 2, -5000000000LL); // 8 octets, negative sign-extends
    size_t total = dws_lwm2m_tlv_finish(&w);
...
void test_zero_length_value()
{
    ...
    Lwm2mTlvWriter w;
    dws_lwm2m_tlv_init(&w, buf, sizeof(buf));
    TEST_ASSERT_TRUE(dws_lwm2m_tlv_write(&w, LWM2M_TLV_RESOURCE, 9, nullptr, 0));
    size_t total = dws_lwm2m_tlv_finish(&w);
...
void test_overflow_and_malformed()
{
    uint8_t small[2];
    Lwm2mTlvWriter w;
    dws_lwm2m_tlv_init(&w, small, sizeof(small));
    dws_lwm2m_tlv_write_string(&w, 0, "too big for two bytes");
    TEST_ASSERT_EQUAL_size_t(0, dws_lwm2m_tlv_finish(&w));
    ...
    TEST_ASSERT_FALSE(dws_lwm2m_tlv_value_int(three, 3, &v)); // 3 octets is not a valid int width
}
...
void test_write_error_paths()
{
    ...
    dws_lwm2m_tlv_init(&w, buf, sizeof(buf));
    TEST_ASSERT_FALSE(dws_lwm2m_tlv_write(nullptr, LWM2M_TLV_RESOURCE, 0, dummy, 1)); // null writer
    TEST_ASSERT_FALSE(dws_lwm2m_tlv_write(&w, LWM2M_TLV_RESOURCE, 0, nullptr, 4));    // len but null value
    ...
    dws_lwm2m_tlv_init(&w, buf, sizeof(buf));
    TEST_ASSERT_FALSE(dws_lwm2m_tlv_write(&w, LWM2M_TLV_RESOURCE, 0, dummy, 0x10000));
    // A value_len too large for even the 24-bit length field.
    dws_lwm2m_tlv_init(&w, buf, sizeof(buf));
    TEST_ASSERT_FALSE(dws_lwm2m_tlv_write(&w, LWM2M_TLV_RESOURCE, 0, dummy, 0x1000000));

    TEST_ASSERT_FALSE(dws_lwm2m_tlv_write_string(&w, 0, nullptr)); // null string
}
...
void test_write_float_roundtrip()
{
    ...
    Lwm2mTlvWriter w;
    dws_lwm2m_tlv_init(&w, buf, sizeof(buf));
    TEST_ASSERT_TRUE(dws_lwm2m_tlv_write_float(&w, 6, 3.5));
    size_t total = dws_lwm2m_tlv_finish(&w);
...
void test_read_id16_and_truncation()
{
    ...
    size_t pos = 0;
    Lwm2mTlv t;
    TEST_ASSERT_TRUE(dws_lwm2m_tlv_read(tlv16, sizeof(tlv16), &pos, &t));
    ...
    pos = 0;
    TEST_ASSERT_FALSE(dws_lwm2m_tlv_read(just_type, sizeof(just_type), &pos, &t));
