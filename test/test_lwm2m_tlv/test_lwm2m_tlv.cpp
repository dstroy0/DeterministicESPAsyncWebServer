...
void test_zero_length_value()
{
    ...
    Lwm2mTlvWriter w;
    pc_lwm2m_tlv_init(&w, buf, sizeof(buf));
    TEST_ASSERT_TRUE(pc_lwm2m_tlv_write(&w, LWM2M_TLV_RESOURCE, 9, nullptr, 0));
    size_t total = pc_lwm2m_tlv_finish(&w);
...
void test_write_error_paths()
{
    ...
    pc_lwm2m_tlv_init(&w, buf, sizeof(buf));
    TEST_ASSERT_FALSE(pc_lwm2m_tlv_write(nullptr, LWM2M_TLV_RESOURCE, 0, dummy, 1)); // null writer
    TEST_ASSERT_FALSE(pc_lwm2m_tlv_write(&w, LWM2M_TLV_RESOURCE, 0, nullptr, 4));    // len but null value
    ...
    TEST_ASSERT_FALSE(pc_lwm2m_tlv_write_string(&w, 0, nullptr)); // null string
}
...
void test_write_after_error_latched()
{
    ...
    // capacity check.
    TEST_ASSERT_FALSE(pc_lwm2m_tlv_write(&w, LWM2M_TLV_RESOURCE, 1, nullptr, 0));
}
...
void test_read_null_args()
{
    ...
    size_t pos = 0;
    Lwm2mTlv t;
    TEST_ASSERT_FALSE(pc_lwm2m_tlv_read(nullptr, sizeof(buf), &pos, &t));
    TEST_ASSERT_FALSE(pc_lwm2m_tlv_read(buf, sizeof(buf), nullptr, &t));
    TEST_ASSERT_FALSE(pc_lwm2m_tlv_read(buf, sizeof(buf), &pos, nullptr));
}
...
{
    int64_t out;
    TEST_ASSERT_FALSE(pc_lwm2m_tlv_value_int(nullptr, 1, &out));

    const uint8_t v1[] = {0x2A};
    TEST_ASSERT_TRUE(pc_lwm2m_tlv_value_int(v1, 1, nullptr));
}
