...
void test_ping_and_split_recv()
{
    ...
    uint8_t ping[9 + 8];
    pc_h2_write_header(ping, sizeof ping, 8, H2FrameType::H2_PING, 0, 0);
    memcpy(ping + 9, op, 8);
    ...
    {
        TEST_ASSERT_TRUE(pc_h2_conn_recv(&c, &in[k], 1));
    }
...
static bool feed_frame(H2Conn &c, uint8_t type, uint8_t flags, uint32_t sid, const uint8_t *pl, size_t pn)
{
    ...
    uint8_t hh[9];
    pc_h2_write_header(hh, sizeof hh, (uint32_t)pn, type, flags, sid);
    v.insert(v.end(), hh, hh + 9);
    if (pn)
    {
        v.insert(v.end(), pl, pl + pn);
    }
    return pc_h2_conn_recv(&c, v.data(), v.size());
}
...
void test_h2_frame_too_big()
{
    ...
    establish(c, cap);
    uint8_t hh[9];
    pc_h2_write_header(hh, sizeof hh, PC_H2_MAX_FRAME + 1, H2FrameType::H2_DATA, 0, 1);
...
void test_h2_continuation_more()
{
    ...
    }
    {
        ...
        H2Conn c;
        establish(c, cap);
        std::vector<uint8_t> frag(PC_H2_HDR_BLOCK - 8, 0);
