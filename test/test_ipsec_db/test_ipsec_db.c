...
void test_selector_range_edges_and_any()
{
    ...
    // Null guards.
    TEST_ASSERT_FALSE(pc_ipsec_selector_match(NULL, &lo_edge));
    TEST_ASSERT_FALSE(pc_ipsec_selector_match(&any, NULL));
}
...
void test_selector_from_ts()
{
    ...
    TEST_ASSERT_FALSE(pc_ipsec_selector_from_ts(&sel, NULL, &tsr));
}
