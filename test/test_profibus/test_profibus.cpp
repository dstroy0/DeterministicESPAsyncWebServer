...
void test_fcs(void)
{
    ...
    TEST_ASSERT_EQUAL_HEX8(0x4E, pc_pb_fcs(b, sizeof(b))); // 0x03+0x02+0x49
}

void test_sd1(void)
{
    uint8_t out[8];
    size_t n = pc_pb_build_sd1(0x03, 0x02, Profibus::PB_FC_REQUEST_FDL_STATUS, out, sizeof(out));
    ...
    PbTelegram t;
    TEST_ASSERT_TRUE(pc_pb_parse(out, n, &t));
...
void test_sd2_roundtrip(void)
{
    ...
    uint8_t out[16];
    size_t n = pc_pb_build_sd2(0x05, 0x02, Profibus::PB_FC_SRD_HIGH, data, 3, out, sizeof(out));
...
void test_sd3_roundtrip(void)
{
    ...
    uint8_t out[16];
    size_t n = pc_pb_build_sd3(0x05, 0x02, 0x7C, data, out, sizeof(out));
    ...
    TEST_ASSERT_EQUAL_size_t(0, pc_pb_build_sd3(0x05, 0x02, 0x7C, data, nullptr, sizeof(out)));
    TEST_ASSERT_EQUAL_size_t(0, pc_pb_build_sd3(0x05, 0x02, 0x7C, nullptr, out, sizeof(out)));
    TEST_ASSERT_EQUAL_size_t(0, pc_pb_build_sd3(0x05, 0x02, 0x7C, data, out, 13)); // needs 14

    // A corrupt FCS is rejected; a truncated SD3 (< 14) is rejected.
    out[12] ^= 0xFF;
    TEST_ASSERT_FALSE(pc_pb_parse(out, n, &t));
    out[12] ^= 0xFF;                             // restore
    TEST_ASSERT_FALSE(pc_pb_parse(out, 13, &t)); // truncated
}
...
void test_parse_rejects(void)
{
    ...
    uint8_t out[16];
    size_t n = pc_pb_build_sd2(0x05, 0x02, Profibus::PB_FC_SRD_LOW, data, 2, out, sizeof(out));
    PbTelegram t;
    out[n - 2] ^= 0xFF; // corrupt FCS
    TEST_ASSERT_FALSE(pc_pb_parse(out, n, &t));
    // Mismatched LE/LEr.
    n = pc_pb_build_sd2(0x05, 0x02, Profibus::PB_FC_SRD_LOW, data, 2, out, sizeof(out));
    out[2] = 0x99;
    TEST_ASSERT_FALSE(pc_pb_parse(out, n, &t));
    // data_len > 246 rejected at build.
    TEST_ASSERT_EQUAL_size_t(0, pc_pb_build_sd2(0x05, 0x02, Profibus::PB_FC_SRD_LOW, data, 300, out, sizeof(out)));
}
...
void test_build_and_parse_guard_subconditions(void)
{
    ...
    TEST_ASSERT_EQUAL_size_t(0, pc_pb_build_sd1(3, 2, 0x6C, nullptr, sizeof(out)));
    TEST_ASSERT_EQUAL_size_t(0, pc_pb_build_sd1(3, 2, 0x6C, out, 5));
    TEST_ASSERT_EQUAL_size_t(0, pc_pb_build_sd2(3, 2, 0x6C, data, sizeof(data), out, 4));
    // Parse guards: null frame, null out, short frame, and an SD2 with LE < 3.
    PbTelegram tg;
    TEST_ASSERT_FALSE(pc_pb_parse(nullptr, 6, &tg));
    TEST_ASSERT_FALSE(pc_pb_parse(out, 6, nullptr));
    TEST_ASSERT_FALSE(pc_pb_parse(out, 3, &tg));
    const uint8_t bad_sd2[9] = {Profibus::PB_SD2, 0x02, 0x02, Profibus::PB_SD2, 1, 2, 3, 4, 5}; // LE=2 (< 3)
    TEST_ASSERT_FALSE(pc_pb_parse(bad_sd2, sizeof(bad_sd2), &tg));
}
...
void test_sd2_build_more_guards(void)
{
    ...
    TEST_ASSERT_EQUAL_size_t(0, pc_pb_build_sd2(0x05, 0x02, Profibus::PB_FC_SRD_LOW, data, 3, nullptr, sizeof(out)));
    // Non-zero data_len with a null data pointer fails closed.
    TEST_ASSERT_EQUAL_size_t(0, pc_pb_build_sd2(0x05, 0x02, Profibus::PB_FC_SRD_LOW, nullptr, 3, out, sizeof(out)));
}
...
void test_sd2_zero_length_data(void)
{
    ...
    uint8_t out[16];
    size_t n = pc_pb_build_sd2(0x05, 0x02, Profibus::PB_FC_SRD_LOW, nullptr, 0, out, sizeof(out));
...
void test_sd1_parse_corruption(void)
{
    ...
    bad_fcs[4] ^= 0xFF;
    TEST_ASSERT_FALSE(pc_pb_parse(bad_fcs, n, &t));

    // FCS correct, but ED mismatch fails closed.
    uint8_t bad_ed[6];
    memcpy(bad_ed, out, n);
    bad_ed[5] = 0x00;
    TEST_ASSERT_FALSE(pc_pb_parse(bad_ed, n, &t));
}
...
void test_parse_unknown_sd(void)
{
    ...
    PbTelegram t;
    TEST_ASSERT_FALSE(pc_pb_parse(frame, sizeof(frame), &t));
}
...
void test_sd2_parse_length_guards(void)
{
    ...
    TEST_ASSERT_FALSE(pc_pb_parse(short_body, sizeof(short_body), &t));
