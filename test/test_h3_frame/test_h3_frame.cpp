...
//
// Unit tests for the HTTP/3 framing layer (network_drivers/presentation/http3/dws_h3_frame, RFC 9114
...
void test_header_roundtrip()
{
    ...
    TEST_ASSERT_EQUAL_INT(2, (int)dws_h3_frame_write_header(b, sizeof b, H3FrameType::H3_SETTINGS, 0));
    const uint8_t exp[2] = {0x04, 0x00};
    TEST_ASSERT_EQUAL_UINT8_ARRAY(exp, b, 2);
    H3Frame f;
    TEST_ASSERT_TRUE(dws_h3_frame_parse(b, 2, &f));
    ...
    TEST_ASSERT_EQUAL_INT(3, (int)dws_h3_frame_write_header(b, sizeof b, H3FrameType::H3_HEADERS, 1000));
    const uint8_t exp2[3] = {0x01, 0x43, 0xE8};
    TEST_ASSERT_EQUAL_UINT8_ARRAY(exp2, b, 3);
    TEST_ASSERT_TRUE(dws_h3_frame_parse(b, 3, &f));
...
{
    uint8_t b[32];
    TEST_ASSERT_EQUAL_INT(7, (int)dws_h3_build_data(b, sizeof b, (const uint8_t *)"hello", 5));
    const uint8_t data[7] = {0x00, 0x05, 'h', 'e', 'l', 'l', 'o'};
    TEST_ASSERT_EQUAL_UINT8_ARRAY(data, b, 7);

    TEST_ASSERT_EQUAL_INT(3, (int)dws_h3_build_goaway(b, sizeof b, 8));
...
void test_settings_roundtrip()
{
    ...
    uint8_t b[32];
    size_t n = dws_h3_build_settings(b, sizeof b, ids, vals, 2);
    ...
    TEST_ASSERT_TRUE(s.dws_qpack_max_table_capacity == 4096);
    TEST_ASSERT_TRUE(s.max_field_section_size == 1048576);
    TEST_ASSERT_TRUE(s.dws_qpack_blocked_streams == 0);
}

void test_reserved()
{
    TEST_ASSERT_TRUE(dws_h3_frame_type_reserved(0x02));
    TEST_ASSERT_TRUE(dws_h3_frame_type_reserved(0x06));
    TEST_ASSERT_TRUE(dws_h3_frame_type_reserved(0x08));
    TEST_ASSERT_TRUE(dws_h3_frame_type_reserved(0x09));
    TEST_ASSERT_FALSE(dws_h3_frame_type_reserved(H3FrameType::H3_DATA));
    TEST_ASSERT_FALSE(dws_h3_frame_type_reserved(H3FrameType::H3_HEADERS));
    TEST_ASSERT_FALSE(dws_h3_frame_type_reserved(H3FrameType::H3_SETTINGS));

    // A SETTINGS payload using a reserved HTTP/2 settings id (0x02) must be rejected.
    H3Settings s;
    dws_h3_settings_defaults(&s);
    const uint8_t bad[2] = {0x02, 0x00};
    TEST_ASSERT_FALSE(dws_h3_parse_settings(bad, 2, &s));
}
...
void test_build_headers()
{
    ...
    uint8_t b[8];
    size_t n = dws_h3_build_headers(b, sizeof b, block, 2);
...
void test_builder_overflow()
{
    ...
    TEST_ASSERT_EQUAL_INT(0, (int)dws_h3_frame_write_header(b, 0, H3FrameType::H3_DATA, 0));
    TEST_ASSERT_EQUAL_INT(0, (int)dws_h3_build_data(b, 2, data, 5)); // header fits, payload does not
    TEST_ASSERT_EQUAL_INT(0, (int)dws_h3_build_headers(b, 2, data, 5));
    TEST_ASSERT_EQUAL_INT(0, (int)dws_h3_build_settings(b, 1, ids, vals, 1));
    TEST_ASSERT_EQUAL_INT(0, (int)dws_h3_build_goaway(b, 1, 0x4000)); // stream id needs a 2-byte varint
}
...
{
    H3Frame f;
    TEST_ASSERT_FALSE(dws_h3_frame_parse((const uint8_t *)"", 0, &f)); // no type
    const uint8_t no_len[1] = {0x04};                                  // type ok, no length varint
    TEST_ASSERT_FALSE(dws_h3_frame_parse(no_len, 1, &f));
    const uint8_t trunc_len[1] = {0x40}; // a 2-byte type varint with only 1 byte present
    TEST_ASSERT_FALSE(dws_h3_frame_parse(trunc_len, 1, &f));

    H3Settings s;
    dws_h3_settings_defaults(&s);
    const uint8_t id_no_val[1] = {0x01}; // QPACK_MAX_TABLE_CAPACITY id, no value
    TEST_ASSERT_FALSE(dws_h3_parse_settings(id_no_val, 1, &s));
