...
void test_lrc(void)
{
    ...
    TEST_ASSERT_EQUAL_HEX8(0x30, pc_dnet_lrc(b, sizeof(b)));
}

void test_header_frame(void)
{
    uint8_t out[16];
    size_t n = pc_dnet_header(1, DnetByte::DNET_READ, 0x0040, 2, out, sizeof(out));
    ...
    uint8_t lrc = pc_dnet_lrc(out + 1, 10);
...
void test_data_frame_roundtrip(void)
{
    ...
    uint8_t buf[16];
    size_t n = pc_dnet_data(payload, 4, buf, sizeof(buf));
    // STX + ABCD + ETX + LRC = 7.
    ...
    size_t dl = 0;
    TEST_ASSERT_TRUE(pc_dnet_data_parse(buf, n, &d, &dl));
...
void test_data_parse_rejects(void)
{
    ...
    uint8_t buf[16];
    size_t n = pc_dnet_data(payload, 2, buf, sizeof(buf));
    const uint8_t *d;
    size_t dl;
    buf[n - 1] ^= 0xFF; // bad LRC
    TEST_ASSERT_FALSE(pc_dnet_data_parse(buf, n, &d, &dl));
    // Missing STX.
    uint8_t bad[4] = {0x00, 0x11, DnetByte::DNET_ETX, 0x11 ^ DnetByte::DNET_ETX};
    TEST_ASSERT_FALSE(pc_dnet_data_parse(bad, sizeof(bad), &d, &dl));
}
...
{
    uint8_t out[16];
    size_t n = pc_dnet_header(0xAB, DnetByte::DNET_WRITE, 0xCDEF, 0xFA, out, sizeof(out));
    TEST_ASSERT_EQUAL_size_t(12, n);
...
// pc_dnet_data_parse's data/data_len out-params are each independently optional; cover the
...
void test_data_parse_null_outputs(void)
{
    ...
    size_t dl = 0;
    TEST_ASSERT_TRUE(pc_dnet_data_parse(buf, n, nullptr, &dl)); // data ptr not wanted
    TEST_ASSERT_EQUAL_size_t(4, dl);

    const uint8_t *d = nullptr;
    TEST_ASSERT_TRUE(pc_dnet_data_parse(buf, n, &d, nullptr)); // data_len not wanted
    TEST_ASSERT_EQUAL_HEX8_ARRAY(payload, d, 4);
}
...
void test_guards(void)
{
    uint8_t out[16];
    uint8_t payload[4] = {'A', 'B', 'C', 'D'};
    TEST_ASSERT_EQUAL_size_t(0, pc_dnet_header(1, DnetByte::DNET_READ, 0x40, 2, nullptr, sizeof(out))); // null out
    TEST_ASSERT_EQUAL_size_t(0, pc_dnet_header(1, DnetByte::DNET_READ, 0x40, 2, out, 5));               // cap too small
    TEST_ASSERT_EQUAL_size_t(0, pc_dnet_data(payload, 4, nullptr, sizeof(out)));                        // null out
    TEST_ASSERT_EQUAL_size_t(0, pc_dnet_data(nullptr, 4, out, sizeof(out))); // len but null data
    TEST_ASSERT_EQUAL_size_t(0, pc_dnet_data(payload, 4, out, 3));           // n > cap

    const uint8_t *d;
    size_t dl;
    TEST_ASSERT_FALSE(pc_dnet_data_parse(nullptr, 5, &d, &dl)); // null frame
