...
// table (register / find / dedup / capacity, with the full pc_ip bind address preserved) and the
...
void test_map_and_find()
{
    ...
    TEST_ASSERT_TRUE(pc_iface_bridge_map("192.168.1.50", 4001, BridgeProto::tcp, &u));
    TEST_ASSERT_EQUAL_UINT8(1, pc_iface_bridge_count());

    const BridgeRule *r = pc_iface_bridge_find(4001, BridgeProto::tcp);
    TEST_ASSERT_NOT_NULL(r);
    TEST_ASSERT_EQUAL(BridgeBus::uart, r->target.bus);
    TEST_ASSERT_EQUAL_UINT16(4001, r->listen_port);
    TEST_ASSERT_EQUAL(pc_ip_family::PC_IP_V4, r->listen_ip.family); // full bind address preserved
    TEST_ASSERT_EQUAL_UINT8(192, r->listen_ip.bytes[0]);
    TEST_ASSERT_EQUAL_UINT8(50, r->listen_ip.bytes[3]);

    TEST_ASSERT_NULL(pc_iface_bridge_find(4001, BridgeProto::udp)); // proto must match
    TEST_ASSERT_NULL(pc_iface_bridge_find(4002, BridgeProto::tcp)); // port must match
}

void test_any_interface_and_dedup()
{
    BridgeTarget i = i2c_target(0x40);
    TEST_ASSERT_TRUE(pc_iface_bridge_map(nullptr, 5000, BridgeProto::tcp, &i)); // any interface
    const BridgeRule *r = pc_iface_bridge_find(5000, BridgeProto::tcp);
    TEST_ASSERT_NOT_NULL(r);
    TEST_ASSERT_EQUAL(pc_ip_family::PC_IP_NONE, r->listen_ip.family);
    TEST_ASSERT_EQUAL_UINT16(0x40, r->target.addr_cs);

    // Same port+proto is a duplicate -> rejected.
    BridgeTarget u = uart_target();
    TEST_ASSERT_FALSE(pc_iface_bridge_map("10.0.0.1", 5000, BridgeProto::tcp, &u));
    // Same port on the other transport is fine.
    TEST_ASSERT_TRUE(pc_iface_bridge_map(nullptr, 5000, BridgeProto::udp, &i));
...
{
    BridgeTarget u = uart_target();
    TEST_ASSERT_FALSE(pc_iface_bridge_map("not.an.ip", 6000, BridgeProto::tcp, &u));
...
void test_table_full()
{
    ...
    {
        TEST_ASSERT_TRUE(pc_iface_bridge_map(nullptr, (uint16_t)(7000 + p), BridgeProto::tcp, &u));
    }
    TEST_ASSERT_EQUAL_UINT8(PC_BRIDGE_MAX_RULES, pc_iface_bridge_count());
    TEST_ASSERT_FALSE(pc_iface_bridge_map(nullptr, 9999, BridgeProto::tcp, &u)); // full
    pc_iface_bridge_clear();
    TEST_ASSERT_EQUAL_UINT8(0, pc_iface_bridge_count());
}
...
void test_txn_roundtrip()
{
    ...
    uint8_t frame[16];
    size_t n = pc_iface_bridge_txn_build(frame, sizeof(frame), wr, 3, 5);
    TEST_ASSERT_EQUAL_size_t(PC_BRIDGE_TXN_HDR + 3, n);
    ...
    size_t used = pc_iface_bridge_txn_parse(frame, n, &wl, &rl, &wd);
...
void test_txn_partial_and_readonly()
{
    ...
    TEST_ASSERT_EQUAL_size_t(0, pc_iface_bridge_txn_parse(hdr2, sizeof(hdr2), nullptr, nullptr, nullptr));
    ...
    TEST_ASSERT_EQUAL_size_t(0, pc_iface_bridge_txn_parse(partial, sizeof(partial), nullptr, nullptr, nullptr));
    ...
    uint16_t rl = 0;
    const uint8_t *wd = nullptr;
    TEST_ASSERT_EQUAL_size_t(PC_BRIDGE_TXN_HDR, pc_iface_bridge_txn_parse(readonly, sizeof(readonly), &wl, &rl, &wd));
...
void test_build_overflow_fails_closed()
{
    ...
    TEST_ASSERT_EQUAL_size_t(0, pc_iface_bridge_txn_build(small, sizeof(small), wr, 4, 0));
    TEST_ASSERT_EQUAL_size_t(0, pc_iface_bridge_txn_parse(nullptr, 10, nullptr, nullptr, nullptr));
}

void test_null_arg_guards()
{
    // add() with a NULL rule pointer fails closed and touches nothing.
    TEST_ASSERT_FALSE(pc_iface_bridge_add(nullptr));
    TEST_ASSERT_EQUAL_UINT8(0, pc_iface_bridge_count());

    // map() with a NULL target fails closed and touches nothing.
    TEST_ASSERT_FALSE(pc_iface_bridge_map("10.0.0.1", 7600, BridgeProto::tcp, nullptr));
    TEST_ASSERT_EQUAL_UINT8(0, pc_iface_bridge_count());
}
...
void test_map_empty_ip_is_any_interface()
{
    ...
    BridgeTarget u = uart_target();
    TEST_ASSERT_TRUE(pc_iface_bridge_map("", 5200, BridgeProto::tcp, &u));
    const BridgeRule *r = pc_iface_bridge_find(5200, BridgeProto::tcp);
    TEST_ASSERT_NOT_NULL(r);
    TEST_ASSERT_EQUAL(pc_ip_family::PC_IP_NONE, r->listen_ip.family);
}
...
void test_txn_parse_null_outputs()
{
    ...
    size_t used = pc_iface_bridge_txn_parse(frame, sizeof(frame), nullptr, nullptr, nullptr);
...
void test_txn_build_edge_cases()
{
    ...
    TEST_ASSERT_EQUAL_size_t(0, pc_iface_bridge_txn_build(nullptr, sizeof(out), nullptr, 0, 0));

    // write_len == 0: header-only frame; the payload copy is skipped even though write_data is NULL.
    size_t n0 = pc_iface_bridge_txn_build(out, sizeof(out), nullptr, 0, 8);
    TEST_ASSERT_EQUAL_size_t(PC_BRIDGE_TXN_HDR, n0);
    ...
    size_t n1 = pc_iface_bridge_txn_build(out, sizeof(out), nullptr, 5, 0);
    TEST_ASSERT_EQUAL_size_t(PC_BRIDGE_TXN_HDR + 5, n1);
