...
void test_map_and_find()
{
    ...
    TEST_ASSERT_TRUE(bridge_map("192.168.1.50", 4001, BridgeProto::tcp, &u));
    TEST_ASSERT_EQUAL_UINT8(1, bridge_count());

    const BridgeRule *r = bridge_find(4001, BridgeProto::tcp);
    ...
    TEST_ASSERT_NULL(bridge_find(4001, BridgeProto::udp)); // proto must match
    TEST_ASSERT_NULL(bridge_find(4002, BridgeProto::tcp)); // port must match
}

void test_any_interface_and_dedup()
{
    BridgeTarget i = i2c_target(0x40);
    TEST_ASSERT_TRUE(bridge_map(nullptr, 5000, BridgeProto::tcp, &i)); // any interface
    const BridgeRule *r = bridge_find(5000, BridgeProto::tcp);
    TEST_ASSERT_NOT_NULL(r);
    ...
    BridgeTarget u = uart_target();
    TEST_ASSERT_FALSE(bridge_map("10.0.0.1", 5000, BridgeProto::tcp, &u));
    // Same port on the other transport is fine.
    TEST_ASSERT_TRUE(bridge_map(nullptr, 5000, BridgeProto::udp, &i));
    TEST_ASSERT_EQUAL_UINT8(2, bridge_count());
}

void test_bad_address_rejected()
{
    BridgeTarget u = uart_target();
    TEST_ASSERT_FALSE(bridge_map("not.an.ip", 6000, BridgeProto::tcp, &u));
    TEST_ASSERT_EQUAL_UINT8(0, bridge_count());
}
...
void test_table_full()
{
    ...
    for (uint16_t p = 0; p < DETWS_BRIDGE_MAX_RULES; p++)
        TEST_ASSERT_TRUE(bridge_map(nullptr, (uint16_t)(7000 + p), BridgeProto::tcp, &u));
    TEST_ASSERT_EQUAL_UINT8(DETWS_BRIDGE_MAX_RULES, bridge_count());
    TEST_ASSERT_FALSE(bridge_map(nullptr, 9999, BridgeProto::tcp, &u)); // full
    bridge_clear();
    TEST_ASSERT_EQUAL_UINT8(0, bridge_count());
}
...
void test_txn_roundtrip()
{
    ...
    uint8_t frame[16];
    size_t n = bridge_txn_build(frame, sizeof(frame), wr, 3, 5);
    ...
    size_t used = bridge_txn_parse(frame, n, &wl, &rl, &wd);
...
void test_txn_partial_and_readonly()
{
    ...
    TEST_ASSERT_EQUAL_size_t(0, bridge_txn_parse(hdr2, sizeof(hdr2), nullptr, nullptr, nullptr));
    ...
    TEST_ASSERT_EQUAL_size_t(0, bridge_txn_parse(partial, sizeof(partial), nullptr, nullptr, nullptr));
    ...
    uint16_t rl = 0;
    const uint8_t *wd = nullptr;
    TEST_ASSERT_EQUAL_size_t(DETWS_BRIDGE_TXN_HDR, bridge_txn_parse(readonly, sizeof(readonly), &wl, &rl, &wd));
...
void test_build_overflow_fails_closed()
{
    ...
    TEST_ASSERT_EQUAL_size_t(0, bridge_txn_build(small, sizeof(small), wr, 4, 0));
    TEST_ASSERT_EQUAL_size_t(0, bridge_txn_parse(nullptr, 10, nullptr, nullptr, nullptr));
}
