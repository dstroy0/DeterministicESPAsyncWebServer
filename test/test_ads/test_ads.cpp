...
void test_build_read_bytes()
{
    ...
    uint8_t buf[64];
    size_t n = pc_ads_build_read(buf, sizeof(buf), &r, AdsIndexGroup::plc_rw_m, 0, 4);
...
void test_parse_read_response()
{
    ...
    AdsAmsHeader h;
    TEST_ASSERT_TRUE(pc_ads_parse_ams_header(resp, sizeof(resp), &h));
    ...
    AdsReadResult rr;
    TEST_ASSERT_TRUE(pc_ads_parse_read(h.data, h.data_len, &rr));
...
void test_build_write()
{
    ...
    size_t n = pc_ads_build_write(buf, sizeof(buf), &r, AdsIndexGroup::plc_rw_m, 8, val, sizeof(val));
...
void test_build_read_write_symbol()
{
    ...
    size_t n = pc_ads_build_read_write(buf, sizeof(buf), &r, AdsIndexGroup::sym_hnd_by_name, 0, 4,
...
void test_read_state_roundtrip()
{
    AdsRequest r = make_req();
    uint8_t buf[64];
    size_t n = pc_ads_build_read_state(buf, sizeof(buf), &r);
    ...
    TEST_ASSERT_TRUE(pc_ads_parse_read_state(h.data, h.data_len, &st));
    TEST_ASSERT_EQUAL_UINT32(0, st.result);
    TEST_ASSERT_EQUAL_UINT16((uint16_t)AdsState::run, st.pc_ads_state);
...
void test_parse_device_info()
{
    ...
    TEST_ASSERT_TRUE(pc_ads_parse_ams_header(resp, sizeof(resp), &h));
    AdsDeviceInfo di;
    TEST_ASSERT_TRUE(pc_ads_parse_read_device_info(h.data, h.data_len, &di));
...
void test_write_control_and_result()
{
    AdsRequest r = make_req();
    uint8_t buf[64];
    size_t n = pc_ads_build_write_control(buf, sizeof(buf), &r, (uint16_t)AdsState::run, 0, nullptr, 0);
    ...
    TEST_ASSERT_TRUE(pc_ads_parse_result(data, sizeof(data), &res));
...
void test_add_notification()
{
    AdsRequest r = make_req();
    uint8_t buf[80];
    size_t n = pc_ads_build_add_notification(buf, sizeof(buf), &r, AdsIndexGroup::sym_val_by_handle, 0x1234, 2,
    ...
    const uint8_t pc_resp_data[] = {0x00, 0x00, 0x00, 0x00, 0x11, 0x22, 0x33, 0x44}; // result 0 + handle
    uint32_t result = 0xFF, handle = 0;
    TEST_ASSERT_TRUE(pc_ads_parse_add_notification(pc_resp_data, sizeof(pc_resp_data), &result, &handle));
...
void test_parse_notification_stream()
{
    ...
    TEST_ASSERT_TRUE(pc_ads_parse_notification(payload, sizeof(payload), on_sample, nullptr));
...
void test_build_overflow_fails_closed()
{
    ...
    TEST_ASSERT_EQUAL_size_t(0, pc_ads_build_read(small, sizeof(small), &r, 0, 0, 4));
    uint8_t tiny[8];
    TEST_ASSERT_EQUAL_size_t(0, pc_ads_build_read_state(tiny, sizeof(tiny), &r)); // needs 38
}

void test_parse_guards()
{
    AdsAmsHeader h;
    TEST_ASSERT_FALSE(pc_ads_parse_ams_header(nullptr, 64, &h));
    const uint8_t short_buf[10] = {0};
    TEST_ASSERT_FALSE(pc_ads_parse_ams_header(short_buf, sizeof(short_buf), &h)); // < 38
    ...
    TEST_ASSERT_FALSE(pc_ads_parse_ams_header(badres, sizeof(badres), &h));
    ...
    TEST_ASSERT_FALSE(pc_ads_parse_ams_header(liar, sizeof(liar), &h));
    ...
    AdsReadResult rr;
    TEST_ASSERT_FALSE(pc_ads_parse_read(bad_read, sizeof(bad_read), &rr));
}
...
void test_build_read_device_info_and_del()
{
    AdsRequest r = make_req();
    uint8_t buf[64];
    size_t n = pc_ads_build_read_device_info(buf, sizeof(buf), &r);
    TEST_ASSERT_EQUAL_size_t(ADS_HDR_LEN, n);
    TEST_ASSERT_EQUAL_HEX8(0x01, buf[22]);                                  // cmd 1 (ReadDeviceInfo)
    TEST_ASSERT_EQUAL_HEX8(0x00, buf[26]);                                  // cbData 0
    TEST_ASSERT_EQUAL_size_t(0, pc_ads_build_read_device_info(buf, 8, &r)); // buffer too small

    n = pc_ads_build_del_notification(buf, sizeof(buf), &r, 0xAABBCCDD);
    ...
    TEST_ASSERT_EQUAL_size_t(0, pc_ads_build_del_notification(buf, 8, &r, 0)); // buffer too small
}
...
void test_build_null_and_small_buffer_guards()
{
    ...
    TEST_ASSERT_EQUAL_size_t(0, pc_ads_build_read_state(nullptr, sizeof(buf), &r));  // write_header !buf
    TEST_ASSERT_EQUAL_size_t(0, pc_ads_build_read_state(buf, sizeof(buf), nullptr)); // write_header !r

    // Write: null data with len>0 rejects; len==0 skips the copy; a too-small buffer fails closed.
    TEST_ASSERT_EQUAL_size_t(0, pc_ads_build_write(buf, sizeof(buf), &r, 0x4020, 0, nullptr, 4));
    TEST_ASSERT_EQUAL_size_t(ADS_HDR_LEN + 12, pc_ads_build_write(buf, sizeof(buf), &r, 0x4020, 0, nullptr, 0));
    TEST_ASSERT_EQUAL_size_t(0, pc_ads_build_write(buf, 40, &r, 0x4020, 0, val, 4)); // needs 54

    // ReadWrite: same shape on write_data / write_len.
    TEST_ASSERT_EQUAL_size_t(0, pc_ads_build_read_write(buf, sizeof(buf), &r, 0xF003, 0, 4, nullptr, 12));
    TEST_ASSERT_EQUAL_size_t(ADS_HDR_LEN + 16, pc_ads_build_read_write(buf, sizeof(buf), &r, 0xF003, 0, 4, nullptr, 0));
    TEST_ASSERT_EQUAL_size_t(0, pc_ads_build_read_write(buf, 40, &r, 0xF003, 0, 4, val, 4)); // needs 58

    // AddNotification into a buffer too small for its 40-octet payload (needs 78).
    TEST_ASSERT_EQUAL_size_t(
        0, pc_ads_build_add_notification(buf, sizeof(buf), &r, 0xF005, 0, 2, AdsTransMode::server_on_change, 0, 0));
}
...
void test_build_write_control_variants()
{
    ...
    size_t n = pc_ads_build_write_control(buf, sizeof(buf), &r, (uint16_t)AdsState::stop, 7, payload, sizeof(payload));
    ...
    TEST_ASSERT_EQUAL_size_t(0, pc_ads_build_write_control(buf, sizeof(buf), &r, 0, 0, nullptr, 4)); // len && !data
    TEST_ASSERT_EQUAL_size_t(0, pc_ads_build_write_control(buf, 40, &r, 0, 0, payload, sizeof(payload))); // needs 48
}
...
void test_parse_payload_guards()
{
    ...
    AdsReadResult rr;
    TEST_ASSERT_FALSE(pc_ads_parse_read(nullptr, 8, &rr));  // !data
    TEST_ASSERT_FALSE(pc_ads_parse_read(data, 8, nullptr)); // !out
    TEST_ASSERT_FALSE(pc_ads_parse_read(data, 4, &rr));     // data_len < 8

    uint32_t res = 0;
    TEST_ASSERT_FALSE(pc_ads_parse_result(nullptr, 4, &res)); // !data
    TEST_ASSERT_FALSE(pc_ads_parse_result(data, 4, nullptr)); // !result
    TEST_ASSERT_FALSE(pc_ads_parse_result(data, 2, &res));    // data_len < 4

    AdsReadStateResult st;
    TEST_ASSERT_FALSE(pc_ads_parse_read_state(nullptr, 8, &st));  // !data
    TEST_ASSERT_FALSE(pc_ads_parse_read_state(data, 8, nullptr)); // !out
    TEST_ASSERT_FALSE(pc_ads_parse_read_state(data, 4, &st));     // data_len < 8

    AdsDeviceInfo di;
    TEST_ASSERT_FALSE(pc_ads_parse_read_device_info(nullptr, 24, &di));  // !data
    TEST_ASSERT_FALSE(pc_ads_parse_read_device_info(data, 24, nullptr)); // !out
    TEST_ASSERT_FALSE(pc_ads_parse_read_device_info(data, 20, &di));     // data_len < 24

    uint32_t result = 0, handle = 0;
    TEST_ASSERT_FALSE(pc_ads_parse_add_notification(nullptr, 8, &result, &handle)); // !data
    TEST_ASSERT_FALSE(pc_ads_parse_add_notification(data, 8, nullptr, &handle));    // !result
    TEST_ASSERT_FALSE(pc_ads_parse_add_notification(data, 8, &result, nullptr));    // !handle
    TEST_ASSERT_FALSE(pc_ads_parse_add_notification(data, 4, &result, &handle));    // data_len < 8
}
...
void test_parse_notification_guards()
{
    ...
    TEST_ASSERT_FALSE(pc_ads_parse_notification(nullptr, 8, on_sample, nullptr)); // !data
    TEST_ASSERT_FALSE(pc_ads_parse_notification(data, 8, nullptr, nullptr));      // !on_sample
    TEST_ASSERT_FALSE(pc_ads_parse_notification(data, 4, on_sample, nullptr));    // data_len < 8

    const uint8_t bad_length[8] = {0x64, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}; // Length = 100 > buffer
    TEST_ASSERT_FALSE(pc_ads_parse_notification(bad_length, sizeof(bad_length), on_sample, nullptr));

    const uint8_t short_stamp[8] = {0x04, 0x00, 0x00, 0x00, 0x01, 0x00, 0x00, 0x00}; // Stamps=1, no stamp header
    TEST_ASSERT_FALSE(pc_ads_parse_notification(short_stamp, sizeof(short_stamp), on_sample, nullptr));
    ...
    TEST_ASSERT_FALSE(pc_ads_parse_notification(short_sample, sizeof(short_sample), on_sample, nullptr));
    ...
    TEST_ASSERT_FALSE(pc_ads_parse_notification(short_data, sizeof(short_data), on_sample, nullptr));
}
