...
// dws_quic_packet, RFC 9000 sec 17): the long-header build/parse round-trip, a Version Negotiation
...
void test_long_header_roundtrip()
{
    ...
    uint8_t out[32];
    size_t n = dws_quic_build_long_header(out, sizeof out, QuicLongPacket::QUIC_LP_INITIAL, QUIC_VERSION_1, dcid, 5,
                                          scid, 2, 1);
    ...
    TEST_ASSERT_TRUE(dws_quic_is_long_header(out[0]));
    QuicLongHeader h;
    TEST_ASSERT_TRUE(dws_quic_parse_long_header(out, n, &h));
...
void test_version_negotiation()
{
    ...
    uint8_t out[32];
    size_t n = dws_quic_build_version_negotiation(out, sizeof out, dcid, 2, scid, 2, vers, 2);
...
void test_short_header_parse()
{
    ...
    QuicShortHeader h;
    TEST_ASSERT_FALSE(dws_quic_is_long_header(buf[0]));
    TEST_ASSERT_TRUE(dws_quic_parse_short_header(buf, sizeof buf, 4, &h));
...
void test_pn_encode()
{
    ...
    size_t n = dws_quic_pn_encode(out, sizeof out, 0xac5c02, 0xabe8b3);
    ...
    n = dws_quic_pn_encode(out, sizeof out, 0xace8fe, 0xabe8b3);
    ...
    TEST_ASSERT_EQUAL_INT(1, (int)dws_quic_pn_length(0, -1));
}

void test_pn_decode()
{
    // RFC 9000 A.3: largest 0xa82f30ea, 16-bit truncated 0x9b32 -> 0xa82f9b32.
    TEST_ASSERT_TRUE(dws_quic_pn_decode(0xa82f30ea, 0x9b32, 16) == 0xa82f9b32ull);

    // Round-trip: encode then decode recovers the full number.
    uint8_t out[4];
    uint64_t full = 0x112233;
    int64_t acked = 0x112200;
    size_t n = dws_quic_pn_encode(out, sizeof out, full, acked);
    uint64_t trunc = 0;
...
void test_reject()
{
    ...
    TEST_ASSERT_FALSE(dws_quic_parse_long_header(bad, sizeof bad, &h));
    // Truncated long header.
    const uint8_t trunc[3] = {0xC0, 0x00, 0x00};
    TEST_ASSERT_FALSE(dws_quic_parse_long_header(trunc, sizeof trunc, &h));
...
void test_build_guards()
{
    ...
    TEST_ASSERT_EQUAL_INT(
        0, (int)dws_quic_build_long_header(out, sizeof out, QuicLongPacket::QUIC_LP_INITIAL, 1, cid, 2, cid, 2, 0));
    TEST_ASSERT_EQUAL_INT(
        0, (int)dws_quic_build_long_header(out, sizeof out, QuicLongPacket::QUIC_LP_INITIAL, 1, cid, 2, cid, 2, 5));
    TEST_ASSERT_EQUAL_INT(
        0, (int)dws_quic_build_long_header(out, sizeof out, QuicLongPacket::QUIC_LP_INITIAL, 1, cid, 21, cid, 2, 1));
    TEST_ASSERT_EQUAL_INT(
    ...
    uint8_t pn[1];
    TEST_ASSERT_EQUAL_INT(0, (int)dws_quic_pn_encode(pn, 1, 0x123456, -1)); // needs 3 bytes
    TEST_ASSERT_EQUAL_INT(4, (int)dws_quic_pn_length(0xFFFFFFFFull, -1));   // huge unacked range -> 4 bytes
}
...
void test_short_header_guards()
{
    ...
    TEST_ASSERT_FALSE(dws_quic_parse_short_header(buf, sizeof buf, 21, &h)); // DCID length > 20
    const uint8_t longform[8] = {0xC0, 1, 2, 3, 4, 5, 6, 7};
    TEST_ASSERT_FALSE(dws_quic_parse_short_header(longform, sizeof longform, 4, &h)); // 0x80 set = long header
