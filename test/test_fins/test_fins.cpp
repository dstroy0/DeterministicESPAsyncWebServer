...
void test_memory_area_write()
{
    ...
    n = pc_fins_build_memory_area_write(buf, sizeof(buf), &h, 0xB0, 100, 0, 0, nullptr, 0);
    TEST_ASSERT_EQUAL_size_t(FINS_HEADER_SIZE + 2 + 6, n);
    // A null data pointer with a nonzero length, and both overflow paths, all return 0.
    TEST_ASSERT_EQUAL_size_t(0, pc_fins_build_memory_area_write(buf, sizeof(buf), &h, 0xB0, 100, 0, 2, nullptr, 4));
...
void test_run_and_stop()
{
    ...
    size_t n = pc_fins_build_run(buf, sizeof(buf), &h, FinsRunMode::MONITOR);
    ...
    n = pc_fins_build_run(buf, sizeof(buf), &h, FinsRunMode::RUN);
    TEST_ASSERT_EQUAL_size_t(FINS_HEADER_SIZE + 2 + 3, n);
    ...
    TEST_ASSERT_EQUAL_size_t(0, pc_fins_build_run(buf, 11, &h, FinsRunMode::RUN));
...
void test_overflow_and_truncation()
{
    ...
    TEST_ASSERT_EQUAL_size_t(0, pc_fins_build_command(small, sizeof(small), &h, 1, 1, nullptr, 0));
    // Null destination / header / a param length with no params array.
    uint8_t big[32];
    TEST_ASSERT_EQUAL_size_t(0, pc_fins_build_command(nullptr, sizeof(big), &h, 1, 1, nullptr, 0));
    TEST_ASSERT_EQUAL_size_t(0, pc_fins_build_command(big, sizeof(big), nullptr, 1, 1, nullptr, 0));
    TEST_ASSERT_EQUAL_size_t(0, pc_fins_build_command(big, sizeof(big), &h, 1, 1, nullptr, 4));
    ...
    TEST_ASSERT_FALSE(pc_fins_parse_command(nullptr, sizeof(short_buf), &c));
    TEST_ASSERT_FALSE(pc_fins_parse_command(short_buf, sizeof(short_buf), nullptr));
    ...
    TEST_ASSERT_FALSE(pc_fins_parse_response(nullptr, sizeof(short_resp), &r));
    TEST_ASSERT_FALSE(pc_fins_parse_response(short_resp, sizeof(short_resp), nullptr));
}
...
void test_build_command_zero_params()
{
    FinsHeader h = make_header();
    uint8_t buf[32];
    size_t n = pc_fins_build_command(buf, sizeof(buf), &h, 0x05, 0x01, nullptr, 0);
    TEST_ASSERT_EQUAL_size_t(FINS_HEADER_SIZE + 2, n);
