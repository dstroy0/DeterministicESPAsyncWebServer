...
void test_quic_server_http3_get()
{
    ...
    TEST_ASSERT_TRUE(pc_quic_server_begin(443, &scfg, app_request, nullptr));
...
void test_quic_server_input_guards()
{
    ...
    TEST_ASSERT_FALSE(pc_quic_server_begin(443, nullptr, app_request, nullptr));
    QuicServerConfig scfg;
    memset(&scfg, 0, sizeof(scfg));
    scfg.cert_der = CERT;
    scfg.cert_len = sizeof(CERT);
    memcpy(scfg.ed25519_seed, SERVER_SEED, 32);
    scfg.rng = nullptr;
    TEST_ASSERT_FALSE(pc_quic_server_begin(443, &scfg, app_request, nullptr));
    ...
    TEST_ASSERT_FALSE(pc_quic_server_respond(999999, 0, 200, "text/plain", nullptr, 0));
    ...
    pc_quic_server_begin(443, &scfg, app_request, nullptr); // reset the ring cursors
    int pushed = 0;
...
void test_quic_server_copy_str_edges()
{
    ...
    uint8_t one[1] = {0x40};
    TEST_ASSERT_TRUE(pc_quic_server_ingest(one, sizeof(one), nullptr, 1));
...
{
    fill();
    pc_quic_server_set_out_sink_cb(nullptr, nullptr); // guard against a previous test's sink lingering
    ...
    pc_quic_server_stop();
    pc_quic_server_set_out_sink_cb(out_sink, nullptr); // restore for any test relying on the global sink
}
...
void test_quic_server_respond_unknown_id_with_active_conn()
{
    ...
    TEST_ASSERT_FALSE(pc_quic_server_respond(2, 0, 200, "text/plain", nullptr, 0)); // no slot has id 2
...
void test_quic_server_begin_default_port()
{
    ...
    scfg.rng = test_rng;
    pc_quic_server_set_out_sink_cb(out_sink, nullptr);
    TEST_ASSERT_TRUE(pc_quic_server_begin(0, &scfg, app_request, nullptr));
    pc_quic_server_stop();
}
...
void test_quic_server_route_header_edges()
{
    ...
    uint8_t hs_hdr[64];
    size_t hs_len = pc_quic_build_long_header(hs_hdr, sizeof(hs_hdr), QuicLongPacket::QUIC_LP_HANDSHAKE, QUIC_VERSION_1,
    ...
    uint8_t ver_hdr[64];
    size_t ver_len = pc_quic_build_long_header(ver_hdr, sizeof(ver_hdr), QuicLongPacket::QUIC_LP_INITIAL, 0xAABBCCDDu,
    ...
    uint8_t scid_hdr[64];
    size_t scid_hdr_len =
        pc_quic_build_long_header(scid_hdr, sizeof(scid_hdr), QuicLongPacket::QUIC_LP_HANDSHAKE, QUIC_VERSION_1,
...
void test_quic_server_close_reaped_before_idle()
{
    ...
    size_t cc_len = pc_quic_build_connection_close(cc_frames, sizeof(cc_frames), 0, 0, nullptr, 0);
    uint8_t dg1[256];
...
// With on_request == nullptr, pc_h3_on_request's "if (s_quic.on_request)" guard must skip the
...
void test_quic_server_on_request_null()
{
    ...
    scfg.rng = test_rng;
    pc_quic_server_set_out_sink_cb(out_sink, nullptr);
    TEST_ASSERT_TRUE(pc_quic_server_begin(443, &scfg, nullptr, nullptr)); // no request callback
