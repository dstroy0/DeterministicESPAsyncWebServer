...
void test_classify()
{
    TEST_ASSERT_EQUAL_INT(pc_ip_class::PC_IP_UNSPECIFIED, pc_dns_resolver_classify(0u));
    TEST_ASSERT_EQUAL_INT(pc_ip_class::PC_IP_BROADCAST, pc_dns_resolver_classify(0xFFFFFFFFu));
    TEST_ASSERT_EQUAL_INT(pc_ip_class::PC_IP_LOOPBACK, pc_dns_resolver_classify(IPV4(127, 0, 0, 1)));
    TEST_ASSERT_EQUAL_INT(pc_ip_class::PC_IP_PRIVATE, pc_dns_resolver_classify(IPV4(10, 0, 0, 5)));
    TEST_ASSERT_EQUAL_INT(pc_ip_class::PC_IP_PRIVATE, pc_dns_resolver_classify(IPV4(172, 16, 0, 1)));
    TEST_ASSERT_EQUAL_INT(pc_ip_class::PC_IP_PRIVATE, pc_dns_resolver_classify(IPV4(192, 168, 1, 1)));
    TEST_ASSERT_EQUAL_INT(pc_ip_class::PC_IP_LINKLOCAL, pc_dns_resolver_classify(IPV4(169, 254, 1, 1)));
    TEST_ASSERT_EQUAL_INT(pc_ip_class::PC_IP_MULTICAST, pc_dns_resolver_classify(IPV4(224, 0, 0, 1)));
    TEST_ASSERT_EQUAL_INT(pc_ip_class::PC_IP_PUBLIC, pc_dns_resolver_classify(IPV4(8, 8, 8, 8)));
    // 172.32.x is OUTSIDE the 172.16/12 private block -> public.
    TEST_ASSERT_EQUAL_INT(pc_ip_class::PC_IP_PUBLIC, pc_dns_resolver_classify(IPV4(172, 32, 0, 1)));
    // 172.10.x is BELOW the 172.16/12 private block (b < 16) -> public.
    TEST_ASSERT_EQUAL_INT(pc_ip_class::PC_IP_PUBLIC, pc_dns_resolver_classify(IPV4(172, 10, 0, 1)));
    // 192.x (x != 168) is outside the 192.168/16 private block -> public.
    TEST_ASSERT_EQUAL_INT(pc_ip_class::PC_IP_PUBLIC, pc_dns_resolver_classify(IPV4(192, 1, 1, 1)));
    // 169.x (x != 254) is outside the 169.254/16 link-local block -> public.
    TEST_ASSERT_EQUAL_INT(pc_ip_class::PC_IP_PUBLIC, pc_dns_resolver_classify(IPV4(169, 1, 1, 1)));
    // 240.x is ABOVE the 224-239 multicast range -> public.
    TEST_ASSERT_EQUAL_INT(pc_ip_class::PC_IP_PUBLIC, pc_dns_resolver_classify(IPV4(240, 0, 0, 1)));
}
...
void test_resolve_verified_paths()
{
    ...
    TEST_ASSERT_TRUE(pc_dns_resolver_resolve_verified("example.com", nullptr)); // null out_ip ok
...
void test_resolve_host_ok_null_out_ip()
{
    ...
    pc_dns_resolver_test_set_resolve(true, IPV4(8, 8, 8, 8));
    TEST_ASSERT_TRUE(pc_dns_resolver_resolve("example.com", nullptr));
    pc_dns_resolver_test_set_resolve(false, 0); // reset the hook
}
