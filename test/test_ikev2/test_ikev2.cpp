...
#include "network_drivers/presentation/ssh/crypto/ssh_ecdsa.h" // ssh_ecdsa_p256_pubkey (test keygen)
...
{
    // dws_ike_dh_compute is X25519(scalar, u) for group 31 - RFC 7748 §5.2 vector 1.
    uint8_t out[32];
    TEST_ASSERT_EQUAL_size_t(32,
                             dws_ike_dh_compute(IKE_DH_CURVE25519, kat_x_scalar, 32, kat_x_u, 32, out, sizeof(out)));
...
void test_dh_x25519_agreement()
{
    ...
    TEST_ASSERT_EQUAL_size_t(32, dws_ike_dh_public(IKE_DH_CURVE25519, kat_alice_priv, 32, apub, sizeof(apub)));
    TEST_ASSERT_EQUAL_MEMORY(kat_alice_pub, apub, 32);
    TEST_ASSERT_EQUAL_size_t(32, dws_ike_dh_public(IKE_DH_CURVE25519, kat_bob_priv, 32, bpub, sizeof(bpub)));
    TEST_ASSERT_EQUAL_MEMORY(kat_bob_pub, bpub, 32);

    uint8_t s_ab[32], s_ba[32];
    TEST_ASSERT_EQUAL_size_t(
        32, dws_ike_dh_compute(IKE_DH_CURVE25519, kat_alice_priv, 32, kat_bob_pub, 32, s_ab, sizeof(s_ab)));
    TEST_ASSERT_EQUAL_size_t(
        32, dws_ike_dh_compute(IKE_DH_CURVE25519, kat_bob_priv, 32, kat_alice_pub, 32, s_ba, sizeof(s_ba)));
...
void test_dh_guards()
{
    ...
    TEST_ASSERT_EQUAL_size_t(0, dws_ike_dh_compute(IKE_DH_ECP256, kat_alice_priv, 32, kat_bob_pub, 32, out, 32));
    TEST_ASSERT_EQUAL_size_t(0, dws_ike_dh_public(IKE_DH_ECP256, kat_alice_priv, 32, out, 32));
    // Bad lengths / small cap / null.
    TEST_ASSERT_EQUAL_size_t(0, dws_ike_dh_compute(IKE_DH_CURVE25519, kat_alice_priv, 31, kat_bob_pub, 32, out, 32));
    TEST_ASSERT_EQUAL_size_t(0, dws_ike_dh_compute(IKE_DH_CURVE25519, kat_alice_priv, 32, kat_bob_pub, 32, out, 31));
...
void test_signed_octets_kat()
{
    ...
    uint8_t scratch[128];
    size_t n = dws_ike_signed_octets(scratch, sizeof(scratch), so_real, sizeof(so_real), so_nonce, sizeof(so_nonce),
    ...
    TEST_ASSERT_EQUAL_size_t(0, dws_ike_signed_octets(scratch, 50, so_real, sizeof(so_real), so_nonce, sizeof(so_nonce),
                                                      so_skp, sizeof(so_skp), so_id, sizeof(so_id)));
}
...
void test_auth_ecdsa_sign_verify()
{
    ...
    uint8_t pub[DWS_IKE_ECDSA_P256_PUB_LEN];
    ...
    uint8_t scratch[128], sig[DWS_IKE_ECDSA_P256_SIG_LEN];

    TEST_ASSERT_TRUE(dws_ike_auth_sign_ecdsa_p256(sig, priv, scratch, sizeof(scratch), real, sizeof(real), nonce,
                                                  sizeof(nonce), skp, sizeof(skp), id, sizeof(id)));
    // The peer verifies the same octets against the matching public key.
    TEST_ASSERT_TRUE(dws_ike_auth_verify_ecdsa_p256(pub, sig, scratch, sizeof(scratch), real, sizeof(real), nonce,
                                                    sizeof(nonce), skp, sizeof(skp), id, sizeof(id)));
    // A tampered nonce -> different signed octets -> verify fails.
    uint8_t bad_nonce[16];
    memcpy(bad_nonce, nonce, 16);
    bad_nonce[0] ^= 0x01;
    TEST_ASSERT_FALSE(dws_ike_auth_verify_ecdsa_p256(pub, sig, scratch, sizeof(scratch), real, sizeof(real), bad_nonce,
                                                     sizeof(bad_nonce), skp, sizeof(skp), id, sizeof(id)));
    // A different key pair -> verify fails (impersonation rejected).
    uint8_t priv2[32], pub2[DWS_IKE_ECDSA_P256_PUB_LEN];
    for (int i = 0; i < 32; i++)
        priv2[i] = (uint8_t)(i + 2);
    TEST_ASSERT_TRUE(ssh_ecdsa_p256_pubkey(pub2, priv2));
    TEST_ASSERT_FALSE(dws_ike_auth_verify_ecdsa_p256(pub2, sig, scratch, sizeof(scratch), real, sizeof(real), nonce,
                                                     sizeof(nonce), skp, sizeof(skp), id, sizeof(id)));
    // Scratch too small -> both fail closed.
    TEST_ASSERT_FALSE(dws_ike_auth_sign_ecdsa_p256(sig, priv, scratch, 8, real, sizeof(real), nonce, sizeof(nonce), skp,
...
void test_auth_verify_rsa()
{
    ...
    TEST_ASSERT_TRUE(dws_ike_auth_verify_rsa_sha256(rsa_n, rsa_e, rsa_sig, 256, scratch, sizeof(scratch), rsa_real, 40,
                                                    rsa_nonce, 16, rsa_skp, 32, rsa_id, 8));
    // A tampered nonce (different signed octets) fails.
    uint8_t bad_nonce[16];
    memcpy(bad_nonce, rsa_nonce, 16);
    bad_nonce[0] ^= 0x01;
    TEST_ASSERT_FALSE(dws_ike_auth_verify_rsa_sha256(rsa_n, rsa_e, rsa_sig, 256, scratch, sizeof(scratch), rsa_real, 40,
                                                     bad_nonce, 16, rsa_skp, 32, rsa_id, 8));
    // A tampered signature fails.
    uint8_t bad_sig[256];
    memcpy(bad_sig, rsa_sig, 256);
    bad_sig[100] ^= 0x01;
    TEST_ASSERT_FALSE(dws_ike_auth_verify_rsa_sha256(rsa_n, rsa_e, bad_sig, 256, scratch, sizeof(scratch), rsa_real, 40,
                                                     rsa_nonce, 16, rsa_skp, 32, rsa_id, 8));
    // Null-argument guard.
    TEST_ASSERT_FALSE(dws_ike_auth_verify_rsa_sha256(nullptr, rsa_e, rsa_sig, 256, scratch, sizeof(scratch), rsa_real,
...
void test_rekey_derive_keys()
{
    ...
    memset(&km, 0, sizeof(km));
    TEST_ASSERT_TRUE(
        dws_ike_rekey_derive_keys(rk_skd, 32, rk_dh, 32, rk_ni, 16, rk_nr, 16, rk_spii, rk_spir, &lens, &km));
    ...
    TEST_ASSERT_TRUE(dws_ike_derive_keys(rk_dh, 32, rk_ni, 16, rk_nr, 16, rk_spii, rk_spir, &lens, &km_init));
    ...
    TEST_ASSERT_FALSE(
        dws_ike_rekey_derive_keys(nullptr, 32, rk_dh, 32, rk_ni, 16, rk_nr, 16, rk_spii, rk_spir, &lens, &km));
    TEST_ASSERT_FALSE(
