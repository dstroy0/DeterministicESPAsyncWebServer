...
void test_build_getversion_kat()
{
    ...
    uint8_t out[16];
    uint16_t n = pc_zwave_build_frame(ZWAVE_REQ, 0x15, NULL, 0, out, sizeof(out));
...
void test_build_then_parse_round_trip()
{
    ...
    uint8_t buf[24];
    uint16_t n = pc_zwave_build_frame(ZWAVE_RES, 0x04, data, 3, buf, sizeof(buf));
...
void test_parse_getversion_kat()
{
    ...
    TEST_ASSERT_EQUAL_INT(5, pc_zwave_parse_frame(frame, sizeof(frame), &type, &cmd, NULL, &pdlen));
...
void test_parse_rejects_bad_sof()
{
    ...
    TEST_ASSERT_EQUAL_INT(-1, pc_zwave_parse_frame(frame, sizeof(frame), NULL, NULL, NULL, NULL));
}
...
{
    const uint8_t frame[5] = {0x01, 0x03, 0x00, 0x15, 0xE9};
    TEST_ASSERT_EQUAL_INT(0, pc_zwave_parse_frame(frame, 1, NULL, NULL, NULL, NULL)); // just SOF
    TEST_ASSERT_EQUAL_INT(0, pc_zwave_parse_frame(frame, 4, NULL, NULL, NULL, NULL)); // missing checksum
}
...
{
    TEST_ASSERT_TRUE(pc_zwave_is_ack(ZWAVE_ACK));
    TEST_ASSERT_TRUE(pc_zwave_is_nak(ZWAVE_NAK));
    TEST_ASSERT_TRUE(pc_zwave_is_can(ZWAVE_CAN));
    TEST_ASSERT_FALSE(pc_zwave_is_ack(ZWAVE_SOF));
    uint8_t ack[1];
...
void test_build_rejects_null_out()
{
    ...
    TEST_ASSERT_EQUAL_UINT16(0, pc_zwave_build_frame(ZWAVE_REQ, 0x13, data, 2, NULL, 16));
}
...
void test_build_rejects_null_data_with_nonzero_len()
{
    ...
    TEST_ASSERT_EQUAL_UINT16(0, pc_zwave_build_frame(ZWAVE_REQ, 0x13, NULL, 5, out, sizeof(out)));
}
...
void test_parse_rejects_null_raw()
{
    uint8_t type = 0, cmd = 0, pdlen = 0;
    const uint8_t *pd = NULL;
    TEST_ASSERT_EQUAL_INT(0, pc_zwave_parse_frame(NULL, 5, &type, &cmd, &pd, &pdlen));
}

void test_parse_needs_more_bytes_on_zero_len()
{
    const uint8_t frame[5] = {0x01, 0x03, 0x00, 0x15, 0xE9};
    TEST_ASSERT_EQUAL_INT(0, pc_zwave_parse_frame(frame, 0, NULL, NULL, NULL, NULL));
}
...
void test_parse_allows_null_out_params()
{
    ...
    const uint8_t frame[5] = {0x01, 0x03, 0x00, 0x15, 0xE9};
    TEST_ASSERT_EQUAL_INT(5, pc_zwave_parse_frame(frame, sizeof(frame), NULL, NULL, NULL, NULL));
}
