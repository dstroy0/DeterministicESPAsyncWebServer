...
static pc_relay_status run_relay(pc_relay *r, int max_steps)
{
    for (int i = 0; i < max_steps; i++)
    {
        ...
        if (st != pc_relay_status::PC_RELAY_RUNNING)
        {
            return st;
        }
    }
    return pc_relay_status::PC_RELAY_RUNNING; // never finished (a bug if it happens)
}
...
void test_bidirectional()
{
    ...
    TEST_ASSERT_EQUAL_INT(pc_relay_status::PC_RELAY_DONE, run_relay(&r, 64));
...
void test_backpressure()
{
    ...
    TEST_ASSERT_EQUAL_INT(pc_relay_status::PC_RELAY_DONE, run_relay(&r, 1000));
...
void test_half_close_shutdown()
{
    ...
    pc_relay_status st = pc_relay_status::PC_RELAY_RUNNING;
    ...
    TEST_ASSERT_EQUAL_INT(pc_relay_status::PC_RELAY_RUNNING, st);
...
void test_send_error()
{
    ...
    pc_relay_status st = pc_relay_status::PC_RELAY_RUNNING;
    for (int i = 0; i < 8 && st == pc_relay_status::PC_RELAY_RUNNING; i++)
    {
        st = pc_relay_step(&r);
    }
    TEST_ASSERT_EQUAL_INT(pc_relay_status::PC_RELAY_ERROR, st);
}
...
void test_one_way_idle_then_close()
{
    ...
    TEST_ASSERT_EQUAL_INT(pc_relay_status::PC_RELAY_DONE, run_relay(&r, 32));
...
void test_note_eof_out_of_band()
{
    ...
    TEST_ASSERT_EQUAL_INT(pc_relay_status::PC_RELAY_RUNNING, pc_relay_step(&r));
    ...
    TEST_ASSERT_EQUAL_INT(pc_relay_status::PC_RELAY_DONE, run_relay(&r, 8));
...
void test_flush_send_error()
{
    ...
    TEST_ASSERT_EQUAL_INT(pc_relay_status::PC_RELAY_ERROR, pc_relay_step(&r));
}
...
void test_null_argument_guards()
{
    ...
    pc_relay_init(nullptr, &ea, &eb); // no crash
    pc_relay_init(&r, nullptr, &eb);  // no crash
    pc_relay_init(&r, &ea, nullptr);  // no crash

    TEST_ASSERT_EQUAL_INT(pc_relay_status::PC_RELAY_ERROR, pc_relay_step(nullptr));

    pc_relay_note_eof(nullptr, false); // no crash
}
...
void test_shutdown_null_seam()
{
    ...
    TEST_ASSERT_EQUAL_INT(pc_relay_status::PC_RELAY_DONE, run_relay(&r, 16));
