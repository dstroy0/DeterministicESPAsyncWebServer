...
void test_full_handshake_roundtrip()
{
    ...
    size_t used = pc_quic_tls_recv_crypto(&qt, QuicEnc::QUIC_ENC_INITIAL, ch, ch_len);
    ...
    const uint8_t *si = pc_quic_tls_flight(&qt, QuicEnc::QUIC_ENC_INITIAL, &si_len);
    const uint8_t *sh_flight = pc_quic_tls_flight(&qt, QuicEnc::QUIC_ENC_HANDSHAKE, &sh_flight_len);
    ...
    used = pc_quic_tls_recv_crypto(&qt, QuicEnc::QUIC_ENC_HANDSHAKE, cfin, sizeof(cfin));
    ...
    TEST_ASSERT_NOT_NULL(pc_quic_tls_keys(&qt, QuicEnc::QUIC_ENC_HANDSHAKE, true));
    TEST_ASSERT_NOT_NULL(pc_quic_tls_keys(&qt, QuicEnc::QUIC_ENC_APP, false));
}
...
void test_reject_bad_client_finished()
{
    ...
    uint8_t ch[512];
    size_t ch_len = build_client_hello(ch, client_pub, ctp_enc, ctp_len);
    pc_quic_tls_recv_crypto(&qt, QuicEnc::QUIC_ENC_INITIAL, ch, ch_len);
    TEST_ASSERT_EQUAL_UINT8(QtlsState::QTLS_WAIT_FINISHED, qt.state);

    // A Finished with the wrong verify_data must be rejected (decrypt_error).
    uint8_t cfin[36] = {TlsHs::TLS_HS_FINISHED, 0x00, 0x00, 0x20};
    memset(cfin + 4, 0x99, 32);
    pc_quic_tls_recv_crypto(&qt, QuicEnc::QUIC_ENC_HANDSHAKE, cfin, sizeof(cfin));
...
void test_partial_client_hello()
{
    ...
    size_t used = pc_quic_tls_recv_crypto(&qt, QuicEnc::QUIC_ENC_INITIAL, ch, ch_len - 10);
    ...
    used = pc_quic_tls_recv_crypto(&qt, QuicEnc::QUIC_ENC_INITIAL, ch, ch_len);
    TEST_ASSERT_EQUAL_UINT(ch_len, used);
    TEST_ASSERT_EQUAL_UINT8(QtlsState::QTLS_WAIT_FINISHED, qt.state);
}
...
void test_reject_malformed_client_hello()
{
    ...
    pc_quic_tls_recv_crypto(&qt, QuicEnc::QUIC_ENC_INITIAL, bad, sizeof(bad));
    TEST_ASSERT_EQUAL_UINT8(QtlsState::QTLS_FAILED, qt.state);
...
void test_quic_tls_more_guards()
{
    ...
    pc_quic_tls_recv_crypto(&qt, QuicEnc::QUIC_ENC_HANDSHAKE, fin_badlen, sizeof(fin_badlen));
    TEST_ASSERT_EQUAL_UINT8(QtlsState::QTLS_FAILED, qt.state);
    TEST_ASSERT_EQUAL_UINT8(50, qt.alert); // decode_error
    ...
    pc_quic_tls_recv_crypto(&qt, QuicEnc::QUIC_ENC_HANDSHAKE, wrong, sizeof(wrong));
    TEST_ASSERT_EQUAL_UINT8(QtlsState::QTLS_FAILED, qt.state);
    ...
    TEST_ASSERT_EQUAL_UINT(sizeof(more), pc_quic_tls_recv_crypto(&qt, QuicEnc::QUIC_ENC_HANDSHAKE, more, sizeof(more)));

    // flight() for a level that is neither Initial nor Handshake -> nullptr / zero length.
    size_t l = 123;
    TEST_ASSERT_NULL(pc_quic_tls_flight(&qt, QuicEnc::QUIC_ENC_APP, &l));
    ...
    TEST_ASSERT_NULL(pc_quic_tls_keys(&fresh, QuicEnc::QUIC_ENC_HANDSHAKE, true));
    TEST_ASSERT_NULL(pc_quic_tls_keys(&fresh, QuicEnc::QUIC_ENC_APP, false));
...
void test_hybrid_hrr_roundtrip()
{
    ...
    size_t used = pc_quic_tls_recv_crypto(&qt, QuicEnc::QUIC_ENC_INITIAL, ch1, ch1_len);
    ...
    size_t hrr_len = 0;
    const uint8_t *hrr = pc_quic_tls_flight(&qt, QuicEnc::QUIC_ENC_INITIAL, &hrr_len);
    ...
    used = pc_quic_tls_recv_crypto(&qt, QuicEnc::QUIC_ENC_INITIAL, ch2, ch2_len);
    TEST_ASSERT_EQUAL_UINT(ch2_len, used);
    TEST_ASSERT_EQUAL_UINT8(QtlsState::QTLS_WAIT_FINISHED, qt.state);

    size_t si_len = 0, sh_flight_len = 0;
    const uint8_t *si = pc_quic_tls_flight(&qt, QuicEnc::QUIC_ENC_INITIAL, &si_len); // HRR || ServerHello
    const uint8_t *sh_flight = pc_quic_tls_flight(&qt, QuicEnc::QUIC_ENC_HANDSHAKE, &sh_flight_len);
...
void test_hybrid_key_share_entry_skipping()
{
    ...
    QuicTls qt;
    pc_quic_tls_server_init(&qt, &cfg);
    TEST_ASSERT_EQUAL_UINT(p, pc_quic_tls_recv_crypto(&qt, QuicEnc::QUIC_ENC_INITIAL, ch, p));
    TEST_ASSERT_EQUAL_UINT8(QtlsState::QTLS_WAIT_FINISHED, qt.state);
}
...
void test_quic_tls_message_dispatch_guards()
{
    ...
    pc_quic_tls_recv_crypto(&qt, QuicEnc::QUIC_ENC_INITIAL, ch2, sizeof(ch2));
    TEST_ASSERT_EQUAL_UINT8(QtlsState::QTLS_FAILED, qt.state);
    TEST_ASSERT_EQUAL_UINT8(10, qt.alert); // unexpected_message
    ...
    pc_quic_tls_server_init(&qt, &cfg);
    pc_quic_tls_recv_crypto(&qt, QuicEnc::QUIC_ENC_INITIAL, fin, sizeof(fin));
    TEST_ASSERT_EQUAL_UINT8(QtlsState::QTLS_FAILED, qt.state);
    ...
    pc_quic_tls_server_init(&qt, &cfg);
    pc_quic_tls_recv_crypto(&qt, QuicEnc::QUIC_ENC_HANDSHAKE, fin, sizeof(fin));
    TEST_ASSERT_EQUAL_UINT8(QtlsState::QTLS_FAILED, qt.state);
    TEST_ASSERT_EQUAL_UINT8(10, qt.alert);
