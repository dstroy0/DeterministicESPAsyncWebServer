...
void test_content_header()
{
    ...
    size_t n = pc_amqp_build_content_header(buf, sizeof(buf), 7, 60, 5, 0, NULL, 0);
    const uint8_t expect[] = {
    ...
    TEST_ASSERT_EQUAL_size_t(0, pc_amqp_build_content_header(buf, sizeof(buf), 1, 60, 5, 0x8000, NULL, 2));
    TEST_ASSERT_EQUAL_size_t(0, pc_amqp_build_content_header(NULL, sizeof(buf), 1, 60, 5, 0, NULL, 0));
    TEST_ASSERT_EQUAL_size_t(0, pc_amqp_build_content_header(buf, 16, 1, 60, 5, 0, NULL, 0)); // needs 22
}
...
{
    uint8_t buf[64];
    size_t n = pc_amqp_build_method(buf, sizeof(buf), 1, 10, 11, NULL, 0);
...
{
    TEST_ASSERT_EQUAL_size_t(0, pc_amqp_protocol_header(NULL, 8));
}
...
void test_build_method_guards()
{
    ...
    uint8_t buf[32];
    TEST_ASSERT_EQUAL_size_t(0, pc_amqp_build_method(NULL, sizeof(buf), 1, 10, 10, args, sizeof(args)));
    TEST_ASSERT_EQUAL_size_t(0, pc_amqp_build_method(buf, sizeof(buf), 1, 10, 10, NULL, 5));
}
...
void test_parse_frame_optional_out_params()
{
    ...
    AmqpFrame f;
    size_t c;
    TEST_ASSERT_FALSE(pc_amqp_parse_frame(NULL, sizeof(buf), &f, &c));
    TEST_ASSERT_FALSE(pc_amqp_parse_frame(buf, sizeof(buf), NULL, &c));
    TEST_ASSERT_TRUE(pc_amqp_parse_frame(buf, sizeof(buf), &f, NULL));
}
...
void test_parse_method_optional_out_params()
{
    ...
    const uint8_t *a;
    size_t alen;
    TEST_ASSERT_FALSE(pc_amqp_parse_method(NULL, 10, &cls, &meth, &a, &alen));

    const uint8_t payload[4] = {0x00, 0x0A, 0x00, 0x14};
    TEST_ASSERT_TRUE(pc_amqp_parse_method(payload, sizeof(payload), NULL, NULL, NULL, NULL));
}
