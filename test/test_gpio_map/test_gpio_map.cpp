...
void test_dir_name()
{
    TEST_ASSERT_EQUAL_STRING("in", pc_gpio_dir_name(pc_gpio_dir::PC_GPIO_IN));
    TEST_ASSERT_EQUAL_STRING("in_pullup", pc_gpio_dir_name(pc_gpio_dir::PC_GPIO_IN_PULLUP));
    TEST_ASSERT_EQUAL_STRING("in_pulldown", pc_gpio_dir_name(pc_gpio_dir::PC_GPIO_IN_PULLDOWN));
    TEST_ASSERT_EQUAL_STRING("out", pc_gpio_dir_name(pc_gpio_dir::PC_GPIO_OUT));
...
{
    pc_gpio_pin pins[2] = {
        {2, "LED", pc_gpio_dir::PC_GPIO_OUT, 1},
        {0, "BOOT", pc_gpio_dir::PC_GPIO_IN_PULLUP, 0},
    };
    char buf[256];
...
{
    char buf[64];
    int n = pc_gpio_json(nullptr, 0, buf, sizeof(buf));
    TEST_ASSERT_EQUAL_INT(0, n); // null table -> empty string, 0
}

void test_json_small_buffer_fails_closed()
{
    pc_gpio_pin pins[1] = {{2, "LED", pc_gpio_dir::PC_GPIO_OUT, 1}};
    char buf[8];
...
{
    pc_gpio_pin pins[2] = {
        {2, "LED", pc_gpio_dir::PC_GPIO_OUT, 0},
        {0, "BOOT", pc_gpio_dir::PC_GPIO_IN_PULLUP, 0},
    };
...
void test_json_null_and_zero_cap()
{
    pc_gpio_pin pins[1] = {{2, "LED", pc_gpio_dir::PC_GPIO_OUT, 1}};
    char buf[64];
    TEST_ASSERT_EQUAL_INT(0, pc_gpio_json(pins, 1, nullptr, sizeof(buf)));
...
{
    pc_gpio_pin pins[1] = {{5, nullptr, pc_gpio_dir::PC_GPIO_IN, 0}};
    char buf[128];
...
{
    pc_gpio_pin pins[1] = {{2, "L", pc_gpio_dir::PC_GPIO_OUT, 1}};
    char full[128];
...
void test_parse_set_null_args()
{
    uint8_t pin = 0, level = 0;
    const char *b = "pin=2&level=1";
    TEST_ASSERT_FALSE(pc_gpio_parse_set(nullptr, 13, &pin, &level));
    TEST_ASSERT_FALSE(pc_gpio_parse_set(b, strlen(b), nullptr, &level));
    TEST_ASSERT_FALSE(pc_gpio_parse_set(b, strlen(b), &pin, nullptr));
}
...
{
    TEST_ASSERT_FALSE(pc_gpio_is_output(nullptr, 2, 2));
}
