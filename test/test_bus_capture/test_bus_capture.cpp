...
{
    uint8_t g[PC_PCAP_GLOBAL_HDR_LEN];
    TEST_ASSERT_EQUAL_UINT(0, pc_pcap_global_header(nullptr, sizeof(g), PC_DLT_CAN_SOCKETCAN)); // null out
    uint8_t tiny[8];
...
void test_pcap_record_header_bounds()
{
    ...
    TEST_ASSERT_EQUAL_UINT(0, pc_pcap_record_header(nullptr, sizeof(r), 1, 2, 3, 4));             // null out
    uint8_t tiny[8];
