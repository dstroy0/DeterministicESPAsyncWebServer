...
void test_checksum(void)
{
    ...
    TEST_ASSERT_EQUAL_HEX8(0x82, pc_hart_checksum(b, sizeof(b)));
}
...
void test_build_command0_short(void)
{
    ...
    uint8_t addr = 0x80;
    uint8_t out[16];
    size_t n = pc_hart_build(HartDelim::HART_DELIM_STX, &addr, 1, 0x00, nullptr, 0, out, sizeof(out));
...
void test_build_with_data(void)
{
    ...
    uint8_t out[16];
    size_t n = pc_hart_build(HartDelim::HART_DELIM_STX, &addr, 1, 0x01, data, 2, out, sizeof(out));
...
void test_build_long_address(void)
{
    ...
    uint8_t out[24];
    size_t n = pc_hart_build((uint8_t)(HartDelim::HART_DELIM_STX | HartDelim::HART_DELIM_LONG_ADDR), addr, 5, 0x03,
...
void test_parse_roundtrip_and_bad_checksum(void)
{
    ...
    uint8_t buf[16];
    size_t n = pc_hart_build(HartDelim::HART_DELIM_STX, &addr, 1, 0x2A, data, 3, buf, sizeof(buf));
    HartFrame f;
    TEST_ASSERT_TRUE(pc_hart_parse(buf, n, &f));
    ...
    buf[n - 1] ^= 0xFF;
    TEST_ASSERT_FALSE(pc_hart_parse(buf, n, &f));
    // Truncated -> fails.
    TEST_ASSERT_FALSE(pc_hart_parse(buf, 3, &f));
}

void test_hartip_header(void)
{
    uint8_t out[8];
    size_t n = pc_hartip_build_header(HartIp::HARTIP_MSG_REQUEST, HartIp::HARTIP_ID_TOKEN_PDU, 0, 0x1234, 13, out,
                                      sizeof(out));
    ...
    TEST_ASSERT_EQUAL_size_t(0, pc_hartip_build_header(0, 0, 0, 0, 0, out, 4));
    // Big enough cap but null out pointer -> 0.
    TEST_ASSERT_EQUAL_size_t(0, pc_hartip_build_header(0, 0, 0, 0, 0, nullptr, sizeof(out)));
}
...
void test_hartip_parse(void)
{
    ...
    size_t hn = pc_hartip_build_header(HartIp::HARTIP_MSG_RESPONSE, HartIp::HARTIP_ID_TOKEN_PDU, 0x00, 0x0042, 13, msg,
                                       sizeof(msg));
    ...
    HartIpHeader h;
    TEST_ASSERT_TRUE(pc_hartip_parse_header(msg, 13, &h));
    ...
    pc_hartip_build_header(HartIp::HARTIP_MSG_REQUEST, HartIp::HARTIP_ID_KEEPALIVE, 0, 1, 8, msg, sizeof(msg));
    TEST_ASSERT_TRUE(pc_hartip_parse_header(msg, 8, &h));
    ...
    pc_hartip_build_header(HartIp::HARTIP_MSG_RESPONSE, HartIp::HARTIP_ID_TOKEN_PDU, 0, 1, 13, msg, sizeof(msg));
    TEST_ASSERT_FALSE(pc_hartip_parse_header(msg, 10, &h)); // total_len 13 > len 10
    msg[6] = 0x00;
    msg[7] = 0x05; // total_len 5 < header length 8
    TEST_ASSERT_FALSE(pc_hartip_parse_header(msg, 8, &h));
    TEST_ASSERT_FALSE(pc_hartip_parse_header(msg, 7, &h)); // short buffer
    TEST_ASSERT_FALSE(pc_hartip_parse_header(nullptr, 8, &h));
    TEST_ASSERT_FALSE(pc_hartip_parse_header(msg, 8, nullptr));
}
...
void test_build_and_parse_guards()
{
    ...
    TEST_ASSERT_EQUAL_size_t(0, pc_hart_build(0x82, nullptr, 5, 0, data, sizeof(data), out, sizeof(out))); // null addr
    ...
    TEST_ASSERT_EQUAL_size_t(0, pc_hart_build(0x82, addr, 1, 0, nullptr, 3, out, sizeof(out)));
    ...
    TEST_ASSERT_EQUAL_size_t(0, pc_hart_build(0x82, addr, 1, 0, big_data, sizeof(big_data), big_out, sizeof(big_out)));

    HartFrame hf;
    TEST_ASSERT_FALSE(pc_hart_parse(nullptr, 10, &hf)); // null frame
    uint8_t tiny[2] = {0x82, 0x00};
    TEST_ASSERT_FALSE(pc_hart_parse(tiny, sizeof(tiny), &hf)); // len < minimum

    uint8_t addr1 = 0x80;
    uint8_t valid_frame[16];
    size_t vn = pc_hart_build(HartDelim::HART_DELIM_STX, &addr1, 1, 0x00, nullptr, 0, valid_frame, sizeof(valid_frame));
    TEST_ASSERT_FALSE(pc_hart_parse(valid_frame, vn, nullptr)); // valid frame, null out struct
    ...
    size_t fn =
        pc_hart_build(HartDelim::HART_DELIM_STX, &addr1, 1, 0x00, data3, sizeof(data3), frame_full, sizeof(frame_full));
