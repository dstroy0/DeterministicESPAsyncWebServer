...
{
    uint8_t out[16];
    size_t n = pc_pn_dcp_header(Pn::PN_FRAMEID_DCP_IDENT_REQ, Pn::PN_DCP_SERVICE_IDENTIFY, Pn::PN_DCP_TYPE_REQUEST,
    ...
    PnDcpHeader h;
    TEST_ASSERT_TRUE(pc_pn_dcp_parse_header(out, n, &h));
...
void test_block_even_padding(void)
{
    ...
    uint8_t out[16];
    size_t n = pc_pn_dcp_block(Pn::PN_DCP_OPT_DEVICE, Pn::PN_DCP_SUB_DEV_NAME_OF_STATION, (const uint8_t *)"plc", 3,
                               out, sizeof(out));
    ...
    // Even value -> no pad.
    n = pc_pn_dcp_block(Pn::PN_DCP_OPT_IP, Pn::PN_DCP_SUB_IP_PARAM, (const uint8_t *)"ABCD", 4, out, sizeof(out));
...
void test_walk_blocks(void)
{
    uint8_t buf[64];
    size_t n = 0;
    n += pc_pn_dcp_block(Pn::PN_DCP_OPT_DEVICE, Pn::PN_DCP_SUB_DEV_NAME_OF_STATION, (const uint8_t *)"plc", 3, buf + n,
                         sizeof(buf) - n);
    n += pc_pn_dcp_block(Pn::PN_DCP_OPT_IP, Pn::PN_DCP_SUB_IP_PARAM, (const uint8_t *)"ABCD", 4, buf + n,
                         sizeof(buf) - n);
...
void test_walk_rejects_truncated(void)
{
    ...
    TEST_ASSERT_FALSE(pc_pn_dcp_walk(bad, sizeof(bad), nullptr, nullptr));
}
...
{
    uint8_t out[16];
    TEST_ASSERT_EQUAL_size_t(0, pc_pn_dcp_header(0, 0, 0, 0, 0, nullptr, sizeof(out))); // null out
    TEST_ASSERT_EQUAL_size_t(0, pc_pn_dcp_header(0, 0, 0, 0, 0, out, 4));               // cap < header len
    TEST_ASSERT_EQUAL_size_t(0, pc_pn_dcp_block(0, 0, (const uint8_t *)"x", 1, nullptr, sizeof(out))); // null out
    TEST_ASSERT_EQUAL_size_t(0, pc_pn_dcp_block(0, 0, nullptr, 5, out, sizeof(out)));         // len but null value
    TEST_ASSERT_EQUAL_size_t(0, pc_pn_dcp_block(0, 0, (const uint8_t *)"sixval", 6, out, 4)); // block > cap

    PnDcpHeader h;
    TEST_ASSERT_FALSE(pc_pn_dcp_parse_header(nullptr, 10, &h)); // null frame
    uint8_t shortf[9] = {0};
    TEST_ASSERT_FALSE(pc_pn_dcp_parse_header(shortf, sizeof(shortf), &h)); // len < header len
}

void test_block_zero_length_value(void)
{
    // value_len == 0 (value may be null) is legal: exercises the "value_len is falsy" path in
    // pc_pn_dcp_block's guard/memcpy-skip, and the blen==0 (-> nullptr value) path in the walk callback.
    uint8_t out[16];
    size_t n = pc_pn_dcp_block(Pn::PN_DCP_OPT_DEVICE, Pn::PN_DCP_SUB_DEV_ID, nullptr, 0, out, sizeof(out));
    TEST_ASSERT_EQUAL_size_t(4, n);
    const uint8_t expect[] = {0x02, 0x03, 0x00, 0x00};
    TEST_ASSERT_EQUAL_HEX8_ARRAY(expect, out, n);

    Seen s = {0, {0}, {0}, {0}};
    TEST_ASSERT_TRUE(pc_pn_dcp_walk(out, n, collect, &s));
...
void test_block_value_len_too_large(void)
{
    ...
    uint8_t out[16];
    uint8_t dummy = 0;
    TEST_ASSERT_EQUAL_size_t(0, pc_pn_dcp_block(0, 0, &dummy, 0x10000, out, sizeof(out)));
}
...
void test_parse_header_null_out(void)
{
    ...
    TEST_ASSERT_FALSE(pc_pn_dcp_parse_header(frame, sizeof(frame), nullptr));
}
...
void test_walk_null_callback(void)
{
    ...
    uint8_t buf[16];
    size_t n =
        pc_pn_dcp_block(Pn::PN_DCP_OPT_IP, Pn::PN_DCP_SUB_IP_PARAM, (const uint8_t *)"ABCD", 4, buf, sizeof(buf));
    TEST_ASSERT_TRUE(pc_pn_dcp_walk(buf, n, nullptr, nullptr));
}
