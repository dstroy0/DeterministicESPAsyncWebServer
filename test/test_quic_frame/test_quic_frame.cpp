...
void test_max_data_and_close()
{
    ...
    n = pc_quic_build_connection_close(b, sizeof b, 0x0a, QuicFrameType::QUIC_FT_STREAM, "bad", 3);
    TEST_ASSERT_EQUAL_INT((int)n, (int)pc_quic_frame_parse(b, n, &f));
...
void test_builders_with_empty_bodies()
{
    ...
    size_t n = pc_quic_build_crypto(out, sizeof out, 4, nullptr, 0);
    TEST_ASSERT_TRUE(n > 0);
    ...
    n = pc_quic_build_stream(out, sizeof out, 8, 0, nullptr, 0, true);
    TEST_ASSERT_TRUE(n > 0);
    TEST_ASSERT_EQUAL_INT((int)n, (int)pc_quic_frame_parse(out, n, &f));
    ...
    n = pc_quic_build_connection_close(out, sizeof out, 0x0a, 0, nullptr, 0);
    TEST_ASSERT_TRUE(n > 0);
    TEST_ASSERT_EQUAL_INT((int)n, (int)pc_quic_frame_parse(out, n, &f));
