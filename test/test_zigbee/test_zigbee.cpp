...
//
// The env sizes PC_ZIGBEE_MAX_DATA = 32.
...
void test_crc16_rst_kat()
{
    ...
    TEST_ASSERT_EQUAL_HEX16(0x38BC, pc_ash_crc16(rst, 1));
}
...
void test_encode_rst_frame_kat()
{
    ...
    uint8_t out[16];
    uint16_t n = pc_ash_frame_encode(Ash::ASH_RST, nullptr, 0, out, sizeof(out));
...
void test_encode_decode_round_trip()
{
    ...
    uint8_t frame[32];
    uint16_t n = pc_ash_frame_encode(0x25, payload, 5, frame, sizeof(frame));
    TEST_ASSERT_GREATER_THAN_UINT16(0, n);

    uint8_t control = 0, pay[16];
    uint16_t plen = 0;
    int c = pc_ash_frame_decode(frame, n, &control, pay, sizeof(pay), &plen);
...
void test_byte_stuffing_round_trip()
{
    ...
    uint8_t frame[64];
    uint16_t n = pc_ash_frame_encode(0x00, payload, 6, frame, sizeof(frame));
    TEST_ASSERT_GREATER_THAN_UINT16(0, n);
    ...
    uint16_t plen = 0;
    TEST_ASSERT_EQUAL_INT((int)n, pc_ash_frame_decode(frame, n, &control, pay, sizeof(pay), &plen));
...
void test_decode_needs_more_without_flag()
{
    ...
    uint8_t pay[8];
    uint16_t plen = 0;
    TEST_ASSERT_EQUAL_INT(0, pc_ash_frame_decode(partial, sizeof(partial), nullptr, pay, sizeof(pay), &plen));
}
...
void test_decode_rejects_bad_crc()
{
    ...
    uint8_t pay[8];
    uint16_t plen = 0;
    TEST_ASSERT_EQUAL_INT(-1, pc_ash_frame_decode(frame, sizeof(frame), nullptr, pay, sizeof(pay), &plen));
}
...
void test_decode_rejects_small_payload_buffer()
{
    ...
    uint8_t frame[32];
    uint16_t n = pc_ash_frame_encode(0x10, payload, 5, frame, sizeof(frame));
    uint8_t tiny[2]; // payload is 5, cap 2
    uint16_t plen = 0;
    TEST_ASSERT_EQUAL_INT(-1, pc_ash_frame_decode(frame, n, nullptr, tiny, sizeof(tiny), &plen));
}
...
void test_encode_bounds()
{
    uint8_t data[40] = {0};
    uint8_t out[128];
    TEST_ASSERT_EQUAL_UINT16(0, pc_ash_frame_encode(0x00, data, 33, out, sizeof(out))); // > MAX_DATA 32
    uint8_t small[3];
    TEST_ASSERT_EQUAL_UINT16(0, pc_ash_frame_encode(0x00, data, 8, small, sizeof(small))); // will not fit
}
...
void test_encode_decode_guards()
{
    ...
    TEST_ASSERT_EQUAL_UINT16(0, pc_ash_frame_encode(0x00, payload, sizeof(payload), out, 2)); // overflow
    uint8_t control = 0;
    uint8_t pay[64];
    uint16_t pay_len = 0;
    uint8_t short_raw[3] = {0x7E, 0x00, 0x7E}; // destuffs to < control+CRC
    TEST_ASSERT_EQUAL_INT(-1, pc_ash_frame_decode(short_raw, sizeof(short_raw), &control, pay, sizeof(pay), &pay_len));
}
...
void test_encode_null_args()
{
    uint8_t out[16];
    uint8_t payload[4] = {1, 2, 3, 4};
    TEST_ASSERT_EQUAL_UINT16(0, pc_ash_frame_encode(0x00, payload, 4, nullptr, sizeof(out)));
    TEST_ASSERT_EQUAL_UINT16(0, pc_ash_frame_encode(0x00, nullptr, 4, out, sizeof(out)));
    TEST_ASSERT_EQUAL_UINT16(4, pc_ash_frame_encode(Ash::ASH_RST, nullptr, 0, out, sizeof(out))); // control
}
...
{
    uint8_t out[16];
    TEST_ASSERT_EQUAL_UINT16(0, pc_ash_frame_encode(Ash::ASH_FLAG, nullptr, 0, out, 1));
    // With room it stuffs into escape + (byte ^ 0x20).
    uint16_t n = pc_ash_frame_encode(Ash::ASH_FLAG, nullptr, 0, out, sizeof(out));
    TEST_ASSERT_GREATER_THAN_UINT16(0, n);
...
{
    uint8_t out[16];
    TEST_ASSERT_EQUAL_UINT16(0, pc_ash_frame_encode(Ash::ASH_RST, nullptr, 0, out, 1)); // no room for CRC hi
    TEST_ASSERT_EQUAL_UINT16(0, pc_ash_frame_encode(Ash::ASH_RST, nullptr, 0, out, 2)); // no room for CRC lo
    TEST_ASSERT_EQUAL_UINT16(0, pc_ash_frame_encode(Ash::ASH_RST, nullptr, 0, out, 3)); // no room for the flag
    TEST_ASSERT_EQUAL_UINT16(4, pc_ash_frame_encode(Ash::ASH_RST, nullptr, 0, out, 4)); // exact fit
}
...
void test_decode_rejects_oversized_frame()
{
    ...
    uint16_t plen = 0;
    TEST_ASSERT_EQUAL_INT(-1, pc_ash_frame_decode(raw, sizeof(raw), &control, pay, sizeof(pay), &plen));
}
...
void test_decode_optional_outputs()
{
    ...
    uint8_t frame[32];
    uint16_t n = pc_ash_frame_encode(0x42, payload, 3, frame, sizeof(frame));
    TEST_ASSERT_GREATER_THAN_UINT16(0, n);

    uint8_t pay[16] = {0};
    TEST_ASSERT_EQUAL_INT((int)n, pc_ash_frame_decode(frame, n, nullptr, pay, sizeof(pay), nullptr));
