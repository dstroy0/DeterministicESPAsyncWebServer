...
void test_telegram_roundtrip(void)
{
    ...
    uint8_t out[16];
    size_t n = pc_sercos_build(Sercos::SERCOS_TEL_MDT, 4, 0x1234, data, 4, out, sizeof(out));
    const uint8_t expect_hdr[] = {Sercos::SERCOS_TEL_MDT, 4, 0x34, 0x12};
    TEST_ASSERT_EQUAL_size_t(8, n);
    TEST_ASSERT_EQUAL_HEX8_ARRAY(expect_hdr, out, 4);

    SercosTelegram t;
    TEST_ASSERT_TRUE(pc_sercos_parse(out, n, &t));
    TEST_ASSERT_EQUAL_HEX8(Sercos::SERCOS_TEL_MDT, t.type);
...
{
    uint8_t out[8];
    size_t n = pc_sercos_build(Sercos::SERCOS_TEL_AT, 0, 1, nullptr, 0, out, sizeof(out));
    TEST_ASSERT_EQUAL_size_t(4, n);
    SercosTelegram t;
    TEST_ASSERT_TRUE(pc_sercos_parse(out, n, &t));
    TEST_ASSERT_EQUAL_HEX8(Sercos::SERCOS_TEL_AT, t.type);
    TEST_ASSERT_EQUAL_size_t(0, t.data_len);
    // Bad type at build; too short + unknown type at parse.
    TEST_ASSERT_EQUAL_size_t(0, pc_sercos_build(0x05, 0, 0, nullptr, 0, out, sizeof(out)));
    TEST_ASSERT_FALSE(pc_sercos_parse(out, 3, &t));
    uint8_t bad[4] = {0x07, 0, 0, 0};
    TEST_ASSERT_FALSE(pc_sercos_parse(bad, 4, &t));
}
...
void test_sercos_build_guards()
{
    ...
    TEST_ASSERT_EQUAL_size_t(
        0, pc_sercos_build(Sercos::SERCOS_TEL_MDT, 0, 0, data, sizeof(data), out, 2)); // cap too small
    TEST_ASSERT_EQUAL_size_t(0,
                             pc_sercos_build(Sercos::SERCOS_TEL_MDT, 0, 0, nullptr, 4, out, sizeof(out))); // null data
    TEST_ASSERT_EQUAL_size_t(0, pc_sercos_build(Sercos::SERCOS_TEL_MDT, 0, 0, nullptr, 0, nullptr, 0));    // null out
}
...
void test_idn_parse_null_out_pointers(void)
{
    ...
    pc_sercos_idn_parse(idn, nullptr, nullptr, nullptr);

    bool prod = false;
    pc_sercos_idn_parse(idn, &prod, nullptr, nullptr);
    TEST_ASSERT_TRUE(prod);

    uint8_t set = 0;
    pc_sercos_idn_parse(idn, nullptr, &set, nullptr);
    TEST_ASSERT_EQUAL_UINT8(5, set);

    uint16_t block = 0;
    pc_sercos_idn_parse(idn, nullptr, nullptr, &block);
...
{
    uint8_t out[8];
    size_t n = pc_sercos_build(Sercos::SERCOS_TEL_MDT, 0, 0, nullptr, 0, out, sizeof(out));
    SercosTelegram t;
    TEST_ASSERT_FALSE(pc_sercos_parse(nullptr, n, &t));  // null frame
    TEST_ASSERT_FALSE(pc_sercos_parse(out, n, nullptr)); // null out
}
