...
void test_telegram_roundtrip(void)
{
    ...
    uint8_t out[16];
    size_t n = detws_sercos_build(Sercos::SERCOS_TEL_MDT, 4, 0x1234, data, 4, out, sizeof(out));
...
{
    uint8_t out[8];
    size_t n = detws_sercos_build(Sercos::SERCOS_TEL_AT, 0, 1, nullptr, 0, out, sizeof(out));
...
void test_sercos_build_guards()
{
    ...
    TEST_ASSERT_EQUAL_size_t(
        0, detws_sercos_build(Sercos::SERCOS_TEL_MDT, 0, 0, data, sizeof(data), out, 2)); // cap too small
    TEST_ASSERT_EQUAL_size_t(
        0, detws_sercos_build(Sercos::SERCOS_TEL_MDT, 0, 0, nullptr, 4, out, sizeof(out))); // null data
}
