...
void setUp()
{
    ...
    pc_coap_server_add_resource("/temp", CoapMethodMask::COAP_ALLOW_GET, h_temp);
    ...
    pc_coaps_server_set_out_sink_cb(out_sink, nullptr);
}
void tearDown()
{
                             ...
                             uint8_t *scid_out = nullptr, size_t *scid_len_out = nullptr)
{
    ...
    size_t off = 0;
    DtlsPlaintext pt;
    size_t rl = pc_dtls_plaintext_parse(flight, fl, &pt);
...
static void assert_coap_205(DtlsRecordKeys *r, const OutDg *dg, const uint8_t *cid = nullptr, size_t cid_len = 0)
{
    ...
    DtlsCiphertext info;
    TEST_ASSERT_TRUE(
        pc_dtls_ciphertext_unprotect(r, 1, dg->buf, dg->len, coap_resp, sizeof(coap_resp), &info, cid, cid_len));
...
static void ingest_bad_client_hello(const char *ip, uint16_t port)
{
    uint8_t garbage[8] = {0};
    uint8_t frag[64];
    size_t fl = pc_dtls_hs_frag_build(0x01, 0, (uint32_t)sizeof(garbage), 0, garbage, (uint32_t)sizeof(garbage), frag,
                                      sizeof(frag));
    uint8_t rec[128];
...
{
    CoapsServerConfig c;
    TEST_ASSERT_FALSE(pc_coaps_server_begin(PC_COAPS_PORT, nullptr)); // null cfg
...
{
    uint8_t d[8] = {0x16, 0, 0, 0, 0, 0, 0, 0};
    TEST_ASSERT_TRUE(pc_coaps_server_ingest(d, sizeof d, nullptr, 1));
...
{
    pc_coaps_server_set_out_sink_cb(nullptr, nullptr);
