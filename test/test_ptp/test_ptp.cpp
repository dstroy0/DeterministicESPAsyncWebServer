...
void test_header_rejects()
{
    pc_ptp_header h;
    uint8_t buf[34];
    TEST_ASSERT_EQUAL_UINT(0, pc_ptp_build_header(nullptr, sizeof(buf), &h, 0));
    ...
    TEST_ASSERT_FALSE(pc_ptp_parse_header(nullptr, 34, &h));
}
...
void test_timestamp_msg_rejects()
{
    ...
    uint8_t buf[64];
    // bad args to build
    TEST_ASSERT_EQUAL_UINT(0, pc_ptp_build_sync(nullptr, sizeof(buf), &h, &ts));
    ...
    TEST_ASSERT_FALSE(pc_ptp_parse_timestamp_msg(buf, n, &h, nullptr));
}
...
void test_delay_resp()
{
    ...
    TEST_ASSERT_EQUAL_UINT(0, pc_ptp_build_delay_resp(buf, sizeof(buf), &h, &t4, nullptr, 7));
    uint8_t sync[44];
    ...
    TEST_ASSERT_FALSE(pc_ptp_parse_delay_resp(buf, n, &g, nullptr));
}
...
void test_announce()
{
    ...
    *p = 0x20; // timeSource = GPS
    ...
    TEST_ASSERT_FALSE(pc_ptp_parse_announce(buf, PC_PTP_HEADER_LEN + 30, &g, nullptr));
}
...
void test_build_announce()
{
    ...
    TEST_ASSERT_EQUAL_UINT(0, pc_ptp_build_announce(buf, sizeof(buf), &h, nullptr));
}
...
void test_compute_symmetric()
{
    ...
    pc_ptp_compute(0, 0, 0, 0, nullptr); // null-safe
}
...
void test_pdelay_mechanism()
{
    ...
    TEST_ASSERT_EQUAL_UINT(0, pc_ptp_build_pdelay_resp(buf, sizeof(buf), &h, &t2, nullptr, 7));
}
