...
static void test_coap_over_dtls(void)
{
    ...
    uint8_t out[256];
    int on = dws_coaps_process(&conn, app_rec, ar, out, sizeof(out));
...
static void test_coaps_no_coap_response(void)
{
    ...
    uint8_t out[256];
    TEST_ASSERT_EQUAL_INT(0, dws_coaps_process(&conn, app_rec, ar, out, sizeof(out)));
}

// After establishment a datagram that is not an epoch-3 application record is routed back to the DTLS
// state machine (dws_coaps_process's fall-through return dws_dtls_conn_process). A zero-length datagram (the
...
static void test_coaps_non_app_record(void)
{
    ...
    TEST_ASSERT_EQUAL_INT(0, dws_coaps_process(&conn, byte, 0, out, sizeof(out))); // len < 1
    TEST_ASSERT_EQUAL_INT(0, dws_coaps_process(&conn, byte, 1, out, sizeof(out))); // not (b0 & 0xE0) == 0x20
    TEST_ASSERT_TRUE(dws_dtls_conn_established(&conn));                            // neither disturbed the connection
}
...
// error (-1), which dws_coaps_process passes through. Covers the epoch (low-two-bits) side of the record test.
static void test_coaps_wrong_epoch_record(void)
{
    ...
    uint8_t out[64];
    TEST_ASSERT_EQUAL_INT(-1, dws_coaps_process(&conn, rec, sizeof(rec), out, sizeof(out)));
}

// Before establishment dws_coaps_process forwards the datagram straight to the DTLS handshake state
// machine (the !dws_dtls_conn_established branch). Driving the ClientHello through dws_coaps_process must emit
// the server's flight, exactly as feeding dws_dtls_conn_process directly does.
static void test_coaps_forwards_handshake(void)
{
    ...
    uint8_t flight[2048];
    int fl = dws_coaps_process(&conn, ch_rec, ch_rl, flight, sizeof(flight));
