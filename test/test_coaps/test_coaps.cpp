...
static void test_quic_aead_open_rejects_short_ciphertext(void)
{
    ...
    uint8_t nonce[12] = {0};
    uint8_t out[16];
    TEST_ASSERT_FALSE(pc_aes128gcm_open(key, nonce, nullptr, 0, nullptr, 0, out));
    uint8_t short_ct[PC_AES128GCM_TAG_LEN - 1] = {0};
    TEST_ASSERT_FALSE(pc_aes128gcm_open(key, nonce, nullptr, 0, short_ct, sizeof(short_ct), out));
}
