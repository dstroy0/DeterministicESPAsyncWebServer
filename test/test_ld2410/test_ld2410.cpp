...
void test_helpers()
{
    ...
    TEST_ASSERT_FALSE(pc_ld2410_present(nullptr));
}
...
void test_host_stubs_and_parse_guards()
{
    ...
    Ld2410Report rep;
    TEST_ASSERT_FALSE(pc_ld2410_parse_report(nullptr, 20, &rep));
...
void test_ld2410b_command_encoders(void)
{
    ...
    TEST_ASSERT_EQUAL_UINT32(0, (uint32_t)pc_ld2410_cmd_get_mac(nullptr, sizeof(buf)));
    TEST_ASSERT_EQUAL_UINT32(0, (uint32_t)pc_ld2410_cmd_set_bt_password(buf, 17, "HiLink"));
    TEST_ASSERT_EQUAL_UINT32(0, (uint32_t)pc_ld2410_cmd_set_bt_password(buf, sizeof(buf), nullptr));
}
...
void test_ld2410b_ack_rejects_malformed(void)
{
    ...
    TEST_ASSERT_FALSE(pc_ld2410_parse_ack(nullptr, 14, &ack));
    TEST_ASSERT_FALSE(pc_ld2410_parse_ack(good, 14, nullptr));
...
void test_parse_report_more_branches(void)
{
    ...
    TEST_ASSERT_FALSE(pc_ld2410_parse_report(BASIC, sizeof(BASIC), nullptr));
...
void test_distance_cm_and_ack_extra_branches(void)
{
    ...
    TEST_ASSERT_EQUAL_UINT16(0, pc_ld2410_distance_cm(nullptr));
    ...
    TEST_ASSERT_FALSE(pc_ld2410_ack_ok(nullptr));
    ...
    uint8_t mac[6];
    TEST_ASSERT_FALSE(pc_ld2410_ack_mac(nullptr, mac));
    ...
    ack.payload_len = 6;
    TEST_ASSERT_FALSE(pc_ld2410_ack_mac(&ack, nullptr)); // mac == nullptr
