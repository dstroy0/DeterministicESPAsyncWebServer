...
void test_ke_record(void)
{
    ...
    uint8_t out[8];
    size_t n = pc_nts_ke_record(true, Nts::NTS_KE_NEXT_PROTOCOL, body, 2, out, sizeof(out));
...
{
    uint8_t out[32];
    size_t n = pc_nts_ke_request(out, sizeof(out));
...
void test_ke_parse(void)
{
    ...
    TEST_ASSERT_FALSE(pc_nts_ke_parse(bad, sizeof(bad), nullptr, nullptr));
    // Truncated body -> false.
    uint8_t trunc[5] = {0x80, 0x01, 0x00, 0x02, 0x00};
    TEST_ASSERT_FALSE(pc_nts_ke_parse(trunc, sizeof(trunc), nullptr, nullptr));
}
...
void test_extension_field_padding(void)
{
    ...
    uint8_t out[64];
    size_t n = pc_nts_ef_unique_id(uid, sizeof(uid), out, sizeof(out));
    ...
    n = pc_nts_ef(NtsEf::NTS_EF_COOKIE, v, 5, out, sizeof(out));
...
void test_ef_wrappers_and_guards()
{
    ...
    TEST_ASSERT_EQUAL_size_t(0, pc_nts_ef(0x0304, data, sizeof(data), nullptr, sizeof(out))); // null out
...
void test_ke_record_guards(void)
{
    uint8_t body[2] = {0x00, 0x00};
    uint8_t out[8];
    TEST_ASSERT_EQUAL_size_t(0, pc_nts_ke_record(true, Nts::NTS_KE_NEXT_PROTOCOL, body, 2, nullptr, sizeof(out)));
    TEST_ASSERT_EQUAL_size_t(0, pc_nts_ke_record(true, Nts::NTS_KE_NEXT_PROTOCOL, nullptr, 2, out, sizeof(out)));
    // A body longer than the 16-bit length field can express is refused (the pointer
    // is never read: the guard returns first).
    TEST_ASSERT_EQUAL_size_t(0, pc_nts_ke_record(true, Nts::NTS_KE_NEXT_PROTOCOL, body, 0x10000, out, sizeof(out)));
    // Header + body must fit the destination.
    TEST_ASSERT_EQUAL_size_t(0, pc_nts_ke_record(true, Nts::NTS_KE_NEXT_PROTOCOL, body, 2, out, 4));
    TEST_ASSERT_EQUAL_size_t(6, pc_nts_ke_record(true, Nts::NTS_KE_NEXT_PROTOCOL, body, 2, out, 6)); // exact fit
}
...
{
    uint8_t out[8];
    size_t n = pc_nts_ke_record(false, Nts::NTS_KE_AEAD_ALGORITHM, nullptr, 0, out, sizeof(out));
...
{
    uint8_t out[32];
    TEST_ASSERT_EQUAL_size_t(0, pc_nts_ke_request(out, 5));   // next-protocol does not fit
    TEST_ASSERT_EQUAL_size_t(0, pc_nts_ke_request(out, 11));  // aead does not fit
    TEST_ASSERT_EQUAL_size_t(0, pc_nts_ke_request(out, 15));  // end-of-message does not fit
    TEST_ASSERT_EQUAL_size_t(16, pc_nts_ke_request(out, 16)); // exact fit
}
...
{
    uint8_t out[64];
    size_t n = pc_nts_ef(NtsEf::NTS_EF_COOKIE, nullptr, 0, out, sizeof(out));
    TEST_ASSERT_EQUAL_size_t(4, n); // type + length only, already 4-aligned
    TEST_ASSERT_EQUAL_UINT16(4, (uint16_t)((out[2] << 8) | out[3]));
    TEST_ASSERT_EQUAL_size_t(0, pc_nts_ef(NtsEf::NTS_EF_COOKIE, nullptr, 8, out, sizeof(out)));
}
...
void test_ef_length_field_overflow(void)
{
    uint8_t out[64];
    uint8_t value[8] = {0};
    TEST_ASSERT_EQUAL_size_t(0, pc_nts_ef(NtsEf::NTS_EF_COOKIE, value, 0xFFFC, out, sizeof(out)));
}
