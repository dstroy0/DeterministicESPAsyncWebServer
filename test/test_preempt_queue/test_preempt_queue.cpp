...
{
    return dws_pq_post(&v, 0);
}

static void stop_all_lanes()
{
    for (int l = 0; l < (int)dws_pq_lane::DWS_PQ_LANE_COUNT; l++)
        dws_pq_stop_lane((dws_pq_lane)l);
}
...
void setUp()
{
    ...
    cfg.core = 1;
    cfg.name = "test_pq";
    dws_pq_start(&cfg); // starts the USER lane (no-arg API)
}
void tearDown()
{
    stop_all_lanes();
...
void test_start_validates_and_runs()
{
    ...
    TEST_ASSERT_FALSE(dws_pq_start(&ok)); // double start is a no-op
}
...
void test_urgent_goes_to_front()
{
    ...
    post_u32(2);
    uint32_t u = 99;
    TEST_ASSERT_TRUE(dws_pq_post_urgent(&u, 0));
    dws_pq_drain();
    TEST_ASSERT_EQUAL_size_t(3, g_seen.size());
...
void test_high_water_tracks_peak()
{
    ...
    post_u32(2);
    post_u32(3);
    TEST_ASSERT_GREATER_OR_EQUAL_size_t(3, dws_pq_high_water());
    dws_pq_drain();
...
{
    uint32_t v = 7;
    TEST_ASSERT_TRUE(dws_pq_post_from_isr(&v));
    dws_pq_drain();
...
void test_internal_lanes_outrank_user()
{
    ...
    TEST_ASSERT_GREATER_THAN_UINT8(dws_pq_lane_priority(dws_pq_lane::DWS_PQ_LANE_FORWARD),
                                   dws_pq_lane_priority(dws_pq_lane::DWS_PQ_LANE_DMA));
    TEST_ASSERT_GREATER_THAN_UINT8(dws_pq_lane_priority(dws_pq_lane::DWS_PQ_LANE_DEVICE),
                                   dws_pq_lane_priority(dws_pq_lane::DWS_PQ_LANE_FORWARD));
    TEST_ASSERT_GREATER_THAN_UINT8(dws_pq_lane_priority(dws_pq_lane::DWS_PQ_LANE_USER),
                                   dws_pq_lane_priority(dws_pq_lane::DWS_PQ_LANE_DEVICE));
}
...
void test_lanes_are_isolated()
{
    ...
    uint32_t u = 11, d = 22;
    TEST_ASSERT_TRUE(dws_pq_post(&u, 0));                                    // -> USER
    TEST_ASSERT_TRUE(dws_pq_post_lane(dws_pq_lane::DWS_PQ_LANE_DMA, &d, 0)); // -> DMA
...
void test_lane_start_stop_running_independent()
{
    ...
    TEST_ASSERT_FALSE(dws_pq_start_lane(dws_pq_lane::DWS_PQ_LANE_DMA, &dma)); // double start is a no-op

    dws_pq_stop_lane(dws_pq_lane::DWS_PQ_LANE_DMA);
    TEST_ASSERT_FALSE(dws_pq_running_lane(dws_pq_lane::DWS_PQ_LANE_DMA));
    TEST_ASSERT_TRUE(dws_pq_running_lane(dws_pq_lane::DWS_PQ_LANE_USER)); // USER unaffected
}
...
void test_lane_high_water_is_per_lane()
{
    ...
    TEST_ASSERT_TRUE(dws_pq_start_lane(dws_pq_lane::DWS_PQ_LANE_DMA, &dma));
    uint32_t v = 5;
    dws_pq_post_lane(dws_pq_lane::DWS_PQ_LANE_DMA, &v, 0);
    dws_pq_post_lane(dws_pq_lane::DWS_PQ_LANE_DMA, &v, 0);
    TEST_ASSERT_GREATER_OR_EQUAL_size_t(2, dws_pq_high_water_lane(dws_pq_lane::DWS_PQ_LANE_DMA));
    TEST_ASSERT_EQUAL_size_t(0, dws_pq_high_water_lane(dws_pq_lane::DWS_PQ_LANE_DEVICE)); // untouched lane
}
...
void test_lane_api_urgent_and_drain()
{
    ...
    uint32_t a = 10, b = 20;
    TEST_ASSERT_TRUE(dws_pq_post_lane(dws_pq_lane::DWS_PQ_LANE_DMA, &a, 0));
    TEST_ASSERT_TRUE(dws_pq_post_lane_urgent(dws_pq_lane::DWS_PQ_LANE_DMA, &b, 0)); // urgent -> jumps the queue
    dws_pq_drain_lane(dws_pq_lane::DWS_PQ_LANE_DMA);
    ...
    TEST_ASSERT_FALSE(dws_pq_post_lane_urgent((dws_pq_lane)dws_pq_lane::DWS_PQ_LANE_COUNT, &a, 0));
    TEST_ASSERT_FALSE(dws_pq_post_lane_urgent(dws_pq_lane::DWS_PQ_LANE_DMA, nullptr, 0));
    dws_pq_drain_lane((dws_pq_lane)dws_pq_lane::DWS_PQ_LANE_COUNT);
    dws_pq_stop_lane(dws_pq_lane::DWS_PQ_LANE_DMA);
}
