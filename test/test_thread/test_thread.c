...
void test_spinel_command_build_parse_round_trip()
{
    ...
    uint16_t n = pc_spinel_command_build(0x81, SPINEL_CMD_PROP_VALUE_SET, 1337, val, 4, buf, sizeof(buf));
    TEST_ASSERT_GREATER_THAN_UINT16(0, n);
...
void test_thread_more_guards()
{
    ...
    TEST_ASSERT_EQUAL_UINT8(0, pc_spinel_pack_uint(5, NULL, 8));
    uint32_t v = 0;
    TEST_ASSERT_EQUAL_INT(0, pc_spinel_unpack_uint(NULL, 4, &v));

    // command_build guards: null out, cap < 1, value==null with a positive length.
    uint8_t val[2] = {0xAA, 0xBB};
    TEST_ASSERT_EQUAL_UINT16(0, pc_spinel_command_build(0x81, 1, 1, val, 2, NULL, 8));
    TEST_ASSERT_EQUAL_UINT16(0, pc_spinel_command_build(0x81, 1, 1, val, 2, out, 0));
    TEST_ASSERT_EQUAL_UINT16(0, pc_spinel_command_build(0x81, 1, 1, NULL, 2, out, 8));
    ...
    uint16_t pvl = 0;
    TEST_ASSERT_EQUAL_INT(-1, pc_spinel_command_parse(NULL, 4, &hdr, &cmd, &prop, &pv, &pvl));
    ...
    uint8_t pay[16];
    uint16_t pl = 0;
    TEST_ASSERT_EQUAL_INT(0, pc_spinel_frame_decode(NULL, 4, pay, sizeof(pay), &pl));
    uint8_t big[80];
    for (int i = 0; i < 70; i++)
    {
        big[i] = 0x00;
...
void test_spinel_writer_overflow_latch()
{
    ...
    // null-buffer guards.
    SpinelWriter nw;
    pc_spinel_writer_init(&nw, NULL, 8);
    ...
    TEST_ASSERT_FALSE(pc_spinel_put_eui64(&w, NULL));
    TEST_ASSERT_FALSE(pc_spinel_put_utf8(&w, NULL));
}
...
{
    TEST_ASSERT_EQUAL_STRING("NCP_VERSION", pc_spinel_prop_name(SPINEL_PROP_NCP_VERSION));
    TEST_ASSERT_EQUAL_STRING("MAC_15_4_PANID", pc_spinel_prop_name(SPINEL_PROP_MAC_15_4_PANID));
    TEST_ASSERT_EQUAL_STRING("UNKNOWN", pc_spinel_prop_name(0xFFFFFF));

    const SpinelPropInfo *e = pc_spinel_prop_lookup(SPINEL_PROP_HWADDR);
    ...
    TEST_ASSERT_EQUAL_INT8('U', pc_spinel_prop_lookup(SPINEL_PROP_NCP_VERSION)->type);
    TEST_ASSERT_EQUAL_INT8('C', pc_spinel_prop_lookup(SPINEL_PROP_PHY_CHAN)->type);
    TEST_ASSERT_EQUAL_INT8('6', pc_spinel_prop_lookup(SPINEL_PROP_IPV6_LL_ADDR)->type);
}

void test_spinel_status_names()
{
    TEST_ASSERT_EQUAL_STRING("OK", pc_spinel_status_name(SPINEL_STATUS_OK));
    TEST_ASSERT_EQUAL_STRING("PROP_NOT_FOUND", pc_spinel_status_name(SPINEL_STATUS_PROP_NOT_FOUND));
    TEST_ASSERT_EQUAL_STRING("RESET", pc_spinel_status_name(SPINEL_STATUS_RESET_POWER_ON));
...
void test_spinel_null_out_params()
{
    ...
    TEST_ASSERT_EQUAL_INT(1, pc_spinel_unpack_uint(one, 1, NULL));

    uint8_t buf[32];
    const uint8_t val[2] = {0xAA, 0xBB};
    uint16_t n = pc_spinel_command_build(0x81, SPINEL_CMD_PROP_VALUE_SET, 1337, val, 2, buf, sizeof(buf));
    TEST_ASSERT_GREATER_THAN_UINT16(0, n);

    // A zero-length payload has no header byte to read.
    TEST_ASSERT_EQUAL_INT(-1, pc_spinel_command_parse(buf, 0, NULL, NULL, NULL, NULL, NULL));
    // Every out-parameter is optional: the parse still succeeds and reports the value offset.
    TEST_ASSERT_GREATER_THAN_INT(0, pc_spinel_command_parse(buf, n, NULL, NULL, NULL, NULL, NULL));
}

void test_spinel_reader_init_variants()
{
    pc_spinel_reader_init(NULL, NULL, 0); // a null cursor is a no-op, not a crash
    ...
    pc_spinel_reader_init(&r, NULL, 0); // no value at all is not an error
    TEST_ASSERT_EQUAL_UINT16(0, r.len);
    TEST_ASSERT_TRUE(pc_spinel_reader_ok(&r));

    pc_spinel_reader_init(&r, NULL, 5); // a null value with a positive length is malformed
    TEST_ASSERT_EQUAL_UINT16(0, r.len);
    TEST_ASSERT_FALSE(pc_spinel_reader_ok(&r));

    TEST_ASSERT_FALSE(pc_spinel_reader_ok(NULL)); // a null cursor is never ok
}
...
void test_spinel_getters_null_reader()
{
    ...
    TEST_ASSERT_FALSE(pc_spinel_get_bool(NULL, &b));
    TEST_ASSERT_FALSE(pc_spinel_get_u8(NULL, &u8));
    TEST_ASSERT_FALSE(pc_spinel_get_i8(NULL, &i8));
    TEST_ASSERT_FALSE(pc_spinel_get_u16(NULL, &u16));
    TEST_ASSERT_FALSE(pc_spinel_get_i16(NULL, &i16));
    TEST_ASSERT_FALSE(pc_spinel_get_u32(NULL, &u32));
    TEST_ASSERT_FALSE(pc_spinel_get_i32(NULL, &i32));
    TEST_ASSERT_FALSE(pc_spinel_get_uint(NULL, &u32));
    TEST_ASSERT_FALSE(pc_spinel_get_eui64(NULL, &p));
    TEST_ASSERT_FALSE(pc_spinel_get_ipv6(NULL, &p));
    TEST_ASSERT_FALSE(pc_spinel_get_utf8(NULL, &s, &l));
    TEST_ASSERT_FALSE(pc_spinel_get_data(NULL, &p, &l));
    TEST_ASSERT_FALSE(pc_spinel_get_data_wlen(NULL, &p, &l));
}
...
void test_spinel_getters_null_out_params()
{
    ...
    SpinelReader r;
    pc_spinel_reader_init(&r, buf, n);
    TEST_ASSERT_TRUE(pc_spinel_get_bool(&r, NULL));
    TEST_ASSERT_TRUE(pc_spinel_get_u8(&r, NULL));
    TEST_ASSERT_TRUE(pc_spinel_get_i8(&r, NULL));
    TEST_ASSERT_TRUE(pc_spinel_get_u16(&r, NULL));
    TEST_ASSERT_TRUE(pc_spinel_get_i16(&r, NULL));
    TEST_ASSERT_TRUE(pc_spinel_get_u32(&r, NULL));
    TEST_ASSERT_TRUE(pc_spinel_get_i32(&r, NULL));
    TEST_ASSERT_TRUE(pc_spinel_get_uint(&r, NULL));
    TEST_ASSERT_TRUE(pc_spinel_get_eui64(&r, NULL));
    TEST_ASSERT_TRUE(pc_spinel_get_ipv6(&r, NULL));
    TEST_ASSERT_TRUE(pc_spinel_get_utf8(&r, NULL, NULL));
    TEST_ASSERT_TRUE(pc_spinel_reader_ok(&r));
    ...
    TEST_ASSERT_TRUE(pc_spinel_get_data_wlen(&r, NULL, NULL));
    pc_spinel_reader_init(&r, dv, sizeof(dv));
    TEST_ASSERT_TRUE(pc_spinel_get_data(&r, NULL, NULL));
}

void test_spinel_writer_init_and_null_writer()
{
    pc_spinel_writer_init(NULL, NULL, 0); // a null cursor is a no-op
    TEST_ASSERT_EQUAL_UINT16(0, pc_spinel_writer_len(NULL));

    SpinelWriter zw;
    pc_spinel_writer_init(&zw, NULL, 0); // no buffer and no capacity is not an error
    ...
    TEST_ASSERT_FALSE(pc_spinel_put_i16(NULL, 1));
    TEST_ASSERT_FALSE(pc_spinel_put_u32(NULL, 1));
    TEST_ASSERT_FALSE(pc_spinel_put_i32(NULL, 1));
    TEST_ASSERT_FALSE(pc_spinel_put_uint(NULL, 1));
    TEST_ASSERT_FALSE(pc_spinel_put_eui64(NULL, eui));
    TEST_ASSERT_FALSE(pc_spinel_put_ipv6(NULL, v6));
    TEST_ASSERT_FALSE(pc_spinel_put_utf8(NULL, "a"));
    TEST_ASSERT_FALSE(pc_spinel_put_data(NULL, eui, 8));
    TEST_ASSERT_FALSE(pc_spinel_put_data_wlen(NULL, eui, 8));
}
...
void test_spinel_put_null_args()
{
    ...
    pc_spinel_writer_init(&w, buf, sizeof(buf));
    TEST_ASSERT_TRUE(pc_spinel_put_data(&w, NULL, 0));
    TEST_ASSERT_EQUAL_UINT16(0, pc_spinel_writer_len(&w));

    // A null data pointer with a positive length latches err.
    pc_spinel_writer_init(&w, buf, sizeof(buf));
    TEST_ASSERT_FALSE(pc_spinel_put_data(&w, NULL, 4));
    TEST_ASSERT_EQUAL_UINT16(0, pc_spinel_writer_len(&w));

    // So does a null IPv6 address.
    pc_spinel_writer_init(&w, buf, sizeof(buf));
    TEST_ASSERT_FALSE(pc_spinel_put_ipv6(&w, NULL));
    TEST_ASSERT_EQUAL_UINT16(0, pc_spinel_writer_len(&w));

    // The same argument guards must not dereference a null writer either.
    TEST_ASSERT_FALSE(pc_spinel_put_eui64(NULL, NULL));
    TEST_ASSERT_FALSE(pc_spinel_put_ipv6(NULL, NULL));
    TEST_ASSERT_FALSE(pc_spinel_put_utf8(NULL, NULL));
    TEST_ASSERT_FALSE(pc_spinel_put_data(NULL, NULL, 4));
...
void test_spinel_frame_edges()
{
    ...
    TEST_ASSERT_EQUAL_UINT16(0, pc_spinel_frame_encode(p4, 4, NULL, sizeof(out)));
    TEST_ASSERT_EQUAL_UINT16(0, pc_spinel_frame_encode(NULL, 4, out, sizeof(out)));

    // A zero-length payload is legal: FCS(lo,hi) + flag only, and it round-trips.
    uint16_t n = pc_spinel_frame_encode(NULL, 0, out, sizeof(out));
    ...
    TEST_ASSERT_GREATER_THAN_UINT16(0, fn);
    TEST_ASSERT_EQUAL_INT((int)fn, pc_spinel_frame_decode(out, fn, pay, sizeof(pay), NULL));
}
