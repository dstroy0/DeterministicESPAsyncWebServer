...
{
    // max-age=60, swr=30.
    TEST_ASSERT_EQUAL_INT(DeliveryVerdict::DELIVERY_FRESH, pc_delivery_swr(0, 60, 30));
    TEST_ASSERT_EQUAL_INT(DeliveryVerdict::DELIVERY_FRESH, pc_delivery_swr(60, 60, 30)); // boundary: <= max-age
    TEST_ASSERT_EQUAL_INT(DeliveryVerdict::DELIVERY_STALE_REVALIDATE, pc_delivery_swr(61, 60, 30));
    TEST_ASSERT_EQUAL_INT(DeliveryVerdict::DELIVERY_STALE_REVALIDATE, pc_delivery_swr(90, 60, 30)); // boundary: max+swr
    TEST_ASSERT_EQUAL_INT(DeliveryVerdict::DELIVERY_EXPIRED, pc_delivery_swr(91, 60, 30));
    // No swr window.
    TEST_ASSERT_EQUAL_INT(DeliveryVerdict::DELIVERY_EXPIRED, pc_delivery_swr(61, 60, 0));
    ...
    TEST_ASSERT_EQUAL_INT(DeliveryVerdict::DELIVERY_STALE_REVALIDATE, pc_delivery_swr(0xFFFFFFF8u, 0xFFFFFFF0u, 0x20u));
}

void test_cache_control(void)
{
    char buf[64];
    size_t n = pc_delivery_cache_control(60, 30, buf, sizeof(buf));
    TEST_ASSERT_EQUAL_size_t(strlen(buf), n);
    TEST_ASSERT_EQUAL_STRING("public, max-age=60, stale-while-revalidate=30", buf);
    pc_delivery_cache_control(3600, 0, buf, sizeof(buf));
    TEST_ASSERT_EQUAL_STRING("public, max-age=3600", buf);
    // Overflow.
    char tiny[8];
    TEST_ASSERT_EQUAL_size_t(0, pc_delivery_cache_control(3600, 30, tiny, sizeof(tiny)));
}
...
void test_sw_manifest(void)
{
    ...
    char buf[128];
    size_t n = pc_delivery_sw_manifest(paths, 3, "v42", buf, sizeof(buf));
    TEST_ASSERT_EQUAL_size_t(strlen(buf), n);
    TEST_ASSERT_EQUAL_STRING("{\"version\":\"v42\",\"precache\":[\"/\",\"/app.js\",\"/style.css\"]}", buf);
    // Empty list.
    pc_delivery_sw_manifest(nullptr, 0, "v1", buf, sizeof(buf));
...
{
    char buf[256];
    TEST_ASSERT_EQUAL_size_t(0, pc_delivery_cache_control(60, 30, nullptr, sizeof(buf))); // null out
    ...
    TEST_ASSERT_EQUAL_size_t(0, pc_delivery_sw_manifest(paths, 1, "1.0", buf, 4)); // tiny cap fails closed
}
...
void test_builder_edge_guards(void)
{
    ...
    TEST_ASSERT_EQUAL_size_t(0, pc_delivery_sw_manifest(paths, 1, "v", nullptr, sizeof(buf))); // null out
    TEST_ASSERT_EQUAL_size_t(0, pc_delivery_sw_manifest(nullptr, 2, "v", buf, sizeof(buf)));   // n>0, null paths
    ...
    size_t n = pc_delivery_sw_manifest(paths, 1, nullptr, buf, sizeof(buf));
    TEST_ASSERT_TRUE(n > 0);
...
{
    const char *paths[PC_DELIVERY_PRECACHE_MAX];
    for (size_t i = 0; i < PC_DELIVERY_PRECACHE_MAX; i++)
    {
        paths[i] = "/assets/chunk-0000.js"; // a realistic worst-case path length
    }

    char buf[PC_DELIVERY_MANIFEST_BUF];
    size_t n = pc_delivery_sw_manifest(paths, PC_DELIVERY_PRECACHE_MAX, "7.15.0", buf, sizeof(buf));
    TEST_ASSERT_TRUE(n > 0);
    TEST_ASSERT_EQUAL_size_t(strlen(buf), n);
    ...
    TEST_ASSERT_EQUAL_size_t(0, pc_delivery_sw_manifest(paths, PC_DELIVERY_PRECACHE_MAX, "7.15.0", buf, n));
}
