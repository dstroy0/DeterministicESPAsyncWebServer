...
{
    int h = pc_fs_open(s_root, "/s.bin", "", PC_MNT_WRITE);
    ...
    h = pc_fs_open(s_root, "/s.bin", "", PC_MNT_READ);
    TEST_ASSERT_TRUE(h >= 0);
    char buf[8] = {0};
...
void test_append_extends()
{
    ...
    int h = pc_fs_open(s_root, "/log", "", PC_MNT_APPEND);
    TEST_ASSERT_TRUE(h >= 0);
    pc_fs_write(h, "line2\n", 6);
    pc_fs_close(h);
...
void test_missing_file_fails_closed()
{
    ...
    TEST_ASSERT_TRUE(pc_fs_open(s_root, "/nope", "", PC_MNT_READ) < 0);
    char buf[8];
...
{
    int h = pc_fs_open(s_root, "/full", "", PC_MNT_WRITE);
    TEST_ASSERT_TRUE(h >= 0);
...
void test_handle_pool_exhaustion()
{
    ...
    {
        handles[i] = pc_fs_open(s_root, "/h", "", PC_MNT_READ);
        TEST_ASSERT_TRUE(handles[i] >= 0);
    }
    TEST_ASSERT_TRUE(pc_fs_open(s_root, "/h", "", PC_MNT_READ) < 0); // no handles left
    pc_fs_close(handles[0]);
    TEST_ASSERT_TRUE(pc_fs_open(s_root, "/h", "", PC_MNT_READ) >= 0); // one freed
}

void test_unmounted_fails_closed()
{
    pc_mnt_mount(NULL);
    TEST_ASSERT_TRUE(pc_fs_open(s_root, "/a", "", PC_MNT_READ) < 0);
...
void test_ram_guard_subconditions()
{
    ...
    TEST_ASSERT_EQUAL_INT(-1, pc_fs_open(s_root, NULL, "", PC_MNT_WRITE));
    char longname[256];
    for (int i = 0; i < 255; i++)
    {
        longname[i] = 'a';
    ...
    longname[255] = '\0';
    TEST_ASSERT_EQUAL_INT(-1, pc_fs_open(s_root, longname, "", PC_MNT_WRITE));
...
void test_handle_validity_edges()
{
    ...
    int h = pc_fs_open(s_root, "/hv", "", PC_MNT_READ);
    TEST_ASSERT_TRUE(h >= 0);
    pc_fs_close(h);
    ...
    TEST_ASSERT_TRUE(pc_fs_open(s_root, "/hv", "", PC_MNT_READ) >= 0); // pool still usable
}
...
void test_write_to_read_handle_rejected()
{
    ...
    int h = pc_fs_open(s_root, "/ro", "", PC_MNT_READ);
    TEST_ASSERT_TRUE(h >= 0);
...
void test_rename_argument_guards()
{
    ...
    TEST_ASSERT_FALSE(pc_fs_rename(s_root, NULL, "", "/r2", ""));
    TEST_ASSERT_FALSE(pc_fs_rename(s_root, "/r1", "", NULL, ""));
...
void test_read_file_handle_exhaustion()
{
    ...
    for (int i = 0; i < PC_MNT_MAX_OPEN; i++)
    {
        handles[i] = pc_fs_open(s_root, "/rf", "", PC_MNT_READ);
        TEST_ASSERT_TRUE(handles[i] >= 0);
    }
    char buf[16];
