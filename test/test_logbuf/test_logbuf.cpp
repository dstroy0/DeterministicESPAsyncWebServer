...
{
    pc_logbuf_reset();
    pc_log_set_trap(0xFF, nullptr); // disable trap between tests
}
void tearDown()
{
}
...
{
    pc_log(pc_log_level::PC_LOG_INFO, "first");
    pc_log(pc_log_level::PC_LOG_WARN, "second");
...
void test_dump()
{
    pc_log(pc_log_level::PC_LOG_ERROR, "boom");
    pc_log(pc_log_level::PC_LOG_DEBUG, "trace");
    char buf[128];
...
void test_rotation_drops_oldest()
{
    ...
    for (int i = 0; i < PC_LOG_LINES + 3; i++) // overflow by 3
    {
        ...
        pc_log(pc_log_level::PC_LOG_INFO, msg);
    }
...
{
    g_traps = 0;
    pc_log_set_trap(pc_log_level::PC_LOG_WARN, trap);
    pc_log(pc_log_level::PC_LOG_INFO, "ignored"); // below threshold
    pc_log(pc_log_level::PC_LOG_DEBUG, "ignored");
    TEST_ASSERT_EQUAL_INT(0, g_traps);
    pc_log(pc_log_level::PC_LOG_WARN, "warned"); // at threshold
    pc_log(pc_log_level::PC_LOG_ERROR, "errored");
    TEST_ASSERT_EQUAL_INT(2, g_traps);
    TEST_ASSERT_EQUAL_UINT8(pc_log_level::PC_LOG_ERROR, g_last_level);
}

void test_log_null_message()
{
    // A null message must not crash and must fall back to an empty string body.
    pc_log(pc_log_level::PC_LOG_INFO, nullptr);
...
{
    char out[64];
    TEST_ASSERT_EQUAL_INT(0, pc_log_dump(nullptr, sizeof(out))); // null out
