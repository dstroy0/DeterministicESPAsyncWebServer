...
{
    uint8_t hdr[2];
    TEST_ASSERT_TRUE(pc_ad9238_build_instruction(false, 0x0009, 1, hdr));
...
{
    uint8_t hdr[2];
    TEST_ASSERT_TRUE(pc_ad9238_build_instruction(true, 0x0001, 1, hdr));
...
void test_instruction_word_byte_count_field()
{
    ...
    TEST_ASSERT_TRUE(pc_ad9238_build_instruction(false, 0x0100, 4, hdr));
...
{
    uint8_t hdr[2];
    TEST_ASSERT_FALSE(pc_ad9238_build_instruction(false, 0x00, 1, nullptr)); // null out
    TEST_ASSERT_FALSE(pc_ad9238_build_instruction(false, 0x00, 0, hdr));     // nbytes 0
    TEST_ASSERT_FALSE(pc_ad9238_build_instruction(false, 0x00, 5, hdr));     // nbytes > 4
    TEST_ASSERT_FALSE(pc_ad9238_build_instruction(false, 0x2000, 1, hdr));   // addr > 13 bits
    TEST_ASSERT_TRUE(pc_ad9238_build_instruction(false, 0x1FFF, 1, hdr));    // max valid addr
}
...
void test_build_write_transaction()
{
    ...
    TEST_ASSERT_EQUAL_size_t(3,
                             pc_ad9238_build_write((uint16_t)Ad9238Reg::AD9238_REG_POWER_DOWN, 0x01, out, sizeof(out)));
    TEST_ASSERT_EQUAL_HEX8(0x00, out[0]);
    TEST_ASSERT_EQUAL_HEX8(0x09, out[1]);
    TEST_ASSERT_EQUAL_HEX8(0x01, out[2]);

    TEST_ASSERT_EQUAL_size_t(0, pc_ad9238_build_write(0x09, 0x01, nullptr, 3)); // null out
    TEST_ASSERT_EQUAL_size_t(0, pc_ad9238_build_write(0x09, 0x01, out, 2));     // undersized cap
    TEST_ASSERT_EQUAL_size_t(0, pc_ad9238_build_write(0x2000, 0x01, out, 3));   // reg_addr past 13-bit field
}

void test_build_read_transaction()
{
    uint8_t out[2] = {};
    TEST_ASSERT_EQUAL_size_t(2, pc_ad9238_build_read((uint16_t)Ad9238Reg::AD9238_REG_CHIP_ID, out, sizeof(out)));
    TEST_ASSERT_EQUAL_HEX8(0x80, out[0]);
    TEST_ASSERT_EQUAL_HEX8(0x01, out[1]);

    TEST_ASSERT_EQUAL_size_t(0, pc_ad9238_build_read(0x01, nullptr, 2)); // null out
    TEST_ASSERT_EQUAL_size_t(0, pc_ad9238_build_read(0x01, out, 1));     // undersized cap
    TEST_ASSERT_EQUAL_size_t(0, pc_ad9238_build_read(0x2000, out, 2));   // reg_addr past 13-bit field
}
