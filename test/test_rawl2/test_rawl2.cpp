...
void test_build_ethernet_ii(void)
{
    ...
    uint8_t out[32];
    size_t n = pc_eth_build(DST, SRC, RawL2::ETHERTYPE_IPV4, pl, 2, out, sizeof(out));
...
void test_build_vlan(void)
{
    ...
    size_t n = pc_eth_build_vlan(DST, SRC, 3, false, 100, RawL2::ETHERTYPE_PROFINET, pl, 2, out, sizeof(out));
...
void test_parse(void)
{
    uint8_t pl[2] = {0xDE, 0xAD};
    uint8_t buf[32];
    size_t n = pc_eth_build(DST, SRC, RawL2::ETHERTYPE_IPV4, pl, 2, buf, sizeof(buf));
    EthFrame f;
    TEST_ASSERT_TRUE(pc_eth_parse(buf, n, &f));
    TEST_ASSERT_FALSE(f.vlan);
    TEST_ASSERT_EQUAL_UINT16(RawL2::ETHERTYPE_IPV4, f.ethertype);
    ...
    // VLAN roundtrip.
    n = pc_eth_build_vlan(DST, SRC, 5, true, 4094, RawL2::ETHERTYPE_GOOSE, pl, 2, buf, sizeof(buf));
    TEST_ASSERT_TRUE(pc_eth_parse(buf, n, &f));
    TEST_ASSERT_TRUE(f.vlan);
    TEST_ASSERT_EQUAL_UINT8(5, f.pcp);
    TEST_ASSERT_EQUAL_UINT16(4094, f.vid);
    TEST_ASSERT_EQUAL_UINT16(RawL2::ETHERTYPE_GOOSE, f.ethertype);

    // Too short -> false.
    TEST_ASSERT_FALSE(pc_eth_parse(buf, 10, &f));
}
...
void test_fcs_check_vector(void)
{
    ...
    TEST_ASSERT_EQUAL_HEX32(0xCBF43926, pc_eth_fcs(msg, sizeof(msg)));
}
...
void test_eth_build_parse_guards()
{
    ...
    TEST_ASSERT_EQUAL_size_t(0, pc_eth_build(nullptr, mac, 0x0800, pay, sizeof(pay), out, sizeof(out))); // null dst
    TEST_ASSERT_EQUAL_size_t(0, pc_eth_build(mac, mac, 0x0800, pay, sizeof(pay), out, 8)); // cap too small
    TEST_ASSERT_EQUAL_size_t(
        0, pc_eth_build_vlan(nullptr, mac, 0, false, 100, 0x0800, pay, sizeof(pay), out, sizeof(out))); // null dst
...
void test_eth_build_null_src_out_and_zero_payload(void)
{
    ...
    // pc_eth_build: null src, null out, zero-length payload (skips the copy), and a
    // non-zero payload_len paired with a null payload pointer (each is a distinct guard branch).
    TEST_ASSERT_EQUAL_size_t(0, pc_eth_build(mac, nullptr, 0x0800, pay, sizeof(pay), out, sizeof(out))); // null src
    TEST_ASSERT_EQUAL_size_t(0, pc_eth_build(mac, mac, 0x0800, pay, sizeof(pay), nullptr, sizeof(out))); // null out
    TEST_ASSERT_EQUAL_size_t(RawL2::ETH_HDR_LEN,
                             pc_eth_build(mac, mac, 0x0800, nullptr, 0, out, sizeof(out)));    // payload_len == 0
    TEST_ASSERT_EQUAL_size_t(0, pc_eth_build(mac, mac, 0x0800, nullptr, 4, out, sizeof(out))); // len>0, null payload

    // pc_eth_build_vlan: the same set of guard branches.
    TEST_ASSERT_EQUAL_size_t(
        0, pc_eth_build_vlan(mac, nullptr, 0, false, 100, 0x0800, pay, sizeof(pay), out, sizeof(out))); // null src
    TEST_ASSERT_EQUAL_size_t(
        0, pc_eth_build_vlan(mac, mac, 0, false, 100, 0x0800, pay, sizeof(pay), nullptr, sizeof(out))); // null out
    TEST_ASSERT_EQUAL_size_t(RawL2::ETH_VLAN_HDR_LEN, pc_eth_build_vlan(mac, mac, 0, false, 100, 0x0800, nullptr, 0,
                                                                        out, sizeof(out))); // payload_len == 0
    TEST_ASSERT_EQUAL_size_t(
        0, pc_eth_build_vlan(mac, mac, 0, false, 100, 0x0800, nullptr, 4, out, sizeof(out))); // len>0, null payload
}
...
void test_eth_parse_null_guards(void)
{
    ...
    EthFrame ef;
    TEST_ASSERT_FALSE(pc_eth_parse(nullptr, sizeof(buf), &ef)); // null frame
    TEST_ASSERT_FALSE(pc_eth_parse(buf, sizeof(buf), nullptr)); // null out
}
