...
{
    TEST_ASSERT_FALSE(dws_lifeline_overdue(1000, 900, 200)); // 100 <= 200
    TEST_ASSERT_FALSE(dws_lifeline_overdue(1000, 800, 200)); // 200, not > 200
    TEST_ASSERT_TRUE(dws_lifeline_overdue(1000, 700, 200));  // 300 > 200
    // Across a millis() rollover: last_feed just before wrap, now just after.
    TEST_ASSERT_FALSE(dws_lifeline_overdue(50, 0xFFFFFF00u, 1000));         // gap 306 <= 1000
    TEST_ASSERT_TRUE(dws_lifeline_overdue(0x00000400u, 0xFFFFFF00u, 1000)); // gap 1284 > 1000
}

void test_register_and_not_overdue_when_fresh(void)
{
    int id = dws_failsafe_register_at("worker", 500, 1000);
    TEST_ASSERT_EQUAL_INT(0, id);
    // Just under the deadline: not overdue, no fire.
    TEST_ASSERT_EQUAL_UINT32(0, dws_failsafe_check_at(1400));
    TEST_ASSERT_EQUAL_INT(0, s_fire_count);
}

void test_breach_fires_once_then_clears_on_feed(void)
{
    int a = dws_failsafe_register_at("a", 500, 1000);
    // b has a huge deadline so it never trips during this test - a stays the only overdue lifeline.
    int b = dws_failsafe_register_at("b", 1000000, 1000);
    TEST_ASSERT_EQUAL_INT(0, a);
    TEST_ASSERT_EQUAL_INT(1, b);

    // Only a is overdue at t=1600 (age 600 > 500).
    uint32_t mask = dws_failsafe_check_at(1600);
    ...
    mask = dws_failsafe_check_at(1700);
    ...
    dws_failsafe_feed_at(a, 1700);
    TEST_ASSERT_EQUAL_UINT32(0, dws_failsafe_check_at(1800));       // fed, healthy
    TEST_ASSERT_EQUAL_UINT32(1u << 0, dws_failsafe_check_at(2300)); // overdue again
...
void test_registry_full(void)
{
    for (int i = 0; i < DWS_FAILSAFE_MAX_LIFELINES; i++)
        TEST_ASSERT_EQUAL_INT(i, dws_failsafe_register_at("x", 100, 0));
    TEST_ASSERT_EQUAL_INT(-1, dws_failsafe_register_at("overflow", 100, 0));
}
...
void test_json(void)
{
    int a = dws_failsafe_register_at("motor", 500, 1000);
    (void)a;
    char buf[128];
...
{
    dws_failsafe_reset();
    int id = dws_failsafe_register("alpha", 1000); // clock-reading wrapper
    TEST_ASSERT_TRUE(id >= 0);
    TEST_ASSERT_TRUE(dws_failsafe_feed(id)); // clock-reading wrapper
    dws_failsafe_check();                    // clock-reading wrapper
    // A second lifeline drives the JSON comma separator.
    dws_failsafe_register("beta", 1000);
    char buf[256];
    TEST_ASSERT_TRUE(dws_failsafe_json_at(0, buf, sizeof(buf)) > 0);
}
