...
void test_peek_reads_type_and_version(void)
{
    ...
    TEST_ASSERT_TRUE(pc_j519_peek(g_buf, 8, nullptr, nullptr));
    TEST_ASSERT_FALSE(pc_j519_peek(g_buf, 7, &type, &ver));
    TEST_ASSERT_FALSE(pc_j519_peek(nullptr, 8, &type, &ver));
}
...
void test_null_guards(void)
{
    ...
    TEST_ASSERT_EQUAL_UINT32(0, (uint32_t)pc_j519_build_start(nullptr, 64, 1));
    TEST_ASSERT_EQUAL_UINT32(0, (uint32_t)pc_j519_build_motion(g_buf, sizeof(g_buf), nullptr));
    TEST_ASSERT_EQUAL_UINT32(0, (uint32_t)pc_j519_build_motion(nullptr, 64, &m));
    TEST_ASSERT_FALSE(pc_j519_parse_motion(nullptr, 64, &got));

    pc_j519_build_motion(g_buf, sizeof(g_buf), &m);
    TEST_ASSERT_FALSE(pc_j519_parse_motion(g_buf, 64, nullptr));
}
...
void test_remaining_null_guard_branches(void)
{
    ...
    TEST_ASSERT_EQUAL_UINT32(0, (uint32_t)pc_j519_build_stop(nullptr, 64, 1));
    TEST_ASSERT_EQUAL_UINT32(0, (uint32_t)pc_j519_build_request(nullptr, 64, &q));
    TEST_ASSERT_EQUAL_UINT32(0, (uint32_t)pc_j519_build_status(nullptr, 200, &s));
    TEST_ASSERT_EQUAL_UINT32(0, (uint32_t)pc_j519_build_ack(nullptr, 200, &a));

    // builders: valid buf but null payload pointer
    TEST_ASSERT_EQUAL_UINT32(0, (uint32_t)pc_j519_build_request(g_buf, sizeof(g_buf), nullptr));
    TEST_ASSERT_EQUAL_UINT32(0, (uint32_t)pc_j519_build_status(g_buf, sizeof(g_buf), nullptr));
    TEST_ASSERT_EQUAL_UINT32(0, (uint32_t)pc_j519_build_ack(g_buf, sizeof(g_buf), nullptr));

    // parsers: null out-param - `!out` short-circuits before hdr_ok touches buf/len at all
    TEST_ASSERT_FALSE(pc_j519_parse_request(g_buf, PC_J519_LEN_REQUEST, nullptr));
    TEST_ASSERT_FALSE(pc_j519_parse_status(g_buf, PC_J519_LEN_STATUS, nullptr));
    TEST_ASSERT_FALSE(pc_j519_parse_ack(g_buf, PC_J519_LEN_ACK, nullptr));
}
