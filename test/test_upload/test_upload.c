...
void setUp()
{
    ...
    pc_sse_init();
    mock_mnt_reset();
    pc_mnt_mount(mock_mnt()); // the upload writes through whatever is mounted
...
{
    pc_upload_begin("/upload", "/dest.bin");
    ...
    TEST_ASSERT_EQUAL_UINT(blen, pc_upload_last_size());
