...
| `native_client` | default | `protocols/transport/test_client` | Outbound TCP client transport (network_drivers/transport/tcp/tcp_client.c), the pooled layer-4 peer of tcp.c used by http_client / mqtt / ws_client / relay / smtp / ssh port-forward (PC_NEED_DET_CLIENT). |
...
| `native_frame` | default | `unit/mmgr/test_frame` | The declarative frame builder (mmgr/frame.h + frame.c): the single engine that turns a static pc_field spec into wire bytes, so the ~160 formatting sites in this library carry a table rather than a ha... |
...
| `native_preempt_queue` | `PC_ENABLE_PREEMPT_QUEUE=1`, `PC_PQ_DEPTH=4`, `PC_PQ_ITEM_SIZE=4` | `unit/session/test_preempt_queue` | Preempting work queue (network_drivers/session/preempt_queue), v5 real-time ingest: the host fixed-ring core - FIFO order, urgent-to-front, fail-closed when full, high-water, and drain/handler dispatch. |
