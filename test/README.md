...
| `native_phy_hot` | `PC_ENABLE_ETHERNET=1`, `PC_ENABLE_IPV6=1`, `PC_ENABLE_RADIO_POWER=1`, `PC_RADIO_WIFI_PS=2` | `unit/physical/test_phy_hot` | Layer 1 driven through a REAL backend: base native_hot_base selects PC_VENDOR_MOCK, so PC_PHYSICAL_HAS_BACKEND is 1 and core_setup/physical/mock stands in for silicon instead of the no-op stubs in phy... |
