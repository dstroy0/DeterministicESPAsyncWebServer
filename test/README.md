...
| `native_arena` | default | `unit/session/test_arena` | Unified double-ended server arena (network_drivers/session/pc_arena): first-fit persistent end (bottom, individual free + coalesce + boundary shrink) + bump scratch end (top, mark/release/reset) shari... |
...
| `native_auth` | `PC_ENABLE_AUTH=1` | `integration/crypto/test_auth` | test_auth against the native_stack_http stack. |
...
| `native_client` | default | `protocols/transport/test_client` | Outbound TCP client transport (network_drivers/transport/tcp/tcp_client.cpp), the pooled layer-4 peer of tcp.cpp used by http_client / mqtt / ws_client / relay / smtp / ssh port-forward (PC_NEED_DET_C... |
...
| `native_coaps` | `PC_ENABLE_DTLS=1`, `PC_ENABLE_COAP=1` | `integration/crypto/test_coaps` | CoAP over DTLS (services/iot/coap/coaps, RFC 7252 sec 9): the bridge that drives a DtlsConn handshake and, once established, unwraps each epoch-3 application record, answers it with coap_server_proces... |
| `native_coaps_server` | `PC_ENABLE_DTLS=1`, `PC_ENABLE_COAP=1` | `integration/crypto/test_coaps_server` | CoAP-over-DTLS server front-end (services/iot/coap/coaps_server): the per-peer DtlsConn pool + ingest/poll seam on top of pc_coaps_process(). |
...
| `native_csrf` | `PC_ENABLE_CSRF=1` | `unit/crypto/test_csrf` | Stateless HMAC-signed CSRF token (services/security/csrf): issue/verify with a fixed secret unit-tests on the host (PC_ENABLE_CSRF set). |
...
| `native_digest_auth` | `PC_ENABLE_AUTH=1` | `integration/crypto/test_digest_auth` | test_digest_auth against the native_stack_http stack. |
| `native_digest_vectors` | `PC_ENABLE_AUTH=1` | `unit/crypto/test_digest_vectors` | test_digest_vectors against the native_stack_http stack. |
...
| `native_dns_resolver` | `PC_ENABLE_DNS_RESOLVER=1` | `unit/network/test_dns_resolver` | DNS resolver answer classifier/verifier (network_drivers/network/dns_resolver): host-tested; the lwIP resolve is ESP32-only. |
| `native_dns_server` | `PC_ENABLE_DNS_SERVER=1` | `protocols/network/test_dns_server` | Authoritative DNS server (services/net/dns_server): the pure A-record response builder (QNAME parse, compressed A answer, NXDOMAIN, non-A query, header flags, malformed guards) and the built-in name->... |
...
| `native_forward` | `PC_ENABLE_FORWARD=1`, `PC_FWD_MAX_IFACES=4`, `PC_FWD_MAX_RULES=4`, `PC_FWD_MAX_ACL=4`, `PC_FWD_MAX_ROUTES=4`, `PC_FWD_INSPECT=1` | `protocols/net/test_forward` | Interface forwarding plane (services/net/forward), v5 bridge / router: default-deny, an ALLOW rule forwards, a DENY wins, multi-destination fan-out, no reflection to the source, the per-rule rate cap ... |
...
| `native_frame` | default | `unit/mmgr/test_frame` | The declarative frame builder (shared_primitives/frame.h + frame.cpp): the single engine that turns a static pc_field spec into wire bytes, so the ~160 formatting sites in this library carry a table r... |
...
| `native_ikev2` | `PC_ENABLE_IKEV2=1` | `unit/crypto/test_ikev2`, `unit/crypto/test_ikev2_natt` | IKEv2 (RFC 7296) message + payload codec (services/security/ikev2): the 28-octet IKE header, the generic payload chain walker, the SA -> proposal -> transform tree (incl. |
...
| `native_jwt` | `PC_ENABLE_JWT=1` | `unit/crypto/test_jwt` | JWT (HS256) bearer-auth verification. |
...
| `native_ntp_server` | `PC_ENABLE_NTP_SERVER=1` | `unit/application/test_ntp_server` | NTP/SNTP server (RFC 5905 server mode) response codec (ntp_server_build_response): version echo, mode/LI/stratum, origin-timestamp copy, reference/receive/transmit stamps, big-endian encoding, and the... |
...
| `native_oidc` | `PC_ENABLE_OIDC=1` | `unit/crypto/test_oidc` | OIDC RS256 ID-token verifier (services/security/oidc). |
...
| `native_plaintext` | default | `unit/mmgr/test_plaintext` | The plaintext pool accessor (server/mmgr/plaintext): bump-allocate + reset semantics, alignment, and fail-closed exhaustion. |
...
| `native_preempt_queue` | `PC_ENABLE_PREEMPT_QUEUE=1`, `PC_PQ_DEPTH=4`, `PC_PQ_ITEM_SIZE=4` | `unit/session/test_preempt_queue` | Preempting work queue (services/system/preempt_queue), v5 real-time ingest: the host fixed-ring core - FIFO order, urgent-to-front, fail-closed when full, high-water, and drain/handler dispatch. |
...
| `native_secure_pool` | default | `unit/mmgr/test_secure_pool` | The secure pool accessor (server/mmgr/secure): the SAME pool mechanism as the plaintext side (server/mmgr/arena) instantiated a second time at a disjoint address, so only what differs is covered here ... |
...
| `native_smbus` | `PC_ENABLE_SMBUS=1` | `unit/smb/test_smbus` | SMBus 3.1 Packet Error Code (services/peripherals/smbus): the address byte with its direction bit, the PEC over a write transaction and over a read transaction (which spans both halves and the repeate... |
...
| `native_span` | default | `unit/mmgr/test_span` | The bounded byte region (server/mmgr/span.h): a pointer and the capacity that belongs to it, bound together. |
...
| `native_udp_transport` | default | `integration/transport/test_udp_transport` | UDP transport multicast receive (network_drivers/transport/udp.cpp): joining an IPv4 multicast group by dotted-quad, rejecting a non-multicast or malformed group, delivering a group datagram to the re... |
