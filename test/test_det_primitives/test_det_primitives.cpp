...
// (shared_primitives/numparse.h - dws_strtol / dws_strtoul / dws_strtof, the
...
void test_strtol()
{
    ...
    TEST_ASSERT_EQUAL_INT(-42, dws_strtol(s, &end)); // leading ws + sign + digits
    TEST_ASSERT_EQUAL_PTR(s + 5, end);               // stopped at 'x'
    TEST_ASSERT_EQUAL_INT(7, dws_strtol("+7", nullptr));

    const char *bad = "abc";
    const char *e2 = nullptr;
    TEST_ASSERT_EQUAL_INT(0, dws_strtol(bad, &e2));
...
void test_strtoul()
{
    const char *s = "  +123abc";
    const char *end = nullptr;
    TEST_ASSERT_EQUAL_UINT32(123, dws_strtoul(s, &end)); // ws + '+' + digits
    TEST_ASSERT_EQUAL_PTR(s + 6, end);

    const char *bad = "  x";
    const char *e2 = nullptr;
    TEST_ASSERT_EQUAL_UINT32(0, dws_strtoul(bad, &e2));
...
{
    const char *end = nullptr;
    TEST_ASSERT_FLOAT_WITHIN(0.0001f, 3.14f, dws_strtof("  3.14", &end)); // ws + int + frac
    TEST_ASSERT_FLOAT_WITHIN(0.0001f, -2.5f, dws_strtof("-2.5", nullptr));
    TEST_ASSERT_FLOAT_WITHIN(1.0f, 1500.0f, dws_strtof("1.5e3", &end));      // exponent
    TEST_ASSERT_FLOAT_WITHIN(0.0001f, 0.0125f, dws_strtof("1.25E-2", &end)); // negative exponent

    const char *bad = "abc";
    const char *e2 = nullptr;
    TEST_ASSERT_FLOAT_WITHIN(0.0001f, 0.0f, dws_strtof(bad, &e2));
    TEST_ASSERT_EQUAL_PTR(bad, e2); // no digits -> end == s
}
