...
static bool cap_uplink(const det_gateway_msg *m, void *)
{
    if (!g_up_accept)
        return false;
    UpMsg u;
...
static bool add_port(uint8_t id, det_gateway_kind kind, uint16_t rate, bool withtx)
{
    det_gateway_port_config c = {};
    c.port_id = id;
    c.kind = kind;
    c.tx = withtx ? cap_tx : nullptr;
    c.rate_cap = rate;
    return det_gateway_add_port(&c);
}

static det_gateway_stats stats()
{
...
{
    TEST_ASSERT_TRUE(add_port(0, det_gateway_kind::DET_GW_LORA, 0, false));
    det_gateway_set_uplink_cb(cap_uplink, nullptr);
    const uint8_t hi[2] = {'h', 'i'};
    TEST_ASSERT_TRUE(det_gateway_uplink(0, 0x42, hi, 2, -50));
    ...
    TEST_ASSERT_EQUAL_UINT8(det_gateway_kind::DET_GW_LORA, g_up[0].kind);
...
{
    add_port(0, det_gateway_kind::DET_GW_LORA, 0, false); // no det_gateway_set_uplink_cb()
    const uint8_t x[1] = {1};
    TEST_ASSERT_FALSE(det_gateway_uplink(0, 1, x, 1, 0));
...
void test_uplink_unknown_port_drops()
{
    det_gateway_set_uplink_cb(cap_uplink, nullptr);
    const uint8_t x[1] = {1};
    TEST_ASSERT_FALSE(det_gateway_uplink(9, 1, x, 1, 0)); // port 9 never registered
    TEST_ASSERT_EQUAL_UINT32(1, stats().up_dropped);
...
{
    add_port(0, det_gateway_kind::DET_GW_NRF24, 2, false); // 2 uplinks / second
    det_gateway_set_uplink_cb(cap_uplink, nullptr);
    const uint8_t x[1] = {7};
    TEST_ASSERT_TRUE(det_gateway_uplink(0, 1, x, 1, 0));
    TEST_ASSERT_TRUE(det_gateway_uplink(0, 1, x, 1, 0));
    TEST_ASSERT_FALSE(det_gateway_uplink(0, 1, x, 1, 0)); // 3rd in the window -> dropped
    TEST_ASSERT_EQUAL_size_t(2, g_up.size());
    TEST_ASSERT_EQUAL_UINT32(1, stats().up_dropped);
    det_gateway_test_set_now(1000); // next window
    TEST_ASSERT_TRUE(det_gateway_uplink(0, 1, x, 1, 0));
...
{
    add_port(0, det_gateway_kind::DET_GW_LORA, 0, false);
    det_gateway_set_uplink_cb(cap_uplink, nullptr);
...
{
    add_port(0, det_gateway_kind::DET_GW_LORA, 0, true); // with a tx callback
    const uint8_t cmd[3] = {'c', 'm', 'd'};
    TEST_ASSERT_TRUE(det_gateway_downlink(0, 0x10, cmd, 3));
...
{
    add_port(0, det_gateway_kind::DET_GW_LORA, 0, false); // receive-only (null tx)
    const uint8_t x[1] = {1};
    TEST_ASSERT_FALSE(det_gateway_downlink(0, 1, x, 1)); // no tx
    TEST_ASSERT_FALSE(det_gateway_downlink(9, 1, x, 1)); // unknown port
    TEST_ASSERT_EQUAL_UINT32(2, stats().down_dropped);
}

void test_downlink_tx_refusal_counted()
{
    add_port(0, det_gateway_kind::DET_GW_LORA, 0, true);
    g_tx_accept = false; // radio refuses
    const uint8_t x[1] = {1};
    TEST_ASSERT_FALSE(det_gateway_downlink(0, 1, x, 1));
...
void test_topic_format()
{
    ...
    m.src_addr = 0x42; // 66
    char buf[32];
    uint16_t n = det_gateway_topic(&m, buf, sizeof(buf));
    TEST_ASSERT_EQUAL_STRING("gw/2/66", buf);
    TEST_ASSERT_EQUAL_UINT16(7, n);

    det_gateway_set_topic_prefix("lora");
    n = det_gateway_topic(&m, buf, sizeof(buf));
    TEST_ASSERT_EQUAL_STRING("lora/2/66", buf);

    char tiny[4];
    TEST_ASSERT_EQUAL_UINT16(0, det_gateway_topic(&m, tiny, sizeof(tiny))); // too small -> 0
}

void test_add_port_validation_and_table_full()
{
    TEST_ASSERT_FALSE(det_gateway_add_port(nullptr));
    TEST_ASSERT_TRUE(add_port(0, det_gateway_kind::DET_GW_LORA, 0, false));
    TEST_ASSERT_FALSE(add_port(0, det_gateway_kind::DET_GW_LORA, 0, false)); // duplicate id
    TEST_ASSERT_TRUE(add_port(1, det_gateway_kind::DET_GW_NRF24, 0, false));
    TEST_ASSERT_TRUE(add_port(2, det_gateway_kind::DET_GW_ZIGBEE, 0, false));
    TEST_ASSERT_TRUE(add_port(3, det_gateway_kind::DET_GW_BLE, 0, false));
    TEST_ASSERT_FALSE(add_port(4, det_gateway_kind::DET_GW_LORA, 0, false)); // table full (DETWS_GW_MAX_PORTS = 4)
}
...
void test_seq_increments_per_uplink()
{
    ...
    det_gateway_set_uplink_cb(cap_uplink, nullptr);
    const uint8_t x[1] = {1};
    det_gateway_uplink(0, 1, x, 1, 0);
    det_gateway_uplink(0, 2, x, 1, 0);
    TEST_ASSERT_EQUAL_size_t(2, g_up.size());
    TEST_ASSERT_EQUAL_UINT32(0, g_up[0].seq);
...
{
    det_gateway_reset();
    det_gateway_set_topic_prefix("gw");
    char buf[64];
    det_gateway_msg m = {};
    m.port_id = 0; // zero -> put_u32 '0' branch
    m.src_addr = 0;
    TEST_ASSERT_TRUE(det_gateway_topic(&m, buf, sizeof(buf)) > 0);
    TEST_ASSERT_EQUAL_UINT16(0, det_gateway_topic(nullptr, buf, sizeof(buf))); // null msg
    TEST_ASSERT_EQUAL_UINT16(0, det_gateway_topic(&m, buf, 0));                // zero buflen
    for (uint16_t cap = 1; cap <= 4; cap++)                                    // fail at successive append steps
        TEST_ASSERT_EQUAL_UINT16(0, det_gateway_topic(&m, buf, cap));
}
