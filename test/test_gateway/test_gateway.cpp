...
//
// The env sizes DWS_GW_MAX_PORTS = 4.
...
static bool cap_uplink(const dws_gateway_msg *m, void *)
{
    if (!g_up_accept)
        return false;
    UpMsg u;
...
static bool add_port(uint8_t id, dws_gateway_kind kind, uint16_t rate, bool withtx)
{
    dws_gateway_port_config c = {};
    c.port_id = id;
    c.kind = kind;
    c.tx = withtx ? cap_tx : nullptr;
    c.rate_cap = rate;
    return dws_gateway_add_port(&c);
}

static dws_gateway_stats stats()
{
    dws_gateway_stats st;
    dws_gateway_get_stats(&st);
    return st;
}
...
{
    TEST_ASSERT_TRUE(add_port(0, dws_gateway_kind::DWS_GW_LORA, 0, false));
    dws_gateway_set_uplink_cb(cap_uplink, nullptr);
    const uint8_t hi[2] = {'h', 'i'};
    TEST_ASSERT_TRUE(dws_gateway_uplink(0, 0x42, hi, 2, -50));
    ...
    TEST_ASSERT_EQUAL_UINT8(dws_gateway_kind::DWS_GW_LORA, g_up[0].kind);
...
{
    add_port(0, dws_gateway_kind::DWS_GW_LORA, 0, false); // no dws_gateway_set_uplink_cb()
    const uint8_t x[1] = {1};
    TEST_ASSERT_FALSE(dws_gateway_uplink(0, 1, x, 1, 0));
...
void test_uplink_unknown_port_drops()
{
    dws_gateway_set_uplink_cb(cap_uplink, nullptr);
    const uint8_t x[1] = {1};
    TEST_ASSERT_FALSE(dws_gateway_uplink(9, 1, x, 1, 0)); // port 9 never registered
    TEST_ASSERT_EQUAL_UINT32(1, stats().up_dropped);
...
{
    add_port(0, dws_gateway_kind::DWS_GW_NRF24, 2, false); // 2 uplinks / second
    dws_gateway_set_uplink_cb(cap_uplink, nullptr);
    const uint8_t x[1] = {7};
    TEST_ASSERT_TRUE(dws_gateway_uplink(0, 1, x, 1, 0));
    TEST_ASSERT_TRUE(dws_gateway_uplink(0, 1, x, 1, 0));
    TEST_ASSERT_FALSE(dws_gateway_uplink(0, 1, x, 1, 0)); // 3rd in the window -> dropped
    TEST_ASSERT_EQUAL_size_t(2, g_up.size());
    TEST_ASSERT_EQUAL_UINT32(1, stats().up_dropped);
    dws_gateway_test_set_now(1000); // next window
    TEST_ASSERT_TRUE(dws_gateway_uplink(0, 1, x, 1, 0));
...
{
    add_port(0, dws_gateway_kind::DWS_GW_LORA, 0, true); // with a tx callback
    const uint8_t cmd[3] = {'c', 'm', 'd'};
    TEST_ASSERT_TRUE(dws_gateway_downlink(0, 0x10, cmd, 3));
...
{
    add_port(0, dws_gateway_kind::DWS_GW_LORA, 0, false); // receive-only (null tx)
    const uint8_t x[1] = {1};
    TEST_ASSERT_FALSE(dws_gateway_downlink(0, 1, x, 1)); // no tx
    TEST_ASSERT_FALSE(dws_gateway_downlink(9, 1, x, 1)); // unknown port
    TEST_ASSERT_EQUAL_UINT32(2, stats().down_dropped);
}

void test_downlink_tx_refusal_counted()
{
    add_port(0, dws_gateway_kind::DWS_GW_LORA, 0, true);
    g_tx_accept = false; // radio refuses
    const uint8_t x[1] = {1};
    TEST_ASSERT_FALSE(dws_gateway_downlink(0, 1, x, 1));
...
void test_topic_format()
{
    ...
    m.src_addr = 0x42; // 66
    char buf[32];
    uint16_t n = dws_gateway_topic(&m, buf, sizeof(buf));
...
{
    TEST_ASSERT_FALSE(dws_gateway_add_port(nullptr));
    TEST_ASSERT_TRUE(add_port(0, dws_gateway_kind::DWS_GW_LORA, 0, false));
    TEST_ASSERT_FALSE(add_port(0, dws_gateway_kind::DWS_GW_LORA, 0, false)); // duplicate id
    TEST_ASSERT_TRUE(add_port(1, dws_gateway_kind::DWS_GW_NRF24, 0, false));
    TEST_ASSERT_TRUE(add_port(2, dws_gateway_kind::DWS_GW_ZIGBEE, 0, false));
    TEST_ASSERT_TRUE(add_port(3, dws_gateway_kind::DWS_GW_BLE, 0, false));
    TEST_ASSERT_FALSE(add_port(4, dws_gateway_kind::DWS_GW_LORA, 0, false)); // table full (DWS_GW_MAX_PORTS = 4)
}
...
{
    dws_gateway_reset();
    dws_gateway_set_topic_prefix("gw");
    char buf[64];
    dws_gateway_msg m = {};
