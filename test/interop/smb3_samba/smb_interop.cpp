...
//
// SMB 3.1.1 interop driver: run the real DWS smb_client engine (services/smb) over a live TCP socket
...
int main()
{
    ...
    cfg.user = env_or("SMB_USER", "dwsuser");
    cfg.pass = env_or("SMB_PASS", "SmbPass123!");
    cfg.domain = env_or("SMB_DOMAIN", "");
    cfg.workstation = "DWSCLIENT";
    cfg.share = env_or("SMB_SHARE", "\\\\127.0.0.1\\dwsshare");
    ...
    const char *payload = "DeterministicESPAsyncWebServer SMB 3.1.1 CMAC interop payload - 0123456789";
    size_t plen = strlen(payload);
    size_t wrote = 0;
    r = smb_write(&h, 0, (const uint8_t *)payload, plen, &wrote, sock_send, sock_recv, &fd);
