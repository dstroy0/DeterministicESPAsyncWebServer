...
A **manual, on-the-wire interop test** for the SMB2/3 client (`src/services/smb`). It drives the real
...
g++ -std=c++14 -O1 -DDWS_ENABLE_SMB=1 -Isrc -Itest/mocks -o /tmp/smb_interop \
...
SMB_HOST=127.0.0.1 SMB_PORT=4455 SMB_USER="$USER" SMB_PASS='SmbPass123!' \
  SMB_SHARE='\\127.0.0.1\dwsshare' SMB_PATH=interop.txt /tmp/smb_interop
```
...
(`dws_ntlm_mic`) makes Samba reject the logon with `STATUS_LOGON_FAILURE` - confirming the server
