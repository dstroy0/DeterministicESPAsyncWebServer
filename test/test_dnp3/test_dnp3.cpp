...
{
    TEST_ASSERT_EQUAL_HEX16(0xEA82, dnp3_crc((const uint8_t *)"123456789", 9));
}
...
void test_build_header_bytes()
{
    ...
    uint8_t buf[32];
    size_t n = dnp3_build_frame(buf, sizeof(buf), 0x44, 0x0004, 0x0001, data, sizeof(data));
...
void test_round_trip_single_block()
{
    ...
    uint8_t buf[32];
    size_t n = dnp3_build_frame(buf, sizeof(buf), 0x44, 0x1234, 0x0A0B, data, sizeof(data));
    TEST_ASSERT_GREATER_THAN(0, (int)n);

    Dnp3Frame f;
    uint8_t user[64];
    size_t user_len;
    TEST_ASSERT_TRUE(dnp3_parse_frame(buf, n, &f, user, sizeof(user), &user_len));
...
void test_round_trip_multi_block()
{
    ...
    for (size_t i = 0; i < sizeof(data); i++)
        data[i] = (uint8_t)(i + 1);
    uint8_t buf[64];
    size_t n = dnp3_build_frame(buf, sizeof(buf), 0x44, 1, 2, data, sizeof(data));
...
{
    uint8_t buf[16];
    size_t n = dnp3_build_frame(buf, sizeof(buf), 0x49, 3, 4, nullptr, 0);
    TEST_ASSERT_EQUAL_size_t(10, n);      // header block only
    TEST_ASSERT_EQUAL_HEX8(0x05, buf[2]); // LEN = 5

    Dnp3Frame f;
    size_t user_len = 999;
    TEST_ASSERT_TRUE(dnp3_parse_frame(buf, n, &f, nullptr, 0, &user_len));
...
void test_parse_rejects_bad()
{
    ...
    corrupt[10] ^= 0xFF;
    TEST_ASSERT_FALSE(dnp3_parse_frame(corrupt, n, &f, user, sizeof(user), &user_len));
    ...
    // Truncated frame.
    TEST_ASSERT_FALSE(dnp3_parse_frame(buf, n - 1, &f, user, sizeof(user), &user_len));

    // out_user too small.
    TEST_ASSERT_FALSE(dnp3_parse_frame(buf, n, &f, user, 2, &user_len));
}
...
void test_build_overflow_fails_closed()
{
    const uint8_t data[] = {'a', 'b', 'c'};
    uint8_t small[8];
    TEST_ASSERT_EQUAL_size_t(0, dnp3_build_frame(small, sizeof(small), 0x44, 1, 2, data, sizeof(data)));
