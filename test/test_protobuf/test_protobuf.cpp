...
void test_zigzag_mapping()
{
    ...
    TEST_ASSERT_EQUAL_INT64(-1, dws_pb_zigzag64(1));
    TEST_ASSERT_EQUAL_INT64(1, dws_pb_zigzag64(2));
    TEST_ASSERT_EQUAL_INT64(-2, dws_pb_zigzag64(3));
    TEST_ASSERT_EQUAL_INT32(-2, dws_pb_zigzag32(3));
...
void test_fixed_and_float_bytes()
{
    ...
    uint8_t buf[32];
    dws_pb_writer_init(&w, buf, sizeof(buf));
    dws_pb_fixed32(&w, 3, 0xDEADBEEF);
    dws_pb_float(&w, 4, 1.5f); // 1.5f bits == 0x3FC00000
    size_t n = dws_pb_writer_finish(&w);
...
void test_round_trip_reader()
{
    ...
    size_t total = dws_pb_writer_finish(&w);
    TEST_ASSERT_GREATER_THAN(0, (int)total);

    size_t pos = 0;
    PbField f;

    TEST_ASSERT_TRUE(dws_pb_read_field(buf, total, &pos, &f));
    ...
    TEST_ASSERT_TRUE(dws_pb_double_bits(f.value) == 2.5);

    TEST_ASSERT_TRUE(dws_pb_read_field(buf, total, &pos, &f));
    TEST_ASSERT_EQUAL_UINT32(5, f.field_number);
    TEST_ASSERT_EQUAL_INT64(-1234567, dws_pb_zigzag64(f.value));

    TEST_ASSERT_FALSE(dws_pb_read_field(buf, total, &pos, &f)); // end of buffer
}
...
void test_varint_and_overflow()
{
    ...
    uint8_t buf[16];
    dws_pb_writer_init(&w, buf, sizeof(buf));
    dws_pb_write_varint(&w, 300); // 0xAC 0x02
    TEST_ASSERT_EQUAL_size_t(2, dws_pb_writer_finish(&w));
    TEST_ASSERT_EQUAL_HEX8(0xAC, buf[0]);
    TEST_ASSERT_EQUAL_HEX8(0x02, buf[1]);
    size_t pos = 0;
    uint64_t v;
    TEST_ASSERT_TRUE(dws_pb_read_varint(buf, 2, &pos, &v));
    TEST_ASSERT_EQUAL_UINT64(300, v);

    // Overflow fails closed.
    PbWriter sw;
    uint8_t small[4];
    dws_pb_writer_init(&sw, small, sizeof(small));
    dws_pb_string(&sw, 1, "this is way too long");
    TEST_ASSERT_EQUAL_size_t(0, dws_pb_writer_finish(&sw));
}
...
void test_malformed_reads()
{
    ...
    TEST_ASSERT_FALSE(dws_pb_read_varint(trunc, sizeof(trunc), &pos, &v));

    PbField f;
    pos = 0;
    const uint8_t bad_len[] = {0x12, 0x05, 'h', 'i'}; // LEN says 5 but only 2 bytes follow
    TEST_ASSERT_FALSE(dws_pb_read_field(bad_len, sizeof(bad_len), &pos, &f));

    pos = 0;
    const uint8_t group[] = {0x0B}; // field 1 wire type 3 (SGROUP) - unsupported
    TEST_ASSERT_FALSE(dws_pb_read_field(group, sizeof(group), &pos, &f));
}
...
void test_varint_width_boundary()
{
    ...
    size_t pos = 0;
    uint64_t v = 0;
    TEST_ASSERT_TRUE(dws_pb_read_varint(max64, sizeof(max64), &pos, &v));
    ...
    pos = 0;
    TEST_ASSERT_FALSE(dws_pb_read_varint(overlong, sizeof(overlong), &pos, &v));
}
...
void test_empty_length_field()
{
    ...
    uint8_t buf[8];
    dws_pb_writer_init(&w, buf, sizeof(buf));
    dws_pb_string(&w, 1, ""); // empty string -> tag + length 0
    size_t total = dws_pb_writer_finish(&w);
...
void test_writer_error_paths()
{
    ...
    dws_pb_writer_init(&w, buf, sizeof(buf));
    TEST_ASSERT_FALSE(dws_pb_write_varint(&w, 0xFFFFFFFFull));
    TEST_ASSERT_TRUE(w.error);

    // fixed32: the tag fits the 2-byte buffer but the 4-byte value does not.
    uint8_t b2[2];
    dws_pb_writer_init(&w, b2, sizeof(b2));
    TEST_ASSERT_FALSE(dws_pb_fixed32(&w, 1, 0x12345678u));
    TEST_ASSERT_TRUE(w.error);

    // bytes: not even the tag fits a zero-length buffer.
    dws_pb_writer_init(&w, buf, 0);
    TEST_ASSERT_FALSE(dws_pb_bytes(&w, 1, (const uint8_t *)"x", 1));

    // string: a null pointer is rejected.
    dws_pb_writer_init(&w, buf, sizeof(buf));
    TEST_ASSERT_FALSE(dws_pb_string(&w, 1, nullptr));
    TEST_ASSERT_TRUE(w.error);
}
...
void test_reader_error_paths()
{
    size_t pos = 0;
    uint64_t v;
    TEST_ASSERT_FALSE(dws_pb_read_varint(nullptr, 4, &pos, &v));

    PbField f;
    uint8_t lone_cont[1] = {0x80}; // tag varint is a lone continuation byte
    pos = 0;
    TEST_ASSERT_FALSE(dws_pb_read_field(lone_cont, 1, &pos, &f));

    uint8_t i64[3] = {0x09, 0x01, 0x02}; // wire type 1 (I64) with < 8 payload bytes
    pos = 0;
    TEST_ASSERT_FALSE(dws_pb_read_field(i64, sizeof(i64), &pos, &f));

    uint8_t i32[2] = {0x0D, 0x01}; // wire type 5 (I32) with < 4 payload bytes
    pos = 0;
    TEST_ASSERT_FALSE(dws_pb_read_field(i32, sizeof(i32), &pos, &f));

    uint8_t len_trunc[2] = {0x0A, 0x80}; // wire type 2 (LEN) with a truncated length varint
    pos = 0;
    TEST_ASSERT_FALSE(dws_pb_read_field(len_trunc, sizeof(len_trunc), &pos, &f));
}
