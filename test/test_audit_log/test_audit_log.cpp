...
void test_append_assigns_monotonic_seq()
{
    ...
    TEST_ASSERT_NULL(dws_audit_at(3));
}
...
void test_ring_evicts_oldest_and_still_verifies()
{
    ...
    TEST_ASSERT_EQUAL_UINT32((uint32_t)(DWS_AUDIT_LOG_ENTRIES + extra),
                             dws_audit_at((uint16_t)(dws_audit_count() - 1))->seq);
    // The moving anchor keeps the retained window a complete, verifiable chain.
    TEST_ASSERT_TRUE(dws_audit_verify(nullptr));
}
...
void test_format_and_dump_json()
{
    ...
    char doc[512];
    int dn = dws_audit_dump_json(doc, sizeof(doc));
...
// dws_audit_format fails closed for a null arg and for every buffer size below
...
void test_format_fails_closed_all_stages()
{
    ...
    TEST_ASSERT_EQUAL_INT(0, dws_audit_format(e, full, 0));
    TEST_ASSERT_EQUAL_INT(0, dws_audit_format(nullptr, full, sizeof(full)));
    for (int cap = 1; cap < flen; cap++)
    {
        char small[256];
        TEST_ASSERT_EQUAL_INT(0, dws_audit_format(e, small, (size_t)cap));
    }
}

// dws_audit_dump_json fails closed for a null arg and every under-sized buffer
...
void test_dump_fails_closed_all_stages()
{
    ...
    TEST_ASSERT_TRUE(flen > 0);
    TEST_ASSERT_EQUAL_INT(0, dws_audit_dump_json(full, 0));
    TEST_ASSERT_EQUAL_INT(0, dws_audit_dump_json(nullptr, sizeof(full)));
    for (int cap = 1; cap < flen; cap++)
    {
        char small[1024];
        TEST_ASSERT_EQUAL_INT(0, dws_audit_dump_json(small, (size_t)cap));
    }
}
