...
void test_bcc_is_xor()
{
    ...
    TEST_ASSERT_EQUAL_HEX8(0x11 ^ 0x22 ^ 0x33, pc_3964r_bcc(d, sizeof(d)));
    // a doubled DLE pair cancels in the XOR (0x10 ^ 0x10 = 0)
    const uint8_t pair[] = {SIMATIC_DLE, SIMATIC_DLE};
    TEST_ASSERT_EQUAL_HEX8(0, pc_3964r_bcc(pair, sizeof(pair)));
}
...
void test_build_block_stuffs_dle_and_terminates()
{
    ...
    uint8_t buf[32];
    size_t n = pc_3964r_build_block(buf, sizeof(buf), data, sizeof(data), true);
    ...
    TEST_ASSERT_EQUAL_HEX8(pc_3964r_bcc(buf, 6), buf[6]); // BCC over stuffed data + DLE ETX
}
...
void test_block_round_trip_with_embedded_dle()
{
    ...
    uint8_t blk[64];
    size_t n = pc_3964r_build_block(blk, sizeof(blk), data, sizeof(data), true);
    TEST_ASSERT_GREATER_THAN_size_t(0, n);
    uint8_t out[64];
    size_t olen = 0;
    TEST_ASSERT_TRUE(pc_3964r_parse_block(blk, n, true, out, sizeof(out), &olen));
...
void test_block_round_trip_no_bcc()
{
    ...
    uint8_t blk[32];
    size_t n = pc_3964r_build_block(blk, sizeof(blk), data, sizeof(data), false);
    uint8_t out[32];
    size_t olen = 0;
    TEST_ASSERT_TRUE(pc_3964r_parse_block(blk, n, false, out, sizeof(out), &olen));
    TEST_ASSERT_EQUAL_HEX8_ARRAY(data, out, olen);
}
...
void test_parse_rejects_bad()
{
    ...
    TEST_ASSERT_FALSE(pc_3964r_parse_block(blk, n, true, out, sizeof(out), &olen));
    // missing DLE ETX terminator
    const uint8_t noterm[] = {0x41, 0x42, 0x43};
    TEST_ASSERT_FALSE(pc_3964r_parse_block(noterm, sizeof(noterm), false, out, sizeof(out), &olen));
    // dangling DLE (truncated)
    const uint8_t dangle[] = {0x41, SIMATIC_DLE};
    TEST_ASSERT_FALSE(pc_3964r_parse_block(dangle, sizeof(dangle), false, out, sizeof(out), &olen));
    // DLE + illegal control byte
    const uint8_t badctl[] = {SIMATIC_DLE, 0x55, SIMATIC_DLE, SIMATIC_ETX};
    TEST_ASSERT_FALSE(pc_3964r_parse_block(badctl, sizeof(badctl), false, out, sizeof(out), &olen));
    // out-buffer overflow (fail-closed)
    const uint8_t big[] = {1, 2, 3, 4, 5};
    uint8_t bigblk[32];
    size_t bn = pc_3964r_build_block(bigblk, sizeof(bigblk), big, sizeof(big), false);
    uint8_t tiny[2];
    TEST_ASSERT_FALSE(pc_3964r_parse_block(bigblk, bn, false, tiny, sizeof(tiny), &olen));
}
...
void test_build_block_rejects_bad_args()
{
    ...
    TEST_ASSERT_EQUAL_size_t(0, pc_3964r_build_block(nullptr, sizeof(buf), d, 1, false));
    TEST_ASSERT_EQUAL_size_t(0, pc_3964r_build_block(buf, sizeof(buf), nullptr, 1, false));
    size_t n = pc_3964r_build_block(buf, sizeof(buf), nullptr, 0, false);
...
void test_build_block_overflow_at_each_stage()
{
    ...
    TEST_ASSERT_EQUAL_size_t(0, pc_3964r_build_block(buf, 0, d, 1, false));   // no room at all
    TEST_ASSERT_EQUAL_size_t(0, pc_3964r_build_block(buf, 1, dle, 1, false)); // room for DLE, not its double
    TEST_ASSERT_EQUAL_size_t(0, pc_3964r_build_block(buf, 2, d, 1, false));   // no room for DLE ETX
    TEST_ASSERT_EQUAL_size_t(0, pc_3964r_build_block(buf, 3, d, 1, true));    // no room for the BCC
    TEST_ASSERT_EQUAL_size_t(3, pc_3964r_build_block(buf, 3, d, 1, false));   // same block fits without one
}
...
void test_parse_block_rejects_null_args()
{
    ...
    TEST_ASSERT_FALSE(pc_3964r_parse_block(nullptr, sizeof(blk), false, out, sizeof(out), &olen));
    TEST_ASSERT_FALSE(pc_3964r_parse_block(blk, sizeof(blk), false, nullptr, sizeof(out), &olen));
    TEST_ASSERT_FALSE(pc_3964r_parse_block(blk, sizeof(blk), false, out, sizeof(out), nullptr));
    TEST_ASSERT_TRUE(pc_3964r_parse_block(blk, sizeof(blk), false, out, sizeof(out), &olen));
...
void test_parse_block_missing_bcc_and_doubled_dle_overflow()
{
    ...
    TEST_ASSERT_FALSE(pc_3964r_parse_block(nobcc, sizeof(nobcc), true, out, sizeof(out), &olen));
    ...
    TEST_ASSERT_FALSE(pc_3964r_parse_block(doubled, sizeof(doubled), false, out, 0, &olen));
}
...
{
    Simatic3964Ctx c;
    pc_3964r_init(&c, true, true, cap_tx, cap_rx, nullptr);
...
void test_sm_receive_path_delivers()
{
    ...
    uint8_t blk[32];
    size_t n = pc_3964r_build_block(blk, sizeof(blk), payload, sizeof(payload), true);
...
{
    Simatic3964Ctx c;
    pc_3964r_init(&c, true, false, cap_tx, cap_rx, nullptr);
...
void test_sm_qvz_timeout_then_abort()
{
    ...
    pc_3964r_init(&c, true, false, cap_tx, cap_rx, nullptr);
    const uint8_t msg[] = {0x01};
    pc_3964r_send(&c, msg, sizeof(msg), 0); // STX at t=0, deadline t=QVZ
    // never ack; tick past the deadline repeatedly -> connection retries then abort
    uint32_t t = PC_SIMATIC_QVZ_MS;
    for (int i = 0; i < SIMATIC_MAX_CONN_RETRY + 1; i++)
    {
        pc_3964r_tick(&c, t);
        t += PC_SIMATIC_QVZ_MS;
    }
    TEST_ASSERT_TRUE(pc_3964r_idle(&c)); // gave up after MAX retries
}
...
void test_sm_priority_arbitration()
{
    ...
    Simatic3964Ctx lo;
    pc_3964r_init(&lo, false, true, cap_tx, cap_rx, nullptr);
    const uint8_t msg[] = {0x01};
    ...
    Simatic3964Ctx hi;
    pc_3964r_init(&hi, true, true, cap_tx, cap_rx, nullptr);
...
static void rx_then_reply(void *user, const uint8_t *d, size_t n)
{
    ...
    if (g_reply_ctx)
    {
        ...
        pc_3964r_send(g_reply_ctx, reply, sizeof(reply), 100);
    }
}
...
{
    Simatic3964Ctx c;
    pc_3964r_init(&c, true, true, cap_tx, rx_then_reply, nullptr);
    g_reply_ctx = &c;
...
void test_sm_send_rejects_when_busy_or_unframeable()
{
    ...
    const uint8_t msg[] = {0x01};
    TEST_ASSERT_TRUE(pc_3964r_send(&c, msg, sizeof(msg), 0));
    TEST_ASSERT_FALSE(pc_3964r_send(&c, msg, sizeof(msg), 0));

    Simatic3964Ctx d;
    pc_3964r_init(&d, true, true, cap_tx, cap_rx, nullptr);
    static uint8_t big[PC_SIMATIC_BLOCK_MAX];
    memset(big, 0x41, sizeof(big)); // no room left for DLE ETX BCC
    TEST_ASSERT_FALSE(pc_3964r_send(&d, big, sizeof(big), 0));
    TEST_ASSERT_TRUE(pc_3964r_idle(&d));
}
...
void test_sm_null_callbacks_are_safe()
{
    ...
    Simatic3964Ctx c;
    pc_3964r_init(&c, true, true, nullptr, nullptr, nullptr);
...
void test_sm_receive_no_bcc_variant_delivers()
{
    ...
    uint8_t blk[16];
    size_t n = pc_3964r_build_block(blk, sizeof(blk), payload, sizeof(payload), false);
    pc_3964r_rx_byte(&c, SIMATIC_STX, 0);
    for (size_t i = 0; i < n; i++)
    {
        pc_3964r_rx_byte(&c, blk[i], (uint32_t)(1 + i));
...
void test_sm_receive_overflow_naks()
{
    ...
    pc_3964r_init(&c, true, true, cap_tx, cap_rx, nullptr);
    pc_3964r_rx_byte(&c, SIMATIC_STX, 0);
    for (size_t i = 0; i < PC_SIMATIC_BLOCK_MAX; i++)
    {
        pc_3964r_rx_byte(&c, 0x41, (uint32_t)(1 + i));
    }
    TEST_ASSERT_FALSE(pc_3964r_idle(&c)); // exactly full, still collecting
    pc_3964r_rx_byte(&c, 0x41, 1000);     // one byte too many
    TEST_ASSERT_EQUAL_HEX8(SIMATIC_NAK, g_tx.back());
    TEST_ASSERT_TRUE(pc_3964r_idle(&c));
}
...
void test_sm_conn_nak_retries_then_gives_up()
{
    ...
    {
        pc_3964r_rx_byte(&c, SIMATIC_NAK, (uint32_t)i);
        TEST_ASSERT_EQUAL_HEX8(SIMATIC_STX, g_tx.back());
        TEST_ASSERT_FALSE(pc_3964r_idle(&c));
    }
    pc_3964r_rx_byte(&c, SIMATIC_NAK, 99);
    TEST_ASSERT_TRUE(pc_3964r_idle(&c));
}
...
void test_sm_await_end_ignores_noise_then_gives_up()
{
    ...
    {
        pc_3964r_rx_byte(&c, SIMATIC_NAK, 3); // reject -> repeat from STX
        TEST_ASSERT_FALSE(pc_3964r_idle(&c));
        pc_3964r_rx_byte(&c, SIMATIC_DLE, 4); // partner connects again
    }
    pc_3964r_rx_byte(&c, SIMATIC_NAK, 5);
    TEST_ASSERT_TRUE(pc_3964r_idle(&c));
}
...
void test_sm_tick_block_timeout_retries_then_gives_up()
{
    ...
    for (int i = 0; i < SIMATIC_MAX_BLOCK_RETRY - 1; i++)
    {
        pc_3964r_rx_byte(&c, SIMATIC_DLE, t); // connect -> awaiting the end DLE
        t += PC_SIMATIC_QVZ_MS;
        pc_3964r_tick(&c, t);
        TEST_ASSERT_EQUAL_HEX8(SIMATIC_STX, g_tx.back());
        TEST_ASSERT_FALSE(pc_3964r_idle(&c));
    }
    pc_3964r_rx_byte(&c, SIMATIC_DLE, t);
    t += PC_SIMATIC_QVZ_MS;
    pc_3964r_tick(&c, t);
    TEST_ASSERT_TRUE(pc_3964r_idle(&c));
}
...
void test_sm_tick_zvz_aborts_receive()
{
    ...
    pc_3964r_init(&c, true, true, cap_tx, cap_rx, nullptr);
    pc_3964r_rx_byte(&c, SIMATIC_STX, 0);
    pc_3964r_rx_byte(&c, 0x41, 1);
    pc_3964r_tick(&c, 1 + PC_SIMATIC_ZVZ_MS);
    TEST_ASSERT_EQUAL_HEX8(SIMATIC_NAK, g_tx.back());
    TEST_ASSERT_TRUE(pc_3964r_idle(&c));
}
...
void test_rk512_build_send_field_order()
{
    ...
    uint8_t buf[32];
    size_t n = pc_rk512_build_send(buf, sizeof(buf), Rk512Area::DB, 5, 0x0010, words, 2);
...
{
    uint8_t buf[16];
    size_t n = pc_rk512_build_fetch(buf, sizeof(buf), Rk512Area::MB, 0, 0x0100, 4);
...
{
    uint8_t buf[8];
    size_t n = pc_rk512_build_reaction(buf, sizeof(buf), 0x0000);
    uint16_t status = 0xFFFF;
    const uint8_t *data = (const uint8_t *)1;
    size_t dlen = 99;
    TEST_ASSERT_TRUE(pc_rk512_parse_reaction(buf, n, &status, &data, &dlen));
    TEST_ASSERT_EQUAL_UINT16(0, status);
    TEST_ASSERT_NULL(data);
    TEST_ASSERT_EQUAL_size_t(0, dlen);
    // a non-zero error status
    pc_rk512_build_reaction(buf, sizeof(buf), 0x8001);
    pc_rk512_parse_reaction(buf, 3, &status, nullptr, nullptr);
...
void test_rk512_parse_rejects()
{
    ...
    TEST_ASSERT_FALSE(pc_rk512_parse_header(shortbuf, sizeof(shortbuf), &h)); // too short
    const uint8_t badarea[] = {0x00, 0x00, 0xEE, 0x00, 0x00, 0x00, 0x00, 0x01};
    TEST_ASSERT_FALSE(pc_rk512_parse_header(badarea, sizeof(badarea), &h)); // invalid area
    // overflow guards
    uint8_t tiny[4];
    const uint16_t w[] = {1};
    TEST_ASSERT_EQUAL_size_t(0, pc_rk512_build_send(tiny, sizeof(tiny), Rk512Area::DB, 0, 0, w, 1));
}
...
void test_rk512_build_guards()
{
    ...
    TEST_ASSERT_EQUAL_size_t(0, pc_rk512_build_send(nullptr, sizeof(buf), Rk512Area::DB, 0, 0, w, 1));
    TEST_ASSERT_EQUAL_size_t(0, pc_rk512_build_send(buf, sizeof(buf), Rk512Area::DB, 0, 0, nullptr, 1));
    // a null word array with count 0 is the legal header-only SEND
    TEST_ASSERT_EQUAL_size_t(8, pc_rk512_build_send(buf, sizeof(buf), Rk512Area::DB, 0, 0, nullptr, 0));
    TEST_ASSERT_EQUAL_size_t(0, pc_rk512_build_fetch(nullptr, sizeof(buf), Rk512Area::DB, 0, 0, 1));
    TEST_ASSERT_EQUAL_size_t(0, pc_rk512_build_fetch(buf, 7, Rk512Area::DB, 0, 0, 1)); // < header
    TEST_ASSERT_EQUAL_size_t(0, pc_rk512_build_reaction(nullptr, sizeof(buf), 0));
...
void test_rk512_parse_header_guards()
{
    ...
    TEST_ASSERT_FALSE(pc_rk512_parse_header(nullptr, sizeof(ok), &h));
    TEST_ASSERT_FALSE(pc_rk512_parse_header(ok, sizeof(ok), nullptr));
    uint8_t lowarea[8];
...
void test_rk512_parse_reaction_guards_and_data()
{
    ...
    uint8_t buf[16];
    uint16_t status = 0;
    pc_rk512_build_reaction(buf, sizeof(buf), 0x0102);
    TEST_ASSERT_FALSE(pc_rk512_parse_reaction(nullptr, 3, &status, nullptr, nullptr));
    TEST_ASSERT_FALSE(pc_rk512_parse_reaction(buf, 3, nullptr, nullptr, nullptr));
    TEST_ASSERT_FALSE(pc_rk512_parse_reaction(buf, 2, &status, nullptr, nullptr));
    const uint8_t notreaction[3] = {(uint8_t)Rk512Cmd::FETCH, 0x00, 0x00};
    TEST_ASSERT_FALSE(pc_rk512_parse_reaction(notreaction, sizeof(notreaction), &status, nullptr, nullptr));
    buf[3] = 0x12; // two data words appended by a FETCH responder
    buf[4] = 0x34;
    const uint8_t *data = nullptr;
    size_t dlen = 0;
    TEST_ASSERT_TRUE(pc_rk512_parse_reaction(buf, 5, &status, &data, &dlen));
