...
{
    TEST_ASSERT_EQUAL_INT(SB_OK, pc_southbound_register(&g_full));
    ...
    TEST_ASSERT_EQUAL_INT(SB_ERR_DUP, pc_southbound_register(&g_full));
    // Bad args.
    TEST_ASSERT_EQUAL_INT(SB_ERR_ARG, pc_southbound_register(NULL));
    SouthboundDriver noname = {NULL, fake_read, NULL, NULL, NULL, NULL};
    TEST_ASSERT_EQUAL_INT(SB_ERR_ARG, pc_southbound_register(&noname));
}
...
void test_read_write_dispatch(void)
{
    ...
    int32_t v = -1;
    TEST_ASSERT_EQUAL_INT(SB_OK, pc_southbound_read("fake", 3, &v));
    TEST_ASSERT_EQUAL_INT32(30, v);
    TEST_ASSERT_EQUAL_INT(SB_OK, pc_southbound_write("fake", 3, 999));
    TEST_ASSERT_EQUAL_INT(SB_OK, pc_southbound_read("fake", 3, &v));
    TEST_ASSERT_EQUAL_INT32(999, v);
    // Unknown driver.
    TEST_ASSERT_EQUAL_INT(SB_ERR_NOT_FOUND, pc_southbound_read("x", 0, &v));
    TEST_ASSERT_EQUAL_INT(SB_ERR_NOT_FOUND, pc_southbound_write("x", 0, 0));
    // Null out.
    TEST_ASSERT_EQUAL_INT(SB_ERR_ARG, pc_southbound_read("fake", 0, NULL));
...
void test_block_atomic(void)
{
    ...
    TEST_ASSERT_EQUAL_INT(SB_ERR_ARG, pc_southbound_read_block("fake", 0, out, 0));
    TEST_ASSERT_EQUAL_INT(SB_ERR_ARG, pc_southbound_write_block("fake", 0, NULL, 3));
}
...
void test_unsupported_capability(void)
{
    ...
    int32_t v = 0, out[2] = {0};
    TEST_ASSERT_EQUAL_INT(SB_OK, pc_southbound_read("ro", 0, &v));
    TEST_ASSERT_EQUAL_INT(SB_ERR_UNSUPPORTED, pc_southbound_write("ro", 0, 1));
    TEST_ASSERT_EQUAL_INT(SB_ERR_UNSUPPORTED, pc_southbound_read_block("ro", 0, out, 2));
    TEST_ASSERT_EQUAL_INT(SB_ERR_UNSUPPORTED, pc_southbound_write_block("ro", 0, out, 2));
}
...
void test_registry_full(void)
{
    ...
    for (int i = 0; i < 9; i++)
    {
        ...
        {
            TEST_ASSERT_EQUAL_INT(SB_ERR_FULL, pc_southbound_register(&drv[i]));
        }
    }
    // Default cap is 8.
...
void test_dispatch_not_found_guards()
{
    ...
    int32_t v = 0;
    TEST_ASSERT_EQUAL_INT(SB_ERR_NOT_FOUND, pc_southbound_read("nope", 0, &v));  // no driver
    TEST_ASSERT_EQUAL_INT(SB_ERR_NOT_FOUND, pc_southbound_write("nope", 0, 42)); // no driver
}

void test_find_null_name(void)
{
    // pc_southbound_find's own null-name guard, independent of any dispatch caller.
    TEST_ASSERT_NULL(pc_southbound_find(NULL));
}
...
void test_read_missing_capability(void)
{
    ...
    TEST_ASSERT_EQUAL_INT(SB_OK, pc_southbound_register(&wo));
    int32_t v = 0;
    TEST_ASSERT_EQUAL_INT(SB_ERR_UNSUPPORTED, pc_southbound_read("wo", 0, &v));
}
...
void test_find_skips_driver_mutated_name_null(void)
{
    ...
    TEST_ASSERT_EQUAL_INT(SB_OK, pc_southbound_register(&mutable_drv));
...
void test_block_not_found_and_arg_edges(void)
{
    ...
    TEST_ASSERT_EQUAL_INT(SB_ERR_NOT_FOUND, pc_southbound_read_block("nope", 0, out, 2));
    TEST_ASSERT_EQUAL_INT(SB_ERR_NOT_FOUND, pc_southbound_write_block("nope", 0, in, 2));
    // Null out pointer (read_block) and zero count with a valid pointer (write_block).
    TEST_ASSERT_EQUAL_INT(SB_ERR_ARG, pc_southbound_read_block("fake", 0, NULL, 2));
    TEST_ASSERT_EQUAL_INT(SB_ERR_ARG, pc_southbound_write_block("fake", 0, in, 0));
}
