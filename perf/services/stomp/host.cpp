...
//
// Host-side microbenchmark for the STOMP 1.2 frame codec: dws_stomp_build_frame (the device emits a SEND) and
// dws_stomp_parse_frame (decode one inbound broker frame - command + headers + content-length body, the
// untrusted-input hot op). Both pure (no sockets, no heap), so they link standalone. The device figure comes
// from the rig /bench dws_stomp_parse_frame op; this host ns/op + MB/s is a RELATIVE baseline. Build + run:
//   g++ -O2 -std=c++17 -Isrc -Itest/mocks -Itest/support -DDWS_ENABLE_STOMP=1 perf/services/stomp/host.cpp \
//       src/services/stomp/stomp.cpp -o /tmp/bstomp && /tmp/bstomp

#define DWS_ENABLE_STOMP 1
...
int main()
{
    ...
    size_t flen = dws_stomp_build_frame(frame, sizeof(frame), "SEND", bk, bv, 2, body, blen);
    ...
    // build a SEND frame (command + 2 escaped headers + body + NUL).
    {
        ...
        double ns = bench_ns(
            2000000, [&] { sink += dws_stomp_build_frame(frame, sizeof(frame), "SEND", bk, bv, 2, body, blen); });
    ...
    // parse an inbound MESSAGE frame (command + 4 headers + content-length body).
    {
        ...
        double ns = bench_ns(2000000, [&] {
            StompFrame f;
            size_t used = 0;
            sink += dws_stomp_parse_frame(msg, mlen, &f, &used) ? (int)used : 0;
        });
