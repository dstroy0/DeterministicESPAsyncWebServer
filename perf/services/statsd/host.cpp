...
//
// Host-side microbenchmark for the StatsD metrics client: dws_statsd_format() builds one `name:value|type
...
// dws_statsd_format op; this host ns/op + MB/s is a RELATIVE baseline:
//   g++ -O2 -std=c++17 -Isrc -Itest/mocks -Itest/support -DDWS_ENABLE_STATSD=1 perf/services/statsd/host.cpp \
...
{
    char out[256];
    size_t len = dws_statsd_format(out, sizeof(out), "api.requests", "1", StatsdType::STATSD_COUNTER, 0.1f,
    ...
    // format one sampled counter line with tags (the per-metric cost before the UDP send).
    {
        ...
        row("statsd", "dws_statsd_format (counter)", ns, (double)len);
        (void)sink;
    }
