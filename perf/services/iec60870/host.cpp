...
//   g++ -O2 -std=c++17 -Isrc -Itest/mocks -Itest/support -DDWS_ENABLE_IEC60870=1 perf/services/iec60870/host.cpp \
//       src/services/iec60870/iec60870.cpp -o /tmp/bi && /tmp/bi

#define DWS_ENABLE_IEC60870 1
...
int main()
{
    ...
    ah.common_addr = 1;
    uint8_t asdu[32];
    size_t al = dws_iec_asdu_build_header(asdu, sizeof(asdu), &ah);
    al += dws_iec_put_ioa(asdu + al, sizeof(asdu) - al, 100);
    asdu[al++] = 0x12; // normalized value LSB
    asdu[al++] = 0x34; // normalized value MSB
    asdu[al++] = 0x00; // quality descriptor

    uint8_t frame[64];
    size_t frame_len = dws_iec104_build_i(frame, sizeof(frame), 0, 0, asdu, al);

    printf("| Feature   | Operation                |     ns/op |    MB/s |\n");
    printf("|-----------|--------------------------|-----------|---------|\n");

    // dws_iec104_build_i: frame an ASDU in an I-format APCI (start + length + Ns/Nr) - the transmit op.
    {
        uint8_t buf[64];
        volatile size_t sink = 0;
        double ns = bench_ns(5000000, [&] { sink += dws_iec104_build_i(buf, sizeof(buf), 0, 0, asdu, al); });
        row("iec60870", "build_i (104 I-frame)", ns, (double)frame_len);
        (void)sink;
    }

    // dws_iec104_parse: validate the APCI start/length + decode the I/S/U format and slice the ASDU - receive op.
    {
        volatile size_t sink = 0;
        double ns = bench_ns(10000000, [&] {
            Iec104Apci a;
            size_t used = 0;
            if (dws_iec104_parse(frame, frame_len, &a, &used))
                sink += a.asdu_len;
        });
    ...
    // dws_iec_asdu_parse_header: decode the 6-octet ASDU header (type / SQ / count / COT / common address).
    {
        volatile size_t sink = 0;
        double ns = bench_ns(10000000, [&] {
            IecAsduHeader h;
            size_t used = 0;
            if (dws_iec_asdu_parse_header(asdu, al, &h, &used))
                sink += used;
        });
