...
//   g++ -O2 -std=c++17 -Isrc -Itest/mocks -Itest/support -DDWS_ENABLE_DNP3=1 perf/services/dnp3/host.cpp \
//       src/services/dnp3/dnp3.cpp -o /tmp/bd && /tmp/bd

#define DWS_ENABLE_DNP3 1
...
int main()
{
    ...
    uint8_t frame[512];
    size_t frame_len = dws_dnp3_build_frame(frame, sizeof(frame), 0x44, 0x0001, 0x0002, user, sizeof(user));
    ...
    // CRC-16/DNP over one 16-octet block - the per-block inner op (a frame CRCs the header + every block).
    {
        ...
        double ns = bench_ns(10000000, [&] { sink ^= dws_dnp3_crc(block, sizeof(block)); });
    ...
    // Build a full data-link frame (header block + CRC'd data blocks) from 32 octets of user data.
    {
        ...
        double ns =
            bench_ns(2000000, [&] { sink += dws_dnp3_build_frame(frame, sizeof(frame), 0x44, 1, 2, user, 32); });
    ...
    // Parse + CRC-validate the frame, de-blocking the user data (every block CRC checked) - the receive op.
    {
        ...
        double ns = bench_ns(2000000, [&] {
            ...
            uint8_t out[256];
            size_t ul = 0;
            if (dws_dnp3_parse_frame(frame, frame_len, &f, out, sizeof(out), &ul))
                sink += ul;
        });
