...
// baseline. Build + run:
//   g++ -O2 -std=c++17 -Isrc -Itest/mocks -Itest/support -DDWS_ENABLE_ENIP=1 perf/services/enip/host.cpp \
//       src/services/enip/enip.cpp -o /tmp/be && /tmp/be

#define DWS_ENABLE_ENIP 1
...
int main()
{
    ...
    h.options = 0;
    uint8_t frame[64];
    size_t frame_len = dws_eip_build(frame, sizeof(frame), &h, cip, sizeof(cip));

    uint8_t rs[64];
    const uint8_t ctx[8] = {1, 2, 3, 4, 5, 6, 7, 8};
    size_t rs_len = dws_eip_build_register_session(rs, sizeof(rs), ctx);

    printf("| Feature | Operation                  |     ns/op |    MB/s |\n");
    printf("|---------|----------------------------|-----------|---------|\n");

    // dws_eip_build: lay down the 24-octet encapsulation header + command data (the transmit op).
    {
        uint8_t buf[64];
        volatile size_t sink = 0;
        double ns = bench_ns(5000000, [&] { sink += dws_eip_build(buf, sizeof(buf), &h, cip, sizeof(cip)); });
        row("enip", "dws_eip_build (encap)", ns, (double)frame_len);
        (void)sink;
    }

    // dws_eip_parse: validate the encapsulation header (command/length/status) + slice the command data (receive
    // op; this is the parser the fuzz attack targets - a length lie must not over-read past the 24-octet header).
    {
        ...
        double ns = bench_ns(10000000, [&] {
            ...
            size_t dlen = 0;
            if (dws_eip_parse(frame, frame_len, &out, &data, &dlen))
                sink += dlen + out.command;
        });
        row("enip", "dws_eip_parse (encap)", ns, (double)frame_len);
        (void)sink;
    }

    // dws_eip_build_register_session: the session-open handshake frame (fixed 28 octets).
    {
        uint8_t buf[64];
        volatile size_t sink = 0;
        double ns = bench_ns(5000000, [&] { sink += dws_eip_build_register_session(buf, sizeof(buf), ctx); });
