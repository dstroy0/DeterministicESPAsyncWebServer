...
// Hello/Acknowledge handshake (dws_opcua_parse_hello + dws_opcua_build_ack, run on every new connection) and the
// per-node DataValue Variant encode (dws_ua_w_datavalue, the Read-service hot op). All pure (no sockets, no
...
//   g++ -O2 -std=c++17 -Isrc -Itest/mocks -Itest/support -DDWS_ENABLE_OPCUA=1 perf/services/opcua/host.cpp \
...
int main()
{
    ...
    {
        volatile int sink = 0;
        double ns = bench_ns(2000000, [&] { sink += dws_opcua_parse_hello(hel, heln, &hello) ? 1 : 0; });
        row("opcua", "parse HELLO", ns, (double)heln);
        (void)sink;
    }
    // Build ACK: negotiate the buffer sizes down to the server limit and emit the reply.
    dws_opcua_parse_hello(hel, heln, &hello);
    {
        volatile size_t sink = 0;
        double ns = bench_ns(2000000, [&] { sink += dws_opcua_build_ack(&hello, ack, sizeof(ack)); });
        size_t n = dws_opcua_build_ack(&hello, ack, sizeof(ack));
