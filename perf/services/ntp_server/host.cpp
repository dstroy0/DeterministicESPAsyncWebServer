...
//
// Host-side microbenchmark for the NTP server (RFC 5905 server mode): dws_ntp_server_build_response() parses a
...
//   g++ -O2 -std=c++17 -Isrc -Itest/mocks -Itest/support -DDWS_ENABLE_NTP_SERVER=1 perf/services/ntp_server/host.cpp \
...
int main()
{
    ...
    // build one mode-4 server reply per client request (the whole per-query server cost).
    {
        ...
        double ns = bench_ns(5000000, [&] {
            sink += dws_ntp_server_build_response(req, sizeof(req), 2, NTP_REFID_LOCL, 0xE9A1B2C3u, 0x80000000u, out,
                                                  sizeof(out));
        });
