...
// dws_modbus_process_adu() takes a complete ADU (MBAP header + PDU) and produces the response ADU - parse
...
//   g++ -O2 -std=c++17 -Isrc -Itest/mocks -Itest/support -DDWS_ENABLE_MODBUS=1 perf/services/modbus/host.cpp \
...
int main()
{
...

    {
        ...
        double ns = bench_ns(1000000, [&] { sink += dws_modbus_process_adu(rd8, sizeof(rd8), resp, sizeof(resp)); });
        row("modbus", "read holding x8 (FC3)", ns, (double)sizeof(rd8));
        (void)sink;
    }
    {
        volatile size_t sink = 0;
        double ns = bench_ns(1000000, [&] { sink += dws_modbus_process_adu(wr2, sizeof(wr2), resp, sizeof(resp)); });
