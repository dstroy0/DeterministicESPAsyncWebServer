...
//   g++ -O2 -std=c++17 -Isrc -DDWS_ENABLE_WAL=1 -DDWS_ENABLE_DBM=1 -DDWS_ENABLE_DOCSTORE=1 \
//       -DDWS_ENABLE_SQLITE=1 -DDWS_ENABLE_REDIS=1 perf/services/wal/host.cpp \
...
//       src/services/sqlite/sqlite_format.cpp src/services/redis_resp/redis_resp.cpp -o /tmp/bench_ds && /tmp/bench_ds
...
int main()
{
    ...
    // ---- WAL ----
    {
        ...
        double ns = bench_ns(500000, [&] { sink += dws_wal_crc32(src, N); });
        row("wal", "crc32 (1 KiB)", ns, (double)N);

        uint8_t rec[256];
        double ns2 = bench_ns(2000000, [&] { sink += (uint32_t)dws_wal_record_encode(rec, sizeof(rec), 1, src, 128); });
    ...
    // ---- SQLite reader ----
    {
        ...
        double nv = bench_ns(5000000, [&] {
            uint64_t v;
            s += dws_sqlite_varint_decode(vi, 2, &v);
        });
        ...
        double ns = bench_ns(20000, [&] {
            ...
            SqliteTableCursor c;
            dws_sqlite_table_cursor_begin(&c, mem_read, &m, DB_MP_PAGE_SIZE, 0, 2, leaf, work);
            uint64_t rid;
            SqliteRecordCursor row;
            uint64_t st;
            const uint8_t *v;
            uint32_t vl;
            while (dws_sqlite_table_cursor_next(&c, &rid, &row))
                while (dws_sqlite_record_next(&row, &st, &v, &vl))
                    s += st;
        });
    ...
    // ---- Redis RESP ----
    {
        ...
        double ne = bench_ns(2000000, [&] { sink += dws_resp_encode_command(out, sizeof(out), argv, lens, 3); });
        ...
        double np = bench_ns(5000000, [&] {
            RespReply r;
            size_t c;
            if (dws_resp_parse(bulk, sizeof(bulk) - 1, &r, &c))
                sink += r.str_len;
        });
