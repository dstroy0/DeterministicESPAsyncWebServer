...
//
// Host-side microbenchmark for the FTP client wire codec (RFC 959 + RFC 2428): dws_ftp_build_command (the
// device emits a control command), dws_ftp_parse_reply (decode the possibly-multiline 3-digit reply - the
// untrusted-input hot op), and dws_ftp_parse_pasv (decode the 227 passive-mode data address). All pure (no
...
//   g++ -O2 -std=c++17 -Isrc -Itest/mocks -Itest/support -DDWS_ENABLE_FTP=1 perf/services/ftp/host.cpp \
//       src/services/ftp/ftp.cpp -o /tmp/bf && /tmp/bf

#define DWS_ENABLE_FTP 1
...
int main()
{
    ...
    char cmd[128];
    size_t clen = dws_ftp_build_command(cmd, sizeof(cmd), "STOR", "dws_rig.txt");
    ...
    // build a STOR command line.
    {
        ...
        double ns = bench_ns(2000000, [&] { sink += dws_ftp_build_command(cmd, sizeof(cmd), "STOR", "dws_rig.txt"); });
    ...
    // parse a multiline reply (scan to the NNN<SP> terminator - the per-reply hot op).
    {
        ...
        double ns = bench_ns(2000000, [&] {
            int code = 0;
            size_t used = 0;
            sink += dws_ftp_parse_reply(feat, featlen, &code, &used) ? code : 0;
        });
    ...
    // parse the 227 passive-mode data address tuple.
    {
        ...
        double ns = bench_ns(2000000, [&] {
            uint8_t ip[4];
            uint16_t port = 0;
            sink += dws_ftp_parse_pasv(pasv, pasvlen, ip, &port) ? port : 0;
        });
