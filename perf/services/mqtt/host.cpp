...
// device runs as an MQTT client (dws_mqtt_build_connect / dws_mqtt_build_publish, and dws_mqtt_parse_publish - the
...
//   g++ -O2 -std=c++17 -Isrc -Itest/mocks -Itest/support -DDWS_ENABLE_MQTT=1 perf/services/mqtt/host.cpp \
...
int main()
{
    ...
    size_t clen = dws_mqtt_build_connect(conn, sizeof(conn), &opts);
    size_t publen = dws_mqtt_build_publish(pub, sizeof(pub), topic, payload, plen, 1, 0x1234, false, false);
    ...
    // build CONNECT: the packet the client sends on connect.
    {
        ...
        double ns = bench_ns(1000000, [&] { sink += dws_mqtt_build_connect(conn, sizeof(conn), &opts); });
    ...
    // build PUBLISH (QoS 1): the publish hot op.
    {
        ...
        double ns = bench_ns(1000000, [&] {
            sink += dws_mqtt_build_publish(pub, sizeof(pub), topic, payload, plen, 1, 0x1234, false, false);
        });
    ...
    // parse PUBLISH: the inbound-message decode (fixed header + variable header + payload).
    {
        ...
        double ns = bench_ns(1000000, [&] {
            ...
            uint32_t rl = 0;
            size_t hlen = 0;
            if (dws_mqtt_parse_fixed_header(pub, publen, &type, &flags, &rl, &hlen))
            {
                const uint8_t *pl = nullptr;
                size_t tl = 0, pll = 0;
                uint16_t pid = 0;
                sink += dws_mqtt_parse_publish(pub + hlen, rl, flags, tbuf, sizeof(tbuf), &tl, &pl, &pll, &pid) ? 1 : 0;
            }
        });
