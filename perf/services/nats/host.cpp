...
//
// Host-side microbenchmark for the NATS client codec: dws_nats_build_pub (the device publishes) and dws_nats_parse
// (decode one inbound server frame - the untrusted-input hot op). Both pure (no sockets, no heap), so they
// link standalone. The device figure comes from the rig /bench dws_nats_parse op; this host ns/op + MB/s is a
// RELATIVE baseline. Build + run:
//   g++ -O2 -std=c++17 -Isrc -Itest/mocks -Itest/support -DDWS_ENABLE_NATS=1 perf/services/nats/host.cpp \
//       src/services/nats/nats.cpp -o /tmp/bnats && /tmp/bnats

#define DWS_ENABLE_NATS 1
...
int main()
{
    ...
    char pub[128];
    size_t publen = dws_nats_build_pub(pub, sizeof(pub), "factory.line1.temp", nullptr, payload, plen);
    ...
    // build a PUB frame.
    {
        ...
        double ns = bench_ns(3000000, [&] {
            sink += dws_nats_build_pub(pub, sizeof(pub), "factory.line1.temp", nullptr, payload, plen);
        });
    ...
    // parse an inbound MSG frame (control line + payload).
    {
        ...
        double ns = bench_ns(3000000, [&] {
            NatsMsg m;
            size_t used = 0;
            sink += dws_nats_parse(msg, mlen, &m, &used) ? (int)used : 0;
        });
