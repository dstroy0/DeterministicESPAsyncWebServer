...
// baseline. Build + run:
//   g++ -O2 -std=c++17 -Isrc -Itest/mocks -Itest/support -DDWS_ENABLE_S7COMM=1 perf/services/s7comm/host.cpp \
//       src/services/s7comm/s7comm.cpp -o /tmp/bs && /tmp/bs

#define DWS_ENABLE_S7COMM 1
...
int main()
{
    ...
    // dws_s7_build_setup: the Setup-Communication job (negotiate AMQ + PDU size) - once per connection.
    {
        uint8_t buf[64];
        volatile size_t sink = 0;
        double ns = bench_ns(5000000, [&] { sink += dws_s7_build_setup(buf, sizeof(buf), 0x0001, 1, 1, 480); });
        row("s7comm", "build_setup", ns, (double)(sink ? 22.0 : 0.0));
        (void)sink;
    }

    // dws_s7_build_read_request: frame an N-item Read Var job (S7-ANY pointers) - the PLC-poll transmit op.
    {
        uint8_t buf[256];
        volatile size_t sink = 0;
        double ns = bench_ns(2000000, [&] { sink += dws_s7_build_read_request(buf, sizeof(buf), 0x0002, items, 3); });
        row("s7comm", "build_read_request (3)", ns, (double)req_len);
        (void)sink;
    }

    // dws_s7_parse_header: validate the protocol id + ROSCTR + lengths and slice param/data - the receive op.
    {
        volatile size_t sink = 0;
        double ns = bench_ns(10000000, [&] {
            S7Header h;
            if (dws_s7_parse_header(req, req_len, &h))
                sink += h.header_len;
        });
