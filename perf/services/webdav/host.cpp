...
// transport/filesystem-free hot op that runs on every PROPFIND response (one dws_webdav_ms_entry per
...
//   g++ -O2 -std=c++17 -Isrc -Itest/mocks -Itest/support -DDWS_ENABLE_WEBDAV=1 \
//       perf/services/webdav/host.cpp src/services/webdav/webdav.cpp -o /tmp/bw && /tmp/bw
...
int main()
{
    ...
    // One <response> entry for a file (the per-child PROPFIND hot op).
    {
        ...
        double ns = bench_ns(1000000, [&] {
            size_t n = dws_webdav_ms_entry(buf, sizeof(buf), 0, "/dav/report.txt", false, 4096, mtime, "text/plain");
            sink += n;
        });
        size_t bytes = dws_webdav_ms_entry(buf, sizeof(buf), 0, "/dav/report.txt", false, 4096, mtime, "text/plain");
    ...
    // A whole Depth-1 directory listing: prolog + 8 children + epilog (a realistic PROPFIND body).
    {
        ...
        double ns = bench_ns(200000, [&] {
            ...
            for (int k = 0; k < 8; k++)
                len =
                    dws_webdav_ms_entry(buf, sizeof(buf), len, "/dav/sensor-log.csv", false, 12800, mtime, "text/csv");
    ...
    // XML-escape an href containing the five escapable characters (per href / property tag).
    {
        char esc[256];
        volatile size_t sink = 0;
        double ns = bench_ns(2000000, [&] { sink += dws_webdav_xml_escape(esc, sizeof(esc), "/dav/a&b<c>\"d'e.txt"); });
        size_t bytes = dws_webdav_xml_escape(esc, sizeof(esc), "/dav/a&b<c>\"d'e.txt");
