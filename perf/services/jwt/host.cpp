...
//
// Host-side microbenchmark for JWT HS256 bearer-auth verification: dws_jwt_verify_hs256() splits the compact
...
int main()
{
    ...
    // verify one bearer token (split + alg check + HMAC-SHA256 + base64url + constant-time compare).
    {
        volatile int sink = 0;
        double ns =
            bench_ns(2000000, [&] { sink += dws_jwt_verify_hs256(tok, tlen, secret, sizeof(secret) - 1) ? 1 : 0; });
        row("jwt", "dws_jwt_verify_hs256", ns, (double)tlen);
        (void)sink;
    }
