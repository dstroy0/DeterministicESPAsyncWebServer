...
//
// Host-side microbenchmark for the Redis RESP2/RESP3 codec: dws_resp_encode_command (the device builds an
// outbound command) and dws_resp_parse (the device decodes a server reply - the untrusted-input hot op).
...
//   g++ -O2 -std=c++17 -Isrc -Itest/mocks -Itest/support -DDWS_ENABLE_REDIS=1 perf/services/redis_resp/host.cpp \
//       src/services/redis_resp/redis_resp.cpp -o /tmp/br && /tmp/br

#define DWS_ENABLE_REDIS 1
...
int main()
{
    ...
    char cmd[128];
    size_t clen = dws_resp_encode_command(cmd, sizeof(cmd), argv, nullptr, 3);
    ...
    // encode a SET command.
    {
        ...
        double ns = bench_ns(2000000, [&] { sink += dws_resp_encode_command(cmd, sizeof(cmd), argv, nullptr, 3); });
