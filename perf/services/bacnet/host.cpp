...
//   g++ -O2 -std=c++17 -Isrc -Itest/mocks -Itest/support -DDWS_ENABLE_BACNET=1 perf/services/bacnet/host.cpp \
//       src/services/bacnet/bacnet.cpp -o /tmp/bb && /tmp/bb

#define DWS_ENABLE_BACNET 1
...
int main()
{
    ...
    uint8_t npdu[64];
    size_t npdu_len =
        dws_npdu_build(npdu, sizeof(npdu), true, NPDU_PRIO_NORMAL, true, 100, dadr, 2, 255, apdu, sizeof(apdu));

    uint8_t frame[128];
    size_t frame_len = dws_bvlc_build(frame, sizeof(frame), BVLC_FUNC_ORIGINAL_UNICAST, npdu, npdu_len);

    printf("| Feature  | Operation                |     ns/op |    MB/s |\n");
    printf("|----------|--------------------------|-----------|---------|\n");

    // dws_bvlc_parse: validate the Annex-J envelope + slice out the NPDU - the first op on every datagram.
    {
        ...
        double ns = bench_ns(10000000, [&] {
            ...
            size_t nl = 0;
            if (dws_bvlc_parse(frame, frame_len, &fn, &np, &nl))
                sink += nl;
        });
        row("bacnet", "dws_bvlc_parse", ns, (double)frame_len);
        (void)sink;
    }

    // dws_npdu_parse: validate version/control + walk the optional addressing + slice the APDU - the receive op.
    {
        volatile size_t sink = 0;
        double ns = bench_ns(10000000, [&] {
            NpduInfo info;
            if (dws_npdu_parse(npdu, npdu_len, &info))
                sink += info.apdu_len;
        });
        row("bacnet", "dws_npdu_parse", ns, (double)npdu_len);
        (void)sink;
    }

    // dws_npdu_build: frame an APDU with destination addressing + hop count - the transmit op.
    {
        volatile size_t sink = 0;
        ...
        row("bacnet", "dws_npdu_build", ns, (double)npdu_len);
        (void)sink;
    }
