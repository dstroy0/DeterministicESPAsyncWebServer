...
//
// Host-side microbenchmark for the authoritative DNS server (RFC 1035): dns_server_build_response() parses
...
//   g++ -O2 -std=c++17 -Isrc -Itest/mocks -Itest/support -DDETWS_ENABLE_DNS_SERVER=1 perf/bench_dns.cpp \
...
int main()
{
    ...
    // A hit: parse question + append the compressed A answer.
    {
        ...
        double ns = bench_ns(
            5000000, [&] { sink += dns_server_build_response(query, qlen, 60, resolve_hit, out, sizeof(out)); });
    ...
    // A miss -> NXDOMAIN (parse only, no answer RR).
    {
        volatile size_t sink = 0;
        double ns = bench_ns(
            5000000, [&] { sink += dns_server_build_response(query, qlen, 60, resolve_miss, out, sizeof(out)); });
