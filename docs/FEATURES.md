...
Opt-in GNSS RTK base station + NTRIP caster (services/gnss). Default off. Turns the device into a differential-GNSS correction source: it surveys in a fixed antenna position and serves RTCM 3.x corrections to rovers over the network (the NTRIP protocol), so a rover applies them for RTK / DGPS accuracy instead of the ~2.5 m of a bare receiver. The shipped foundation is the pure, zero-heap, host-tested RTCM3 codec (services/gnss/rtcm3): the transport frame (0xD3 preamble, 6 reserved + 10-bit length, payload, 24-bit CRC-24Q), the MSB-first bit I/O every RTCM3 field is packed with, and the Stationary Antenna Reference Point messages 1005 (no height) and 1006 (with antenna height) that advertise the base's surveyed ECEF position (38-bit signed coordinates at 0.0001 m resolution). Frames are verified byte-for-byte against pyrtcm. The caster server (rover connections + sourcetable) and the receiver bring-up (UBX / NMEA over UART, survey-in position averaging) are the ESP32 step; the base pairs with the NMEA 0183 codec to read fix quality back. Generating RTCM3 _observation_ messages (the MSM sets 1074/1077/1084/... that let a rover fix carrier-phase ambiguities for centimeter RTK) requires a receiver that outputs raw measurements (u-blox RXM-RAWX: F9P / M8T class); a raw-less module (NEO-6/7, GT-U7) can still survey in and serve the reference point + sourcetable. See src/services/gnss/rtcm3.h.
