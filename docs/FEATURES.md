...
Opt-in HTTP delivery optimizations. Three pure cores for cheaper HTTP serving, each a real web standard: RFC 5861 stale-while-revalidate (dws_delivery_swr decision + dws_delivery_cache_control header), RFC 7233 byte-range delta/offset fetch (dws_delivery_range parse of X-Y / X- / -N + dws_delivery_content_range for a 206), and a versioned service-worker precache manifest (dws_delivery_sw_manifest). No heap/stdlib. Default off.
