...
| Advanced attack  | ~HTTP + 6 | HTTP/1.1 desync+chunk+obs-fold+CRLF+range+upload, WS frame-abuse+ping, **SSE exhaustion+churn**, **WebDAV Depth:inf/XXE/traversal/PROPPATCH-bomb/LOCK-flood**, auth, csrf, throttle; coap/snmp/modbus/opcua basic fuzz; tls handshake |
| Throughput bench |       ~12 | codecs (base64/hex/mime/mtconnect/**sse**/**webdav**), storage, request-path (host), smb/dnc/relay/radio, CCOUNT primitives                                                                                                           |
