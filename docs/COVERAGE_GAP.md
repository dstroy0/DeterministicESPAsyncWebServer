...
| Advanced attack  | ~HTTP + 7 | HTTP/1.1 desync+chunk+obs-fold+CRLF+range+upload, WS frame-abuse+ping, **SSE exhaustion+churn**, **WebDAV Depth:inf/XXE/traversal/PROPPATCH-bomb/LOCK-flood**, **CoAP option-abuse/block-bomb/observe-flood**, auth, csrf, throttle; snmp/modbus/opcua basic fuzz; tls handshake |
| Throughput bench |       ~13 | codecs (base64/hex/mime/mtconnect/**sse**/**webdav**/**coap**), storage, request-path (host), smb/dnc/relay/radio, CCOUNT primitives                                                                                                                                             |
