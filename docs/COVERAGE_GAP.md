...
| Advanced attack  | ~HTTP + 11 | HTTP/1.1 desync+chunk+obs-fold+CRLF+range+upload, WS frame-abuse+ping+**deflate-bomb**, **SSE exhaustion+churn**, **WebDAV Depth:inf/XXE/traversal/PROPPATCH-bomb/LOCK-flood**, **CoAP option-abuse/block-bomb/observe-flood**, **SNMP getbulk-amp/BER-abuse/varbind-flood**, **OPC-UA hello-buffer-abuse/UACP-abuse/chunk-flood**, **Modbus illegal-requests/MBAP-abuse**, auth, csrf, throttle; tls handshake |
...
| MQTT                                                                                                              |   OK    |   OK    |      -      | mqtt codec host+device bench (publish 4.2us); attack NEEDS a device-as-client malicious-broker harness (rig connect-out route + a broker feeding malformed CONNACK/PUBLISH/SUBACK) - next MQTT tick |
