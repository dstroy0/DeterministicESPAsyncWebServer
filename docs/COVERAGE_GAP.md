...
| ---------------- | --------: | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Interop peers    |         8 | http, ws, **sse**, coap, mqtt, modbus, snmp, opcua                                                                                                                      |
| Advanced attack  | ~HTTP + 5 | HTTP/1.1 desync+chunk+obs-fold+CRLF+range+upload, WS frame-abuse+ping, **SSE exhaustion+churn**, auth, csrf, throttle; coap/snmp/modbus/opcua basic fuzz; tls handshake |
| Throughput bench |       ~11 | codecs (base64/hex/mime/mtconnect/**sse**), storage, request-path (host), smb/dnc/relay/radio, CCOUNT primitives                                                        |

The library implements **~50+ network protocols**; interop is the biggest gap by far (8 of ~50).
