...
- **Keyboard-interactive authentication** (RFC 4256) - opt-in via `DWS_ENABLE_SSH_KEYBOARD_INTERACTIVE`; the server sends one non-echoed `Password:` prompt (`USERAUTH_INFO_REQUEST`) and verifies the `USERAUTH_INFO_RESPONSE` through the same password callback, so it is the challenge-response face of password auth (the common `ssh -o PreferredAuthentications=keyboard-interactive` / PAM-password case). The response is wiped from the stack; a response with no armed exchange is refused. HW-verified against OpenSSH 10 on an ESP32-P4 (authenticated + byte-exact channel echo)
...
- **Remote port forwarding** (`forwarded-tcpip`, the `ssh -R` remote forward, RFC 4254 §7) - same `DWS_SSH_PORT_FORWARD` gate: the client's `tcpip-forward` global request makes the device open a listener, and each inbound connection is bridged back to the client over a server-initiated `forwarded-tcpip` channel (`dws_ssh_conn_open_forwarded`), gated by the same policy callback. HW-verified on an ESP32-P4: a stock OpenSSH `ssh -R` through the device tunnels a byte-exact reverse round trip. (X11 forwarding is a deliberate non-goal.)
...
| User authentication           | RFC 4252 §5, §7, §8; RFC 4256 | Implemented - `publickey` (`rsa-sha2-512`, `rsa-sha2-256`, `ecdsa-sha2-nistp256`, `ssh-ed25519`), `password`, and (opt-in `DWS_ENABLE_SSH_KEYBOARD_INTERACTIVE`) `keyboard-interactive` methods                              |
