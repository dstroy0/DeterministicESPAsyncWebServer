...
fixed-size buffers, a host-testable core where possible, and a `DWS_ENABLE_*`
...
- [x] Thread-safe transport boundary _(shipped)_ - the cross-thread connection
      fields (state, RX ring head/tail) are `DWSAtomic` (acquire/release), so the
...
- [x] Core-partitioned parallel workers _(shipped)_ - `DWS_WORKER_COUNT` (default 1) workers each own a disjoint set of connection slots (round-robin at
      ...
      single pipeline.
- [x] Thread-safe app push path _(shipped)_ - `DWS::defer(slot, fn, arg)`
...
- [x] Pluggable monotonic clock _(shipped)_ - `services/clock.h`: all library
      timing flows through `dws_millis()` (a single 1000 Hz source). Feed your own
      clock with `dws_set_clock(fn, ticks_per_second)` and it is divided down to
...
- [x] Notification-driven worker drain _(shipped)_ - the worker blocks on its
      ...
      producers (`listener_enqueue`, `dws_defer`) nudge it the moment work is
      queued, so events are serviced immediately. This decouples event latency from
      the idle-sweep cadence: `DWS_WORKER_POLL_TICKS` is now purely the idle
...
- [x] \*Preempting task queue (L) _(shipped)_ - `DWS_ENABLE_PREEMPT_QUEUE`:
      `services/preempt_queue`, static (zero-heap) queues feeding dedicated
      core-pinned tasks. **From a task**, `dws_pq_post()` /
      `dws_pq_post_urgent()` (`xQueueSendToBack` / `xQueueSendToFront`) with a
      wait timeout; the scheduler preempts the lower-priority producer the instant
      the item is queued. **From an ISR**, `dws_pq_post_from_isr()`
...
- [x] \*User-configurable task priorities / affinity (M) _(shipped)_. Each lane's
      task priority + core pinning are set at `dws_pq_start[_lane]()` and the queue
      depth is compile-time (`DWS_PQ_DEPTH`); the no-arg API drives the USER lane so
      users own their task priorities, while internal lanes default above the user lane.
- [x] \*DMA UART / I2C / SPI transfer (L) _(shipped)_ - `DWS_ENABLE_DMA`:
      ...
      Zero-heap, fail-closed. The ingress/egress **simulator** (`DWS_DMA_SIMULATE`,
      default on) runs the whole pipeline with no physical loopback - host bench and
      on-device; a real silicon driver plugs into `dws_dma_hw_*`. Host-tested
...
- [x] \*Interface forwarding (L), **DMA-driven** _(shipped)_ - `DWS_ENABLE_FORWARD`:
      ...
      cap). A frame arriving on one interface (`dws_forward_ingress()`, wired from a
...
The **generic gateway framework is shipped** (`DWS_ENABLE_GATEWAY`, `services/gateway`):
ports, address-aware northbound enveloping + topic (`dws_gateway_uplink` / `dws_gateway_topic`),
...
- [~] \*LoRa / LoRaWAN gateway (L) - Semtech SX127x / SX126x, RFM95/96 over SPI.
  **Codec + SX127x driver shipped** (`DWS_ENABLE_LORA`, `services/lora`): the
...
- [~] \*nRF24L01+ gateway (M) - Nordic 2.4 GHz over SPI; cheap point-to-multipoint
  sensor links bridged to the web stack. **Driver shipped** (`DWS_ENABLE_NRF24`,
...
- [~] \*Zigbee NCP gateway (L) - Silicon Labs EZSP (EFR32) / Digi XBee / TI ZNP over
  ...
  shipped** (`DWS_ENABLE_ZIGBEE`, `services/zigbee`): the byte-stuffed, CRC-16/CCITT
...
- [~] \*EnOcean gateway (M) - energy-harvesting 868 MHz ESP3 protocol over UART.
  **ESP3 codec shipped** (`DWS_ENABLE_ENOCEAN`, `services/enocean`): telegram
...
- [~] \*Sigfox uplink (S) - Wisol / Murata module over UART; tiny low-power uplinks.
  **AT-command codec shipped** (`DWS_ENABLE_SIGFOX`, `services/sigfox`): the
...
- [~] \*NFC / RFID gateway (M) - PN532 (I2C / SPI / UART) or MFRC522 (SPI); tag
  ...
  (`DWS_ENABLE_PN532`, `services/pn532`): normal-information-frame build/parse +
...
- [~] \*BLE GATT bridge (M) - on-chip ESP32 BLE (and external HCI-UART modules):
  ...
  (`DWS_ENABLE_BLE_GATT`, `services/ble_gatt`): the Attribute Protocol PDUs under GATT (Bluetooth Core
...
- [x] \*Bus listen-only capture (M) _(CAN shipped, v4.85.0)_ - `DWS_ENABLE_BUS_CAPTURE`
...
- [~] \*Radio channel sniff (L) - the RF gateways above in receive-only mode (sniff a
  ...
  shipped** (`DWS_ENABLE_RADIO_SNIFF`, `services/radio_sniff`): wraps each received 802.15.4 MAC frame
  in the Wireshark IEEE 802.15.4 TAP pseudo-header (per-frame RSSI + channel TLVs, an exact int->float32
  RSSI encode) and a pcap record (new `DWS_DLT_IEEE802_15_4_TAP` / `_NOFCS` link types in
...
- [~] \*Capacitive / proximity field sensing (S) - FDC2x14 / MPR121 (I2C): touch,
  ...
  MPR121 12-channel touch/proximity (`DWS_ENABLE_MPR121`, `services/mpr121`) and the FDC2114/2214
  capacitance-to-digital field sensor (`DWS_ENABLE_FDC2214`, `services/fdc2214`): the 28-bit data
...
- [~] \*Inductive / EM field sensing (S) - LDC1614 (I2C) inductance-to-digital for
  ...
  (`DWS_ENABLE_LDC1614`, `services/ldc1614`): the LDC1614 28-bit data combine + error flags + frequency
...
- [x] Real-time **dashboard** _(shipped, phase 1)_ - `DWS_ENABLE_DASHBOARD`: a compile-time `DWSWidget` table served as hand-rolled SVG (gauge / value / bar / sparkline, no external JS) via the `web_assets` pipeline, live over SSE; `dws_dashboard_set` / `_publish` (example Dashboard). **Flagship.**
...
- [~] Southbound protocol-driver framework _(framework shipped)_ - `DWS_ENABLE_SOUTHBOUND`
  ...
  a rolling txid) + `dws_sb_modbus_driver` install the full vtable over an app-supplied request/response
  transaction seam (`DwsSbModbusTxn`, bound to dws_client for Modbus TCP or a serial gateway): `read`
  ...
  `DWS_SB_MODBUS_EXCEPTION` with the raw code captured, distinct from a propagated transport error. The
  matching **master write codec is shipped too** (`dws_modbus_build_write_single` / `_build_write_multiple`
  / `_parse_write_response`). Host-tested end to end against the real slave codec (`dws_modbus_process_adu`)
  ...
  **The bit-access function codes are shipped too**, completing the master's standard set: read coils
  (FC 0x01) and discrete inputs (FC 0x02) via `dws_modbus_build_read_bits` / `_parse_read_bits_response`
  ...
  shipped too** - Mask Write Register (FC 0x16, `dws_modbus_build_mask_write` + `_parse_mask_write_response`:
  ...
  `dws_modbus_build_read_write_multiple`: a write span applied before a read span in one transaction) - both
  implemented on **both** the master and the slave codec (`dws_modbus_process_adu`) and host-tested end to
  ...
  0x17). _Remaining:_ an example driving a real Modbus TCP server over `dws_client` (HW-verify).
...
- [x] Egress-interface reporting _(shipped)_ - `dws_net_egress()` / `dws_net_egress_ip()` read the live lwIP default route so the app always knows which interface (WiFi STA / softAP / Ethernet) its traffic is leaving on; the stack owns the actual failover, so no manager or polling tick is added (example NetEgress).
- [~] Ethernet PHY bring-up (L, greenlit) - a wired-PHY init alongside WiFi; failover + the egress report
  ...
  _(escalation policy shipped)_ - `DWS_ENABLE_LINK_MANAGER` (`services/link_manager`): a table of
  ...
  (RFC 4303). Splits into three tractability tiers, built in order:
    1. **IKEv2 message + payload codec** (M) - SHIPPED (`services/ikev2`, `DWS_ENABLE_IKEV2`, host-tested
       ...
       **AUTH**, **IDi/IDr**, **CERT** / **CERTREQ**, **TSi/TSr** traffic selectors, **N** notify, **D** delete,
       ...
       SK-payload AEAD is shipped too** (RFC 5282 AES-256-GCM-16): `dws_ike_sk_aead_seal` /
    ...
    2. **IKE SA handshake state machine** (L) - IKE_SA_INIT -> IKE_AUTH (PSK and/or certificate auth) ->
       ...
       **The IKE_AUTH encrypted-message assembly is shipped too** (RFC 7296 §3.14 / RFC 5282):
       ...
       `dws_ike_signed_octets` assembles RealMessage | Nonce | prf(SK_p, id) into caller scratch, and
       `dws_ike_auth_sign_ecdsa_p256` / `_verify_ecdsa_p256` sign / verify those octets with the library's
       ...
       SPIs, role, negotiated `IkeSuite`, and `IkeKeyMaterial`; `dws_ike_suite_keylengths` maps the
       ...
       emits the IKE_SA_INIT request from caller-supplied ephemeral material (SPI, X25519 key pair, nonce
        - RNG stays out of the pure core) and `dws_ike_initiator_on_sa_init` consumes the responder's
          ...
          initiator handshake is now COMPLETE**: `dws_ike_initiator_on_auth_psk` decrypts the responder's
          ...
          **The responder IKE_SA_INIT is shipped too**: `dws_ike_responder_on_sa_init` consumes the
          ...
          `dws_ike_auth_verify_rsa_sha256` verifies a peer's RSA-2048 PKCS#1 v1.5 SHA-256 AUTH signature
...
- [x] Source-IP allowlist / firewall in the accept callback _(shipped)_ - `listener_ip_allow_add_cidr("192.168.1.0/24")` / `listener_ip_allowed` (IPv4 + IPv6 CIDR rules matched on the full address, `DWS_ENABLE_IP_ALLOWLIST`; example IpAllowlist).
- [x] Brute-force per-IP exponential lockout _(shipped)_ - `DWS_ENABLE_AUTH_LOCKOUT`; `auth_lockout_*` table (keyed on the full IPv4/IPv6 address) issues 429 + Retry-After on the HTTP auth gate (example AuthLockout).
- [x] CSRF token verification _(shipped)_ - `DWS_ENABLE_CSRF`; global enforcement on POST/PUT/PATCH/DELETE via a stateless HMAC-signed `X-CSRF-Token` (built-in `GET /csrf` issues it; example Csrf).
- [x] Granular API-token scoping _(shipped)_ - `dws_jwt_claim_str()` reads string claims (sub / role / scope) and `dws_jwt_scope_allows()` matches a space-separated OAuth2 scope claim, so a handler can authorize per role/scope on the verified JWT (example JWTAuth).
...
- [x] MAC-derived UUID _(shipped)_ - `dws_uuid_from_mac` / `dws_device_uuid` (RFC 4122 v5; example DeviceUuid).
...
- [x] **Post-quantum hybrid key exchange - SSH** (SHIPPED v6.6.0, `DWS_ENABLE_PQC_KEX`) - the **headline
...
- [x] **SSH cipher / host-key breadth** _(shipped)_ - closed the easy deltas vs CycloneSSH:
    ...
    - [x] **`ecdsa-sha2-nistp256`** host key + client auth _(shipped)_ - ECDSA over NIST P-256 (RFC 5656,
          ...
          the sign path is byte-exact against the RFC 6979 A.2.5 vectors. Installed via `dws_ssh_hostkey_ecdsa_set()`;
...
- [x] **SFTP + SCP subsystem over SSH** (XL, high value - ties to G-code deployment) _(done, HW-verified)_ -
      Cyclone has both SFTP and SCP client+server; we now have the **server** side. `DWS_ENABLE_SSH_SFTP`
      ...
      `dws_ssh_sftp_begin(fs, root)`: OPEN/READ/WRITE/OPENDIR/READDIR/STAT/MKDIR/RMDIR/REMOVE/RENAME/REALPATH,
      the `..` traversal guard (server/fs_path). `DWS_ENABLE_SSH_SCP` (services/scp + server/ssh_scp) - the
...
- [x] **DTLS 1.3** (L) _(full server stack + CoAP-over-DTLS front-end shipped)_ - Cyclone does DTLS 1.0/1.2/1.3;
      ...
      **HelloRetryRequest** group renegotiation (§5.1, address-bound cookie) and **ACK/timeout retransmission**
      ...
      is complete - the `dws_coaps_process` bridge plus `coaps_server`, a per-peer DtlsConn pool binding UDP 5684
      (`coaps://`), driven by one `dws_coaps_server_poll()` (`native_coaps` / `native_coaps_server`; example
...
- [~] PSRAM web buffers / zero-copy net buffers (`heap_caps_calloc(MALLOC_CAP_SPIRAM)` at begin) + asset
  ...
  `DWS_ENABLE_PSRAM_POOL` (`services/psram_pool`): `dws_psram_place` picks DRAM vs PSRAM for a buffer
  by size, DMA requirement, and free-heap headroom (large/cold to PSRAM, small/hot + DMA to DRAM, always
  leaving an internal-DRAM reserve), and `dws_pingpong_*` keeps the SPI DMA double-buffer bookkeeping
...
- [x] Fail-safe safe-state + deadlock-detection WDT + watchdog-protected coroutine lifelines (M)
      ...
      "lifeline" (a task / worker / control loop) that must check in (`dws_failsafe_feed`) within its
      deadline; `dws_failsafe_check()` detects one that wedged (hang / deadlock) via a wrap-safe time
...
- [~] Hardware health (M): power-rail voltage-drop logger, SPI-bus CRC audit + clock backoff, GPIO
  short-circuit test, capacitor-leakage diag _(decision cores shipped)_ - `DWS_ENABLE_HW_HEALTH`
  (`services/hw_health`): `dws_hwhealth_rail_sample` tracks a rail's worst droop + sag/brownout counts
  (with a `/health` JSON serializer), `dws_hwhealth_spi_result` is a hysteretic clock-backoff state
  machine (halve on a CRC-fail streak, step up on an ok streak, clamped to a floor/ceiling),
  `dws_hwhealth_gpio_short` classifies a driven-vs-readback mismatch as a short to ground / Vcc, and
  `dws_hwhealth_cap_leak` compares a measured RC decay to expected (too fast = leak, too slow = high
  ESR). Pure, host-tested (`native_hw_health`). _Remaining:_ the ADC / SPI / GPIO sampling glue that
  feeds them on real hardware (M).

## Build / tooling

- [x] Runtime build-flag reporter _(shipped)_ - `server.diag()` / `DWS_ENABLE_DIAG` serves a build-info JSON (example Diagnostics); the feature enumeration could be extended.
...
- [~] **TLS 1.2** (L, RFC 5246) - explicit TLS 1.2 record/handshake support with a
  ...
  cipher **policy** shipped as `DWS_ENABLE_TLS_POLICY` (`services/tls_policy`):
  `dws_tls_negotiate_version` picks the version server-style, `dws_tls_version_name` makes it
  observable, `dws_tls_select_cipher` pins the suite allowlist by server preference, and
  `dws_tls_is_aead` classifies suites - pure + host-tested (`native_tls_policy`). _Remaining:_ wire the
...
- [~] **TLS 1.3** (L, RFC 8446) - TLS 1.3 handshake (1-RTT, optional 0-RTT early
  ...
  hardened profile is the shared `services/tls_policy` (`dws_tls_is_aead` + the 1.3 suites 0x1301-0x1303
...
- [x] **HTTP caching** (RFC 9111) _(shipped)_ - conditional GET on served files via
      `DWS_ENABLE_ETAG`: a strong `ETag` + `Last-Modified`, and `304 Not Modified`
...
- [~] **Raw L2 frame TX/RX** (M, platform enabler) _(Ethernet frame codec shipped)_ -
  `DWS_ENABLE_RAWL2` (`services/rawl2`): the host-testable core - build/parse Ethernet II + 802.1Q
...
- [x] **umati / OPC UA for Machine Tools** (M, OPC 40501-1) _(MachineTool model shipped)_ -
      `DWS_ENABLE_UMATI` (`services/umati`, requires `DWS_ENABLE_OPCUA`): the umati companion-spec
...
- [~] **MTConnect** (L, ANSI/MTC1.4-2018) _(streams/error response codec shipped)_ -
  `DWS_ENABLE_MTCONNECT` (`services/mtconnect`): the MTConnect agent response documents over the
...
- [~] **Fieldbuses** (L, classic serial/CAN buses) - the pre-Ethernet field protocols
  ...
  `DWS_ENABLE_CANOPEN` (`services/canopen`); **PROFIBUS-DP** is `DWS_ENABLE_PROFIBUS`
  (`services/profibus`); **DeviceNet** (CIP-over-CAN) is `DWS_ENABLE_DEVICENET`; Modbus TCP + RTU ship
  as [modbus](../src/services/modbus/) (`DWS_ENABLE_MODBUS` / `_MODBUS_RTU`). All host-tested behind
...
- [~] **PROFINET / PROFIBUS** (XL, Siemens automation) _(PROFINET DCP + PROFIBUS-DP FDL codecs shipped)_ -
  **PROFINET DCP**: `DWS_ENABLE_PROFINET` (`services/profinet`) - the DCP frame codec (10-octet header +
  option/suboption blocks with even-padding, built by `dws_pn_dcp_header` / `_block`, walked by
  `dws_pn_dcp_walk`) for Identify + Set (assign NameOfStation / IP) over raw L2 (ethertype 0x8892);
  host-tested (`native_profinet`). **PROFIBUS-DP**: `DWS_ENABLE_PROFIBUS` (`services/profibus`) - the
...
- [~] **HART** (M, FieldComm) _(frame + HART-IP codec shipped)_ - `DWS_ENABLE_HART` (`services/hart`):
  the HART command-frame codec - `dws_hart_build` / `_parse` with the longitudinal XOR checksum and
...
- [~] **CC-Link / CC-Link IE** (L, CLPA) _(cyclic frame codec shipped)_ - `DWS_ENABLE_CCLINK`
  (`services/cclink`): the CC-Link cyclic frame - `dws_cclink_build` / `_parse` for
...
- [~] **CANopen** (M, CiA 301) _(message codec shipped)_ - `DWS_ENABLE_CANOPEN`
...
- [~] **IO-Link** (M, IEC 61131-9) _(data-link codec shipped)_ - `DWS_ENABLE_IOLINK`
...
- [~] **POWERLINK** (XL, EPSG) _(basic frame codec shipped)_ - `DWS_ENABLE_POWERLINK`
  (`services/powerlink`): the EPL basic frames - `dws_epl_build` / `_parse` (and the SoC / PReq /
  ...
  Fixed BSS, no heap.
- [~] **SERCOS III** (XL, motion bus) _(telegram + IDN codec shipped)_ - `DWS_ENABLE_SERCOS`
  (`services/sercos`): the cyclic **MDT/AT** telegram codec - `dws_sercos_build` / `_parse` for
  ...
  BSS, no heap.
- [~] **DeviceNet** (L, CIP over CAN) _(link-adaptation codec shipped)_ - `DWS_ENABLE_DEVICENET`
...
- [~] **LonWorks / LON** (L, ISO/IEC 14908) _(network-variable codec shipped)_ - `DWS_ENABLE_LONWORKS`
  (`services/lonworks`): the LonTalk network-variable PDU - `dws_lon_build_nv` / `_parse_nv` for
...
- [~] **Modbus Plus** (L, HDLC token bus) _(frame + token MAC shipped)_ - `DWS_ENABLE_MBPLUS`
  (`services/mbplus`): the Modbus Plus HDLC frame codec - `dws_mbplus_build` / `_parse` for
  `[7E][addr][ctrl][payload][CRC-16/X-25][7E]` and `dws_mbplus_next_token` (the next station in the
...
- [~] **INTERBUS** (L, Phoenix Contact) _(summation-frame codec shipped)_ - `DWS_ENABLE_INTERBUS`
  (`services/interbus`): the summation frame - `dws_interbus_build` / `_parse` assemble the single
...
- [~] **SNP** (M, GE Fanuc Series Ninety Protocol) _(frame codec shipped)_ - `DWS_ENABLE_SNP`
  (`services/snp`): the SNP master-slave serial frame - `dws_snp_build` / `_parse` for
...
- [~] **DirectNET** (M, AutomationDirect) _(frame codec shipped)_ - `DWS_ENABLE_DIRECTNET`
  (`services/directnet`): the DirectNET master-slave serial frames - `dws_dnet_header` (SOH +
  ASCII-hex slave/type/address/blocks + ETB + LRC) and `dws_dnet_data` / `_data_parse` (STX + data + ETX + LRC) - for V-memory read/write on an AutomationDirect DirectLOGIC PLC, with the
...
- [~] **IEC 60870-5-101 / -104** (L, power-grid SCADA) _(codec shipped)_ - `DWS_ENABLE_IEC60870`
...
- [~] **IEC 61850** (XL, substation automation) _(GOOSE publisher + MMS Read shipped)_ - **GOOSE**:
  `DWS_ENABLE_GOOSE` (`services/goose`) - the raw-L2 multicast event publish for fast trips (the
  BER-encoded IECGoosePdu wrapped in the 8-octet GOOSE header + Ethernet frame, ethertype 0x88B8, on the
  shipped raw-L2 codec; host-tested `native_goose`). **MMS**: `DWS_ENABLE_MMS` (`services/mms`) - the
...
- [~] **IEEE 2030.5 (SEP 2.0)** (M-L, DER / smart energy) _(resource codec shipped)_ -
  `DWS_ENABLE_SEP2` (`services/sep2`): the core 2030.5 XML resource documents in the
...
- [~] **OpenADR** (L, demand response) _(3.0 event/report codec shipped)_ - `DWS_ENABLE_OPENADR`
  (`services/openadr`): the OpenADR 3.0 REST/JSON objects - `dws_openadr_event` (a demand-response
  signal: programID + eventName + interval payload points with start / duration / type / value) and
  `dws_openadr_report` (a VEN reading back to the VTN) - built into a caller buffer over the
...
- [~] **ICCP / TASE.2** (XL, IEC 60870-6) _(Data_Value codec shipped)_ - `DWS_ENABLE_ICCP`
  (`services/iccp`): the TASE.2 indication-point Data_Value BER structures - `dws_iccp_state_q`
  (StateQ: a discrete state + quality flags) and `dws_iccp_real_q` (RealQ: a scaled INTEGER value +
...
- [x] **SCPI / IEEE 488.2** (S, universal instrument control - **high value**) - SHIPPED
      (`services/scpi`, `DWS_ENABLE_SCPI`, example `L7-Application/Scpi`). Standard Commands for Programmable Instruments,
      ...
      builder (`dws_scpi_build` - headers, `:` hierarchy, parameter formatting), the 13 mandatory IEEE
      488.2 common commands (`dws_scpi_common`: `*IDN?` / `*RST` / `*OPC?` / `*CLS` / `*ESR?` /
      ...
      header matcher (`dws_scpi_match`). Makes the device a bench-instrument controller or a wireless
...
- [~] **NTCIP** (L, US ITS) _(object OIDs shipped)_ - `DWS_ENABLE_NTCIP` (`services/ntcip`): the NTCIP
...
- [~] **UTMC** (L, UK/EU ITS) _(common-database codec shipped)_ - `DWS_ENABLE_UTMC` (`services/utmc`):
  ...
  id), `dws_utmc_response` (a UTMCResponse carrying the object value + a data-quality flag +
  timestamp), and `dws_utmc_parse_request` (extract the requested id) - over the existing HTTP
  server, XML-escaped; host-tested (`native_utmc`). _Remaining:_ the fuller object model + the
  DATEX-II profile. Fixed BSS, no heap.
- [~] **OCIT** (L, DE/AT/CH ITS) _(message codec shipped)_ - `DWS_ENABLE_OCIT` (`services/ocit`): the
  OCIT-Outstations object message - `dws_ocit_build` / `_parse` for
...
- [~] **V2X / SAE J2735** (XL, connected vehicle) _(UPER codec + BSMcore + SPaT shipped)_ -
  `DWS_ENABLE_J2735` (`services/j2735`): the ASN.1 **UPER** (Unaligned Packed Encoding Rules) bit-level
...
- [~] **IEEE 1609 (WAVE)** (XL, vehicular radio stack) _(WSMP + 1609.2 envelope codec shipped)_ -
  `DWS_ENABLE_WAVE` (`services/wave`): the 1609.3 **WSMP** (WAVE Short Message Protocol) header -
  `dws_wsmp_build` / `_parse` (version + a P-encoded **PSID** + length + payload) with the PSID
  variable-length p-encoding (`dws_wave_encode_psid` / `_decode_psid`) - and the **1609.2**
  secured-message envelope header (`dws_wave_1609dot2_wrap`: protocolVersion + contentType); these
...
- [~] **NEMA TS 2** (M, traffic cabinet bus) _(SDLC frame codec shipped)_ - `DWS_ENABLE_NEMA_TS2`
  (`services/nema_ts2`): the traffic-cabinet SDLC bus frame - `dws_nema_ts2_build` / `_parse` for
...
- [~] **ATC** (S, platform note) _(interop shipped)_ - the Advanced Traffic Controller
  ...
  `DWS_ENABLE_ATC` (`services/atc`): the field-I/O map serialized as
...
- [~] **gRPC / Protocol Buffers** (L) - _Protobuf wire codec shipped._
  ...
  **HTTP/2** with `application/grpc`) remains gated on the HTTP/2 roadmap item above. Fixed
  BSS, no heap.
- [~] **DDS** (XL, OMG DDS) _(RTPS framing shipped)_ - `DWS_ENABLE_DDS` (`services/dds`): the RTPS
  ...
  flag, built by `dws_rtps_header` / `_submessage` and walked by `dws_rtps_parse`; host-tested
...
- [x] **WAMP** (M, web messaging) _(shipped)_ - `DWS_ENABLE_WAMP` (`services\wamp`): a
...
- [x] **MQTT-SN** (M, sensor networks) _(shipped)_ - `DWS_ENABLE_MQTT_SN`
...
- [x] **NetFlow / IPFIX** (M, flow export) _(shipped)_ - `DWS_ENABLE_FLOW_EXPORT`
      ...
      flow cache (5-tuple + counters) and the UDP send (`dws_udp_sendto`) are the app's.
...
- [~] **XMPP (IoT profile)** (L, XSF) _(stanza codec shipped)_ - `DWS_ENABLE_XMPP` (`services/xmpp`):
...
- [~] **DShot** (M, RMT) _(codec shipped)_ - `DWS_ENABLE_DSHOT` (`services/dshot`): the DShot frame
  codec - `dws_dshot_encode` / `_decode` build + validate the 16-bit packet (11-bit throttle /
...
- [~] **Bidirectional / Extended DShot (EDShot)** (M, RMT) _(outbound frame shipped)_ - the
  bidirectional inverted-CRC frame is built by `dws_dshot_encode(..., bidirectional=true)` (and
...
- [~] **ProShot** (S, RMT) _(shares the DShot packet)_ - ProShot carries the same 16-bit DShot value
  (built by `dws_dshot_encode`, shipped) as four pulse-position symbols. _Remaining:_ the
  nibble->pulse-position encoding + the RMT emission (the packet + CRC are done).
- [~] **OneShot / Multishot** (S, MCPWM) _(pulse-width mapping shipped)_ - `dws_esc_pwm_ns()`
...
`DWS_ENABLE_NTP` client trusts whatever the network hands it, so a MITM can shift
...
> The plain (unauthenticated) NTP **server** already ships (`DWS_ENABLE_NTP_SERVER`,
...
- [~] **NTS-KE (Key Establishment)** (L) _(record codec shipped)_ - `DWS_ENABLE_NTS`
  (`services/nts`): the NTS-KE TLV record codec - `dws_nts_ke_request` builds the standard client
  request (Next Protocol NTPv4 + AEAD AES-SIV-CMAC-256 + End-of-Message, all critical) and
  `dws_nts_ke_parse` walks a response, surfacing each record (cookies, negotiated AEAD, server,
...
- [~] **NTS-protected NTP** (L) _(extension-field framing shipped)_ - `services/nts` also builds the NTS
  ...
  anti-replay unique-id echo + cookie rotation, then feed the validated offset to `dws_clock`.
...
- [x] **Sphinx + Breathe bridge** (L) _(shipped)_ - the Sphinx project lives in `docs/sphinx`: `docs/Doxyfile`
      ...
      (the `DWS` public surface), and **MyST** pulls the hand-written Markdown guides
...
> defaults `DWS_ENABLE_WEBSOCKET`, `DWS_ENABLE_SSE`, `DWS_ENABLE_FILE_SERVING`,
> `DWS_ENABLE_AUTH`, ... to `1`). The default should be a single-purpose HTTP server and
...
- [ ] **Slim the default config to a single-purpose HTTP server** (M) - flip the `src/ServerConfig.h`
      `DWS_ENABLE_*` defaults so only the core request/response path is on out of the box; every other
...
> `src/dwserver.cpp` is a ~4,000-line monolith holding the whole DWS implementation
...
- [x] **Waveshare HMMD 24 GHz mmWave presence sensor** (M) - human micro-motion detection (moving /
      ...
      `DWS_ENABLE_HMMD` / `services/hmmd`: a pure, host-tested **little-endian** frame codec - the default
...
- [ ] **FSoE / Fail Safe over EtherCAT (IEC 61784-3-12, ETG)** (L) - the SIL3 safety layer over EtherCAT.
      `DWS_ENABLE_FSOE` / `services/fsoe`: the FSoE frame (Command + SafeData + a CRC per data slot +
...
- [ ] **IO-Link Safety (IO-Link Community)** (M) - functional safety over the shipped IO-Link (SDCI,
      IEC 61131-9) codec. `DWS_ENABLE_IOLINK_SAFETY` / `services/iolink_safety`: the OSSDe signal handling
...
- [ ] **PI (PROFIBUS & PROFINET International) - PROFINET Security** (L) - PI is the steward of the shipped
      ...
      identity). `DWS_ENABLE_PROFINET_SECURITY` / extend `services/profinet`: the SecurityControl +
...
- [ ] **Wireless coexistence management (IEC 62657-2)** (M) - adhere to IEC 62657-2 so the device behaves in
      ...
      manager can consume, and a backoff / channel-blacklist policy. `DWS_ENABLE_COEXISTENCE` /
...
- [ ] **802.11r Fast BSS Transition (FT)** (M) - pre-authenticate / pre-derive the PTK with the target AP
      ...
      enable + configure (`DWS_ENABLE_FAST_ROAM`; PSK or WPA2/3-Enterprise FT), expose the handover timing,
...
- [ ] **802.11k Radio Resource Measurement (neighbor reports)** (S) - request the AP's neighbor report (the
      ...
      that list to the decision layer below. `DWS_ENABLE_RRM`.
- [ ] **802.11v BSS Transition Management (BTM)** (S) - accept and act on network-suggested roams (BTM
      ...
      feeding the hint into the decision layer. `DWS_ENABLE_BTM`.
...
> on a classic board, so examples have to hand-dial the tunables down in `build_opt.h` (as MeshCache does:
