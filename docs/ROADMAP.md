...
- [~] Southbound protocol-driver framework _(framework shipped)_ - `DWS_ENABLE_SOUTHBOUND`
  ...
  **The bit-access function codes are shipped too**, completing the master's standard set: read coils
  ...
  packed-pattern check (`native_modbus_master` +5, now 24 cases). The master now speaks every standard
  Modbus data-access function code (0x01-0x06, 0x0F, 0x10). _Remaining:_ an example driving a real Modbus
  TCP server over `dws_client` (HW-verify).
