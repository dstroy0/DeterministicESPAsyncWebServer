...
<tr>
  ...
  <td align="left" width="20%"><a href="FEATURES.md#wear-leveling" title="Opt-in flash wear-leveling slot selector. When set, server/filesystem/wearlevel provides pc_wearlevel_pick(): given per-slot write counts it returns the least-worn slot to write next, so repeated flash/NVS writes spread evenly and the region ages together instead of burning out one block. Pure core (the app owns the slots + persisted counts). Default off.">Wear Leveling</a></td>
