...
<tr>
  ...
  <td align="left" width="20%"><a href="FEATURES.md#j1939" title="SAE J1939 codec. Default off. services/j1939 is a zero-heap codec for the heavy-duty-vehicle / agriculture / marine / genset CAN higher-layer protocol over 29-bit extended frames (`shared_primitives/can.h`): `dws_j1939_encode_id` / `dws_j1939_decode_id` pack and unpack the priority / PGN / source / destination identifier (both the PDU1 peer-to-peer and PDU2 broadcast forms), `dws_j1939_build_message` emits single frames, `dws_j1939_build_request` and `dws_j1939_build_address_claim` (with `dws_j1939_build_name` for the 64-bit NAME) handle the Request PGN and Address Claimed messages, and the Transport Protocol (BAM announce + TP.DT data packets) reassembles multi-packet messages up to `DWS_J1939_TP_MAX` octets via `dws_j1939_tp_feed`. Typed decoders lift the two canonical engine PGNs into engineering units (SAE J1939-71): `dws_j1939_decode_eec1` (EEC1 / PGN 61444 - engine speed at 0.125 rpm/bit and the driver's-demand / actual percent torque at the -125 offset) `dws_j1939_decode_et1` (ET1 / PGN 65262 - coolant, fuel, and oil temperature), `dws_j1939_decode_lfe` (LFE / PGN 65266 - engine fuel rate at 0.05 L/h/bit, instantaneous and average fuel economy at 1/512 km/L, and throttle position), `dws_j1939_decode_amb` (AMB / PGN 65269 - barometric pressure at 0.5 kPa/bit and the cab-interior / ambient-air / air-inlet / road-surface temperatures), and `dws_j1939_decode_ic1` (IC1 / PGN 65270 - the particulate-trap-inlet / boost / air-inlet / air-filter / coolant-filter pressures, the intake-manifold temperature, and the exhaust gas temperature at 0.03125 degC/bit), each applying the SPN scale + offset and clearing a validity flag for a raw in the not-available range. Identifier layout, NAME bit fields, and the TP control bytes verified against SAE J1939-21 / -81, and the EEC1/ET1 decoders against worked examples (1500 rpm, 90 degC coolant); pure and host-tested. Drive it from the ESP32 TWAI peripheral or an MCP2515 over SPI to bridge a J1939 bus onto Wi-Fi. See src/services/j1939/j1939.h.">J1939</a></td>
