...
<tr>
  <td align="left" width="20%"><a href="FEATURES.md#nmea-2000" title="NMEA 2000 codec. Default off; implies J1939 (NMEA 2000 is J1939 at the transport layer). services/nmea2000 is a zero-heap codec for the marine instrumentation network over CAN: it reuses the J1939 29-bit identifier codec and adds the NMEA-specific Fast Packet transport. `dws_n2k_fastpacket_num_frames` sizes a transfer, `dws_n2k_fastpacket_build_frame` emits frame N of a 9..223-octet message (a control octet of sequence counter + frame counter, the first frame carrying the total length and 6 data octets, continuations carrying 7), and `dws_n2k_fastpacket_feed` reassembles a sequence (matching source / PGN / sequence counter, rejecting out-of-order frames and ignoring interleaved sequences); `dws_n2k_build_single` wraps a single-frame message. Typed decoders lift common single-frame PGNs into engineering units: `dws_n2k_decode_position_rapid` (PGN 129025 - latitude / longitude at 1e-7 deg/bit into decimal degrees), `dws_n2k_decode_wind_data` (PGN 130306 - wind speed at 0.01 m/s, angle at 0.0001 rad, and the reference), `dws_n2k_decode_water_depth` (PGN 128267 - depth below the transducer at 0.01 m and the transducer offset at 0.001 m), and `dws_n2k_decode_vessel_heading` (PGN 127250 - heading / deviation / variation at 0.0001 rad and the true/magnetic reference), each clearing a validity flag for a not-available raw (0x7FFFFFFF for the signed position, all-ones for a wind / depth / heading field). Fast Packet framing verified against the NMEA 2000 / J1939 layout, and the PGN decoders against worked examples; pure and host-tested. Drive it from the ESP32 TWAI peripheral or an MCP2515 over SPI to bridge an NMEA 2000 backbone (GPS, wind, depth, engine PGNs) onto Wi-Fi. See src/services/nmea2000/nmea2000.h.">NMEA 2000</a></td>
