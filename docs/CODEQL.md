...
| `cpp/constant-comparison`        | `services/auth_lockout/auth_lockout.cpp:105`                        | **Benign (defensive)** - the `dur > (0xFFFFFFFFu >> 1)` overflow guard in the backoff loop is dead under the default `DWS_AUTH_LOCKOUT_MAX_MS` (the `>= MAX_MS` break fires first) but protects a pathologically large `MAX_MS` config. |
...
| `cpp/poorly-documented-function` | `services/modbus/modbus.cpp:112`                                    | **Won't fix (style)** - `dws_modbus_process_pdu` is a function-code dispatcher (documented with a header comment); the heuristic dislikes the size / comment ratio.                                                                     |
