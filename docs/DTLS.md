...
> complete: the bridge (`dws_coaps_process`) drives the handshake and then runs `dws_coap_server_process`
...
With `DWS_ENABLE_TLS_RPK`, the server can present a **bare public key** instead of an X.509
