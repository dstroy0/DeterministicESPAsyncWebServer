...
- **SMB/CIFS client** - _in progress_: increments 1 (SMB2 wire codec) + 2 (NTLM digests) + 3 (NTLMv2
  response) + 4 (NTLMSSP messages) + 5 (SPNEGO GSS wrapping) + 6 (SESSION_SETUP framing) **shipped**;
  next = the file commands (TREE_CONNECT / CREATE / READ / WRITE / CLOSE).
