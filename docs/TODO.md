...
- **SMB/CIFS client** - _codec + engine complete_: all 8 codec increments (SMB2 wire codec, NTLM
  digests, NTLMv2 response, NTLMSSP messages, SPNEGO wrapping, SESSION_SETUP framing, TREE_CONNECT /
  CREATE / CLOSE, READ / WRITE) **plus the `smb_client` dialogue engine** (`smb_open` runs NEGOTIATE ->
  NTLMv2 SESSION_SETUP -> TREE_CONNECT -> CREATE, `smb_close` releases the handle) - host-tested end to
  end with a scripted mock SMB2 server, `native_smb` 42 cases. Remaining: `smb_read` / `smb_write` on
  the handle, a runnable example over `det_client`, and HW-verify against a real Samba/Windows share.
