...
- **CDN caching tier** - the RAM-tier reverse-proxy edge cache + cache key / invalidation / purge shipped and
  **HW-verified**, the **SD (L2) persistence tier** (`DETWS_ENABLE_DBM`: dbm/WAL spill + promote + reboot
  survival, host-tested) shipped, and **Range/`206`-from-cache** shipped (`DETWS_ENABLE_RANGE`, single-range
  window + `Content-Range` / `416` / `Accept-Ranges`, host-tested) (`DETWS_ENABLE_EDGE_CACHE`,
  services/edge_cache, example 79). Remaining: `https://` origins and cross-device mesh distribution.
