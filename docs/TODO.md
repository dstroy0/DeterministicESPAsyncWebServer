...
- **CDN caching tier** - the RAM-tier reverse-proxy edge cache + cache key / invalidation / purge shipped and
  **HW-verified**, and the **SD (L2) persistence tier** (`DETWS_ENABLE_DBM`: dbm/WAL spill + promote +
  reboot survival, host-tested) shipped (`DETWS_ENABLE_EDGE_CACHE`, services/edge_cache, example 79).
  Remaining: Range/`206`-from-cache, `https://` origins, and cross-device mesh distribution.
