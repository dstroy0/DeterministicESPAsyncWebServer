...
- **SMB/CIFS client** - _codec + engine complete_: all 8 codec increments (SMB2 wire codec, NTLM
  ...
  NTLMv2 SESSION_SETUP -> TREE_CONNECT -> CREATE; `smb_read` / `smb_write` loop the file bytes;
  `smb_close` releases the handle) - a POSIX-like open/read/write/close surface, host-tested end to end
  with a scripted mock SMB2 server, `native_smb` 46 cases (incl. a byte-exact write-then-read round
  trip). Remaining: a runnable example over `det_client` + HW-verify against a real Samba/Windows share.
