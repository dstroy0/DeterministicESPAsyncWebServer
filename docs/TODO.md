...
- **Ethernet DNC** - the transport-agnostic `dnc_stream` drip-feed engine is shipped (leader / `%`
  markers / blocks over a send/recv seam, XOFF/XON pacing; `native_dnc` 22 cases). Remaining: a
  runnable example binding the seam to `det_client` (a raw program port) + HW-verify against a
  controller that accepts network drip-feed.
