...
- [~] MTConnect follow-ups: the `probe` (device model) document is **done** - `detws_mtc_devices_begin/add_item/end` build an MTConnectDevices doc (a `<Device>` with its `<DataItems>`, optional `name`/`units`), tested in `test_mtconnect`. The `asset` document is **done** too - `detws_mtc_assets_begin` + `detws_mtc_assets_cutting_tool_begin`/`_tool_life`/`_cutting_tool_end` + `detws_mtc_assets_end` build an MTConnectAssets doc (a `<CuttingTool>` with its `<CuttingToolLifeCycle>`/`<ToolLife>`, optional `serialNumber`/`toolId`/`deviceUuid`/`timestamp`/`limit`), tested in `test_mtconnect`. Still open: a streaming `sample` sequence cursor (from/count long-poll over a ring of observations) to round out the already-shipped `current` / `sample` agent.
