...
- **CDN caching tier** - the RAM-tier reverse-proxy edge cache + cache key / invalidation / purge shipped and
  **HW-verified** (`DETWS_ENABLE_EDGE_CACHE`, services/edge_cache, example 79). Remaining: the SD (L2)
  persistence tier, Range/`206`-from-cache, `https://` origins, and cross-device mesh distribution.
