...
- [ ] MTConnect follow-ups: the `probe` (device model) document is **done** - `detws_mtc_devices_begin/add_item/end` build an MTConnectDevices doc (a `<Device>` with its `<DataItems>`, optional `name`/`units`), tested in `test_mtconnect`. Still open: the `asset` documents and a streaming `sample` sequence cursor to round out the already-shipped `current` / `sample` agent.
