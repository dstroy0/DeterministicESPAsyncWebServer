...
- **SMB/CIFS client** - _in progress_: increments 1 (SMB2 wire codec) + 2 (NTLM digests) + 3 (NTLMv2
  response) + 4 (NTLMSSP messages) + 5 (SPNEGO GSS wrapping) **shipped**; next = the SMB2 SESSION_SETUP
  request/response flow, then the file commands.
