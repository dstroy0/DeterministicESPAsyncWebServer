...
- **SMB/CIFS client** - _codec complete_: all 8 increments shipped (SMB2 wire codec, NTLM digests,
  NTLMv2 response, NTLMSSP messages, SPNEGO wrapping, SESSION_SETUP framing, TREE_CONNECT / CREATE /
  CLOSE, READ / WRITE) - the full NEGOTIATE -> auth -> open -> read/write -> close path, `native_smb`
  36 cases. Remaining: a usable example over the `det_client` TCP transport + HW-verify against a real
  Samba/Windows share (like the FTP follow-up).
