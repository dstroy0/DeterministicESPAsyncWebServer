...
- **Port forwarding / DNAT** - complete: the `det_relay` bidirectional TCP byte pump (backpressure +
  independent half-close, host-tested, `native_relay` 6 cases) **plus the `PROTO_RELAY` server-side
  listener** (`server.listen(port, PROTO_RELAY)` + `det_relay_publish(...)` dials the origin via
  `det_client` and pumps the relay from the server poll loop) **plus a runnable example**
  (`70.PortForward`, ESP32-link-verified). Only remainder: HW-verify an inbound->origin round trip on
  a real device (the README shows a machine-less `python -m http.server` + `curl` test).
- **Ethernet DNC** - the transport-agnostic `dnc_stream` drip-feed engine + a runnable example
  ...
  controller that accepts network drip-feed (the README shows a machine-less `nc`-listener test).
