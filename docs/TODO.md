...
- **CDN caching tier** - the RAM-tier reverse-proxy edge cache + cache key / invalidation / purge, the
  **SD (L2) persistence tier** (`DETWS_ENABLE_DBM`), **Range/`206`-from-cache** (`DETWS_ENABLE_RANGE`), and
  **`https://` origins** (`DETWS_ENABLE_EDGE_ORIGIN_TLS`: fetch over the shared client-TLS session, CA/pin
  verification, HW-verified) are all shipped (`DETWS_ENABLE_EDGE_CACHE`, services/edge_cache, example 79).
  Remaining: cross-device mesh distribution; and two TLS-origin follow-ups (async handshake so a cold https
  MISS never blocks the worker, and a multi-session pool for >1 concurrent TLS fetch).
...
- **Ethernet DNC** - the transport-agnostic `dnc_stream` drip-feed engine + a runnable example
  (`69.EthernetDnc`) are shipped (leader / `%` markers / blocks over a send/recv seam, XOFF/XON pacing;
...
- **SMB/CIFS client** - complete and shipped: all 8 codec increments + the `smb_client` engine
  ...
  (`68.SmbFileClient`, ESP32-compile-verified). Host-tested end to end with a scripted mock SMB2 server
