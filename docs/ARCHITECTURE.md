...
src/network_drivers/
  ...
  network/      IPv4 / interface tagging (DWSIface: STA vs AP)
  transport/    lwIP raw-API callbacks, the per-connection RX ring, the TcpEvt
                event queue, and the dws_conn_* I/O API. OWNS all socket I/O.
  ...
  application/  DWS: routes, handlers, serve_file / send_chunked pumps,
                WebDAV
...
src/shared_primitives/  layer-agnostic header-only primitives shared across the
                ...
                server-adjacent code AND dws_client, so a client has one DNS owner).
```
...
- `DWSAtomic<>` (acquire/release) on `TcpConn::state`, `rx_head`, `rx_tail`.
...
```
app / presentation  -->  dws_conn_send / dws_conn_flush / dws_conn_sndbuf
                         (TLS slots route through dws_tls_write)
                              |
                         dws_tcp_marshal (DWS_OP_*)  -->  tcpip_thread: tcp_write / tcp_output
```

The **same marshal rule covers every raw lwIP call, not just app data**: the TCP
listener bring-up (`listener_add` / `listener_stop`), the UDP transport (`dws_udp_*`,
used by SNMP / CoAP / captive-DNS / syslog / telemetry), the outbound client
(`dws_client`), and the DNS resolver all route their `tcp_*` / `udp_*` through
...
ACKs on copy. The worker calls `dws_conn_ack_consumed(slot)` once per slot per loop
...
`dws_conn_available` / `dws_conn_read_byte` / `dws_conn_read` / `dws_conn_peek` /
`dws_conn_consume` (inline in tcp.h, single-consumer per slot). Migrated:
...
`dws_conn_ack_consumed()` per loop - so there is exactly one place that touches the
ring indices for draining and one that ACKs.

## Outbound clients - the unified client transport (dws_client)

The device's clients (`http_client`, `mqtt`, `ws_client`) do not each own a raw lwIP
stack: they share **`dws_client`** (`network_drivers/transport/dws_client.*`), the
...
per-connection wire ring, and **ack-on-consume** (`dws_client_read()` reopens the
window as the caller drains; `DWS_CLIENT_RX_BUF >= TCP_WND`), so client and server
share one flow-control model. TLS clients layer `dws_tls_client_session_*` on top, pointing
the BIO at `dws_client_send` / `dws_client_read` (the ring carries ciphertext).
...
`DWSAtomic` SPSC index wrapper + the drain math `dws_ring_available / read_byte /
read / peek / consume` AND the fill math `dws_ring_free / dws_ring_write_span`). The
server (`dws_conn_*` + `recv_cb`) and client (`dws_client_*` + `cc_recv`) are thin
...
`volatile`; they are now `DWSAtomic` like the server (correct cross-core
...
| -------------------- | ------------------------ | ------------------------------------------------- |
| Socket TX            | transport `dws_conn_*`   | DONE                                              |
| RX receive window    | transport                | DONE (`dws_conn_ack_consumed`, ack-on-consume)    |
| RX ring read/drain   | transport (read API)     | DONE (`dws_conn_read*`; consumers off the ring)   |
...
| Outbound client I/O  | transport (`dws_client`) | DONE (pooled, ack-on-consume; all clients use it) |

## Straightening plan (phased; each phase host + HW regresses every consumer)

1. **DONE - RX read API in transport** - `dws_conn_available` / `dws_conn_read_byte`
   / `dws_conn_read` / `dws_conn_peek` / `dws_conn_consume` (inline, tcp.h).
2. **DONE - migrate the consumers** - HTTP / websocket / telnet / ssh / tls + the
   ...
   `rx_tail` modulo remains. The read functions consume only; `dws_conn_ack_consumed`
...
4. **DONE - outbound clients** - all clients (http_client / mqtt / ws_client) share
   `dws_client`, the unified client transport; brought to the same ack-on-consume
   flow control as the server (`DWS_CLIENT_RX_BUF >= TCP_WND`). Each module's `s_rx`
...
handler reads its bytes through the transport RX API (`dws_conn_read` copy-out, or
`dws_conn_peek`+`dws_conn_consume` zero-copy - never the ring internals) and writes
through `dws_conn_send`/`dws_conn_flush`. So Telnet, SSH (+ `PROTO_SSH_RFWD`), Modbus,
...
a _different_ but deliberately separate seam - `dws_udp_listen(port, handler, arg)`, one
...
1. **`session.cpp` (L5) is now protocol-agnostic - DONE.** The dispatcher owns only the
   ...
   `begin()`. Adding a protocol = write its module + one guarded line in `proto_builtins.cpp`
    - never editing the dispatcher. (The SSH remote-forward listener still self-registers from
      its own opt-in `dws_ssh_forward_begin()`.)
...
## Unified arena primitive (`session/dws_arena`)
...
onto it: that pool uses only a bump end, so routing it through `dws_arena` would leave
