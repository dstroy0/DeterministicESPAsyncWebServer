        ...
        if s.startswith("#if ") or s.startswith("#ifdef ") or s.startswith("#ifndef "):
            ...
            if "requires" in msg:
                for prereq in prereqs:
                    if prereq.startswith("PC_ENABLE_"):
...
def dot(hard, theme):
    ...
    out = [
        "// GENERATED by ci_tooling/generate/gen_flag_deps.py - do not edit by hand.",
...
def build_block():
    ...
    # Pre-render to a PNG (via ci_tooling/assets/render_diagrams.sh) so it shows in the GitHub app + Doxygen too.
    ...
    lines = [
        ...
        "> Generated from the `#error` / `#if` guards in [src/protocore_config.h](src/protocore_config.h) by"
        " `ci_tooling/generate/gen_flag_deps.py` - do not edit by hand. Pre-rendered PNG (shows in the GitHub"
