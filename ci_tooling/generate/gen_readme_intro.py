...
def test_numbers():
    ...
    return (int(passed.group(1)) if passed else None,
            int(envs.group(1)) if envs else None)


def coverage_exclusions():
    """Globs excluded from coverage measurement, read from the sonar config.

    Stating the excluded surface is the difference between an honest percentage
    and a flattering one, and reading it here means the caveat cannot go stale
    when the exclusion list changes.
    """
    m = re.search(r"^sonar\.coverage\.exclusions=(.+)$", read(SONAR), re.M)
    if not m:
        return []
...
def render_status():
    ...
    ex = coverage_exclusions()
    if ex:
        out.append("")
        out.append("Coverage is measured over `src/`, excluding "
                   + ", ".join(f"`{g}`" for g in ex)
                   + " (vendor-backed glue with nothing to measure on a native build). "
                     "Numbers come from `test/coverage.xml` and `test/TEST_REPORT.md`, both regenerated by CI.")
    return "\n".join(out)
