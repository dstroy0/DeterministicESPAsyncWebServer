...
pc_aes128 *pc_aes128_wants(void)
{
    ...
    return pc_span_ok(ws) ? reinterpret_cast<pc_aes128 *>(ws.buf) : nullptr;
}
...
inline void set_j0(Aes128GcmWork *w, const uint8_t nonce[12])
{
    memcpy(w->j0, nonce, 12);
    w->j0[12] = 0;
    w->j0[13] = 0;
...
void gcm_tag(Aes128GcmWork *w, const uint8_t *aad, size_t aad_len, const uint8_t *cipher, size_t cipher_len,
             uint8_t tag_out[16])
{
    memset(w->acc, 0, 16);
    ghash_update(&w->ghk, w->acc, aad, aad_len);
...
{
    Aes128GcmWork *w = reinterpret_cast<Aes128GcmWork *>(storage);
    pc_aes128_init(&w->aes, key);
    memset(w->h, 0, 16);
    pc_aes128_encrypt_block(&w->aes, w->h, w->h); // H = E(K, 0^128)
    ghash_key_init(&w->ghk, w->h);
    return reinterpret_cast<pc_aes128gcm_key *>(w);
}

void pc_aes128gcm_key_wipe(pc_aes128gcm_key *k)
{
    Aes128GcmWork *w = reinterpret_cast<Aes128GcmWork *>(k);
    pc_aes128_wipe(&w->aes);
    pc_secure_wipe(reinterpret_cast<uint8_t *>(w), sizeof(Aes128GcmWork));
}
