...
{
    mbedtls_gcm_context *g = reinterpret_cast<mbedtls_gcm_context *>(storage);
    mbedtls_gcm_init(g);
    ...
    return reinterpret_cast<pc_aes128gcm_key *>(g);
}

void pc_aes128gcm_key_wipe(pc_aes128gcm_key *k)
{
    mbedtls_gcm_context *g = reinterpret_cast<mbedtls_gcm_context *>(k);
    mbedtls_gcm_free(g); // releases whatever the vendor attached
    pc_secure_wipe(reinterpret_cast<uint8_t *>(g), sizeof(mbedtls_gcm_context));
}
