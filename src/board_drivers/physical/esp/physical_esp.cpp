...
bool init_eth_physical(void)
{
    return false; // Ethernet not enabled (PC_ENABLE_ETHERNET)
}
bool eth_ready(void)
{
    return false;
...
bool init_ipv6_physical(void)
{
    return false; // IPv6 not enabled (PC_ENABLE_IPV6)
}
...
struct PhyMonitorCtx
{
    pc_phy_frame_fn sink;
};
PhyMonitorCtx s_phy_monitor = {nullptr};

// Vendor shape in, neutral shape out. sig_len includes the 4-byte FCS, which no caller wants.
void phy_monitor_trampoline(void *buf, wifi_promiscuous_pkt_type_t)
{
