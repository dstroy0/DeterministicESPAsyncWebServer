...
/**
 * @file radio_sniff.h
 * @brief Receive-only radio channel sniffer -> pcap capture records (DETWS_ENABLE_RADIO_SNIFF).
 *
 ...
 * and Wi-Fi captures (shared_primitives/det_pcap). For 802.15.4 that means wrapping each frame in the
 ...
 *
 * This is that pure framing: ::detws_radiosniff_global writes the pcap global header (DLT TAP), and
 * ::detws_radiosniff_tap_record writes one record (TAP header + RSSI/channel TLVs + the MAC frame). The
