...
#include "services/system/clock.h" // pc_millis() (pulls in Arduino millis())
#endif
pc_ota_action pc_ota_decide(uint8_t img_state, bool self_test_ok, uint32_t ms_since_boot, uint32_t window_ms)
{
    ...
    {
        return pc_ota_action::PC_OTA_WAIT; // not a freshly-updated image: nothing to do
    }
    if (self_test_ok)
    {
        return pc_ota_action::PC_OTA_COMMIT;
    }
    if (ms_since_boot >= window_ms)
    {
        return pc_ota_action::PC_OTA_ROLLBACK; // never confirmed in time -> self-heal
    }
    return pc_ota_action::PC_OTA_WAIT;
}
...
pc_ota_action pc_ota_rollback_tick(bool self_test_ok)
{
    ...
    else if (a == pc_ota_action::PC_OTA_ROLLBACK)
    {
        pc_ota_rollback();
    }
    return a;
...
pc_ota_action pc_ota_rollback_tick(bool)
{
    return pc_ota_action::PC_OTA_WAIT;
}
