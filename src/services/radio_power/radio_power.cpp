...
const char *dws_radio_ps_name(uint8_t mode)
{
    switch (mode)
    {
    case DetwsRadioPs::DWS_PS_MIN_MODEM:
        return "min_modem";
    case DetwsRadioPs::DWS_PS_MAX_MODEM:
        return "max_modem";
    case DetwsRadioPs::DWS_PS_NONE:
        return "none";
    default:
        return "none";
    }
...
{
    if (mode == DetwsRadioPs::DWS_PS_MIN_MODEM)
        return WIFI_PS_MIN_MODEM;
    if (mode == DetwsRadioPs::DWS_PS_MAX_MODEM)
...
{
    esp_wifi_set_ps(to_esp_ps(DWS_RADIO_WIFI_PS));
#if DWS_RADIO_MAX_TX_DBM > 0
    esp_wifi_set_max_tx_power((int8_t)(DWS_RADIO_MAX_TX_DBM * 4)); // API unit: 0.25 dBm
#endif
}
...
uint8_t dws_radio_ps_get(void)
{
    ...
    if (esp_wifi_get_ps(&m) != ESP_OK)
        return DetwsRadioPs::DWS_PS_NONE;
    if (m == WIFI_PS_MIN_MODEM)
        return DetwsRadioPs::DWS_PS_MIN_MODEM;
    if (m == WIFI_PS_MAX_MODEM)
        return DetwsRadioPs::DWS_PS_MAX_MODEM;
    return DetwsRadioPs::DWS_PS_NONE;
}
...
void dws_radio_busy_release(void)
{
    if (s_busy.held > 0 && --s_busy.held == 0)
        dws_radio_power_apply(); // last transfer done: restore the configured mode
}
...
void dws_radio_busy_release(void)
{
    // no-op on the host build (see dws_radio_busy_hold).
}
