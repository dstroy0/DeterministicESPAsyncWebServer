...
void dws_rtcm_bw_s(RtcmBitWriter *w, int64_t val, uint8_t nbits)
{
    ...
    dws_rtcm_bw_u(w, (uint64_t)val & mask, nbits);
}
...
{
    uint64_t v = dws_rtcm_br_u(buf, pos, nbits);
...
size_t dws_rtcm3_frame_parse(const uint8_t *buf, size_t len, Rtcm3Frame *out)
{
    if (len < RTCM3_HDR_LEN || buf[0] != RTCM3_PREAMBLE)
        return 0; // not aligned to a preamble (caller runs dws_rtcm3_sync first)
    ...
    uint32_t crc_calc = dws_rtcm3_crc24q(buf, (size_t)RTCM3_HDR_LEN + payload_len);
    uint32_t crc_frame =
    ...
    if (out)
    {
        ...
        size_t p = 0;
        out->msg_type = (payload_len >= 2) ? (uint16_t)dws_rtcm_br_u(payload, &p, 12) : 0;
    }
    return frame_len;
}
...
size_t dws_rtcm3_frame_build(uint8_t *out, size_t cap, const uint8_t *payload, uint16_t payload_len)
{
    ...
    uint32_t crc = dws_rtcm3_crc24q(out, (size_t)RTCM3_HDR_LEN + payload_len);
                 ...
                 bool with_h, uint16_t h)
{
    ...
    RtcmBitWriter w;
    dws_rtcm_bw_init(&w, payload, sizeof payload);
    dws_rtcm_bw_u(&w, msg, 12);                // DF002 message number
    dws_rtcm_bw_u(&w, station_id & 0xFFF, 12); // DF003 reference station id
    dws_rtcm_bw_u(&w, 0, 6);                   // DF021 ITRF realization year (unspecified)
    dws_rtcm_bw_u(&w, 1, 1);                   // DF022 GPS indicator
    dws_rtcm_bw_u(&w, 0, 1);                   // DF023 GLONASS indicator
    dws_rtcm_bw_u(&w, 0, 1);                   // DF024 Galileo indicator
    dws_rtcm_bw_u(&w, 0, 1);                   // DF141 reference-station indicator (0 = real/physical)
    dws_rtcm_bw_s(&w, x, 38);                  // DF025 ECEF-X
    dws_rtcm_bw_u(&w, 0, 1);                   // DF142 single receiver oscillator indicator
    dws_rtcm_bw_u(&w, 0, 1);                   // reserved
    dws_rtcm_bw_s(&w, y, 38);                  // DF026 ECEF-Y
    dws_rtcm_bw_u(&w, 0, 2);                   // DF364 quarter cycle indicator
    dws_rtcm_bw_s(&w, z, 38);                  // DF027 ECEF-Z
...
bool dws_rtcm3_parse_1005(const uint8_t *payload, uint16_t payload_len, Rtcm3StationArp *out)
{
    if (!payload || !out || payload_len < 19)
        return false;
    size_t p = 0;
    uint16_t msg = (uint16_t)dws_rtcm_br_u(payload, &p, 12);
    if (msg != 1005 && msg != 1006)
        return false;
    if (msg == 1006 && payload_len < 21)
        return false;
    out->station_id = (uint16_t)dws_rtcm_br_u(payload, &p, 12);
    dws_rtcm_br_u(payload, &p, 6); // DF021
    dws_rtcm_br_u(payload, &p, 1); // DF022
    dws_rtcm_br_u(payload, &p, 1); // DF023
    dws_rtcm_br_u(payload, &p, 1); // DF024
    dws_rtcm_br_u(payload, &p, 1); // DF141
    out->ecef_x_01mm = dws_rtcm_br_s(payload, &p, 38);
    dws_rtcm_br_u(payload, &p, 1); // DF142
    dws_rtcm_br_u(payload, &p, 1); // reserved
    out->ecef_y_01mm = dws_rtcm_br_s(payload, &p, 38);
    dws_rtcm_br_u(payload, &p, 2); // DF364
    out->ecef_z_01mm = dws_rtcm_br_s(payload, &p, 38);
    if (msg == 1006)
    {
        out->antenna_height_01mm = (uint16_t)dws_rtcm_br_u(payload, &p, 16);
        out->has_height = true;
    }
    else
    {
