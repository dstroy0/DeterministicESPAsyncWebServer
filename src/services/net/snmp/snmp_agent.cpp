...
struct SnmpAgentCtx
{
    ...
    size_t mib_count = 0;
    char ro[SNMP_COMMUNITY_MAX] = PC_SNMP_DEFAULT_RO_COMMUNITY;
...
static_assert(sizeof(PC_SNMP_DEFAULT_RO_COMMUNITY) > 1,
...
bool pc_snmp_agent_add_string(const uint32_t *oid, size_t oid_len, const char *value, SnmpSetFn setter)
{
...
bool pc_snmp_agent_add_integer(const uint32_t *oid, size_t oid_len, long value, SnmpSetFn setter)
{
    SnmpMibEntry *e = mib_alloc(s_agent, oid, oid_len);
    if (!e)
    {
                              ...
                              long services)
{
    ...
    pc_snmp_agent_add_string(o_descr, 9, descr);
    ...
    pc_snmp_agent_add_dynamic(o_uptime, 9, (uint8_t)SnmpTag::SNMP_TIMETICKS, sys_uptime_get);
    pc_snmp_agent_add_string(o_contact, 9, contact);
    pc_snmp_agent_add_string(o_name, 9, name);
    pc_snmp_agent_add_string(o_loc, 9, location);
    pc_snmp_agent_add_integer(o_svc, 9, services);
}
...
static void enc_value(BerEnc *e, const SnmpValue *v)
{
    ...
    case (uint8_t)SnmpTag::SNMP_IPADDRESS: {
        uint8_t ip[4] = {(uint8_t)(v->uval >> 24), (uint8_t)(v->uval >> 16), (uint8_t)(v->uval >> 8),
                         (uint8_t)(v->uval)};
        pc_ber_put_octet_string(e, (uint8_t)SnmpTag::SNMP_IPADDRESS, ip, 4);
        break;
    }
    ...
    default:
        ...
        pc_ber_put_tlv(e, v->type, nullptr, 0);
        break;
    }
}
...
// 4 deep (S134).
static void pc_snmp_apply_set_all(size_t nvb, bool v2c, long *err_status, long *err_index)
{
                         ...
                         size_t cap)
{
    ...
    size_t pdu = pc_ber_seq_begin(&e, (uint8_t)SnmpTag::SNMP_PDU_RESPONSE);
                            ...
                            size_t out_cap)
{
    ...
    uint8_t vbl_tag;
    size_t vbl_len;
    if (!pc_ber_read_header(&d, &vbl_tag, &vbl_len) || vbl_tag != (uint8_t)SnmpTag::BER_SEQUENCE)
    {
        return 0;
    }
    ...
    while (d.pos < vbl_end && d.ok) // GCOVR_EXCL_LINE  d.ok is always true here (see above)
    {
        ...
        uint8_t vt;
        size_t vlen;
        if (!pc_ber_read_header(&d, &vt, &vlen) || vt != (uint8_t)SnmpTag::BER_SEQUENCE)
        {
            return 0;
        }
...
size_t pc_snmp_agent_process(const uint8_t *req, size_t req_len, uint8_t *resp, size_t pc_resp_cap)
{
    ...
    uint8_t tag;
    size_t len;
    if (!pc_ber_read_header(&d, &tag, &len) || tag != (uint8_t)SnmpTag::BER_SEQUENCE) // message wrapper
    {
        return 0;
    }
    ...
    uint8_t ctag;
    size_t clen;
    if (!pc_ber_read_header(&d, &ctag, &clen) || ctag != (uint8_t)SnmpTag::BER_OCTET_STRING)
    {
        return 0;
    }
    ...
    size_t pn = pc_snmp_dispatch_pdu(req + d.pos, req_len - d.pos, is_rw, version == (int)SnmpVersion::SNMP_V2C, pdubuf,
                                     sizeof(pdubuf));
    if (pn == 0)
    {
        return 0;
    ...
    pc_ber_put_octet_string(&e, (uint8_t)SnmpTag::BER_OCTET_STRING, (const uint8_t *)community, community_len);
...
{
    pc_udp_listen(port, pc_snmp_udp_handler, nullptr);
}
