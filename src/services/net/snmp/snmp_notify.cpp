...
{
    size_t t = pc_ber_seq_begin(e, (uint8_t)SnmpTag::BER_SEQUENCE);
    ...
    case (uint8_t)SnmpVbType::SNMP_VB_STRING:
        pc_ber_put_octet_string(e, (uint8_t)SnmpTag::BER_OCTET_STRING, vb->bytes, vb->blen);
        break;
    case (uint8_t)SnmpVbType::SNMP_VB_OID:
        pc_ber_put_oid(e, vb->oid_val, vb->oid_val_len);
        break;
    case (uint8_t)SnmpVbType::SNMP_VB_COUNTER32:
        pc_ber_put_uint(e, (uint8_t)SnmpTag::SNMP_COUNTER32, (uint32_t)vb->ival);
        break;
    case (uint8_t)SnmpVbType::SNMP_VB_GAUGE32:
        pc_ber_put_uint(e, (uint8_t)SnmpTag::SNMP_GAUGE32, (uint32_t)vb->ival);
        break;
    case (uint8_t)SnmpVbType::SNMP_VB_TIMETICKS:
        pc_ber_put_uint(e, (uint8_t)SnmpTag::SNMP_TIMETICKS, (uint32_t)vb->ival);
        break;
    case (uint8_t)SnmpVbType::SNMP_VB_IPADDR:
        pc_ber_put_octet_string(e, (uint8_t)SnmpTag::SNMP_IPADDRESS, vb->bytes, vb->blen);
        break;
    default:
        e->ok = false;
        break;
                                ...
                                size_t trap_oid_len, uint32_t uptime_ticks, const SnmpVarbind *vbs, size_t n)
{
    ...
    // sysUpTime.0 = TimeTicks
    {
        ...
        pc_ber_put_uint(e, (uint8_t)SnmpTag::SNMP_TIMETICKS, uptime_ticks);
        pc_ber_seq_end(e, t);
    }
    // snmpTrapOID.0 = OID
    {
                                ...
                                const SnmpVarbind *vbs, size_t n)
{
    ...
    pc_ber_put_octet_string(&e, (uint8_t)SnmpTag::BER_OCTET_STRING, (const uint8_t *)community,
                      ...
                      size_t trap_oid_len, const SnmpVarbind *vbs, size_t n)
{
    ...
    size_t len = pc_snmp_notify_build_v2c(buf, sizeof(buf), community, (uint8_t)SnmpTag::SNMP_PDU_TRAPV2,
...
bool pc_snmp_trap_v2c(const char *, uint16_t, const char *, const uint32_t *, size_t, const SnmpVarbind *, size_t)
{
    return false;
}
bool pc_snmp_inform_v2c(const char *, uint16_t, const char *, uint32_t, const uint32_t *, size_t, const SnmpVarbind *,
                        size_t)
{
    return false;
}
