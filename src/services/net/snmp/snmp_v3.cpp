...
// working buffers (staggered lifetimes; see pc_snmp_v3_process), grouped so it is one named owner,
...
void pc_snmp_v3_set_user(const char *user, const char *auth_pass, const char *priv_pass)
{
...
void pc_snmp_v3_set_boots(uint32_t boots)
{
    s_v3.boots = boots;
}
uint32_t pc_snmp_v3_get_boots()
{
    return s_v3.boots;
}
                         ...
                         const uint8_t **pdu, size_t *pdu_len)
{
    ...
    uint8_t tag;
    size_t l;
    if (!pc_ber_read_header(&d, &tag, &l) || tag != (uint8_t)SnmpTag::BER_SEQUENCE)
    {
        return false;
    }
    if (!pc_ber_read_header(&d, &tag, &l) || tag != (uint8_t)SnmpTag::BER_OCTET_STRING) // contextEngineID
    {
        return false;
    }
    d.pos += l;
    if (!pc_ber_read_header(&d, &tag, &l) || tag != (uint8_t)SnmpTag::BER_OCTET_STRING) // contextName
    {
        return false;
    }
    *ctxname = d.buf + d.pos;
    *ctxname_len = l;
    ...
    if (!pc_ber_read_header(&d, &tag, &l)) // PDU header (kept)
    {
        return false;
    }
    *pdu = buf + pdu_start;
...
static size_t build_message(long msg_id, bool auth, bool priv, const uint8_t *scoped, size_t scoped_len, uint8_t *resp,
                            size_t pc_resp_cap)
{
    uint32_t now = pc_snmp_v3_uptime_s();
    ...
    if (priv)
    {
        pc_wr32be(salt, s_v3.boots);
        pc_wr32be(salt + 4, ++s_v3.salt_ctr);
        uint8_t iv[16];
        ...
        pc_snmp_aes128_cfb(s_v3.priv_key, iv, scoped, s_v3.v3_d, scoped_len, true);
        data_ptr = s_v3.v3_d;
    }

    // msgSecurityParameters (a SEQUENCE, later wrapped in an OCTET STRING).
    BerEnc se;
    pc_ber_enc_init(&se, s_v3.v3_sec, sizeof(s_v3.v3_sec));
    size_t ss = pc_ber_seq_begin(&se, (uint8_t)SnmpTag::BER_SEQUENCE);
    pc_ber_put_octet_string(&se, (uint8_t)SnmpTag::BER_OCTET_STRING, s_v3.engine_id, s_v3.engine_id_len);
    pc_ber_put_integer(&se, (long)s_v3.boots);
    pc_ber_put_integer(&se, (long)now);
    pc_ber_put_octet_string(&se, (uint8_t)SnmpTag::BER_OCTET_STRING, (const uint8_t *)s_v3.user,
    ...
    if (auth)
    {
        ...
        pc_ber_put_octet_string(&se, (uint8_t)SnmpTag::BER_OCTET_STRING, zeros, SNMP_V3_AUTH_PARAM_LEN);
    }
    else
    {
        pc_ber_put_octet_string(&se, (uint8_t)SnmpTag::BER_OCTET_STRING, nullptr, 0);
    }
    if (priv)
    {
        pc_ber_put_octet_string(&se, (uint8_t)SnmpTag::BER_OCTET_STRING, salt, SNMP_V3_PRIV_PARAM_LEN);
    }
    else
    {
        pc_ber_put_octet_string(&se, (uint8_t)SnmpTag::BER_OCTET_STRING, nullptr, 0);
    ...
    size_t msg = pc_ber_seq_begin(&e, (uint8_t)SnmpTag::BER_SEQUENCE);
    pc_ber_put_integer(&e, (int)SnmpVersion::SNMP_V3);
    size_t hdr = pc_ber_seq_begin(&e, (uint8_t)SnmpTag::BER_SEQUENCE);
    pc_ber_put_integer(&e, msg_id);
    pc_ber_put_integer(&e, 65507); // msgMaxSize
    uint8_t fl = (uint8_t)((auth ? 0x01 : 0) | (priv ? 0x02 : 0));
    pc_ber_put_octet_string(&e, (uint8_t)SnmpTag::BER_OCTET_STRING, &fl, 1);
    pc_ber_put_integer(&e, 3); // msgSecurityModel = USM
    pc_ber_seq_end(&e, hdr);
    pc_ber_put_octet_string(&e, (uint8_t)SnmpTag::BER_OCTET_STRING, s_v3.v3_sec, sec_len);
    size_t sec_value_pos = e.len - sec_len;
    if (priv)
    {
        pc_ber_put_octet_string(&e, (uint8_t)SnmpTag::BER_OCTET_STRING, data_ptr, data_len);
    }
    else
    {
        pc_ber_put_raw(&e, data_ptr, data_len);
    }
                           ...
                           size_t pc_resp_cap)
{
    ...
    BerEnc e;
    pc_ber_enc_init(&e, s_v3.v3_b, sizeof(s_v3.v3_b));
    size_t pdu = pc_ber_seq_begin(&e, (uint8_t)SnmpTag::SNMP_PDU_REPORT);
    pc_ber_put_integer(&e, request_id);
    pc_ber_put_integer(&e, 0);
    pc_ber_put_integer(&e, 0);
    size_t vbl = pc_ber_seq_begin(&e, (uint8_t)SnmpTag::BER_SEQUENCE);
    size_t vb = pc_ber_seq_begin(&e, (uint8_t)SnmpTag::BER_SEQUENCE);
    pc_ber_put_oid(&e, oid, 11);
    pc_ber_put_uint(&e, (uint8_t)SnmpTag::SNMP_COUNTER32, count);
    pc_ber_seq_end(&e, vb);
    ...
    BerEnc sc;
    pc_ber_enc_init(&sc, s_v3.v3_c, sizeof(s_v3.v3_c));
    size_t s = pc_ber_seq_begin(&sc, (uint8_t)SnmpTag::BER_SEQUENCE);
    pc_ber_put_octet_string(&sc, (uint8_t)SnmpTag::BER_OCTET_STRING, s_v3.engine_id, s_v3.engine_id_len);
    pc_ber_put_octet_string(&sc, (uint8_t)SnmpTag::BER_OCTET_STRING, nullptr, 0);
    ...
    return build_message(msg_id, auth, false, s_v3.v3_c, sc.len, resp, pc_resp_cap);
}
...
size_t pc_snmp_v3_process(const uint8_t *req, size_t req_len, uint8_t *resp, size_t pc_resp_cap)
{
    ...
    long version;
    if (!pc_ber_read_integer(&d, &version) || version != (int)SnmpVersion::SNMP_V3)
    {
        return 0;
    }
    ...
    long msg_max;
    long sec_model;
    if (!pc_ber_read_integer(&d, &msg_id) || !pc_ber_read_integer(&d, &msg_max))
    {
        return 0;
    }
    (void)msg_max;
    size_t flags_len;
    if (!pc_ber_read_header(&d, &tag, &flags_len) || tag != (uint8_t)SnmpTag::BER_OCTET_STRING || flags_len < 1)
    {
        return 0;
    }
    uint8_t flags = d.buf[d.pos];
    d.pos += flags_len;
    if (!pc_ber_read_integer(&d, &sec_model) || sec_model != 3) // USM only
    {
        return 0;
    }
    ...
    size_t sec_len;
    if (!pc_ber_read_header(&d, &tag, &sec_len) || tag != (uint8_t)SnmpTag::BER_OCTET_STRING)
    {
        return 0;
    }
    ...
    BerDec sd;
    pc_ber_dec_init(&sd, sec, sec_len);
    if (!pc_ber_read_header(&sd, &tag, &l) || tag != (uint8_t)SnmpTag::BER_SEQUENCE)
    {
        return 0;
    }
    size_t eid_len;
    if (!pc_ber_read_header(&sd, &tag, &eid_len) || tag != (uint8_t)SnmpTag::BER_OCTET_STRING)
    {
        return 0;
    }
    ...
    long req_boots;
    long req_time;
    if (!pc_ber_read_integer(&sd, &req_boots) || !pc_ber_read_integer(&sd, &req_time))
    {
        return 0;
    }
    size_t uname_len;
    if (!pc_ber_read_header(&sd, &tag, &uname_len) || tag != (uint8_t)SnmpTag::BER_OCTET_STRING)
    {
        return 0;
    }
    ...
    sd.pos += uname_len;
    size_t aparm_len;
    if (!pc_ber_read_header(&sd, &tag, &aparm_len) || tag != (uint8_t)SnmpTag::BER_OCTET_STRING)
    {
        return 0;
    }
    ...
    sd.pos += aparm_len;
    size_t pparm_len;
    if (!pc_ber_read_header(&sd, &tag, &pparm_len) || tag != (uint8_t)SnmpTag::BER_OCTET_STRING)
    {
        return 0;
    }
    ...
    if (!engine_match)
    {
        ...
        return build_report(msg_id, false, (int)UsmStat::USM_STAT_UNKNOWN_ENGINE, s_v3.stat_unknown_engine,
                            inner_request_id(mdata, mdata_len, req_priv), resp, pc_resp_cap);
    }
    ...
    if (req_priv)
    {
        ...
        {
            s_v3.stat_decrypt++;
            return build_report(msg_id, true, (int)UsmStat::USM_STAT_DECRYPT, s_v3.stat_decrypt, 0, resp, pc_resp_cap);
        }
        BerDec md;
        pc_ber_dec_init(&md, mdata, mdata_len);
        if (!pc_ber_read_header(&md, &tag, &l) || tag != (uint8_t)SnmpTag::BER_OCTET_STRING)
        {
            return 0;
        }
        ...
        pc_snmp_aes128_cfb(s_v3.priv_key, iv, ct, s_v3.v3_a, ct_len, false);
        scoped = s_v3.v3_a;
        scoped_len = ct_len;
    }
    else
    ...
    size_t rpdu = pc_snmp_dispatch_pdu(pdu, pdu_len, true, true, s_v3.v3_b, sizeof(s_v3.v3_b));
    if (rpdu == 0)
    {
        return 0;
    }
    ...
    size_t s = pc_ber_seq_begin(&sc, (uint8_t)SnmpTag::BER_SEQUENCE);
    pc_ber_put_octet_string(&sc, (uint8_t)SnmpTag::BER_OCTET_STRING, s_v3.engine_id, s_v3.engine_id_len);
    pc_ber_put_octet_string(&sc, (uint8_t)SnmpTag::BER_OCTET_STRING, ctxname, ctxname_len);
    pc_ber_put_raw(&sc, s_v3.v3_b, rpdu);
    pc_ber_seq_end(&sc, s);
    if (!sc.ok)
    {
        return 0;
    ...
    return build_message(msg_id, true, req_priv, s_v3.v3_c, sc.len, resp, pc_resp_cap);
}
...
// pc_snmp_v3_init()/pc_snmp_v3_set_user() and the same build_message() as responses.
                           ...
                           const uint32_t *trap_oid, size_t trap_oid_len, const SnmpVarbind *vbs, size_t n)
{
    ...
    BerEnc e;
    pc_ber_enc_init(&e, s_v3.v3_b, sizeof(s_v3.v3_b));
    pc_snmp_notify_build_pdu(&e, pdu_tag, request_id, trap_oid, trap_oid_len, pc_snmp_v3_uptime_s() * 100, vbs, n);
    if (!e.ok)
    {
        return false;
    }
...
// pc_snmp_inform_v2c: builds + sends the InformRequest. The caller owns the
