...
enum class pc_gateway_kind : uint8_t
{
    PC_GW_OTHER = 0,
    PC_GW_LORA,
    PC_GW_NRF24,
 ...
 */
typedef bool (*pc_gateway_uplink_fn)(const pc_gateway_msg *msg, void *ctx);
 ...
 */
typedef bool (*pc_gateway_tx_fn)(uint8_t port_id, uint16_t dst_addr, const uint8_t *payload, uint16_t len, void *ctx);

/** @brief Southbound port (radio / bus) configuration passed to pc_gateway_add_port(). */
struct pc_gateway_port_config
{
 ...
 */
bool pc_gateway_add_port(const pc_gateway_port_config *cfg);
 ...
 */
bool pc_gateway_uplink(uint8_t port_id, uint16_t src_addr, const uint8_t *payload, uint16_t len, int16_t rssi);
 ...
 */
bool pc_gateway_downlink(uint8_t port_id, uint16_t dst_addr, const uint8_t *payload, uint16_t len);
