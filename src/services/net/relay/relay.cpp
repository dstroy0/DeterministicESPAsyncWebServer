...
static int pump_refill(pc_relay_end *src, pc_relay_end *dst, uint8_t *buf, uint16_t *len, uint16_t *off, bool *src_eof,
                       uint32_t *counter)
{
...
static int pump(pc_relay_end *src, pc_relay_end *dst, uint8_t *buf, uint16_t *len, uint16_t *off, bool *src_eof,
...
pc_relay_status pc_relay_step(pc_relay *r)
{
    if (!r)
    {
        return pc_relay_status::PC_RELAY_ERROR;
    }
    ...
    return (r->a2b_done && r->b2a_done) ? pc_relay_status::PC_RELAY_DONE : pc_relay_status::PC_RELAY_RUNNING;
}
