...
#include "services/system/radio_power/radio_power.h" // keep the radio awake during a relayed transfer
#endif
#include <string.h>
...
struct RelayBind
{
    bool active;
    uint8_t listener_id;
    char host[PC_RELAY_HOST_MAX];
    uint16_t port;
};

// One live relayed connection: an inbound conn slot bridged to an origin pc_client.
struct RelayBridge
{
    bool active;
    uint8_t conn_slot;
...
{
    RelayBind binds[PC_RELAY_MAX_PUBLISH];
...
int a_recv(void *c, uint8_t *buf, size_t cap)
{
    ...
    {
        return (int)pc_conn_read(br->conn_slot, buf, cap);
    }
    return 0;
}
...
void service(uint8_t slot)
{
    ...
    // so one poll forwards the whole buffered origin RX ring (PC_CLIENT_RX_BUF) instead of a single
    // PC_RELAY_BUF chunk. Bounded by PC_RELAY_DRAIN_MAX so one busy bridge cannot starve others.
    for (int pass = 0; pass < PC_RELAY_DRAIN_MAX; pass++)
    {
        uint32_t moved = br->relay.bytes_a2b + br->relay.bytes_b2a;
        pc_relay_status st = pc_relay_step(&br->relay);
        if (st == pc_relay_status::PC_RELAY_ERROR || st == pc_relay_status::PC_RELAY_DONE)
        {
            teardown(br, true);
            return;
        }
...
void relay_on_accept(uint8_t slot)
{
    ...
    br->conn_slot = slot;
    br->origin_cid = cid;
    pc_relay_end a = {a_recv, a_send, nullptr, br};
    pc_relay_end b = {b_recv, b_send, nullptr, br};
...
bool pc_relay_publish(uint8_t listener_id, const char *origin_host, uint16_t origin_port)
{
    ...
    size_t hl = strnlen(origin_host, PC_RELAY_HOST_MAX + 1);
    if (hl == 0 || hl >= PC_RELAY_HOST_MAX)
    {
        return false;
    }
    int idx = -1;
