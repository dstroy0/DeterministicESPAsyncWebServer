...
struct ForwardCtx
{
    ...
    pc_fwd_action acl_default = pc_fwd_action::PC_FWD_ALLOW; // frames matching no ACL entry (opt-in ACL)
#if PC_FWD_INSPECT
    pc_fwd_inspect_fn inspector = nullptr; // opt-in ingress inspection hook
...
resolve_result resolve(const ForwardCtx &f, uint8_t src, uint8_t dst, int *allow_idx)
{
    ...
    for (uint8_t i = 0; i < PC_FWD_MAX_RULES; i++)
    {
        ...
        if (f.rules[i].action == pc_fwd_action::PC_FWD_DENY)
        {
            deny = true;
        }
        else if (allow < 0)
...
bool acl_permits(const ForwardCtx &f, uint8_t src, const uint8_t *data, uint16_t len)
{
    for (uint8_t i = 0; i < PC_FWD_MAX_ACL; i++)
    {
        ...
        {
            return f.acl[i].action == pc_fwd_action::PC_FWD_ALLOW;
        }
    }
    return f.acl_default == pc_fwd_action::PC_FWD_ALLOW;
}
} // namespace
...
void pc_forward_reset(void)
{
    ...
    s_fwd.acl_default = pc_fwd_action::PC_FWD_ALLOW;
#if PC_FWD_INSPECT
...
uint8_t pc_forward_ingress(uint8_t src_if, const uint8_t *data, uint16_t len)
{
    ...
    if (s_fwd.inspector && s_fwd.inspector(src_if, data, len, s_fwd.inspect_ctx) == pc_fwd_verdict::PC_FWD_INSPECT_DROP)
    {
