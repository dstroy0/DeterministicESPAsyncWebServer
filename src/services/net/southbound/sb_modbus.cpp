...
int sb_modbus_read_span(pc_sb_modbus_ctx *c, uint32_t first, int32_t *out, size_t n)
{
...
// a malformed reply.
int sb_modbus_write_txn(pc_sb_modbus_ctx *c, const uint8_t *req, size_t req_len)
{
    uint8_t resp[MODBUS_ADU_MAX];
    ...
    uint8_t ex = 0;
    int w = pc_modbus_parse_write_response(resp, (size_t)pn, nullptr, &ex);
    if (w < 0)
    {
        return Sb::SB_ERR_ARG; // malformed / short frame
    }
