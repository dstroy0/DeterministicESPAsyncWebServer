...
void write_reg(const pc_cc1101_bus *b, uint8_t addr, uint8_t val)
{
...
uint8_t read_reg(const pc_cc1101_bus *b, uint8_t addr, bool status)
{
...
void strobe(const pc_cc1101_bus *b, uint8_t cmd)
{
    uint8_t tx[1] = {cmd};
    uint8_t rx[1] = {0};
    b->spi(tx, rx, 1, b->ctx);
...
uint8_t status_byte(const pc_cc1101_bus *b)
{
