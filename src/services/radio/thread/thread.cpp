...
bool pc_spinel_get_uint(SpinelReader *r, uint32_t *out)
{
    if (!r || r->err)
    {
        return false;
...
bool pc_spinel_put_uint(SpinelWriter *w, uint32_t v)
{
    if (!w || w->err)
    {
        return false;
...
bool pc_spinel_put_utf8(SpinelWriter *w, const char *s)
{
    if (!s)
    {
        if (w)
...
bool pc_spinel_put_data(SpinelWriter *w, const uint8_t *d, uint16_t n)
{
    if (d == nullptr && n > 0)
    {
        if (w)
...
bool pc_spinel_put_data_wlen(SpinelWriter *w, const uint8_t *d, uint16_t n)
{
...
{
    if (!out || len > PC_THREAD_MAX_DATA || (payload == nullptr && len > 0))
    {
        return 0;
    }
