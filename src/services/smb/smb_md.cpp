...
// Compress one 64-byte block into the running hash state h[0..7] (FIPS 180-4 §6.2.2).
static void sha256_block(uint32_t h[8], const uint8_t blk[64])
{
    uint32_t W[64];
    for (int i = 0; i < 16; i++)
        W[i] = dws_rd32be(blk + i * 4); // the first 16 words are the block, big-endian
    for (int i = 16; i < 64; i++)
    {
...
void dws_sha256_final(Sha256Ctx *c, uint8_t out[32])
{
    ...
    while (c->buflen < 56)
        c->buf[c->buflen++] = 0x00;
    dws_wr64be(c->buf + 56, bitlen);
    sha256_block(c->s, c->buf);
    for (int i = 0; i < 8; i++)
        dws_wr32be(out + i * 4, c->s[i]);
}
...
void dws_sha256(const uint8_t *data, size_t len, uint8_t out[32])
{
    Sha256Ctx c;
    dws_sha256_init(&c);
    dws_sha256_update(&c, data, len);
    dws_sha256_final(&c, out);
}

// --- SHA-512 (FIPS 180-4) --------------------------------------------------

// Round constants K = frac(cbrt(first 80 primes)) and initial hash H0 = frac(sqrt(first 8 primes)),
// each the leading 64 bits (FIPS 180-4 §4.2.3 / §5.3.5).
...
// Compress one 128-byte block into the running hash state h[0..7] (FIPS 180-4 §6.4.2).
static void sha512_block(uint64_t h[8], const uint8_t blk[128])
{
    uint64_t W[80];
    for (int i = 0; i < 16; i++)
        W[i] = dws_rd64be(blk + i * 8); // the first 16 words are the block, big-endian
    for (int i = 16; i < 80; i++)
    {
...
void dws_sha512_final(Sha512Ctx *c, uint8_t out[64])
{
    ...
    while (c->buflen < 112)
        c->buf[c->buflen++] = 0x00;
    dws_wr64be(c->buf + 112, 0); // the high 64 bits of the 128-bit length are zero at these sizes
    dws_wr64be(c->buf + 120, bitlen);
    sha512_block(c->s, c->buf);
    for (int i = 0; i < 8; i++)
        dws_wr64be(out + i * 8, c->s[i]);
}
...
void dws_sha512(const uint8_t *data, size_t len, uint8_t out[64])
{
    Sha512Ctx c;
    dws_sha512_init(&c);
    dws_sha512_update(&c, data, len);
    dws_sha512_final(&c, out);
}
...
void dws_hmac_sha256(const uint8_t *key, size_t key_len, const uint8_t *msg, size_t msg_len, uint8_t out[32])
{
    ...
    memset(k, 0, sizeof(k));
    if (key_len > 64)
        dws_sha256(key, key_len, k); // a key longer than the block is hashed to 32 octets first
    else
        memcpy(k, key, key_len);
    ...
    dws_sha256_update(&c, msg, msg_len);
