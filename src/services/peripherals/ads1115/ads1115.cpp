...
{
    s_ads.addr = addr ? addr : (uint8_t)PC_ADS1115_I2C_ADDR;
    pc_i2c_begin();
    return true;
}
...
bool pc_ads1115_read_raw(uint8_t channel, uint8_t gain, int16_t *raw)
{
    ...
    static const uint16_t pc_ads1115_sps[8] = {8, 16, 32, 64, 128, 250, 475, 860};
    pcdelay(1000u / pc_ads1115_sps[dr] + 1);
    uint16_t v = 0;
...
{
    int16_t raw = 0;
    if (!pc_ads1115_read_raw(channel, gain, &raw))
    {
        return false;
    }
    if (microvolts)
    {
        *microvolts = pc_ads1115_raw_to_uv(raw, gain);
    }
    return true;
}
...
bool pc_ads1115_begin(uint8_t)
{
    return false;
}
bool pc_ads1115_read_raw(uint8_t, uint8_t, int16_t *)
{
    return false;
}
bool pc_ads1115_read_uv(uint8_t, uint8_t, int32_t *)
{
    return false;
}
