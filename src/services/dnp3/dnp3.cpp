                        ...
                        const uint8_t *user_data, size_t user_data_len)
{
    if (!buf || (user_data_len && !user_data) || user_data_len > DNP3_MAX_USER_DATA)
        return 0;
    size_t nblocks = (user_data_len + 15) / 16;
    size_t total = 10 + user_data_len + nblocks * 2; // 8 header + 2 header CRC + data + per-block CRCs
    if (total > cap)
        return 0;
    ...
    buf[p++] = DNP3_START0;
    buf[p++] = DNP3_START1;
    buf[p++] = (uint8_t)(5 + user_data_len); // LEN
    buf[p++] = control;
    buf[p++] = (uint8_t)(dest & 0xFF); // LE
    buf[p++] = (uint8_t)(dest >> 8);
    buf[p++] = (uint8_t)(src & 0xFF);
    buf[p++] = (uint8_t)(src >> 8);
    p += put_crc(buf + p, buf, 8); // header CRC over the 8 header octets
                      ...
                      size_t *out_user_len)
{
    ...
    if (length < 5) // LEN must at least cover CTRL + DEST + SRC
        return false;
    size_t user_len = (size_t)length - 5;
    if (user_len > DNP3_MAX_USER_DATA)
        return false;

    uint16_t hcrc = dnp3_crc(buf, 8);
    if ((uint16_t)(buf[8] | (buf[9] << 8)) != hcrc)
        return false; // header CRC mismatch

    size_t nblocks = (user_len + 15) / 16;
    size_t total = 10 + user_len + nblocks * 2;
    if (total > len)
