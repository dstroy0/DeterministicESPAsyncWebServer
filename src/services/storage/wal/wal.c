...
// CRC-32 core with no init/final xor, so it composes across a header + payload in two steps.
static uint32_t crc32_step(uint32_t crc, const uint8_t *d, size_t n)
{
    for (size_t i = 0; i < n; i++)
    {
        crc = CRC32_TABLE[(crc ^ d[i]) & 0xFFu] ^ (crc >> 8);
    }
    return crc;
}
