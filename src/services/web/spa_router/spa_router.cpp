...
pc_spa_action pc_spa_route(const char *path, const char *api_prefix)
{
    ...
    {
        return pc_spa_action::PC_SPA_SERVE_SHELL; // "" or "/" -> the app shell
    }
    ...
    if (api_prefix && api_prefix[0])
    {
        ...
        {
            return pc_spa_action::PC_SPA_PASSTHROUGH; // "/api/..." -> handlers
        }
    }

    return pc_spa_has_extension(path) ? pc_spa_action::PC_SPA_SERVE_FILE : pc_spa_action::PC_SPA_SERVE_SHELL;
}
...
pc_spa_action pc_spa_route_ex(const char *path, const pc_spa_ctx *ctx)
{
    if (!ctx)
    {
        return pc_spa_route(path, nullptr);
    }
    ...
    if (a != pc_spa_action::PC_SPA_SERVE_SHELL)
    {
        return a;
    }
    if (!ctx->shell_available || !ctx->client_scripting || ctx->degraded)
    {
        return pc_spa_action::PC_SPA_SERVE_FALLBACK;
    }
    return a;
}
