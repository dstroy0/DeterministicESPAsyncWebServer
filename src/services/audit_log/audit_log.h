 ...
 * record breaks the chain, which detws_audit_verify() detects in O(n). All
 * storage is a fixed RAM ring of DETWS_AUDIT_LOG_ENTRIES records (no heap,
 ...
 * and verification. Install a sink with detws_audit_set_sink() to forward every
 ...
 * detws_audit_format() to render a record as one JSON line for that sink.
 *
...
enum DetwsAuditCat
{
    ...
    DETWS_AUDIT_AUTH_FAIL = 2, ///< Authentication failure.
    DETWS_AUDIT_ACCESS = 3,    ///< Resource access (request served / denied).
    DETWS_AUDIT_CONFIG = 4,    ///< Configuration change.
    DETWS_AUDIT_ADMIN = 5,     ///< Privileged / administrative action.
};
...
typedef void (*detws_audit_sink_fn)(const DetwsAuditEntry *entry);
 ...
 *
 * The sink runs synchronously inside detws_audit_append(); keep it short and
 * non-reentrant (do not call detws_audit_append() from it).
 */
 ...
 */
int detws_audit_format(const DetwsAuditEntry *entry, char *out, size_t cap);
