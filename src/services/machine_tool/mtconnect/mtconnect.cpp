...
namespace
{
void put(pc_mtc_streams *s, const char *text)
{
...
void put_escaped(pc_mtc_streams *s, const char *text)
{
    // null text is a no-op: harden the helper itself, not just every call site.
...
void put_u64(pc_mtc_streams *s, uint64_t v)
{
    char tmp[20];
    int n = 0;
    do
...
const char *mtc_cat_str(pc_mtc_category cat)
{
    if (cat == pc_mtc_category::PC_MTC_SAMPLE)
    {
        return "SAMPLE";
    }
    if (cat == pc_mtc_category::PC_MTC_EVENT)
    {
        return "EVENT";
    }
    return "CONDITION";
                        ...
                        const char *timestamp, const char *value)
{
    ...
    else if (cat == pc_mtc_category::PC_MTC_EVENT)
    {
        wrap = "Events";
    }
    put(s, "<");
    put(s, wrap);
    put(s, ">");
    if (cat == pc_mtc_category::PC_MTC_CONDITION)
    {
...
void mtc_streams_begin_windowed(pc_mtc_streams *s, char *buf, size_t cap, uint64_t instance_id, uint64_t first_seq,
