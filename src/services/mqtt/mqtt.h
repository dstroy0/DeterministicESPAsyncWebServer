...
/**
 * @file mqtt.h
 * @brief Zero-heap MQTT 3.1.1 publish/subscribe client (DWS_ENABLE_MQTT).
 *
 ...
 *  - dws_mqtt_connect() / dws_mqtt_publish() / dws_mqtt_subscribe() / dws_mqtt_loop() resolve the
...
/** @brief Register the inbound-message callback (call before dws_mqtt_connect). */
void dws_mqtt_set_message_cb(MqttMessageCb cb);

/**
 * @brief Connect to a broker and complete the MQTT handshake (blocking).
 *
 * Resolves @p host, opens TCP (TLS when @p use_tls and DWS_ENABLE_MQTT_TLS),
