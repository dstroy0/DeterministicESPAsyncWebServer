...
bool pc_ber_int(uint8_t *out, size_t cap, size_t *n, uint8_t tag, uint32_t v)
{
    uint8_t buf[5];
    size_t k = 0;
    // big-endian minimal
...
bool pc_ber_str(uint8_t *out, size_t cap, size_t *n, uint8_t tag, const char *s)
{
    return tlv(out, cap, n, tag, (const uint8_t *)(s ? s : ""), s ? strnlen(s, cap + 1) : 0);
}

bool pc_ber_bool(uint8_t *out, size_t cap, size_t *n, uint8_t tag, bool v)
{
...
size_t pc_goose_pdu(const pc_goose *g, uint8_t *out, size_t cap)
{
    ...
    bool ok = pc_ber_str(out, cap, &n, 0x80, g->gocb_ref) && pc_ber_int(out, cap, &n, 0x81, g->time_allowed_to_live) &&
