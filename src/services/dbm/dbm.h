 ...
 *    call ::dws_dbm_sync to checkpoint the WAL and make them durable.
 ...
 *
 * The index is a fixed BSS array of ::DWS_DBM_SLOTS slots (no heap); keys are bounded by
 * ::DWS_DBM_KEY_MAX and values by ::DWS_DBM_VAL_MAX. Everything fails closed at those bounds. Like the
