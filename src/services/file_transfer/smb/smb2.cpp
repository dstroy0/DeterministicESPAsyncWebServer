                            ...
                            uint32_t tree_id, uint64_t session_id)
{
    ...
    memset(buf, 0, PC_SMB2_HEADER_SIZE);
...
bool pc_smb2_parse_header(const uint8_t *buf, size_t len, Smb2Header *out)
{
    ...
    if (memcmp(buf, SMB2_PROTOCOL_ID, 4) != 0 || pc_rd16le(buf + 4) != 64)
    {
        return false;
    }
...
size_t pc_smb2_build_negotiate(uint8_t *buf, size_t cap, const uint8_t client_guid[16], uint16_t security_mode)
{
    ...
    if (pc_smb2_build_header(buf, cap, Smb2Command::SMB2_NEGOTIATE, 1, 0, 0, 0) == 0)
    {
        return 0;
    }
    // GCOVR_EXCL_STOP
...
bool pc_smb2_parse_negotiate_response(const uint8_t *msg, size_t len, Smb2NegotiateResp *out)
{
    ...
    Smb2Header h;
    if (!pc_smb2_parse_header(msg, len, &h) || h.command != Smb2Command::SMB2_NEGOTIATE)
    {
        return false;
    }
                                   ...
                                   uint8_t security_mode, const uint8_t *sec_buf, size_t sec_len)
{
    ...
    if (pc_smb2_build_header(buf, cap, Smb2Command::SMB2_SESSION_SETUP, 1, message_id, 0, session_id) == 0)
    {
        return 0;
    }
    // GCOVR_EXCL_STOP
...
bool pc_smb2_parse_session_setup_response(const uint8_t *msg, size_t len, Smb2SessionSetupResp *out)
{
    ...
    Smb2Header h;
    if (!pc_smb2_parse_header(msg, len, &h) || h.command != Smb2Command::SMB2_SESSION_SETUP)
    {
        return false;
    }
                                  ...
                                  const uint8_t *path_utf16, size_t path_len)
{
    ...
    if (pc_smb2_build_header(buf, cap, Smb2Command::SMB2_TREE_CONNECT, 1, message_id, 0, session_id) == 0)
    {
        return 0;
    }
    // GCOVR_EXCL_STOP
...
bool pc_smb2_parse_tree_connect_response(const uint8_t *msg, size_t len, Smb2TreeConnectResp *out)
{
    ...
    Smb2Header h;
    if (!pc_smb2_parse_header(msg, len, &h) || h.command != Smb2Command::SMB2_TREE_CONNECT)
    {
        return false;
    }
                            ...
                            uint32_t create_options, const uint8_t *name_utf16, size_t name_len)
{
    ...
    if (pc_smb2_build_header(buf, cap, Smb2Command::SMB2_CREATE, 1, message_id, tree_id, session_id) == 0)
    {
        return 0;
    }
    // GCOVR_EXCL_STOP
...
bool pc_smb2_parse_create_response(const uint8_t *msg, size_t len, Smb2CreateResp *out)
{
    ...
    Smb2Header h;
    if (!pc_smb2_parse_header(msg, len, &h) || h.command != Smb2Command::SMB2_CREATE)
    {
        return false;
    }
                           ...
                           const uint8_t file_id[16])
{
    ...
    if (pc_smb2_build_header(buf, cap, Smb2Command::SMB2_CLOSE, 1, message_id, tree_id, session_id) == 0)
    {
        return 0;
    }
    // GCOVR_EXCL_STOP
...
bool pc_smb2_parse_close_response(const uint8_t *msg, size_t len, Smb2CloseResp *out)
{
    ...
    Smb2Header h;
    if (!pc_smb2_parse_header(msg, len, &h) || h.command != Smb2Command::SMB2_CLOSE)
    {
        return false;
    }
                          ...
                          const uint8_t file_id[16], uint32_t length, uint64_t offset)
{
    ...
    if (pc_smb2_build_header(buf, cap, Smb2Command::SMB2_READ, 1, message_id, tree_id, session_id) == 0)
    {
        return 0;
    }
    // GCOVR_EXCL_STOP
...
bool pc_smb2_parse_read_response(const uint8_t *msg, size_t len, Smb2ReadResp *out)
{
    ...
    Smb2Header h;
    if (!pc_smb2_parse_header(msg, len, &h) || h.command != Smb2Command::SMB2_READ)
    {
        return false;
    }
                           ...
                           const uint8_t file_id[16], const uint8_t *data, size_t data_len, uint64_t offset)
{
    ...
    if (pc_smb2_build_header(buf, cap, Smb2Command::SMB2_WRITE, 1, message_id, tree_id, session_id) == 0)
    {
        return 0;
    }
    // GCOVR_EXCL_STOP
...
bool pc_smb2_parse_write_response(const uint8_t *msg, size_t len, Smb2WriteResp *out)
{
    ...
    Smb2Header h;
    if (!pc_smb2_parse_header(msg, len, &h) || h.command != Smb2Command::SMB2_WRITE)
    {
        return false;
    }
...
{
    uint8_t mac[32];
    pc_hmac_sha256(key, 16, msg, len, mac);
...
static void smb2_sign_framed(const uint8_t key[16], uint8_t *msg, size_t msg_len, Smb2MacFn mac)
{
    ...
    memcpy(msg + PC_SMB2_SIGNATURE_OFF, tag, PC_SMB2_SIGNATURE_LEN);
}
...
static bool smb2_verify_framed(const uint8_t key[16], uint8_t *msg, size_t msg_len, Smb2MacFn mac)
{
    ...
    memcpy(received, msg + PC_SMB2_SIGNATURE_OFF, PC_SMB2_SIGNATURE_LEN);
    memset(msg + PC_SMB2_SIGNATURE_OFF, 0, PC_SMB2_SIGNATURE_LEN);
    uint8_t tag[PC_SMB2_SIGNATURE_LEN];
    mac(key, msg, msg_len, tag);
    memcpy(msg + PC_SMB2_SIGNATURE_OFF, received, PC_SMB2_SIGNATURE_LEN); // restore; the message is unchanged
    uint8_t diff = 0;
                                   ...
                                   size_t key_len, uint8_t *out_key)
{
    ...
    if (dialect == (uint16_t)Smb2Dialect::SMB2_DIALECT_0311)
    {
        ...
        memcpy(fixed + n, c2s ? c2s_label : s2c_label, sizeof(c2s_label));
        n += sizeof(c2s_label);
        fixed[n++] = 0x00;              // Label||0x00 separator
        memcpy(fixed + n, preauth, 64); // Context = the 64-byte preauth-integrity hash
        n += 64;
                       ...
                       uint64_t session_id, const uint8_t *msg, size_t msg_len, uint8_t *out, size_t out_cap)
{
    ...
    memset(out, 0, PC_SMB2_TRANSFORM_HDR_LEN);
    pc_wr32le(out + 0, PC_SMB2_TRANSFORM_PROTOCOL_ID); // ProtocolId 0xFD 'S' 'M' 'B'
    memcpy(out + 20, nonce, PC_SMB2_NONCE_FIELD_LEN);  // Nonce field (16 bytes; AEAD uses the leading bytes)
