 ...
 * unit-tested without pulling in the server. Requires DWS_ENABLE_SSE.
 */
...
#include "dwserver.h"
#include "network_drivers/application/web_assets.h" // DWS_DASHBOARD_PAGE
...
static void dash_page_handler(uint8_t slot_id, HttpReq *req)
{
    (void)req;
    if (s_dashr.srv)
        s_dashr.srv->send(slot_id, 200, DWS_MIME_TEXT_HTML, DWS_DASHBOARD_PAGE);
}
...
static void dash_ws_message(uint8_t ws_id)
{
    ...
    if (ws_id < MAX_WS_CONNS)
        dws_dashboard_dispatch_control(ws_payload(ws_id));
}
