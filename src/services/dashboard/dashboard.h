 ...
 * dws_dashboard_set(key, value) and pushes them with dws_dashboard_publish().
 *
 * The widget-table -> JSON serializers (layout + values) are pure and have no
 * server dependency, so they unit-test on the host. Requires DWS_ENABLE_SSE.
 *
...
enum class DWSWidgetType : uint8_t
{
    ...
    DWS_WIDGET_GAUGE,     ///< radial arc gauge over [min, max]
    DWS_WIDGET_BAR,       ///< horizontal bar over [min, max]
    DWS_WIDGET_SPARKLINE, ///< recent-history SVG line over [min, max]
    DWS_WIDGET_CHART,     ///< dense Canvas line chart over [min, max]
    // Control widgets - send values back to the device over WebSocket.
    DWS_WIDGET_BUTTON, ///< momentary button -> control value 1
    DWS_WIDGET_TOGGLE, ///< on/off toggle -> control value 0/1 (reflects SSE state)
    DWS_WIDGET_SLIDER  ///< range slider over [min, max] -> control value
};
