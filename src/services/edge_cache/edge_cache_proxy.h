 ...
 * @brief CDN edge-cache tier - server glue (DWS_ENABLE_EDGE_CACHE).
 *
 * Wires the pure engine (edge_cache) + async fetch (edge_fetch) into a DWS: registers the
 * cache as a middleware and installs the async-fetch poll hook, maps request path prefixes to upstream
 * origins (fetched over dws_client), and serves hits with the constant-memory send-pump. A miss or a
 ...
 *        bind the origin transport to dws_client, and clear the L1 store. Call once.
 */
 ...
 * `https://` origin is fetched over TLS when DWS_ENABLE_EDGE_ORIGIN_TLS is set, otherwise rejected.
 ...
 *        a reboot. Pass nullptr to detach. Call after ::dws_edge_cache_enable.
 *
