 ...
 * @brief Dual-stack destination selection + Happy Eyeballs fallback (DWS_ENABLE_HAPPY_EYEBALLS).
 *
 * On a dual-stack device (DWS_ENABLE_IPV6), an outbound connection often has both IPv6 and IPv4
 ...
 *
 * This is the pure decision layer on top of the shipped `DWSIp`: a preference score, the ordering +
...
#ifndef DWS_HE_MAX
#define DWS_HE_MAX 16 ///< candidate list size the interleave step handles (larger lists are only sorted).
#endif

/** @brief RFC 8305 recommended Connection Attempt Delay (ms); the spec floor is 100, default 250. */
#define DWS_HE_ATTEMPT_DELAY_MS 250
 ...
 *        Lists longer than DWS_HE_MAX are sorted but not interleaved.
 */
