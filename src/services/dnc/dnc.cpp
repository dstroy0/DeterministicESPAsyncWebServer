...
static size_t dnc_put_eob(const DncCfg *cfg, uint8_t *out, size_t cap, size_t n)
{
    ...
    if (cfg->crlf)
    {
        uint8_t cr = cfg->even_parity ? dws_dnc_iso_add_parity(0x0D) : 0x0D;
        if (n >= cap)
            return 0;
        out[n++] = cr;
    }
    uint8_t lf = cfg->even_parity ? dws_dnc_iso_add_parity(0x0A) : 0x0A;
    if (n >= cap)
        return 0;
    out[n++] = lf;
    return n;
...
size_t dws_dnc_encode_block(const DncCfg *cfg, const char *line, size_t line_len, uint8_t *out, size_t out_cap)
{
    ...
    for (size_t i = 0; i < line_len; i++)
    {
        ...
        if (cfg->code == DncCode::DNC_CODE_EIA)
        {
            uint8_t e = dws_dnc_iso_to_eia(line[i]);
            if (e == 0xFF)
...
size_t dws_dnc_encode_marker(const DncCfg *cfg, uint8_t *out, size_t out_cap)
{
    ...
    {
        uint8_t pct = cfg->even_parity ? dws_dnc_iso_add_parity(0x25) : 0x25;
        if (n >= out_cap)
            return 0;
        out[n++] = pct;
    }
...
DncEvent dws_dnc_decode_feed(DncDecoder *d, uint8_t wire)
{
    ...
    // (controller -> sender), handled by dws_dnc_flow_feed; this decodes the forward program
    ...
    if (d->code == DncCode::DNC_CODE_EIA)
    {
        if (wire == (uint8_t)DncEiaCode::DNC_EIA_EOB)
            ...
            ascii = (uint8_t)dws_dnc_eia_to_iso(wire); // 0 for blank/runout/unknown -> ignored
    }
    else
    {
