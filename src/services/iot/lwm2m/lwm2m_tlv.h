...
bool pc_lwm2m_tlv_write(Lwm2mTlvWriter *w, uint8_t id_type, uint16_t id, const uint8_t *value, size_t value_len);

/** @brief Write a Resource integer (shortest of 1/2/4/8 octets, big-endian two's complement). */
bool pc_lwm2m_tlv_write_int(Lwm2mTlvWriter *w, uint16_t id, int64_t v);

/** @brief Write a Resource boolean (one octet, 0/1). */
bool pc_lwm2m_tlv_write_bool(Lwm2mTlvWriter *w, uint16_t id, bool v);

/** @brief Write a Resource UTF-8 string. */
bool pc_lwm2m_tlv_write_string(Lwm2mTlvWriter *w, uint16_t id, const char *s);

/** @brief Write a Resource float (8-octet IEEE-754, big-endian). */
bool pc_lwm2m_tlv_write_float(Lwm2mTlvWriter *w, uint16_t id, double v);
 ...
 */
bool pc_lwm2m_tlv_read(const uint8_t *buf, size_t len, size_t *pos, Lwm2mTlv *out);

/** @brief Decode a TLV integer value (1/2/4/8 octets, big-endian two's complement). */
bool pc_lwm2m_tlv_value_int(const uint8_t *value, size_t len, int64_t *out);
