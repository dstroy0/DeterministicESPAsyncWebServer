...
#include "services/system/clock.h"   // pc_millis() - idle-reap clock (the DTLS PTO uses it internally too)
...
bool serialize_peer(const char *ip, uint16_t port, uint8_t out[PC_COAPS_PEER_SER])
{
    uint32_t oct = 0;
    int octets = 0;
    int ndig = 0;
...
CoapsSlot *open_conn(const char *ip, uint16_t port)
{
    ...
    pc_dtls_conn_init(&s->conn, &s->cfg, ok ? paddr : nullptr, ok ? sizeof paddr : 0);
...
#if defined(ARDUINO)
    return pc_udp_listen(s_coaps.port, udp_ingest_cb, nullptr);
#else
    return true; // host: fed through pc_coaps_server_ingest()
#endif
}
