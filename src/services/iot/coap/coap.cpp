...
struct CoapOpt
{
    static constexpr uint8_t COAP_OPT_OBSERVE = 6;
    static constexpr uint8_t COAP_OPT_URI_PATH = 11;
    static constexpr uint8_t COAP_OPT_CONTENT_FORMAT = 12;
    static constexpr uint8_t COAP_OPT_URI_QUERY = 15;
    static constexpr uint8_t COAP_OPT_BLOCK2 = 23; ///< block-wise responses (RFC 7959)
    static constexpr uint8_t COAP_OPT_BLOCK1 = 27; ///< block-wise request uploads (RFC 7959)
};
                                 ...
                                 int32_t observe_seq)
{
    ...
    if (ver != 1 || tkl > COAP_MAX_TOKEN)
    {
        return (type == CoapType::COAP_TYPE_CON)
                   ? emit_header(resp, pc_resp_cap, CoapType::COAP_TYPE_RST, 0, mid, nullptr, 0)
                   : 0;
    }
    ...
    if (bad)
    {
        return emit_header(resp, pc_resp_cap, rsp_type, (uint8_t)CoapResponseCode::COAP_RSP_BAD_REQUEST, mid, token,
                           tkl);
    }
    if (bad_option)
    {
        return emit_header(resp, pc_resp_cap, rsp_type, (uint8_t)CoapResponseCode::COAP_RSP_BAD_OPTION, mid, token,
                           tkl);
    }
    ...
    {
        return emit_header(resp, pc_resp_cap, rsp_type, (uint8_t)CoapResponseCode::COAP_RSP_METHOD_NOT_ALLOWED, mid,
                           token, tkl);
    }
    // GCOVR_EXCL_STOP
    ...
    else
    {
        ...
        if (!r)
        {
            return emit_header(resp, pc_resp_cap, rsp_type, (uint8_t)CoapResponseCode::COAP_RSP_NOT_FOUND, mid, token,
                               tkl);
        }
        ...
        if (req_block1 >= 0 && (code == (uint8_t)CoapMethod::COAP_POST || code == (uint8_t)CoapMethod::COAP_PUT))
        {
            ...
            {
                return emit_header(resp, pc_resp_cap, rsp_type, (uint8_t)CoapResponseCode::COAP_RSP_BAD_OPTION, mid,
                                   token, tkl);
            }
            ...
            {
                s_coap.b1_len = 0;
                return emit_header(resp, pc_resp_cap, rsp_type, (uint8_t)CoapResponseCode::COAP_RSP_REQUEST_INCOMPLETE,
                                   mid, token, tkl);
            }
            if (s_coap.b1_len + payload_len > sizeof(s_coap.b1))
            {
                s_coap.b1_len = 0;
                return emit_header(resp, pc_resp_cap, rsp_type, (uint8_t)CoapResponseCode::COAP_RSP_REQUEST_TOO_LARGE,
                                   mid, token, tkl);
            }
            if (payload_len)
            {
            ...
            if (more)
            {
                ...
                size_t cn = emit_header(resp, pc_resp_cap, rsp_type, (uint8_t)CoapResponseCode::COAP_RSP_CONTINUE, mid,
                                        token, tkl);
                if (cn == 0)
                {
                    return 0;
                }
                return emit_options_payload(resp, pc_resp_cap, cn, (uint8_t)CoapResponseCode::COAP_RSP_CONTINUE, -1,
    ...
    if ((cresp.code >> 5) == 2)
    {
        ...
        if (block_wise)
        {
            ...
            {
                return emit_header(resp, pc_resp_cap, rsp_type, (uint8_t)CoapResponseCode::COAP_RSP_BAD_REQUEST, mid,
                                   token, tkl);
            }
...
static bool coap_dedup_replay(const uint8_t *data, size_t len, const struct pc_udp_peer *peer, const char *ip,
...
static void coap_udp_handler(const uint8_t *data, size_t len, const struct pc_udp_peer *peer, void *ctx)
{
    ...
    char ip[16];
    uint16_t pport = 0;
    bool have_peer = pc_udp_peer_addr(peer, ip, sizeof(ip), &pport);
...
void pc_coap_server_begin(uint16_t port)
{
    ...
    pc_udp_listen(port, coap_udp_handler, nullptr);
}
