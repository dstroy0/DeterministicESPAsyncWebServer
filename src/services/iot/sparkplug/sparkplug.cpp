...
bool pc_spb_parse_payload(const uint8_t *buf, size_t len, SpbPayloadHeader *out)
{
    if (!buf || !out)
    {
        return false;
...
bool pc_spb_payload_next_metric(const uint8_t *buf, size_t len, size_t *pos, const uint8_t **metric, size_t *metric_len)
{
...
bool pc_spb_parse_metric(const uint8_t *buf, size_t len, SpbMetricDecoded *out)
{
    if (!buf || !out)
    {
        return false;
