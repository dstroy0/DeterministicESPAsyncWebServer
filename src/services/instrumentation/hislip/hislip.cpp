                                           ...
                                           uint16_t session_id)
{
    ...
    return pc_hislip_build_header(buf, cap, HislipMsg::INITIALIZE_RESPONSE, control, parameter, 0);
}

size_t pc_hislip_build_async_initialize(uint8_t *buf, size_t cap, uint16_t session_id)
{
    return pc_hislip_build_header(buf, cap, HislipMsg::ASYNC_INITIALIZE, 0, session_id, 0);
}

size_t pc_hislip_build_async_initialize_response(uint8_t *buf, size_t cap, uint8_t control, uint16_t server_vendor_id)
{
    return pc_hislip_build_header(buf, cap, HislipMsg::ASYNC_INITIALIZE_RESPONSE, control, server_vendor_id, 0);
}
...
{
    HislipHeader h;
    if (!out || !pc_hislip_parse_header(buf, len, &h) || h.type != HislipMsg::INITIALIZE)
    {
        return false;
    }
...
{
    HislipHeader h;
    if (!out || !pc_hislip_parse_header(buf, len, &h) || h.type != HislipMsg::INITIALIZE_RESPONSE)
    {
        return false;
    }
    out->protocol_version = (uint16_t)(h.parameter >> 16);
