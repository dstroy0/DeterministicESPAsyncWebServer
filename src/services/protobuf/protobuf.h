 ...
 * embedded messages are built into a separate buffer and added with @ref dws_pb_bytes. The
...
bool dws_pb_uint64(PbWriter *w, uint32_t field, uint64_t v); ///< varint (uint32/uint64/enum)
bool dws_pb_int64(PbWriter *w, uint32_t field, int64_t v);   ///< varint, two's complement (int32/int64)
bool dws_pb_sint64(PbWriter *w, uint32_t field, int64_t v);  ///< ZigZag varint (sint32/sint64)
bool dws_pb_bool(PbWriter *w, uint32_t field, bool v);
bool dws_pb_fixed32(PbWriter *w, uint32_t field, uint32_t v); ///< wire type 5
bool dws_pb_fixed64(PbWriter *w, uint32_t field, uint64_t v); ///< wire type 1
bool dws_pb_float(PbWriter *w, uint32_t field, float v);
bool dws_pb_double(PbWriter *w, uint32_t field, double v);
bool dws_pb_bytes(PbWriter *w, uint32_t field, const uint8_t *data, size_t len); ///< wire type 2
