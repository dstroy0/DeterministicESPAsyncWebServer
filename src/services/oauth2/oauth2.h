...
/**
 * @file oauth2.h
 * @brief OAuth2 token-endpoint client - authorization-code + refresh (DETWS_ENABLE_OAUTH2).
 *
 ...
 *  - **ESP32 convenience (needs DETWS_ENABLE_HTTP_CLIENT):**
 *    detws_oauth2_exchange_code() / _refresh() POST to the token URL over the
...
{
    char access_token[DETWS_OAUTH2_TOKEN_LEN];
    char id_token[DETWS_OAUTH2_TOKEN_LEN]; ///< OIDC ID token (verify with services/oidc).
    char refresh_token[DETWS_OAUTH2_RT_LEN];
...
enum DetwsOAuth2Result
{
    ...
    DETWS_OAUTH2_ERR_RESPONSE = -3,  ///< response was not a valid token JSON (no access_token).
};
