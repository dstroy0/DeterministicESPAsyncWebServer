...
size_t pc_nts_ke_request(uint8_t *out, size_t cap)
{
    ...
    size_t n = 0;
    size_t r;
    r = pc_nts_ke_record(true, Nts::NTS_KE_NEXT_PROTOCOL, proto, 2, out + n, cap - n);
    if (!r)
    {
        return 0;
    }
    n += r;
    r = pc_nts_ke_record(true, Nts::NTS_KE_AEAD_ALGORITHM, aead, 2, out + n, cap - n);
    if (!r)
    {
        return 0;
    }
    n += r;
    r = pc_nts_ke_record(true, Nts::NTS_KE_END_OF_MESSAGE, nullptr, 0, out + n, cap - n);
    if (!r)
    {
        return 0;
    }
