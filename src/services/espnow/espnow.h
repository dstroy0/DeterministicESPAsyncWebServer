...
/**
 * @file espnow.h
 * @brief ESP-NOW peer messaging with a typed envelope (DWS_ENABLE_ESPNOW).
 *
 ...
 *    (dws_espnow_encode / decode) and a bounded peer registry
 *    (DWS_ESPNOW_MAX_PEERS, no heap). Same on host and ESP32.
 *  - **ESP32 binding (ARDUINO):** dws_espnow_begin / add_peer / send /
 ...
 * DWS_ESPNOW_MAX_PAYLOAD for data. No stdlib, no heap.
 *
...
typedef void (*dws_espnow_recv_fn)(const uint8_t mac[6], uint8_t type, const uint8_t *payload, size_t len);
