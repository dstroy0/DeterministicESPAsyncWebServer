...
size_t pc_bvlc_build(uint8_t *buf, size_t cap, uint8_t function, const uint8_t *npdu, size_t pc_npdu_len)
{
    ...
    size_t total = BVLC_HEADER_SIZE + pc_npdu_len;
...
bool pc_bvlc_parse(const uint8_t *buf, size_t len, uint8_t *function, const uint8_t **npdu, size_t *pc_npdu_len)
{
    ...
    if (pc_npdu_len)
    {
        *pc_npdu_len = total - BVLC_HEADER_SIZE;
    }
    return true;
}

size_t pc_npdu_build(uint8_t *buf, size_t cap, bool expecting_reply, uint8_t priority, bool has_dest, uint16_t dnet,
...
bool pc_npdu_parse(const uint8_t *buf, size_t len, NpduInfo *out)
{
...
size_t pc_apdu_build_who_is(uint8_t *buf, size_t cap, uint32_t low_limit, uint32_t high_limit, bool has_limits)
{
    if (!buf)
    {
        return 0;
...
bool pc_apdu_parse(const uint8_t *apdu, size_t len, BacnetApdu *out)
{
    if (!apdu || !out || len < 1)
    {
        return false;
