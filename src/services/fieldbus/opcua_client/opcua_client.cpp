                                      ...
                                      size_t cap)
{
    ...
    pc_ua_w_localizedtext(&w, nullptr, "pc_opcua_client"); // ApplicationName
    ...
    pc_ua_w_string(&w, nullptr, -1);                       // ServerUri
    pc_ua_w_string(&w, endpoint_url, endpoint_url ? (int32_t)strnlen(endpoint_url, w.cap) : -1); // EndpointUrl
    pc_ua_w_string(&w, session_name, session_name ? (int32_t)strnlen(session_name, w.cap) : -1); // SessionName
    pc_ua_w_string(&w, nullptr, -1); // ClientNonce (ByteString)
    pc_ua_w_string(&w, nullptr, -1); // ClientCertificate
...
size_t pc_opcua_client_activate_session(OpcUaClient *c, uint8_t *out, size_t cap)
{
    ...
    pc_ua_w_string(&w, nullptr, -1); // ClientSignature.Algorithm
    pc_ua_w_string(&w, nullptr, -1); // ClientSignature.Signature
    ...
    pc_ua_w_string(&w, nullptr, -1);    // UserTokenSignature.Algorithm
    pc_ua_w_string(&w, nullptr, -1);    // UserTokenSignature.Signature
    return cw_patch(&w);
}
...
size_t pc_opcua_client_close_session(OpcUaClient *c, uint8_t *out, size_t cap)
{
    ...
    cw_request_header(c, &w, true);
    pc_ua_w_bool(&w, true); // DeleteSubscriptions
    return cw_patch(&w);
}
