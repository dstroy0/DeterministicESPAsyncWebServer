...
{
    uint8_t epath[12];
    size_t elen = pc_cip_build_epath(epath, sizeof(epath), class_id, instance_id, attribute_id, true);
    ...
    // GCOVR_EXCL_STOP
    return pc_cip_build_request(buf, cap, CIP_SC_GET_ATTR_SINGLE, epath, elen, nullptr, 0);
}
...
size_t pc_cip_build_get_attr_all(uint8_t *buf, size_t cap, uint16_t class_id, uint16_t instance_id)
{
    ...
    size_t elen = pc_cip_build_epath(epath, sizeof(epath), class_id, instance_id, 0, false);
    ...
    // GCOVR_EXCL_STOP
    return pc_cip_build_request(buf, cap, CIP_SC_GET_ATTR_ALL, epath, elen, nullptr, 0);
}
