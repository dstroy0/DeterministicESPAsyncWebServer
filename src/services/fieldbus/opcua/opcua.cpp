                                    ...
                                    uint32_t seq_number, int64_t now_ft, uint32_t lifetime, uint8_t *out, size_t cap)
{
    ...
    pc_ua_w_string(&w, nullptr, -1);   // ServerNonce (null for None)
...
void pc_ua_w_endpoint_description(UaWriter *w, const OpcUaServerInfo *info)
{
    ...
    pc_ua_w_localizedtext(w, nullptr, aname);                // ApplicationName
    pc_ua_w_u32(w, 0);                                       // ApplicationType = Server
    pc_ua_w_string(w, nullptr, -1);                          // GatewayServerUri
    pc_ua_w_string(w, nullptr, -1);                          // DiscoveryProfileUri
    pc_ua_w_i32(w, -1);                                      // DiscoveryUrls[] (null)
    pc_ua_w_string(w, nullptr, -1);                          // ServerCertificate (ByteString, null)
    ...
    pc_ua_w_string(w, nullptr, -1);                                                     // IssuedTokenType
    pc_ua_w_string(w, nullptr, -1);                                                     // IssuerEndpointUrl
                                              ...
                                              int64_t now_ft, uint8_t *out, size_t cap)
{
    ...
    pc_ua_w_string(&w, nullptr, -1);           // ServerNonce (none for SecurityPolicy None)
    pc_ua_w_string(&w, nullptr, -1);           // ServerCertificate
    ...
    pc_ua_w_string(&w, nullptr, -1); // ServerSignature.Algorithm (null String)
    pc_ua_w_string(&w, nullptr, -1); // ServerSignature.Signature (null ByteString)
...
void pc_ua_w_variant(UaWriter *w, const OpcUaVariant *v)
{
    ...
    {
        pc_ua_w_u8(w, (uint8_t)OpcUaVariantType::OPCUA_VAR_NULL); // Null Variant (encoding byte 0)
        return;
    }
                                    ...
                                    uint32_t seq, int64_t now_ft, uint8_t *out, size_t cap)
{
    ...
    {
        pc_ua_w_datavalue(&w, values ? &values[i] : nullptr, statuses ? statuses[i] : OPCUA_STATUS_GOOD);
    }
    pc_ua_w_i32(&w, 0); // DiagnosticInfos[] (empty)
    return patch_size(&w);
}
...
bool pc_opcua_parse_write(const uint8_t *msg, size_t len, OpcUaWriteRequest *out)
{
    ...
    for (int32_t i = 0; i < cnt; i++)
    {
        ...
        OpcUaVariant val;
        if (!pc_ua_r_datavalue(&r, &val, nullptr)) // Value (DataValue)
        {
            return false;
        }
...
void pc_ua_w_reference(UaWriter *w, const OpcUaReference *ref)
{
    ...
    pc_ua_w_localizedtext(w, nullptr, ref->display_name);            // DisplayName
                                      ...
                                      int64_t now_ft, uint8_t *out, size_t cap)
{
    ...
    for (uint32_t i = 0; i < req->count; i++)
    {
        ...
        pc_ua_w_string(&w, nullptr, -1); // ContinuationPoint (ByteString, null)
...
void pc_opcua_rx(uint8_t slot)
{
    ...
    for (;;)
    {
        ...
        else if (memcmp(h.type, "OPN", 3) == 0)
        {
            ...
            int64_t now = pc_opcua_filetime_from_unix((int64_t)time(nullptr));
...
static const ProtoHandler s_opcua_handler = {nullptr, pc_opcua_rx, nullptr, nullptr};
