...
/**
 * @brief Set the mount root every request path is resolved against (e.g. "/gcode/").
 *
 * One root, for the device, not one per protocol: the root is a property of what is mounted, and
 * two servers exposing the same storage under different roots would be two answers to the same
 * question. A NULL or empty @p root selects "/".
 */
...
// --- operations ------------------------------------------------------------------------------
// Each takes the REQUEST path as its two pieces - a @p dir and a leaf @p name, "" when the dir is
// the whole path - and frames them onto the root here, once. A caller that joined them itself
// would build the same bytes twice. Handle-taking calls carry no path and dispatch straight
// through.

/** @brief Open request path @p dir + @p name. @return a handle (>= 0), or -1. */
