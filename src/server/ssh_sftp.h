...
/**
 * @file ssh_sftp.h
 * @brief SFTP server subsystem - the fs::FS binding (DWS_ENABLE_SSH_SFTP).
 *
 ...
 * streamed reads/writes (no heap beyond the fs layer's own file handles). Call dws_ssh_sftp_begin() once
 * after dws_ssh_conn_setup(); it installs the channel subsystem + data callbacks.
 *
 ...
 *        Installs the channel subsystem + data callbacks. Call once, after dws_ssh_conn_setup().
 */
