 ...
 * @brief WebSocket (RFC 6455) and Server-Sent Events upgrade + public API for PC.
 *
 * Split out of protocore.cpp (single-purpose server files). The WS handshake (Sec-WebSocket-Accept
...
#include "network_drivers/presentation/http/websocket/websocket.h" // ws_pool, WsConn, ws_alloc/send_frame/close
#endif
#if PC_ENABLE_SSE
#include "network_drivers/presentation/http/sse/sse.h" // pc_sse_pool, SseConn, pc_sse_alloc/write
#endif
#include <stdio.h>
#include <string.h>
...
bool ws_do_upgrade(uint8_t slot_id, HttpReq *req, WsConnectHandler on_connect)
{
    ...
    pc_sb sb_hdr = {hdr, sizeof(hdr), 0, true};
    pc_sb_put(
        &sb_hdr,
...
#else
    pc_sb sb_hdr2 = {hdr, sizeof(hdr), 0, true};
    pc_sb_put(
        &sb_hdr2,
        "HTTP/1.1 101 Switching Protocols\r\nUpgrade: websocket\r\nConnection: Upgrade\r\nSec-WebSocket-Accept: ");
