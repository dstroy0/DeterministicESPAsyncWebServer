 ...
 *
 * There is no logic here on purpose. Every function is a store, a copy, or a forward, because
 * signaling originates nothing - the loop establishes each fact and deposits it, so anything this
 * file computed would be a second derivation of something already known. See signaling.h.
 */
...
namespace
{
// The deposited bucket, owned by one instance (internal linkage). This is storage for facts the loop
// established, not a source of them: nothing outside this file writes it and nothing inside computes
// it. Zeroed, so a read taken before the first tick answers an idle server rather than garbage.
struct SignalingCtx
{
