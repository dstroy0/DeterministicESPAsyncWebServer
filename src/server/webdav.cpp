...
/**
 * @file webdav.cpp
 * @brief WebDAV (RFC 4918) filesystem-backed request handler for PC.
 *
 * Split out of protocore.cpp (single-purpose server files). The pure WebDAV core - method
 * classification, the 207 Multi-Status XML builder, header parsing - lives in
 * services/file_transfer/webdav/webdav.{h,cpp} and is host-tested; this file is the PC glue that
 * needs a real filesystem (PROPFIND/PUT/COPY/MOVE/... over an fs::FS subtree). WEBDAV requires
 * FILE_SERVING (enforced in protocore_config.h), so the file-serving helpers it borrows are always
 * present. Behavior is identical to the pre-split code - a pure move.
 */
...
// services/file_transfer/webdav/webdav.{h,cpp} and is host-tested; this part needs a real FS.
// ---------------------------------------------------------------------------
...
// of streaming. protocore_config.h documents "do not combine" but nothing enforced it; the buffered-PUT
...
// A Depth-1 PROPFIND stops on whichever comes first: the 207 buffer filling, or PC_WEBDAV_MAX_ENTRIES
...
//
// The fixed text of one <D:response> (pc_webdav_ms_entry) is 204 bytes: 27 href prologue + 66
...
static const size_t PC_WEBDAV_MIN_ENTRY_BYTES = 192;
...
static bool dav_write_blocked(HttpReq *req, const char *path)
{
    ...
    const char *presented = (if_hdr && pc_dav_if_token(if_hdr, tok, sizeof(tok))) ? tok : nullptr;
...
bool dav_stream_put_begin(HttpReq *req)
{
    ...
    for (uint8_t i = 0; i < pc_route_count(); i++)
    {
        ...
        if (r->iface_filter != pc_iface::PC_IFACE_ANY && r->iface_filter != pc_conn_iface(slot))
        {
            continue;
        }
        // GCOVR_EXCL_STOP
...
void dav(const char *url_prefix, const pc_mnt_backend *file_sys, const char *fs_root)
{
    ...
    {
        pc_sb sb_pat2 = {pat, sizeof(pat), 0, true};
...
void serve_dav_request(uint8_t slot_id, HttpReq *req, const Route *r)
{
    ...
    case WebDavMethod::DAV_M_MOVE: {
        ...
        bool is_move = pc_webdav_method(req->method) == WebDavMethod::DAV_M_MOVE;
    ...
    case WebDavMethod::DAV_M_LOCK: {
        ...
        {
            pc_sb sb_token = {token, sizeof(token), 0, true};
...
#endif
            pc_sb sb_token2 = {token, sizeof(token), 0, true};
        ...
        pc_sb sb_buf = {s_dav.buf, sizeof(s_dav.buf), 0, true};
        pc_sb_put(
            &sb_buf,
        ...
        char lt[64];
        pc_sb sb_lt = {lt, sizeof(lt), 0, true};
        pc_sb_put(&sb_lt, "<");
    ...
    case WebDavMethod::DAV_M_PROPFIND: {
        ...
        pc_sb sb_self_href = {self_href, sizeof(self_href), 0, true};
