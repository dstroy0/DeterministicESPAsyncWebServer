...
{
    dws_hkdf_expand_label(secret, label, out, out_len, kdf->label_prefix);
}
                             ...
                             const uint8_t transcript_hash[TLS13_SECRET_LEN], uint8_t out[TLS13_SECRET_LEN])
{
    ...
    dws_hkdf_expand_label_ctx(secret, label, transcript_hash, TLS13_SECRET_LEN, out, TLS13_SECRET_LEN,
                              kdf->label_prefix);
}
...
void dws_tls13_ks_early(const Tls13Kdf *kdf, Tls13KeySchedule *ks)
{
    ...
    dws_hkdf_extract(nullptr, 0, zeros, sizeof(zeros), ks->early_secret);
}
                            ...
                            size_t ecdhe_len)
{
    ...
    dws_hkdf_extract(derived, sizeof(derived), ecdhe, ecdhe_len, ks->handshake_secret);
...
void dws_tls13_ks_master(Tls13KeySchedule *ks, const uint8_t ch_sfin_hash[TLS13_SECRET_LEN])
{
    ...
    uint8_t zeros[TLS13_SECRET_LEN];
    memset(zeros, 0, sizeof(zeros));
    dws_hkdf_extract(derived, sizeof(derived), zeros, sizeof(zeros), ks->master_secret);
                            ...
                            const uint8_t transcript_hash[TLS13_SECRET_LEN], uint8_t out[TLS13_SECRET_LEN])
{
    ...
    dws_hkdf_expand_label(base_secret, "finished", finished_key, sizeof(finished_key), kdf->label_prefix);
