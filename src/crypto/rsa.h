 ...
 * (dws_rsa_sign_sw, raw n/d) is the native-only reference path used by the tests; on-device signing
...
#ifndef DETERMINISTICESPASYNCWEBSERVER_CRYPTO_RSA_H
#define DETERMINISTICESPASYNCWEBSERVER_CRYPTO_RSA_H

#include <stddef.h>
#include <stdint.h>

/** @brief RSA modulus / signature size in bytes (RSA-2048). */
#define DWS_RSA_KEY_BYTES 256

/** @brief PKCS#1 v1.5 signature size for RSA-2048 in bytes. */
#define DWS_RSA_SIG_BYTES 256
...
#define DWS_PKCS1_DIGESTINFO_LEN 19

/** @brief Length of the DER DigestInfo wrapper for SHA-512. */
#define DWS_PKCS1_SHA512_DIGESTINFO_LEN 19

/** @brief The DER-encoded DigestInfo wrapper for SHA-256 (prepend to the 32-byte digest). */
extern const uint8_t dws_pkcs1_sha256_digestinfo[DWS_PKCS1_DIGESTINFO_LEN];

/** @brief The DER-encoded DigestInfo wrapper for SHA-512 (prepend to the 64-byte digest). */
extern const uint8_t dws_pkcs1_sha512_digestinfo[DWS_PKCS1_SHA512_DIGESTINFO_LEN];
 ...
 * @param sig_len  Signature length (must equal DWS_RSA_KEY_BYTES).
 ...
 * @param msg_len  Message length.
 * @param hash     Digest algorithm (SHA-256 / SHA-512).
 * @param sig      Output signature, DWS_RSA_SIG_BYTES big-endian.
 * @return 0 on success.
 */
...
#endif // DETERMINISTICESPASYNCWEBSERVER_CRYPTO_RSA_H
