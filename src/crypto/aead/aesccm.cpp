...
#include <string.h>
#ifndef ARDUINO
#include "crypto/cipher/aes_block.h" // native software AES-128/256 (mbedtls path uses its own on Arduino)
#endif
PC_CRYPTO_HOT
                        ...
                        uint8_t tag_out[PC_AESCCM_TAG_LEN])
{
    ...
    SecureBorrow ws_b(sizeof(mbedtls_ccm_context), alignof(mbedtls_ccm_context));
    const pc_span &ws = ws_b.span();
    if (!pc_span_ok(ws))
    {
        return false;
    }
    mbedtls_ccm_context *c = reinterpret_cast<mbedtls_ccm_context *>(ws.buf);
    mbedtls_ccm_init(c);
...
void tag_encrypt(CcmWork *w, const uint8_t *nonce, size_t nonce_len, uint8_t out_tag[PC_AESCCM_TAG_LEN])
{
                        ...
                        uint8_t tag_out[PC_AESCCM_TAG_LEN])
{
    ...
    SecureBorrow ws_b(sizeof(CcmWork), alignof(CcmWork));
    const pc_span &ws = ws_b.span();
    if (!pc_span_ok(ws))
    {
        return false;
    }
    CcmWork *w = reinterpret_cast<CcmWork *>(ws.buf);
                        ...
                        uint8_t *out)
{
    ...
    if (!pc_ct_eq(w->blk, tag, PC_AESCCM_TAG_LEN))
    {
