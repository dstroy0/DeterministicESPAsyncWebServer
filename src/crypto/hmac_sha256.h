 ...
 * message signing / the SP800-108 KDF. Built over the dws_sha256 streaming functions so the inner
...
#ifndef DETERMINISTICESPASYNCWEBSERVER_CRYPTO_HMAC_SHA256_H
#define DETERMINISTICESPASYNCWEBSERVER_CRYPTO_HMAC_SHA256_H
 ...
 * @param mac      Output buffer, must be DWS_HMAC_SHA256_LEN bytes.
 */
 ...
 *   dws_hmac_sha256_update(&ctx, seq_bytes, 4);
 *   dws_hmac_sha256_update(&ctx, packet, pkt_len);
 *   dws_hmac_sha256_final(&ctx, mac_out);
 */
typedef struct
{
    DwsSha256Ctx inner; ///< Inner hash context (key XOR ipad prepended).
...
#endif // DETERMINISTICESPASYNCWEBSERVER_CRYPTO_HMAC_SHA256_H
