...
#else
#include "crypto/cipher/aes_block.h" // native software AES S-box/blocks (ARDUINO uses the mbedtls block above)
#endif
PC_CRYPTO_HOT
...
static_assert(sizeof(Aes256CtrWork) <= PC_WORK_AES256CTR,
              "Aes256CtrWork outgrew PC_WORK_AES256CTR - raise it in protocore_config.h, which derives "
                        ...
                        const uint8_t *in, uint8_t *out, size_t len)
{
    ...
    SecureBorrow ws_b(sizeof(Aes256CtrWork), alignof(Aes256CtrWork));
    const pc_span &ws = ws_b.span();
    if (!pc_span_ok(ws))
    {
        return;
    }
    Aes256CtrWork *w = reinterpret_cast<Aes256CtrWork *>(ws.buf);
                        ...
                        const uint8_t *in, uint8_t *out, size_t len)
{
    ...
    for (size_t i = 0; i < len; i++)
    {
        if (pos == 0)
        {
            pc_aes_encrypt_block(rk, 14, counter, ks); // keystream = AES(counter)
                                 ...
                                 const uint8_t enc4[4])
{
    ...
    pc_aes_encrypt_block(w->rk, 14, counter, ks);
#endif
