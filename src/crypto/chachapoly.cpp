                                   ...
                                   const uint8_t enc_len[DWS_CHACHAPOLY_AAD_LEN])
{
    ...
    seq_nonce(seqnr, iv);
    uint8_t len[4];
    dws_chacha20_xor(key + 32, iv, 0, enc_len, len, 4); // header key, counter 0
                            ...
                            const uint8_t *src, uint32_t payload_len)
{
    ...
    seq_nonce(seqnr, iv);
    uint8_t poly_key[32];
    dws_chacha20_xor(key, iv, 0, nullptr, poly_key, 32);          // Poly1305 key = K_main block 0
    dws_chacha20_xor(key + 32, iv, 0, src, dest, 4);              // length field: K_header, counter 0
    dws_chacha20_xor(key, iv, 1, src + 4, dest + 4, payload_len); // payload: K_main, counter 1
    dws_poly1305(dest + 4 + payload_len, dest, 4 + payload_len, poly_key);
}
                            ...
                            const uint8_t *src, uint32_t payload_len)
{
    ...
    seq_nonce(seqnr, iv);
    uint8_t poly_key[32];
    dws_chacha20_xor(key, iv, 0, nullptr, poly_key, 32);
    uint8_t tag[16];
    dws_poly1305(tag, src, 4 + payload_len, poly_key); // MAC over the ciphertext (length || payload)
    if (!ct_eq16(tag, src + 4 + payload_len))
        return false; // authentication failed - produce no plaintext
    dws_chacha20_xor(key + 32, iv, 0, src, dest, 4);
    dws_chacha20_xor(key, iv, 1, src + 4, dest + 4, payload_len);
    return true;
}
