 ...
 * shared crypto scratch (crypto_work) and is wiped on the way out - no cipher state on the stack or in BSS.
 */
...
static_assert(sizeof(mbedtls_ccm_context) <= DWS_CRYPTO_WORK_SIZE, "mbedtls_ccm_context must fit crypto_work");
                         ...
                         uint8_t tag_out[DWS_AESCCM_TAG_LEN])
{
    if (!key || !nonce || !ct_out || !tag_out || (key_len != 16 && key_len != 32))
        return false;
    mbedtls_ccm_context *c = reinterpret_cast<mbedtls_ccm_context *>(crypto_work);
    mbedtls_ccm_init(c);
    if (mbedtls_ccm_setkey(c, MBEDTLS_CIPHER_ID_AES, key, (unsigned)(key_len * 8)) != 0)
    {
        mbedtls_ccm_free(c);
        dws_crypto_wipe(crypto_work, sizeof(mbedtls_ccm_context));
        return false;
    }
    int rc =
        mbedtls_ccm_encrypt_and_tag(c, pt_len, nonce, nonce_len, aad, aad_len, pt, ct_out, tag_out, DWS_AESCCM_TAG_LEN);
    mbedtls_ccm_free(c);
    dws_crypto_wipe(crypto_work, sizeof(mbedtls_ccm_context));
    return rc == 0;
}
...
// the tag encrypted with the counter block A0. The whole working set (CcmWork) lives in crypto_work.
// ===========================================================================
...
static_assert(sizeof(CcmWork) <= DWS_CRYPTO_WORK_SIZE, "CcmWork must fit the shared crypto scratch (crypto_work)");

inline void ccm_key_init(CcmWork *w, const uint8_t *key, size_t key_len)
{
    if (key_len == 32)
    {
    ...
    {
        dws_aes_key_expand(key, 4, w->rk);
        w->nr = 10;
    }
}

inline void ecb(const CcmWork *w, const uint8_t in[16], uint8_t out[16])
{
    dws_aes_encrypt_block(w->rk, w->nr, in, out);
}
...
void cbc_mac(CcmWork *w, const uint8_t *nonce, size_t nonce_len, const uint8_t *aad, size_t aad_len, const uint8_t *pt,
             size_t pt_len)
{
    const size_t L = 15 - nonce_len;
    memset(w->X, 0, 16);
    ...
    memset(w->blk, 0, 16);
    w->blk[0] = (uint8_t)((aad_len > 0 ? 0x40 : 0x00) | (((DWS_AESCCM_TAG_LEN - 2) / 2) << 3) | (L - 1));
...
void ctr_crypt(CcmWork *w, const uint8_t *nonce, size_t nonce_len, size_t i0, const uint8_t *in, size_t len,
               uint8_t *out)
{
    size_t off = 0;
    size_t i = i0;
...
void tag_encrypt(CcmWork *w, const uint8_t *nonce, size_t nonce_len, uint8_t out_tag[DWS_AESCCM_TAG_LEN])
{
                         ...
                         uint8_t tag_out[DWS_AESCCM_TAG_LEN])
{
    if (!key || !nonce || !ct_out || !tag_out || (key_len != 16 && key_len != 32) || nonce_len < 7 || nonce_len > 13)
        return false;
    CcmWork *w = reinterpret_cast<CcmWork *>(crypto_work);
    ...
    dws_crypto_wipe(crypto_work, sizeof(CcmWork));
    return true;
}
                         ...
                         uint8_t *out)
{
    ...
    if (!dws_ct_eq(w->blk, tag, DWS_AESCCM_TAG_LEN))
    {
