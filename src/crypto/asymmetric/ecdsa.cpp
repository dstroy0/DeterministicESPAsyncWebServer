...
#if defined(ARDUINO) && !defined(PC_ECDSA_MPI_HW)
                        ...
                        const uint8_t priv[PC_ECDSA_P256_PRIV_LEN])
{
    ...
    if (mbedtls_ecp_group_load(&grp, MBEDTLS_ECP_DP_SECP256R1) == 0 &&
        mbedtls_mpi_read_binary(&d, priv, PC_ECDSA_P256_PRIV_LEN) == 0 &&
        mbedtls_ecdsa_sign(&grp, &r, &s, &d, h, PC_SHA256_DIGEST_LEN, ecdsa_rng, nullptr) == 0 &&
...
void pc_hmac_cat(uint8_t out[32], const uint8_t key[32], const uint8_t *v, size_t vlen, const int tag, const uint8_t *x,
                 const uint8_t *e)
{
...
bool ecdsa_sign_core(uint8_t sig[64], const uint8_t h1[32], const uint32_t d[8])
{
    ...
    pc_hmac_cat(V, K, V, 32, -1, nullptr, nullptr);
    ...
    {
        pc_hmac_cat(V, K, V, 32, -1, nullptr, nullptr); // T = HMAC_K(V), one block
        uint32_t k[8];
