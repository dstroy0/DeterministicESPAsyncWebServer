 ...
 * (DWS_PHYSICAL_HAS_BACKEND == 0 - host/native builds, or a vendor whose PHY driver is not written).
 * Each vendor's real bring-up lives in physical/<vendor>/ (physical/esp/physical_esp.cpp today), chosen
 * by the DWS_VENDOR_* selector. The stubs never bring a link up, so a new target builds and runs headless
...
#endif // !DWS_PHYSICAL_HAS_BACKEND
