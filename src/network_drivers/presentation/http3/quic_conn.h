 ...
 * runs via dws_quic_tls), dispatches the frames, reassembles the CRYPTO stream to advance the handshake,
 * tracks packet numbers to generate ACKs, and coalesces the outbound Initial / Handshake / 1-RTT
 * packets back into datagrams. Application streams are surfaced to HTTP/3 (dws_h3_conn) through a small
 * callback + send API, so the transport engine has no HTTP dependency.
 *
 * It is transport-free (no lwIP): dws_quic_conn_recv() takes a received datagram and dws_quic_conn_send()
 * pulls the next datagram to transmit, so the engine is host-testable by shuttling byte buffers
 * between a server QuicConn and a client written in the test. dws_quic_server wires it to dws_udp.
 *
 ...
 * Loss recovery is a Probe Timeout (RFC 9002): dws_quic_conn_on_timeout() retransmits the outstanding
...
struct QuicPnSpace
{
    ...
    uint64_t crypto_rx_off; ///< in-order CRYPTO bytes already delivered to dws_quic_tls
 ...
 * so the next dws_quic_conn_send() re-sends it, and back the timer off exponentially. @p now_ms is the
 * caller's monotonic millisecond clock (dws_quic_conn stays clock-free). A no-op once the flight is
 * acknowledged or the handshake completes. Call once per poll before dws_quic_conn_send().
 */
 ...
 * @p error_code so the next dws_quic_conn_send() reports the error to the peer instead of leaving it to
