...
/**
 * @file dws_quic_crypto.cpp
 * @brief QUIC packet protection and Initial secrets (see dws_quic_crypto.h).
 */
                                 ...
                                 const QuicPacketKeys *keys, bool is_long, uint8_t *out, uint64_t *out_pn)
{
    ...
    uint64_t full_pn = dws_quic_pn_decode(largest_pn, truncated_pn, (uint8_t)(pn_len * 8));
    if (out_pn)
        *out_pn = full_pn;
