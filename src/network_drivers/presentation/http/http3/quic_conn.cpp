...
QuicPacketKeys *open_keys(QuicConn *qc, int level)
{
    if (level == QuicEnc::QUIC_ENC_INITIAL)
    {
        return &qc->initial.client;
    }
    return pc_quic_tls_keys(&qc->tls, level, /*is_server=*/false);
}
QuicPacketKeys *seal_keys(QuicConn *qc, int level)
{
    if (level == QuicEnc::QUIC_ENC_INITIAL)
    {
        return &qc->initial.server;
    }
    return pc_quic_tls_keys(&qc->tls, level, /*is_server=*/true);
}
...
QuicStream *stream_get(QuicConn *qc, uint64_t id, bool create)
{
    ...
    {
        return nullptr; // pc_quic_conn_stream_send), so the lookup-only arm is never taken
    }
...
size_t recv_packet(QuicConn *qc, const uint8_t *dg, size_t len)
{
    ...
    bool is_long = pc_quic_is_long_header(dg[0]);
...
size_t build_frames(QuicConn *qc, int level, uint8_t *buf, size_t cap, bool *ae)
{
    ...
    *ae = false;
    ...
    {
        return pc_quic_build_connection_close(buf, cap, qc->close_error, qc->close_frame_type, nullptr, 0);
    }
...
int pc_quic_highest_sealed_level(QuicConn *qc)
{
