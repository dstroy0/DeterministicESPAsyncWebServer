...
namespace
{
H3Stream *pc_h3_stream_get(H3Conn *h3, uint64_t id, bool create)
{
...
bool pc_h3_classify_uni_stream(H3Stream *st)
{
    uint64_t type = 0;
    size_t c = 0;
...
void pc_h3_consume_control(H3Conn *h3, H3Stream *st)
{
    size_t off = 0;
    while (off < st->buf_len)
    {
...
void on_stream_data(void *app, QuicConn *, uint64_t stream_id, const uint8_t *data, size_t len, bool fin)
{
    ...
    H3Stream *st = pc_h3_stream_get(h3, stream_id, true);
    if (!st)
    {
        return;
    }
...
bool pc_h3_conn_respond(H3Conn *h3, uint64_t stream_id, int status, const char *content_type, const uint8_t *body,
                        size_t body_len)
{
    H3Stream *st = pc_h3_stream_get(h3, stream_id, false);
    if (st)
    {
        st->responded = true;
    }
    ...
    return pc_quic_conn_stream_send(h3->qc, stream_id, out, op, true) == op;
}
