...
bool pc_tls13_parse_client_hello(const uint8_t *msg, size_t len, Tls13ClientHello *out, bool dtls)
{
...
size_t pc_tls13_build_encrypted_extensions_empty(uint8_t *out, size_t cap, bool rpk_server_cert)
{
    Writer w = {out, cap, 0, true};
...
size_t pc_tls13_build_cert_verify(uint8_t *out, size_t cap, const uint8_t transcript_hash[32], const uint8_t seed[32])
{
    ...
    size_t clen = pc_tls13_cert_verify_content(content, sizeof(content), transcript_hash, true);
