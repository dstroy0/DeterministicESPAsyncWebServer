...
#if defined(ARDUINO) && !PC_QUIC_SERVER_IN_PSRAM && !PC_QUIC_SERVER_ACK_DRAM
#error                                                                                                                 \
...
#endif
#if PC_QUIC_SERVER_IN_PSRAM && defined(ARDUINO)
#include <esp_attr.h>
...
PC_QUIC_POOL_ATTR QuicServerPoolCtx s_qpool;
...
void pc_h3_on_request(void *app, H3Conn * /*h3*/, uint64_t stream_id, const char *method, const char *path,
...
QuicSlot *route(const uint8_t *dg, size_t len, bool *is_initial, QuicLongHeader *lh_out)
{
    *is_initial = false;
    ...
    {
        return nullptr; // GCOVR_EXCL_LINE  dead unless the len<1 branch above is taken
    }
...
#if defined(ARDUINO)
void udp_ingest_cb(const uint8_t *data, size_t len, const pc_udp_peer *peer, void * /*ctx*/)
{
    char ip[16];
    uint16_t port = 0;
...
bool pc_quic_server_begin(uint16_t port, const QuicServerConfig *cfg, QuicServerRequestFn on_request, void *app)
{
    ...
    s_quic.running = true;
#if defined(ARDUINO)
    return pc_udp_listen(s_quic.port, udp_ingest_cb, nullptr);
#else
    return true; // host: fed through pc_quic_server_ingest()
#endif
}
