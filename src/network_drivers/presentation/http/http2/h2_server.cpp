...
#if PC_H2_POOL_IN_PSRAM && defined(ARDUINO)
...
{
    bool ok = pc_h2_conn_respond(&s_h2.pool[slot], conn_pool[slot].pc_h2_stream, code, content_type, body, len);
...
void pc_h2_server_close(uint8_t slot)
{
    ...
    conn_pool[slot].pc_resp_sink = nullptr;
}
