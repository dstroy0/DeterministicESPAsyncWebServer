...
void dws_ssh_server_set_emit_cb(SshEmitCb cb)
{
    s_srv.emit_cb = cb;
}
...
int dws_ssh_server_dispatch(uint8_t i, uint8_t msg_type, const uint8_t *payload, size_t len)
{
    if (i >= MAX_SSH_CONNS)
        return -1;
    ...
    case SSH_MSG_SERVICE_REQUEST:
        ...
        if (s->phase != SshPhase::SSH_PHASE_SERVICE)
            return -1;
        if (dws_ssh_auth_handle_service_request(payload, len, buf, &n, sizeof(buf)) != 0)
            return -1;
        emit(i, buf, n);
    ...
    case SSH_MSG_USERAUTH_REQUEST:
        if (s->phase != SshPhase::SSH_PHASE_AUTH)
            return -1;
        if (dws_ssh_auth_handle_request(i, payload, len, buf, &n, sizeof(buf)) != 0)
            return -1;
    ...
    case SSH_MSG_CHANNEL_OPEN:
        if (!s->authed)
            return -1;
        if (dws_ssh_channel_handle_open(i, payload, len, buf, &n, sizeof(buf)) != 0)
            return -1;
        emit(i, buf, n);
        return 0;
    ...
    case SSH_MSG_CHANNEL_OPEN_CONFIRM:
        ...
        if (!s->authed)
            return -1;
        dws_ssh_channel_handle_open_confirm(i, payload, len);
        return 0;
    ...
    case SSH_MSG_CHANNEL_OPEN_FAILURE:
        ...
        if (!s->authed)
            return -1;
        dws_ssh_channel_handle_open_failure(i, payload, len);
        return 0;

    case SSH_MSG_CHANNEL_REQUEST:
        if (!s->authed)
            return -1;
        if (dws_ssh_channel_handle_request(i, payload, len, buf, &n, sizeof(buf)) != 0)
            return -1;
    ...
    case SSH_MSG_CHANNEL_DATA:
        if (!s->authed)
            return -1;
        if (dws_ssh_channel_handle_data(i, payload, len, buf, &n, sizeof(buf)) != 0)
            return -1;
        emit(i, buf, n); // WINDOW_ADJUST when the receive window is replenished
        return 0;

    case SSH_MSG_CHANNEL_WINDOW_ADJUST:
        dws_ssh_channel_handle_window_adjust(i, payload, len);
        return 0;
