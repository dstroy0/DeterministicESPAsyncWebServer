...
static void build_kex_list(char *out, size_t cap)
{
    ...
    {
        pc_sb sb157 = {out + n, cap - n, 0, true};
        pc_sb_put(&sb157, dh);
        pc_sb_put(&sb157, ",");
        pc_sb_put(&sb157, ec);
        pc_sb_put(&sb157, ",");
    ...
    {
        pc_sb sb159 = {out + n, cap - n, 0, true};
        pc_sb_put(&sb159, c1);
        pc_sb_put(&sb159, ",");
        pc_sb_put(&sb159, c2);
        pc_sb_put(&sb159, ",");
...
static void build_hostkey_list(char *out, size_t cap)
{
    ...
    const bool ed = pc_ssh_hostkey_ed25519_available();
    const bool ec = pc_ssh_hostkey_ecdsa_available();
    struct HostkeyCand
    {
        const char *name;
        bool ok;
    ...
    for (int k = 0; k < 4; k++)
    {
        ...
        pc_sb sb194 = {out + l, cap - l, 0, true};
        pc_sb_put(&sb194, l ? "," : "");
                     ...
                     const char **sig_name)
{
    ...
    const pc_rsa_hash rh = sha512 ? pc_rsa_hash::SHA512 : pc_rsa_hash::SHA256;
                               ...
                               uint8_t k_out[32])
{
    ...
    uint8_t k_pq[32];
    bool ok = pc_mlkem768_encaps(ek, m, s_reply, k_pq); // ciphertext -> s_reply[0..1087]
