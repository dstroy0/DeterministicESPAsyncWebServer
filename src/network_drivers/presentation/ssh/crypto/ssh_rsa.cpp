...
// lifetime (as an SSH host key normally does); the mutex serialises signs because mbedtls mutates the
// blinding values per operation. Loaded once at startup by dws_ssh_rsa_load_pubkey().
struct SshRsaCtx
{
...
int dws_ssh_rsa_load_pubkey(void)
{
    ...
    if (mbedtls_rsa_get_len(rsa) != DWS_RSA_KEY_BYTES)
    {
        mbedtls_pk_free(&s_rsa.pk);
        return -1;
    }
    ...
    mbedtls_mpi_write_binary(&n_mpi, ssh_host_pubkey.n, DWS_RSA_KEY_BYTES);
...
int ssh_rsa_sign(const uint8_t *msg, size_t msg_len, DwsRsaHash hash, uint8_t sig[DWS_RSA_SIG_BYTES])
{
    ...
    // Serialise: mbedtls mutates the context's blinding state on each private op.
    if (s_rsa.lock)
        xSemaphoreTake(s_rsa.lock, portMAX_DELAY);
    size_t sig_len = 0;
#if MBEDTLS_VERSION_MAJOR >= 3
    int rc = mbedtls_pk_sign(&s_rsa.pk, md, digest, dlen, sig, DWS_RSA_SIG_BYTES, &sig_len, ssh_mbedtls_rng, nullptr);
#else
...
uint8_t _test_rsa_n[DWS_RSA_KEY_BYTES];
uint8_t _test_rsa_d[DWS_RSA_KEY_BYTES];
uint8_t _test_rsa_e[4];

int dws_ssh_rsa_load_pubkey(void)
{
    memcpy(ssh_host_pubkey.n, _test_rsa_n, DWS_RSA_KEY_BYTES);
...
int ssh_rsa_sign(const uint8_t *msg, size_t msg_len, DwsRsaHash hash, uint8_t sig[DWS_RSA_SIG_BYTES])
{
    return dws_rsa_sign_sw(_test_rsa_n, _test_rsa_d, msg, msg_len, hash, sig);
}
...
int ssh_rsa_encode_pubkey(uint8_t *out, size_t *out_len, size_t out_cap)
{
    ...
    p = put_mpint(p, ssh_host_pubkey.n, DWS_RSA_KEY_BYTES);
