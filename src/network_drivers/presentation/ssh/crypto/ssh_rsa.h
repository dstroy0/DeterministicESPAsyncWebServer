 ...
 * crypto/rsa (dws_rsa_verify / dws_rsa_sign_sw); peers' signatures are verified via that primitive.
 *
 * ═══════════════════════════════════════════════════════════════════════════
 ...
 * (dws_rsa_sign_sw) wipes its own temporaries. The signature scheme is PKCS#1 v1.5 (RFC 8017 §8.2),
...
#include "crypto/rsa.h" // DwsRsaHash, DWS_RSA_KEY_BYTES/SIG_BYTES, dws_rsa_verify / dws_rsa_sign_sw
#include <stddef.h>
#include <stdint.h>
...
struct SshRsaPubKey
{
    uint8_t n[DWS_RSA_KEY_BYTES]; ///< Modulus n (256 bytes, big-endian).
 ...
 * @return 0 on success, -1 on failure. @p sig receives DWS_RSA_SIG_BYTES big-endian.
 */
