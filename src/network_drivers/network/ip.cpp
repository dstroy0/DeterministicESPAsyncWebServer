...
pc_ip_scope classify_v4(const uint8_t *b)
{
    if (b[0] == 0 && b[1] == 0 && b[2] == 0 && b[3] == 0)
    {
        return pc_ip_scope::PC_IP_SCOPE_UNSPECIFIED; // 0.0.0.0
    }
    if (b[0] == 127)
    {
        return pc_ip_scope::PC_IP_SCOPE_LOOPBACK; // 127/8
    }
    if (b[0] == 169 && b[1] == 254)
    {
        return pc_ip_scope::PC_IP_SCOPE_LINK_LOCAL; // 169.254/16
    }
    if (b[0] == 10 || (b[0] == 172 && b[1] >= 16 && b[1] <= 31) || (b[0] == 192 && b[1] == 168))
    {
        return pc_ip_scope::PC_IP_SCOPE_PRIVATE; // RFC 1918
    }
    if (b[0] >= 224 && b[0] <= 239)
    {
        return pc_ip_scope::PC_IP_SCOPE_MULTICAST; // 224/4
    }
    return pc_ip_scope::PC_IP_SCOPE_GLOBAL;
}

/** Classify the sixteen v6 bytes at @p b (v4-mapped addresses defer to their embedded v4). */
pc_ip_scope classify_v6(const uint8_t *b)
{
    bool allzero = true;
    for (int k = 0; k < 16; k++)
    {
    ...
    if (allzero)
    {
        return pc_ip_scope::PC_IP_SCOPE_UNSPECIFIED; // ::
    }
    ...
    if (loopback)
    {
        return pc_ip_scope::PC_IP_SCOPE_LOOPBACK; // ::1
    }
    ...
    if (b[0] == 0xff)
    {
        return pc_ip_scope::PC_IP_SCOPE_MULTICAST; // ff00::/8
    }
    if (b[0] == 0xfe && (b[1] & 0xc0) == 0x80)
    {
        return pc_ip_scope::PC_IP_SCOPE_LINK_LOCAL; // fe80::/10
    }
    if ((b[0] & 0xfe) == 0xfc)
    {
        return pc_ip_scope::PC_IP_SCOPE_PRIVATE; // fc00::/7 (unique-local)
    }
    return pc_ip_scope::PC_IP_SCOPE_GLOBAL;
}
} // namespace
...
bool pc_ip_parse(const char *s, pc_ip *out)
{
    ...
    if (dot)
    {
        ...
        out->family = pc_ip_family::PC_IP_V4;
        return true;
    }
    return false;
}
...
size_t pc_ip_format(const pc_ip *ip, char *out, size_t cap)
{
    ...
    if (ip->family == pc_ip_family::PC_IP_V4)
    {
        return format_v4(ip->bytes, out, cap);
    }
    if (ip->family != pc_ip_family::PC_IP_V6)
    {
        return 0;
    }
...
{
    return ip && ip->family == pc_ip_family::PC_IP_V6 && is_v4_mapped_bytes(ip->bytes);
}
...
bool pc_ip_equal(const pc_ip *a, const pc_ip *b)
{
    ...
    int n = 0;
    if (a->family == pc_ip_family::PC_IP_V4)
    {
        n = 4;
    }
    else if (a->family == pc_ip_family::PC_IP_V6)
    {
        n = 16;
    }
    if (n == 0)
    {
        return true; // both the same non-address family (pc_ip_family::PC_IP_NONE)
    }
...
pc_ip pc_ip_from_v4_octets(uint8_t a, uint8_t b, uint8_t c, uint8_t d)
{
    ...
    ip.family = pc_ip_family::PC_IP_V4;
    ip.bytes[0] = a;
    ip.bytes[1] = b;
    ip.bytes[2] = c;
    ip.bytes[3] = d;
...
{
    pc_ip ip;
    ip.family = pc_ip_family::PC_IP_V6;
...
bool pc_ip_is_unspecified(const pc_ip *ip)
{
    ...
    int n = (ip->family == pc_ip_family::PC_IP_V4) ? 4 : 16;
    for (int i = 0; i < n; i++)
    {
        if (ip->bytes[i])
        {
...
bool pc_ip_prefix_match(const pc_ip *addr, const pc_ip *net, uint8_t prefix_len)
{
    ...
    int bits = (addr->family == pc_ip_family::PC_IP_V4) ? 32 : (addr->family == pc_ip_family::PC_IP_V6 ? 128 : 0);
