 ...
 * layers the shared client-TLS session (`dws_tls_client_session_*`) on top, pointing its
 * BIO at dws_client_send() / dws_client_read().
 *
 ...
 * deadline. Only dws_client_open() blocks, on DNS + connect.
 */
 ...
 * @return A client id in [0, DWS_CLIENT_CONNS) on success, or < 0 on failure
