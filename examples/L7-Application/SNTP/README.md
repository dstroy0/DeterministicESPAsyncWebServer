...
**Layer:** L7 Application · **Build flags:** `DWS_ENABLE_NTP`

## What this example teaches

An ESP32 boots with no idea what time it is. `dws_ntp_begin(tz)` starts the
ESP-IDF SNTP client (the first sync lands a few seconds later), and
`dws_ntp_http_date()` formats the current time as an RFC 7231 date string -
...
```cpp
dws_ntp_begin("UTC0"); // POSIX TZ string; set your zone for local time
```

```cpp
char date[40];
if (dws_ntp_http_date(date, sizeof(date)) == 0) {  // 0 = not synced yet
...
`dws_ntp_http_date()` returns 0 until the clock is set, so the handler can
...
pio ci --board=esp32dev --project-option="framework=arduino" \
  --project-option="build_flags=-DDWS_ENABLE_NTP=1" \
...
void handle_time(uint8_t slot_id, HttpReq *req)
{
    (void)req;
    char date[40];
    if (dws_ntp_http_date(date, sizeof(date)) == 0) // 0 -> clock not set yet
    {
        server.send(slot_id, 503, "text/plain", "Time not synced yet");
        return;
    }
