...
static size_t exchange(WiFiClient &sock, size_t reqlen) {
    if (reqlen == 0 || sock.write(c_req, reqlen) != reqlen) return 0;
    size_t got = 0; uint32_t deadline = millis() + 3000;
    while (got < 8 && millis() < deadline) if (sock.available()) got += sock.read(c_resp + got, 8 - got);
    if (got < 8) return 0;
    ...
    while (got < size && millis() < deadline) if (sock.available()) got += sock.read(c_resp + got, size - got);
...
The complete sketch ([56.OpcUaClient.ino](56.OpcUaClient.ino)), reproduced verbatim
