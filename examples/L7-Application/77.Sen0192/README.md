...
Detect motion with a **DFRobot SEN0192** — a 10.525 GHz microwave Doppler sensor (`DWS_ENABLE_SEN0192`).
...
presence signal**: presence asserts on motion and is held for `DWS_SEN0192_HOLD_MS` after the last
...
- `DWS_SEN0192_PIN` (default `4`) — the GPIO the OUT line is on
- `DWS_SEN0192_HOLD_MS` (default `2000`) — how long presence is held after the last motion
- `DWS_SEN0192_ACTIVE_HIGH` (default `1`) — set `0` if your module's OUT idles high and drops on motion
...
pio ci examples/L7-Application/77.Sen0192 --board esp32dev --lib "." \
  --project-option="build_flags=-DDWS_ENABLE_SEN0192=1"
```
