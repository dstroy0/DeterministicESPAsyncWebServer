...
/**
 * @file 18.SNTP.ino
 * @brief Wall-clock time via SNTP (DWS_ENABLE_NTP).
 *
 * dws_ntp_begin(tz) starts the ESP-IDF SNTP client; the first sync lands a few
 * seconds later. dws_ntp_http_date() formats the current time as an RFC 7231
 * date string (returns 0 until synced). GET /time returns it.
 *
 * NOTE: this service is compiled into the library only when DWS_ENABLE_NTP
 * is set for the whole build (a .ino #define does not reach the separately
 * compiled library). In platformio.ini:
 *     build_flags = -DDWS_ENABLE_NTP=1
...
void handle_time(uint8_t slot_id, HttpReq *req)
{
    (void)req;
    char date[40];
    if (dws_ntp_http_date(date, sizeof(date)) == 0)
    {
