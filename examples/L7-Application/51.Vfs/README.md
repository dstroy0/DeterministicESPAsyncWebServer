...
**Layer:** L7 Application · **Build flags:** `DWS_ENABLE_VFS`

## What this example teaches

The `dws_vfs_*` API is one file interface with swappable backends: a RAM pool in
...
```cpp
LittleFS.begin(true);
dws_vfs_mount(dws_vfs_fs(&LittleFS)); // real flash...
// dws_vfs_mount(dws_vfs_ram());      // ...or pure RAM - endpoints identical
```

**The file operations are backend-agnostic:**

```cpp
dws_vfs_write_file(path, data, strlen(data)); // create / overwrite
long n = dws_vfs_read_file(path, buf, cap);   // n < 0 if absent
long sz = dws_vfs_size(path);                 // -1 if absent
dws_vfs_remove(path);
```
...
pio ci --board=esp32dev --project-option="framework=arduino" \
  --project-option="build_flags=-DDWS_ENABLE_VFS=1" \
...
void setup()
{
    ...
    server.on("/save", HTTP_GET, [](uint8_t id, HttpReq *req) {
        ...
        bool ok = dws_vfs_write_file(path, data, strlen(data));
    ...
    server.on("/load", HTTP_GET, [](uint8_t id, HttpReq *req) {
        ...
        char buf[512];
        long n = dws_vfs_read_file(path, buf, sizeof(buf) - 1);
        if (n < 0)
        {
    ...
    server.on("/size", HTTP_GET, [](uint8_t id, HttpReq *req) {
        char path[DWS_VFS_NAME_MAX];
        long n = path_of(req, path, sizeof(path)) ? dws_vfs_size(path) : -1;
        char b[24];
    ...
    server.on("/rm", HTTP_GET, [](uint8_t id, HttpReq *req) {
        char path[DWS_VFS_NAME_MAX];
        bool ok = path_of(req, path, sizeof(path)) && dws_vfs_remove(path);
