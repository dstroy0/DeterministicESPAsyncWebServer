 ...
 * @brief Turn the ESP32 into a network<->hardware-bus device server (DWS_ENABLE_IFACE_BRIDGE).
 *
 ...
 * `dws_iface_bridge_publish()` binds it to a target and brings the bus up. The server poll loop does the rest.
 *
 ...
 * build: `build_flags = -DDWS_ENABLE_IFACE_BRIDGE=1`. In the Arduino IDE it is set in build_opt.h.
 */

#define DWS_ENABLE_IFACE_BRIDGE 1

#include "dwserver.h"
#include "network_drivers/physical/physical.h"
#include "services/iface_bridge/iface_bridge_hw.h" // dws_iface_bridge_publish
#include <Arduino.h>
#include <WiFi.h>
...
void setup()
{
    ...
    if (lu < 0 || !dws_iface_bridge_publish((uint8_t)lu, UART_PORT, BridgeProto::tcp, &uart))
    ...
    if (ls < 0 || !dws_iface_bridge_publish((uint8_t)ls, SPI_PORT, BridgeProto::tcp, &spi))
