...
**Layer:** L7 Application · **Build flags:** `DETWS_ENABLE_TOTP`
...
```cpp
int n = detws_base32_decode(SECRET_B32, g_secret, sizeof(g_secret)); // base32 -> bytes
g_secret_len = (n > 0) ? (size_t)n : 0;
```

```cpp
uint32_t code = detws_totp(g_secret, g_secret_len, now_unix(), 30, 6);          // 30s step, 6 digits
bool ok = detws_totp_verify(g_secret, g_secret_len, now_unix(), code, 30, 6, 1); // +/-1 step tolerance
```

The last argument to `detws_totp_verify` is the allowed step skew, which absorbs
...
pio ci --board=esp32dev --project-option="framework=arduino" \
  --project-option="build_flags=-DDETWS_ENABLE_TOTP=1" \
