...
/**
 * @file sysadmin.ino
 * @brief System Administration example featuring a dark-themed HTML/CSS dashboard, stats API, and reboot sequencer.
 *
 * This example showcases:
 *   1. Serving a single-page web dashboard using PROGMEM static resources.
 *   2. Responsive CSS Grid styling, sleek dark-mode aesthetics, and micro-animations.
 *   3. Hardware statistics endpoint (/api/sysinfo) tracking CPU, memory heap, and WiFi RSSI.
 *   4. Clean reboot sequence: responding to the client first, then restarting the device
 *      after a 1-second delay in loop() to allow lwIP buffers to flush cleanly.
 *   5. Input validation and config updates via POST endpoints.
 *   6. Restricting admin APIs with a custom headers check (X-Admin-Token).
 *
 * To run this example:
 *   - Configure SSID/PASSWORD, and upload to an ESP32.
 *   - Find the ESP32 IP in the Serial Monitor, open it in any web browser.
...
// --- Embedded Dashboard HTML (Premium Dark Theme) ---
    ...
    <div class="container">
        ...
        <div class="grid">
            <div class="card">
                ...
                <ul class="data-list">
                    <li class="data-item">
                        <span class="data-label">Min Free Heap</span>
                        <span class="data-value" id="min-heap">-- KB</span>
                    </li>
                    <li class="data-item">
                        <span class="data-label">Max Alloc Block</span>
                        <span class="data-value" id="max-alloc">-- KB</span>
                    </li>
                </ul>
            </div>
            ...
            <div class="card">
                ...
                <ul class="data-list" style="margin-top: 0.5rem;">
                    <li class="data-item">
                        ...
                        <span class="data-value" id="uptime">-- s</span>
                    </li>
                    <li class="data-item">
                        <span class="data-label">Reset Reason</span>
                        <span class="data-value" id="reset-reason">--</span>
                    </li>
                    <li class="data-item">
                        <span class="data-label">Chip Revision</span>
                        <span class="data-value" id="chip-rev">--</span>
                    </li>
                    <li class="data-item">
                        <span class="data-label">CPU Freq</span>
                        <span class="data-value" id="cpu-freq">-- MHz</span>
                    </li>
                </ul>
            </div>
            ...
            <div class="card">
                ...
                <ul class="data-list">
                    <li class="data-item">
                        ...
                        <span class="data-value" id="wifi-ssid">--</span>
                    </li>
                    <li class="data-item">
                        <span class="data-label">Channel</span>
                        <span class="data-value" id="wifi-channel">--</span>
                    </li>
                </ul>
            </div>
        </div>
    ...
    <script>
        ...
        async function triggerReboot() {
            ...
            try {
                ...
                alert('Connection lost. Node is rebooting...');
            }
        }
...
/**
 * @brief Translates ESP32 reset reason enum into user-friendly text string.
 */
...
/**
 * @brief Verifies the custom administration security token header.
 */
...
/**
 * @brief GET /
 * Serves the embedded single-page HTML console dashboard.
 */
...
/**
 * @brief GET /api/sysinfo
 * Collects and serializes hardware telemetry to JSON. Requires valid auth token header.
 */
...
/**
 * @brief POST /api/restart
 * Receives reboot command and arms reboot timer.
 */
void handle_post_restart(uint8_t slot_id, HttpReq *req)
{
    ...
    // Acknowledge request immediately to send response headers back to client
    server.send(slot_id, 200, "application/json", "{\"status\":\"rebooting\"}");

    // Schedule the physical restart after response dispatch completes
    pending_reboot = true;
...
void setup()
{
    ...
    Serial.println("\nWiFi Online!");
    Serial.print("Access the Admin Dashboard via: http://");
    Serial.println(WiFi.localIP());

    server.set_cors("*");

    // Register UI & REST services
    ...
    if (server.begin(80))
    {
        Serial.println("SysAdmin Node Console successfully initialized.");
    }
}
...
void loop()
{
    ...
    // Perform scheduled reboot sequence
    if (pending_reboot && (millis() - reboot_trigger_ms >= 1000))
    {
        Serial.println("Reboot sequence triggered. Rebooting hardware now!");
        delay(100);
        ESP.restart();
    }
}
