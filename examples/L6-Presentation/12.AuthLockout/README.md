...
**Layer:** L6 Presentation · **Build flags:** `DETWS_ENABLE_AUTH_LOCKOUT` (requires `DETWS_ENABLE_AUTH`, on by default)
...
`DETWS_AUTH_LOCKOUT_SLOTS` (how many addresses are tracked). Set them as build
flags alongside the enable flag.

**Build dependency.** `DETWS_ENABLE_AUTH_LOCKOUT` requires `DETWS_ENABLE_AUTH`
...
pio ci --board=esp32dev --project-option="framework=arduino" \
  --project-option="build_flags=-DDETWS_ENABLE_AUTH_LOCKOUT=1" \
