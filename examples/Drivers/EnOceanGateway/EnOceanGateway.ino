...
// test/test_enocean.
//
// Build flags (whole build): DWS_ENABLE_ENOCEAN=1 DWS_ENABLE_GATEWAY=1
...
void setup()
{
    ...
    p.kind = dws_gateway_kind::DWS_GW_ENOCEAN;
    dws_gateway_add_port(&p);
    dws_gateway_set_uplink_cb(northbound_publish, nullptr);
    dws_gateway_set_topic_prefix("enocean");
...
void loop()
{
    ...
    for (;;)
    {
        ...
        if (pkt.type == dws_esp3_type::ESP3_RADIO_ERP1 && pkt.data_len >= 6)
        {
            ...
            dws_gateway_uplink(RADIO_PORT, src, pkt.data, (uint16_t)(pkt.data_len - 5), 0);
        }
